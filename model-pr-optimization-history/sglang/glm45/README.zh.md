...
- 动机: 该 PR 围绕 GLM-4.5 补齐模型支持入口或运行时能力，标题为「GLM-4.5 Model Support」，变更集中在 `python/sglang/srt/models/glm4_moe.py`, `python/sglang/srt/models/glm4_moe_nextn.py`, `python/sglang/srt/function_call/glm4_moe_detector.py`。PR 描述补充为：The SGLang version of the complete implementation of the GLM-4.5 model, which includes: 1. Model implementation (with MTP and without MTP) 2. Tool call and Reasoning parser 3. t...
...
- 状态/时间: merged / 2025-07-28
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/function_call/glm4_moe_detector.py`；关联提交 `581e7dcb92a7`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 6 个文件，+44/-15，可读 patch 168 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 GLM-4.5 补齐模型支持入口或运行时能力，标题为「GLM-4.5 Model Support Follow-up」，变更集中在 `python/sglang/srt/function_call/glm4_moe_detector.py`。PR 描述补充为：## Motivation - Address PR comments in https://github.com/sgl-project/sglang/pull/8224 - support more than one tool_choice logic for GLM4-moe - add key_value_separator for qwen3...
...
- 动机: 该 PR 围绕 GLM-4.5 修复已暴露的启动、加载、解析或数值问题，标题为「fix GLM4_MOE launch with compressed_tensor quant model」，变更集中在 `python/sglang/srt/models/glm4_moe.py`。PR 描述补充为：## Motivation fix-compressed_tensor_quant Without the change: With the fix, it could launch successfully, and gsm8k benchmark is good: ## Modifications ## Checklist - [ ] Format...
...
- 动机: 该 PR 围绕 GLM-4.5 补齐模型支持入口或运行时能力，标题为「Disable tp for shared experts under expert parallelism for GLM4.5 model (#8647)」，变更集中在 `python/sglang/srt/models/glm4_moe.py`。PR 描述补充为：## Motivation Currently when we enable **EPMoe** of the **glm4_moe** model, it still TP splits the shared experts, which blocks us to launch **FP8 per block quant GLM4.5** with...
...
- 动机: 该 PR 围绕 GLM-4.5 调整模型相关实现，标题为「use fp32 for e_score_correction_bias in GLM-4.5」，变更集中在 `python/sglang/srt/models/glm4_moe.py`。PR 描述补充为：Force e_score_correction_bias to FP32 instead of using the model's own dtype. This is to maintain consistency with what was passed during training.
...
- 动机: 该 PR 围绕 GLM-4.5 补齐模型支持入口或运行时能力，标题为「GLM-4.5 and GLM-4.5-Air both support」，变更集中在 `python/sglang/srt/models/glm4_moe.py`。PR 描述补充为：determine_num_fused_shared_experts changed
...
- 动机: 该 PR 围绕 GLM-4.5 修复已暴露的启动、加载、解析或数值问题，标题为「fix glm4 moe」，变更集中在 `python/sglang/srt/models/glm4_moe.py`。PR 描述补充为：## Motivation GLM4 inherits deepseek but #8539's update is not applied to GLM4. Some logits are broken. ## Modifications ## Accuracy Test ## Benchmark & Profiling ## Checklist -...
...
- 动机: 该 PR 围绕 GLM-4.5 修复已暴露的启动、加载、解析或数值问题，标题为「Quick Fix GLM」，变更集中在 `python/sglang/srt/models/glm4_moe.py`, `test/srt/test_nightly_gsm8k_eval.py`。PR 描述补充为：## Motivation GLM inference is failing after https://github.com/sgl-project/sglang/pull/8846/files#diff-5b9e34dd492bd8a14702a18b594721091092276fad1cf8736fba6ef1f33c1b04 Long ter...
...
- 动机: 该 PR 围绕 GLM-4.5 优化关键推理路径或后端选择，标题为「Optimized deepseek-v3/r1 model performance on mxfp4 run」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/quantization/quark/utils.py`, `python/sglang/srt/layers/quantization/quark/schemes/quark_w4a4_mxfp4.py`。PR 描述补充为：## Motivation In order to decrease the activation tensor quantized overhead, fused the quantized behavior into the different ops (activation, layernorm, gemm, flatten) ## Modifi...
...
- 动机: 该 PR 围绕 GLM-4.5 补强部署文档、测试或 CI 验证面，标题为「Update GLM-4.5 Model Doc」，变更集中在 `python/sglang/srt/function_call/glm4_moe_detector.py`, `python/sglang/srt/models/glm4_moe.py`, `python/sglang/srt/models/glm4_moe_nextn.py`。PR 描述补充为：This is a update of GLM-4.5 Doc, including format and model change
...
- 动机: 该 PR 围绕 GLM-4.5 修复已暴露的启动、加载、解析或数值问题，标题为「Fix missing a2a backend init of GLM4.5 MoE Block」，变更集中在 `python/sglang/srt/models/glm4_moe.py`。PR 描述补充为：## Motivation Fix https://github.com/sgl-project/sglang/actions/runs/18546407737/job/52865228007#step:4:8006 ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ##...
...
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-1，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 GLM-4.5 补齐模型支持入口或运行时能力，标题为「fix(glm45): disable reduce scatter」，变更集中在 `python/sglang/srt/models/glm4_moe.py`。PR 描述补充为：## Motivation glm45_moe not support reduce-scatter but enable it ## Modifications disable reduce scatter
...
- 动机: 该 PR 围绕 GLM-4.5 补齐模型支持入口或运行时能力，标题为「[9/N] MoE Refactor: cleanup dispatcher interfaces」，变更集中在 `python/sglang/srt/layers/moe/token_dispatcher/deepep.py`, `python/sglang/srt/layers/moe/ep_moe/layer.py`, `python/sglang/srt/layers/moe/fused_moe_triton/layer.py`。PR 描述补充为：- Unified initialization for `StandardDispatcher` and `DeepEPDispatcher`. - Added `get_is_extend_in_batch` so that there is no need to pass `forward_batch` to dispatcher. - Adde...
...
- 动机: 该 PR 围绕 GLM-4.5 调整模型相关实现，标题为「Refactoring GLM-4.5 and GLM-4.5V related implementations」，变更集中在 `python/sglang/srt/models/glm4_moe.py`, `python/sglang/srt/models/glm4_moe_nextn.py`。PR 描述补充为：To resolve the inheritance conflict with the DeepSeek-V2 model, the GLM-4.5 model implementation has been refactored, streamlining the code and inference logic, with relevant be...
...
- 动机: 该 PR 围绕 GLM-4.5 调整模型相关实现，标题为「Register allgather/reducescatter buffers with symm memory」，变更集中在 `python/sglang/srt/layers/quantization/modelopt_quant.py`, `python/sglang/srt/layers/dp_attention.py`, `python/sglang/srt/layers/moe/topk.py`。PR 描述补充为：## Motivation Rebase version of https://github.com/sgl-project/sglang/pull/9358 on top main (including https://github.com/sgl-project/sglang/pull/12524) ## Modifications ## Accu...
...
- 动机: 该 PR 围绕 GLM-4.5 补齐模型支持入口或运行时能力，标题为「Refactor KTransformers heterogeneous compute with unified GPU-quantization backend」，变更集中在 `python/sglang/srt/layers/quantization/compressed_tensors/compressed_tensors_moe.py`, `python/sglang/srt/layers/moe/kt_ep_wrapper.py`, `python/sglang/srt/layers/moe/fused_moe_triton/layer.py`。PR 描述补充为：## Motivation Replace scattered, hard-coded paths and env-var checks with one clean architecture that can load any GPU quant method. ## Modifications Introduced KTEPWrapperMetho...
...
- 动机: 该 PR 围绕 GLM-4.5 补齐模型支持入口或运行时能力，标题为「clean redundant code in previous PR」，变更集中在 `python/sglang/srt/models/glm4_moe.py`。PR 描述补充为：## Motivation #12834 add some redundant code in python/sglang/srt/models/glm4_moe.py. This PR remove it. ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Chec...
...
- 动机: 该 PR 围绕 GLM-4.5 补齐模型支持入口或运行时能力，标题为「[fused-moe] Add TP2 RTX Pro 6000 for GLM-4.5-Air and GLM-4.5V」，变更集中在 `python/sglang/srt/layers/moe/fused_moe_triton/configs/triton_3_4_0/E=128,N=704,device_name=NVIDIA_RTX_PRO_6000_Blackwell_Workstation_Edition,dtype=fp8_w8a8.json`, `python/sglang/srt/layers/moe/fused_moe_triton/configs/triton_3_4_0/E=129,N=704,device_name=NVIDIA_RTX_PRO_6000_Blackwell_Workstation_Edition,dtype=fp8_w8a8.json`, `python/sglang/srt/layers/moe/fused_moe_triton/configs/triton_3_5_1/E=128,N=704,device_name=NVIDIA_RTX_PRO_6000_Blackwell_Workstation_Edition,dtype=fp8_w8a8,per_channel_quant=True.json`。PR 描述补充为：## Motivation Support GLM-4.5-Air and GLM-4.5V with 2x RTX Pro 6000 ## Modifications Added a new fused_moe_triton config ## Checklist - [x] Format your code according to the For...
...
- 动机: 该 PR 围绕 GLM-4.5 优化关键推理路径或后端选择，标题为「Overlap glm moe gemms in two cuda streams」，变更集中在 `python/sglang/srt/models/glm4_moe.py`。PR 描述补充为：## Modifications Before this pr After this pr ## Accuracy Tests ## Benchmarking and Profiling Launch Bench serving before this pr: 60.40 token/s, after this pr: 66.31 token/s ##...
...
- 动机: 该 PR 围绕 GLM-4.5 补齐模型支持入口或运行时能力，标题为「Feat: GLM-4.6 supports shared experts fusion」，变更集中在 `python/sglang/srt/layers/moe/fused_moe_triton/configs/triton_3_4_0/E=161,N=192,device_name=NVIDIA_H200,dtype=fp8_w8a8,per_channel_quant=True.json`, `python/sglang/srt/models/glm4_moe.py`, `python/sglang/srt/layers/moe/fused_moe_triton/fused_moe_triton_config.py`。PR 描述补充为：Hi from novita.ai team 👋 ## Motivation Fuse shared experts with routed experts for better performance. ## Modifications The changes involve modifying the model initialization to...
...
- 动机: 该 PR 围绕 GLM-4.5 补齐模型支持入口或运行时能力，标题为「[GLM-4.7] GLM-4.7 Tool Parser and Doc Update」，变更集中在 `test/registered/function_call/test_function_call_parser.py`, `python/sglang/srt/function_call/glm47_moe_detector.py`, `python/sglang/srt/function_call/glm4_moe_detector.py`。PR 描述补充为：Support for GLM-4.7 model's Tool Parser and partial documentation
...
- 动机: 该 PR 围绕 GLM-4.5 补齐模型支持入口或运行时能力，标题为「[Feature] Enable return routed experts」，变更集中在 `python/sglang/srt/layers/moe/routed_experts_capturer.py`, `python/sglang/srt/model_executor/model_runner.py`, `python/sglang/srt/managers/detokenizer_manager.py`。PR 描述补充为：## Motivation As per the request from the RL community, this PR enables sglang to return routed experts (topk) during fwd for later usage in training phase. Thanks the MiMo team...
...
- 动机: 该 PR 围绕 GLM-4.5 修复已暴露的启动、加载、解析或数值问题，标题为「Fix GLM-4.7 MoE Detector complex JSON Schema type parsing」，变更集中在 `test/registered/function_call/test_glm47_moe_detector.py`, `python/sglang/srt/function_call/utils.py`, `python/sglang/srt/function_call/glm47_moe_detector.py`。PR 描述补充为：- Modify get_argument_type function to correctly parse complex JSON Schema structures including anyOf/oneOf/allOf and OpenAI-style array types like ["string", "null"] - Improve...
...
- 动机: 该 PR 围绕 GLM-4.5 补齐模型支持入口或运行时能力，标题为「Add GLM45 tool interruption support」，变更集中在 `test/registered/parser/test_reasoning_parser.py`, `python/sglang/srt/parser/reasoning_parser.py`。PR 描述补充为：# Motivation This PR adds support for GLM-4.5 models where tool calls can interrupt reasoning blocks without an explicit closing tag. Specifically, GLM-4.5 uses the pattern ` .....
...
- 动机: 该 PR 围绕 GLM-4.5 补齐模型支持入口或运行时能力，标题为「fix(serving_responses): check enable_thinking for qwen3/glm45 models」，变更集中在 `python/sglang/srt/layers/attention/flashattention_backend.py`, `python/sglang/srt/entrypoints/openai/serving_responses.py`, `PR_DESCRIPTION.md`。PR 描述补充为：## Motivation For reasoning-capable models like Qwen3, GLM45, Nemotron-3, and InternS1, the /v1/responses endpoint was not properly checking the enable_thinking parameter when d...
...
- 动机: 该 PR 围绕 GLM-4.5 修复已暴露的启动、加载、解析或数值问题，标题为「Fix: forward continue_final_message kwargs in Glm45Detector」，变更集中在 `test/registered/unit/parser/test_reasoning_parser.py`, `python/sglang/srt/parser/reasoning_parser.py`。PR 描述补充为：## Motivation `Glm45Detector.__init__` does not accept `continue_final_message` or `previous_content`, while `ReasoningParser.__init__` passes these kwargs whenever the request...
