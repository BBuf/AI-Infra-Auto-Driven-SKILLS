...
- Git-traced PRs: 128
- Extra PRs preserved from existing docs: 99
- Total PRs in this document: 227
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "AMD DeepSeek_V3 FP8 Numerical fix". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation Together with core changes from: https://github.com/sgl-project/sglang/pull/2637 ## Modifications Several places in deepseek_v2.py needs extra normalization to wei...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Feature, Hardware] Enable DeepseekV3 on AMD GPUs". The diff centers on `python/sglang/srt/layers/moe/fused_moe_triton/fused_moe.py`, `python/sglang/srt/layers/attention/triton_ops/decode_attention.py`. PR body context: ## Motivation - Support DeepseekV3 on AMD Instinct MI300X GPU ## Modifications - Add proper fix for AMD FP8 to support DeepseekV3 FP8 model - Bypass to cast FP8 to BF16 in MLA -...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Feature/docs deepseek usage and add multi-node". The diff centers on the file-level diff. PR body context: ## Motivation People are using the Deepseek V3 model, our guidance are not centralised for the ease of usage. ## Modifications Update: docs\references\deepseek.md Update: docs\r...
...
- Motivation: For DeepSeek V3/R1, this PR extends deployment docs, tests, or CI coverage. Title: "refine deepseek_v3 launch server doc". The diff centers on `benchmark/deepseek_v3/README.md`. PR body context: RT
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `70f894b810c0`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "feat: support flashinfer mla attention for deepseek v3". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation Kudos to @yzh119 Throughout the integration process, we have identified and resolved numerous issues with the exceptional support from the FlashInfer team. Current...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Support NextN (MTP) speculative decoding for DeepSeek-V3/R1". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation We implemented NextN (MTP) speculative decoding for DeepSeek-V3/R1 based on EAGLE 2 on Triton backend (https://github.com/sgl-project/sglang/pull/3466) and achieve...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `b110084654a1`
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "Refactor flashinfer logic for deepseek v3 and fix accuracy bug". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation `flashinfer_backend.py` for attention is too complex, this PR extract the logic of MLA and creates a new `flashinfer_mla_backend.py` Also, #3716 #3751 reports an a...
...
- Status/date: merged / 2025-02-24
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `1a6e97577acb`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Feature DeepSeek V3/R1 INT8 Quantization (block-wise)". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation Support block-wise INT8 quantization for DeepSeek V3/R1. INT8 is a friendly type for most hardware platforms. ## Modifications - Fork FP8 implementation and suppor...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `6ce9dbe82882`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[ROCm] Enable Fused MLA Triton kernel for DeepSeekV3". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: This PR introduces the concept of fusing MLA rope into the grouped attention on ROCm. To use this feature use the env variable : SGLANG_ROCM_FUSED_DECODE_MLA=1. Triton Kernel au...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "add deepgemm and sglang fp8 block-wise gemm benchmark". The diff centers on `benchmark/kernels/deepseek/benchmark_deepgemm_fp8_gemm.py`, `benchmark/kernels/deepseek/README.md`. PR body context: The script benchmark all FP8 w8a8 block-wise matrix multiplications involved in DeepSeek V3/R1 under the current tensor parallelism (TP) setting with DeepGemm and vLLM/SGLang. *...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `c7f254468fca`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Feature] DeepSeek V3/R1 INT8 Quantization (channel-wise)". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation Support channel-wise INT8 quantization for DeepSeek V3/R1. INT8 is a friendly type for most hardware platforms. ## Modifications Co-author: @yych0745 @sleepcoo @b0...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Support nextn for flashinfer mla attention backend". The diff centers on `python/sglang/srt/layers/attention/flashinfer_mla_backend.py`, `python/sglang/srt/models/deepseek_v2.py`, `test/srt/test_mla_flashinfer.py`. PR body context: ## Motivation Support the compatibility of nextn and flashinfer mla attention backend. Currently topk can only be set to 1 due to lack of custom mask support for flashinfer MLA...
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "DeepGemm integrate to sgl-kernel". The diff centers on `sgl-kernel/tests/test_deep_gemm.py`, `sgl-kernel/setup.py`, `sgl-kernel/build.sh`. PR body context: ## Motivation Integrate DeepGemm in setup. Linear usage: #4199 . ## Modifications ## Checklist - [x] Format your code according to the Code Formatting with Pre-Commit. - [x] Add...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "linear support deepgemm". The diff centers on `python/sglang/srt/layers/quantization/fp8_kernel.py`, `python/sglang/test/test_block_fp8.py`, `test/srt/test_fp8_kernel.py`. PR body context: ## Motivation Use deepgemm linear gemm, dependent on https://github.com/sgl-project/sglang/pull/4165 ## Test case **MMLU Average accuracy: 0.878** ### performence test size | or...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "add INT8 example into dsv3 README". The diff centers on `benchmark/deepseek_v3/README.md`. PR body context: ## Motivation add INT8 example into benchmark/deepseek_v3/README.md ## Modifications ## Checklist - [x] Format your code according to the Code Formatting with Pre-Commit. - [x]...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Support FlashMLA backend". The diff centers on `python/sglang/srt/layers/attention/flashmla_backend.py`, `python/sglang/srt/layers/attention/utils.py`, `python/sglang/srt/model_executor/model_runner.py`. PR body context: ## Motivation Integrate flashmla for decoding, and the accuracy test is currently okay. The current implementation is quite simple, directly integrating flashmla as the backend....
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Support FlashMLA backend cuda graph". The diff centers on `python/sglang/srt/layers/attention/flashmla_backend.py`, `python/sglang/srt/layers/attention/utils.py`, `python/sglang/srt/server_args.py`. PR body context: ## Motivation Support FlashMLA backend cuda graph. Optimize index calculation, complete the calculation in init_forward ## Modifications * Optimize the FlashInfer block table ca...
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "Set deepgemm to the default value in the hopper architecture.". The diff centers on `python/sglang/srt/layers/quantization/fp8_kernel.py`, `python/sglang/srt/utils.py`. PR body context: ## Motivation Set deepgemm to the default value in the hopper architecture. ## Modifications
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[ROCm] Enable MTP (NextN) on AMD GPU". The diff centers on `sgl-kernel/csrc/speculative/pytorch_extension_utils_rocm.h`, `sgl-kernel/csrc/torch_extension_rocm.cc`, `python/sglang/srt/speculative/build_eagle_tree.py`. PR body context: ## Motivation To enable MTP (NextN) on AMD GPU. ## Modifications Added a few kernels to `torch_extension_rocm.cc`. Enabled MTP test for AMD GPU. ## Benchmark result ## Correctne...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Feature] Support FA3 backend for MLA". The diff centers on `python/sglang/srt/layers/attention/flashattention_backend.py`, `python/sglang/srt/model_executor/model_runner.py`, `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation Support FlashAttention 3 backend for MLA. FA3 official example of MLA can be found in this file. TODO: - [x] Cuda Graph - [x] Benchmark Results (page_size=1) Maybe...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Add deepseek style fused moe group gate selection kernel". The diff centers on `sgl-kernel/csrc/moe/moe_fused_gate.cu`, `sgl-kernel/benchmark/bench_moe_fused_gate.py`, `sgl-kernel/tests/test_moe_fused_gate.py`. PR body context: ## Motivation PR adapted and improved from #3191 Rewrite Macro. Extended to support all power of 2 `# expert` & `# expert group`, also all `# topk_group` & `# topk` use cases +...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `924ca7c92c86`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Add DeepSeek V3/R1 shared experts fusion". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation I gain idea mainly from https://github.com/vllm-project/vllm/pull/15502 , thanks for the author's work.I will add references in the modifications for `grouped_topk...
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "reduce moe_align_block_size_kernel small batch mode overhead". The diff centers on `python/sglang/srt/layers/moe/fused_moe_triton/fused_moe.py`, `sgl-kernel/csrc/moe/moe_align_kernel.cu`, `sgl-kernel/benchmark/bench_moe_align_block_size.py`. PR body context: ## Motivation - [x] Fix `moe_align_block_size` kernel benchmark bug. - [x] Reduce moe_align_block_size_kernel small batch mode ovehead caused by `torch.zeros` . - [x] After this...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "fix: use deepgemm only on hopper". The diff centers on `python/sglang/srt/layers/quantization/fp8_kernel.py`. PR body context: ## Motivation ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests as outlined in the Running Unit Tests....
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "feat: use fa3 mla by default on hopper". The diff centers on `python/sglang/srt/model_executor/model_runner.py`, `python/sglang/srt/layers/attention/flashattention_backend.py`, `python/sglang/srt/utils.py`. PR body context: ## Motivation ## Modifications - Support DP MLA for FA3 ## Benchmark ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests a...
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "Apply deepseek cuda rope". The diff centers on `python/sglang/srt/layers/rotary_embedding.py`. PR body context: ## Motivation The torch.compile issue described in #4959 arises from overriding the `forward` method of `CustomOp`. When torch.compile is applied, the `_forward_method` of `Cust...
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "apply fused moe gate in ds v3/r1". The diff centers on `python/sglang/srt/layers/moe/topk.py`. PR body context: ## torch profile ### main branch !图片 ### pr !图片 Only one kernel now. 36us->8us. ## fused_moe_gate gsm8k acc test ## fused moe gate performance(in H200) |qps|Input token throughp...
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Fix] Turn off DeepGEMM by default". The diff centers on `python/sglang/srt/layers/quantization/fp8_kernel.py`, `docs/references/deepseek.md`. PR body context: ## Motivation Currently DeepGEMM is turned on by default for deepseek models, which caused confusions (#5223) on its performance due to warmup time. This PR makes DeepGEMM turne...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "kernel: support slightly faster merge_state_v2 cuda kernel". The diff centers on `sgl-kernel/tests/test_merge_state_v2.py`, `sgl-kernel/csrc/attention/merge_attn_states.cu`, `sgl-kernel/python/sgl_kernel/attention.py`. PR body context: support slightly faster merge_state_v2 cuda kernel - [x] float32 - [x] float16 - [x] bfloat16 - [x] dispatch by scalar_t - [x] unit tests (performance & correctness) - [x] suppo...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Support MHA with chunked prefix cache for DeepSeek chunked prefill". The diff centers on `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/model_executor/forward_batch_info.py`, `python/sglang/srt/layers/attention/flashattention_backend.py`. PR body context: ## Motivation The current implementation of MLA is slow when when handling long prefix lengths, such as sequences with 32k input length, as highlighted in issues like #5031. Pro...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `53dcf3887639`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Introduce moe_dense_tp_size to fix dense layer errors in DeepSeek V3 + 4x8xH100". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: This code depends on https://github.com/sgl-project/sglang/pull/5190 and https://github.com/sgl-project/sglang/pull/5205 and please subtract diff from there ## Speed tests 4x8xH...
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "use sglang_per_token_group_quant_fp8 from sgl-kernel instead of trion kernel". The diff centers on `python/sglang/srt/layers/quantization/fp8_kernel.py`, `python/sglang/srt/layers/quantization/fp8_utils.py`. PR body context: ## Motivation This pr implemented a faster kernel for per_token_group_quant_fp8, which can replace triton kernel. https://github.com/sgl-project/sglang/pull/4817 I tested the ke...
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "Remove one kernel in per_tensor_quant_mla_fp8". The diff centers on `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/quantization/fp8_kernel.py`, `python/sglang/srt/models/deepseek_nextn.py`. PR body context: ## Motivation Thanks @Alcanderian for discussing it is acceptable to change APIs in the caller site ### Accuracy PR: 93.3 (baseline: roughly 93 when I tested in https://github.c...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[perf] introduce deep gemm group_gemm_masked as bmm". The diff centers on `python/sglang/srt/layers/quantization/fp8_kernel.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/test/test_block_fp8.py`. PR body context: ## Motivation per-token-group quant+deep_gemm's grouped_gemm_masked is generally faster than per-tensor quant+bmm_fp8 NOTE: Multi-batch throughput is hardly comparable because e...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "enable DeepSeek V3 shared_experts_fusion in sm90". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## H200 Benchmark I tested the benchmark using the command provided by https://github.com/sgl-project/sglang/issues/5514. To avoid warmup, I turned off deepgemm. Below are the r...
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "Remove extra copy in deepseek forward absorb". The diff centers on `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/rotary_embedding.py`, `.github/workflows/pr-test-amd.yml`. PR body context: ## Benchmark ### Performance main branch: this PR: 0.5% improvement for bs 1 and 1.5% improvement for bs 32. main branch: this PR: removed 3 copy with 2 cat added (can overlap o...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Turn on DeepGemm By Default and Update Doc". The diff centers on `python/sglang/srt/layers/quantization/deep_gemm.py`, `docs/references/deepseek.md`. PR body context: ## Motivation Followup of #5580 ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests as outlined in the Ru...
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "Fuse q_a_proj and kv_a_proj for DeepSeek models". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation In self attention module of DeepSeek v3, the `q_a_proj` and `kv_a_proj` both takes hidden state as input, so they can be fused into one module, and one launching o...
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[BugFix] Fix combination of MTP and `--n-share-experts-fusion`with R1". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation We found bugs while using MTP and `-n-share-experts-fusion` together. 1. While using `--n-share-experts-fusion ${TP_SIZE}` with MTP, there will be weight loading e...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "update triton 3.2.0 h200 fused moe triton config and add warning about triton fused_moe_kernel performance degradation due to different Triton versions.". The diff centers on `python/sglang/srt/layers/moe/fused_moe_triton/configs/E=264,N=256,device_name=NVIDIA_H200,dtype=fp8_w8a8,block_shape=[128, 128].json`, `python/sglang/srt/layers/moe/fused_moe_triton/fused_moe.py`. PR body context: ## Motivation Reproduction commands : ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests as outlined in...
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "perf: update H20 fused_moe_triton kernel config to get higher throughput during prefilling". The diff centers on `python/sglang/srt/layers/moe/fused_moe_triton/configs/E=272,N=128,device_name=NVIDIA_H20,dtype=fp8_w8a8,block_shape=[128, 128].json`. PR body context: ## Motivation We have provided fused_moe_triton config in PR(https://github.com/sgl-project/sglang/pull/5641) to improve model performance. However, we found that after using th...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Fuse MLA set kv cache kernel". The diff centers on `python/sglang/srt/layers/attention/flashattention_backend.py`, `python/sglang/srt/layers/radix_attention.py`, `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation Fuse MLA set kv cache kernel and remove k concat operation. Currently only support FA3 backend. Can be applied to other backend with subsequent verification. ## Be...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Add Cutlass MLA attention backend". The diff centers on `python/sglang/srt/layers/attention/cutlass_mla_backend.py`, `python/sglang/srt/model_executor/model_runner.py`, `python/sglang/srt/layers/attention/utils.py`. PR body context: ## Motivation Enables use of the blackwell cutlass MLA decode kernel with deepseek models. ## Modifications Adds "cutlass_mla" option for attention backend. ## Deepseek-R1 Bench...
...
- Motivation: For DeepSeek V3/R1, this PR changes model-related implementation. Title: "Auto set draft model path for MTP". The diff centers on `python/sglang/srt/models/deepseek_nextn.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/model_executor/model_runner.py`. PR body context: ## Motivation - load nextn layer weights directly from target model - refactor to reuse common weight loading logic for draft model and target model - auto set draft model path...
...
- Motivation: For DeepSeek V3/R1, this PR extends deployment docs, tests, or CI coverage. Title: "Update ci test and doc for MTP api change". The diff centers on `test/srt/test_mla_deepseek_v3.py`, `python/sglang/srt/server_args.py`, `docs/references/deepseek.md`. PR body context: ## Motivation Update ci test and doc for MTP api change.
...
- Trace source: `git log --name-only -- <model-files>` found it through `examples/chat_template/tool_chat_template_deepseekv3.jinja`; associated commits `170d1f218a4e`
...
- Motivation: For DeepSeek V3/R1, this PR changes model-related implementation. Title: "feat: Refactor DeepSeekV3 function call". The diff centers on `examples/chat_template/tool_chat_template_deepseekv3.jinja`. PR body context: ## Motivation Instead of keeping editing the prompt engineering for `deepseekv3` in `adapter.py`, this PR provides a chat template instead. Implements: https://github.com/sgl-pr...
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "Overlap qk norm with two streams". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation Overlap qk rms norm with two streams in cuda graph. ### Accuracy ### Performance main branch: this PR: ### Profile main branch: this PR: 23.9μm -> 21.5μm
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "feat: flashinfer_mla speculative decoding with custom mask". The diff centers on `python/sglang/srt/layers/attention/flashinfer_mla_backend.py`, `test/srt/test_mla_flashinfer.py`, `python/sglang/srt/speculative/eagle_worker.py`. PR body context: status: not ready. dependent on https://github.com/flashinfer-ai/flashinfer/pull/1017. ## Motivation Support top_k > 1 Eagle speculative decoding with flashinfer MLA backend on...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Update doc for MLA attention backends". The diff centers on `docs/references/deepseek.md`, `docs/backend/server_arguments.md`. PR body context: ## Motivation ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests as outlined in the Running Unit Tests....
...
- Status/date: merged / 2025-05-08
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `73600673bb1d`
...
- Motivation: For DeepSeek V3/R1, this PR changes model-related implementation. Title: "Clean logs for DeepSeek-V3 launching". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation When launching Deepseek-V3 model, some of the logs only need to be printed once (on tp_rank=0). ## Modifications This PR adds a `should_rank` util function. And pr...
...
- Status/date: merged / 2025-05-08
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `5e02330137a1`
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "[perf] dsv3 bmm fallback to bf16". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation Align to official impl according to released profile data: https://github.com/deepseek-ai/profile-data/blob/ad8f61ed5e590c4984bb6579f370f9689c427416/decode.json Is...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Feat] Support FlashMLA backend with MTP and FP8 KV cache". The diff centers on `python/sglang/srt/layers/attention/flashmla_backend.py`, `python/sglang/srt/layers/attention/flashinfer_mla_backend.py`, `python/sglang/srt/model_executor/cuda_graph_runner.py`. PR body context: ## Motivation This PR improves flashmla backend by accelerating decode stage with mtp. The implementation utilizes the feature that flashmla can handle `seq_len_q > 1`. To use f...
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "[Feat] optimize Qwen3 on H20 by hybrid Attention Backend". The diff centers on `python/sglang/srt/model_executor/model_runner.py`, `python/sglang/srt/model_executor/cuda_graph_runner.py`, `python/sglang/srt/server_args.py`. PR body context: ## Motivation ### FA3 decode performance is significantly lower than flashinfer on H20 #5630 https://github.com/Dao-AILab/flash-attention/issues/1572 I have profiled the Qwen3-2...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/function_call/deepseekv3_detector.py`; associated commits `461a73028070`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "fix(deepseekv3): Fix DeepSeekV3Detector tool_index assignment and multi-tool call streaming support". The diff centers on `python/sglang/srt/function_call/deepseekv3_detector.py`. PR body context: ## Motivation This PR focuses to fix the current two issues in `DeepSeekV3Detector`. 1. tool_index: Current the tool_index of each ToolCallItem is assigned based on the index of...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Partially supports using MHA kernels in MLA forward when page-size > 1.". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation add Partially support to use MHA kernels when page-size > 1. related to #6411 ## Modifications add AttnForwardMethod.MHA_PAGED_PREFILL, and when any(forward_batch....
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `51cdd81f9720`
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[fix][RL] Fix DeepSeekV3ForCausalLM.post_load_weights for multiple update weight". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation When doing RL, which will call `post_load_weights` a second time apart from the intialization, the origin `post_load_weights` will somehow corrupt the weight when...
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "Use deepgemm instead of triton for fused_qkv_a_proj_with_mqa". The diff centers on `python/sglang/srt/layers/quantization/fp8_utils.py`. PR body context: ## Motivation ~~do not merge now, I will test and check more first~~ Test: * triton: 41us * change to DeepGEMM: 12us thus more than 3x speedup for this kernel change in my scena...
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "Fuse routed scaling factor in topk_reduce kernel". The diff centers on `python/sglang/srt/layers/moe/fused_moe_triton/fused_moe.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/moe/fused_moe_triton/layer.py`. PR body context: ## Motivation ### gsm8k acc in h200 ### benchmark in H200 command: ### Micro benchmark in h200 micro benchmark in h200: And I use the below code to select triton or `torch.compi...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `c2c4f57f6311`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[DeepseekR1-FP4] Add Support for nvidia/DeepSeekR1-FP4 model". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation Adds Model support for DeepSeek R1 FP4 Model. (Functional enablement - kernel optimizations underway) ## Modifications Adds ModelOptFP4FusedMoEMethod and CutlassMo...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Fuse routed scaling factor in deepseek". The diff centers on `python/sglang/srt/layers/moe/fused_moe_triton/fused_moe.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/moe/fused_moe_triton/layer.py`. PR body context: ## Motivation ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests as outlined in the Running Unit Tests....
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "FlowMLA: zero-overhead DP MLA throughput boost via memory optimization". The diff centers on `python/sglang/srt/layers/linear.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/model_executor/cuda_graph_runner.py`. PR body context: ## Motivation This PR is part of our upcoming work FlowMLA @antgroup. Targeting MLA in Data Parallel scenarios, we enable serving DeepSeek-like models on fewer devices through s...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Support new DeepGEMM format in per token group quant". The diff centers on `sgl-kernel/csrc/gemm/per_token_group_quant_8bit.cu`, `sgl-kernel/tests/test_per_token_group_quant_8bit.py`, `sgl-kernel/include/sgl_kernel_ops.h`. PR body context: ## Motivation ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests as outlined in the Running Unit Tests....
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Support new DeepGEMM format in per token group quant (part 2: srt)". The diff centers on `python/sglang/srt/layers/quantization/fp8_kernel.py`, `python/sglang/srt/entrypoints/engine.py`, `python/pyproject.toml`. PR body context: ## Motivation ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests as outlined in the Running Unit Tests....
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Re-quantize DeepSeek model weights to support DeepGEMM new input format". The diff centers on `python/sglang/srt/layers/quantization/fp8_utils.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/math_utils.py`. PR body context: ## Motivation ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests as outlined in the Running Unit Tests....
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Refactor DeepGEMM integration". The diff centers on `python/sglang/srt/layers/quantization/deep_gemm_wrapper/compile_utils.py`, `python/sglang/srt/layers/quantization/deep_gemm_wrapper/entrypoint.py`, `python/sglang/srt/layers/moe/ep_moe/layer.py`. PR body context: ## Motivation ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests as outlined in the Running Unit Tests....
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Support new DeepGEMM". The diff centers on `python/sglang/srt/layers/quantization/deep_gemm_wrapper/entrypoint.py`, `python/sglang/srt/layers/quantization/fp8_kernel.py`, `python/sglang/srt/layers/moe/ep_moe/layer.py`. PR body context: ## Motivation ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests as outlined in the Running Unit Tests....
...
- Status/date: merged / 2025-06-14
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `ed54bf9d19f1`
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[fix] fix dsv3 weight loader tqdm and simplify shared experts fusion". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation Using `weights = [w for w in weights_list if w[0] not in names_to_remove]` to update `weights` will change it from async weight iterator to a list. Thats will make...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "feat: mtp support dp-attention". The diff centers on `python/sglang/srt/models/deepseek_nextn.py`, `python/sglang/srt/model_executor/cuda_graph_runner.py`, `python/sglang/srt/layers/attention/triton_backend.py`. PR body context: ## Motivation mtp support dp-attention - implemented MTP for DP-attention, also fixed related bugs #4783 #4847 . - Enabled CUDA Graph support for both target and draft models at...
...
- Status/date: merged / 2025-06-17
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `8c16da334e93`
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "Fix Deepseek R1 0528 FP4 tensor name mismatch issue during weights loading.". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation This PR tries to fix a weights loading issue for Deepseek R1 0528 Fp4 from Nvidia. To reproduce the issue: (i'm using blackwell image) If we look into the weight f...
...
- Status/date: merged / 2025-06-17
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `4f204db57cf9`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "fix: resolve b200 dsv3 mtp issue". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests as outlined in the Running Unit Tests....
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `a06912ad8b35`
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "Fix judgment condition for enabling Deepseek V3/R1 shared expert fusion optimization". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation ref: https://github.com/sgl-project/sglang/pull/7180 ## Modifications Modify the compatibility version judgment conditions. ## Benchmark The following results were...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `bdbb8d009ae1`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[perf] slightly imporve DeepSeek-R1-FP4 TP8". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation Tuned dual stream kernel order, 79->81 ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests...
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "Fix MTP with Deepseek R1 Fp4". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation https://github.com/sgl-project/sglang/issues/7365 ## Modifications Update quant config to None for MTP module in Deepseek modelopt fp4. It's not quantized. See htt...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `00aec6ad6c34`
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "Apply dsv3_fused_a_gemm kernel". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation Apply `dsv3_fused_a_gemm kernel` in https://github.com/sgl-project/sglang/pull/7630. Profile and accuracy test are already listed in the previous PR. Dependency: r...
...
- Status/date: merged / 2025-07-02
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `88f484ce4c73`
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "Apply dsv3 router gemm kernel for deepseek-r1 fp4". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation Apply kernel added in #7627 to dpsk-r1 fp4 model. Currently we have to convert the dtype of router logits back to bf16, otherwise there will be cuda illegal memory...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `84f2e4a0f847`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "fix awq and dsv3 fused gemm compatible". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation added a check to avoid compatibility issues when loading AWQ weights with dsv3 fused gemm enabled related issue https://github.com/sgl-project/sglang/issues/7728 #...
...
- Status/date: merged / 2025-07-03
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `b58226510f7d`
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "fix dsv3 fused proj check". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation fix dsv3 fused proj check in https://github.com/sgl-project/sglang/pull/7735 ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting...
...
- Status/date: merged / 2025-07-03
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `ac49dac00946`
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[fix] fix dsv3_router_gemm filter". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation fix ci on main: https://github.com/sgl-project/sglang/actions/runs/16045971205/job/45277302233#step:5:3827 ## Modifications ## Checklist - [ ] Format your code acc...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `54411f6afa27`
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "fix: disable dsv3_router_gemm in dsv3_nextn". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation It has been confirmed that using dsv3_router_gemm leads to a degradation in the acc_len of the dsr1-fp4 nextn module.: https://github.com/sgl-project/sglang/pull/7...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "feat: support DeepSeek-R1-W4AFP8 model with ep-moe mode". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation This PR supports running DeepSeek-R1-W4AFP8 model with ep-moe mode(deepep mode support is on the way~) Due to the reduced space required for model weights and decr...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/function_call/deepseekv3_detector.py`; associated commits `624a3b8d1f10`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Fix incomplete tool call capture issue in streaming response of DeepSeek-V3 when enable MTP". The diff centers on `python/sglang/srt/function_call/deepseekv3_detector.py`. PR body context: ## Motivation When using DeepSeeker V3 and enabling MTP and tool call parsing, we found that there are have some unexpected string in the streaming reponse like " \n ". The prob...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `e7e5a3050a64`
...
- Motivation: For DeepSeek V3/R1, this PR changes model-related implementation. Title: "Update batch size limitation of dsv3_router_gemm kernel to 16". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation Raise the batch size limitation to from 4 to 16 for dsv3_router_gemm kernel ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. -...
...
- Trace source: `git log --name-only -- <model-files>` found it through `examples/chat_template/tool_chat_template_deepseekv3.jinja`, `python/sglang/srt/function_call/deepseekv3_detector.py`; associated commits `c9dd70fbde92`
...
- Motivation: For DeepSeek V3/R1, this PR extends deployment docs, tests, or CI coverage. Title: "tool-call(dsv3): Improve deepseek-v3 chat template and tool_choice = `required`". The diff centers on `python/sglang/srt/function_call/deepseekv3_detector.py`, `examples/chat_template/tool_chat_template_deepseekv3.jinja`. PR body context: ## Motivation Improve deepseek-v3 chat template, following #8576 ## Modifications - Improve deepseek-v3 chat template - Remove spaces in the `build_ebnf` ## Accuracy Tests ### A...
...
- Trace source: `git log --name-only -- <model-files>` found it through `examples/chat_template/tool_chat_template_deepseekv31.jinja`, `python/sglang/srt/function_call/deepseekv31_detector.py`; associated commits `b9683be6538e`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Support DeepSeek-V3.1 tool call". The diff centers on `python/sglang/srt/function_call/deepseekv31_detector.py`, `examples/chat_template/tool_chat_template_deepseekv31.jinja`. PR body context: ## Motivation Support tool call for DeepSeek-V3.1 The tool call format of DeepSeek-V3.1 is different from DeepSeek-V3/R1: DeepSeek-V3.1: tool_call_name tool_call_arguments DeepS...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `8abe8deae6cd`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "fix: dsv3 lite q_lora_rank none". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[feat] Support tp mode for DeepSeek-R1-W4AFP8". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation Support tp mode for DeepSeek w4a8 model, which has a better performace than ep mode. ## Modifications 1. Add W4AFp8MoEMethod and associated `create_weights`, `proc...
...
- Trace source: `git log --name-only -- <model-files>` found it through `examples/chat_template/tool_chat_template_deepseekv31.jinja`; associated commits `cc9a31c66226`
...
- Motivation: For DeepSeek V3/R1, this PR extends deployment docs, tests, or CI coverage. Title: "Update tool_chat_template_deepseekv31.jinja". The diff centers on `examples/chat_template/tool_chat_template_deepseekv31.jinja`. PR body context: 增加tojson，解决多轮工具调用报错的问题： https://github.com/sgl-project/sglang/issues/9891 ## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Forma...
...
- Status/date: merged / 2025-09-03
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `0dfd54d11d06`, `1b2ff4fb7f05`
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "Optimized deepseek-v3/r1 model performance on mxfp4 run". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation In order to decrease the activation tensor quantized overhead, fused the quantized behavior into the different ops (activation, layernorm, gemm, flatten) ## Modifi...
...
- Status/date: merged / 2025-09-03
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `1b2ff4fb7f05`
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "Revert "Optimized deepseek-v3/r1 model performance on mxfp4 run (#9671)"". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: This reverts commit 0dfd54d11d06eb8363bc7fb3cf9a1f464368caf8. Because #9671 breaks on B200 ## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Chec...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `e96973742c32`
...
- Status/date: merged / 2025-09-04
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `918e3d4c27c3`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Fix accuracy drop of dsv3 run in dp enablement". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation Got an accuracy issue of dsv3 model run on aiter backend with dp enablement. Issue link: https://github.com/sgl-project/sglang/issues/7692 ## Modifications For ext...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `012584ecd539`
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "perf: Avoid unnecessary data type conversions for DeepSeek-V3 on Blackwell". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: Optimize the performance of DeepSeek-V3 on Blackwell for small batch sizes by - Change the output data type of router GEMM to FP32 - Convert the data type of `correction_bias` i...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `400d3b97aebc`
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "Fix run time error in dsv3-fp8 model on mi35x". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation Got run-time error in running dsv3-fp8 model on mi35x. The error is caused by the activation tensor is changed to tuple of tensors type for the next mxfp4 linear c...
...
- Trace source: `git log --name-only -- <model-files>` found it through `examples/chat_template/tool_chat_template_deepseekv3.jinja`; associated commits `2cd94dd07eea`
- Diff scope read: GitHub Pull Request files API returned 1 files, +1/-1, 9 readable patch lines; this card prioritizes model-related and high-change files.
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "tool-call(dsv3): Fixed a parse problem when there are multiple function definitions in tool_calls". The diff centers on `examples/chat_template/tool_chat_template_deepseekv3.jinja`. PR body context: ## Motivation When there are multiple function definitions in tool_calls, an error will be reported **TypeError: can only concatenate str (not "dict") to str** ## Modifications...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Fix GPU fault issue when run dsv3 with dp mode and enable torch-compile". The diff centers on `python/sglang/srt/layers/dp_attention.py`, `python/sglang/srt/layers/logits_processor.py`. PR body context: ## Motivation Got GPU fault when running "python3 -m sglang.launch_server --model-path DeepSeek-V3.1 --tp 8 --trust-remote-code --chunked-prefill-size 131072 --dp-size 8 --enabl...
...
- Trace source: `git log --name-only -- <model-files>` found it through `examples/chat_template/tool_chat_template_deepseekv32.jinja`; associated commits `85c1f7937781`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Add DeepSeek-V3.2 Tool Call Template". The diff centers on `examples/chat_template/tool_chat_template_deepseekv32.jinja`. PR body context: ## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Support DeepSeek V3.2 Exp". The diff centers on `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/tilelang_kernel.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`. PR body context: We now support DeepSeek V3.2. Try this model in the tracking issue #11060 --- Authors: General: @fzyzcjy @DarkSharpness @hnyls2002 @hebiao064 @Fridge003; AMD: @HaiShaw @kkHuang-...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Update DeepSeek-R1-FP4 default config on blackwell". The diff centers on `python/sglang/srt/server_args.py`. PR body context: ## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`; associated commits `384733639a91`
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "[DSv32] Use torch.compile for _get_logits_head_gate". The diff centers on `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`. PR body context: ## Motivation fuse 2 unary mul + 1 binary mul in _get_logits_head_gate. ## Modifications ## Accuracy Tests ## Benchmarking and Profiling 1.8% e2e total throughput With torch.com...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[1/N]Support DeepSeek-R1 w4a8 normal deepep". The diff centers on `python/sglang/srt/layers/moe/cutlass_w4a8_moe.py`, `python/sglang/srt/layers/quantization/w4afp8.py`, `python/sglang/srt/layers/moe/ep_moe/layer.py`. PR body context: ## Motivation Support deepep normal mode for DeepSeek w4a8 model with @yangsijia-serena @rainj-me ## Modifications add forward_cutlass_w4a8 for deepep normal mode ## Command `SG...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `3cceaa381ad3`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Bugfix] Fix Qwen3/DSV3/DSV3.2 model support". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation This pr fixes very models that failed to start on npu after dsv3.2 support pr was merged. ci and image release infra was also improved. ## Modifications - Fix mla...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa/utils.py`; associated commits `20b8d2306c3d`
...
- Motivation: For DeepSeek V3/R1, this PR extends deployment docs, tests, or CI coverage. Title: "Cleaning indexer for DeepSeek V3.2". The diff centers on `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa/utils.py`. PR body context: ## Motivation - Fake indexer is for testing accuracy during early development, so can be deprecated now. - Cleaning forward logic. ## Modifications ## Accuracy Tests ## Benchmar...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`; associated commits `efa473348bc9`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Spec Decoding] Support MTP for dsv3.2". The diff centers on `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`. PR body context: ## Motivation Based on https://github.com/sgl-project/sglang/pull/11109 We have implemented MTP support for DS v3.2 and ***cuda graph*** in our in-house maintained version of sg...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[1/2] deepseek deterministic: support deterministic inference for deepseek arch models on a single GPU". The diff centers on `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/batch_invariant_ops/batch_invariant_ops.py`, `python/sglang/srt/server_args.py`. PR body context: ## Motivation Part of this Issue: https://github.com/sgl-project/sglang/issues/10278 As part of deepseek deterministic inference support, this change ensures deterministic infer...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[2/N]Support DeepSeek-R1 w4a8 low latency deepep". The diff centers on `python/sglang/srt/layers/moe/ep_moe/kernels.py`, `python/sglang/srt/layers/moe/cutlass_w4a8_moe.py`, `python/sglang/srt/layers/quantization/w4afp8.py`. PR body context: ## Motivation Follow https://github.com/sgl-project/sglang/pull/8247 https://github.com/sgl-project/sglang/pull/7762. Based on https://github.com/sgl-project/sglang/pull/8311. S...
...
- Status/date: merged / 2025-10-25
- Trace source: `git log --name-only -- <model-files>` found it through `docs/references/multi_node_deployment/rbg_pd/deepseekv32_pd.md`; associated commits `729b242934cb`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Doc] Add documentation for DeepSeek V3.2". The diff centers on `docs/references/multi_node_deployment/rbg_pd/deepseekv32_pd.md`. PR body context: Depends on #11844 and #11876 Also waiting for @whybeyoung on some PD updates ## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Fo...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[doc] add example of using w4fp8 for Deepseek". The diff centers on `benchmark/deepseek_v3/README.md`. PR body context: ## Motivation This PR adds a basic example of deploying w4fp8 Deepseek using SGLang. Below are the evaluation of accuracy. | Model | MMLU | |-------|------| | novita/Deepseek-V3...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Support running FP4 Deepseek on SM120.". The diff centers on `python/sglang/srt/layers/quantization/modelopt_quant.py`, `python/sglang/srt/models/gpt_oss.py`, `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation sm120 is not runnable for deepseek because of sm check and invalid kernels. ## Modifications - add `is_blackwell_supported` as an extend to `is_sm100_supported` -...
...
- Status/date: merged / 2025-10-28
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/dequant_k_cache.py`, `python/sglang/srt/layers/attention/nsa/quant_k_cache.py`, `python/sglang/srt/layers/attention/nsa_backend.py`; associated commits `81a632ace647`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[DeepseekV32] Enable flashmla_prefill kernel with fp8 kvcache". The diff centers on `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/dequant_k_cache.py`, `python/sglang/srt/layers/attention/nsa/quant_k_cache.py`. PR body context: ## Motivation Add logics to dequant the kvcache from fp8 to bf16 in a separate kernel and use `flashmla_prefill` kernel with fp8 kvcache. - [x] Verify accuracy and perf on both...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa_backend.py`; associated commits `42e1a72efb78`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Deepseek V3.2] Enable flashmla_auto with MTP". The diff centers on `python/sglang/srt/layers/attention/nsa_backend.py`. PR body context: ## Motivation Follow up to https://github.com/sgl-project/sglang/pull/11655 ## Modifications With fp8 kvcache on B200, flashmla_sparse can be used in the extend (not draft_exten...
...
- Status/date: merged / 2025-10-29
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `e39628fd07b3`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[2/2] Deepseek deterministic: support deepseek v3 deterministic inference on 8 x H200". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation part of roadmap Previous PR supported deepseek arch model's deterministic inference on a single Hopper GPU. This PR is to further support full deepseek v3 model's...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/models/deepseek_v2.py`; associated commits `9ff9fa7f95be`
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "Fuse wk and weight_proj in Indexer for DeepSeekV3.2-FP4". The diff centers on `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation Since wk and weight_proj in Indexer have the same input, they can be fused into a single gemm by concatenating weights. For the base DeepSeekV3.2-Exp model, wk is...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/transform_index.py`; associated commits `662725b936a2`
- Diff scope read: GitHub Pull Request files API returned 1 files, +1/-1, 9 readable patch lines; this card prioritizes model-related and high-change files.
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[DeepSeekV32] Bug fix to ensure `page_table` and `result` in same type". The diff centers on `python/sglang/srt/layers/attention/nsa/transform_index.py`. PR body context: Running transform_index.py in NSA attention path fails at torch.gather(...) with a dtype mismatch: the index/out tensor is int (int32) while PyTorch requires Long (int64) for ga...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/models/deepseek_v2.py`; associated commits `f235498eca7a`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "DeepSeek-V3.2: Add Adaptive MHA Attention Pathway for Short-Sequence Prefill". The diff centers on `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation For DeepSeek-V3.2 models, using MLA (Multi-Latent Attention) uniformly across all sequence lengths during prefill is suboptimal. For short sequences, the overhead...
...
- Motivation: For DeepSeek V3/R1, this PR changes model-related implementation. Title: "Update dsv3 quantization auto setting for sm100". The diff centers on `python/sglang/srt/server_args.py`. PR body context: ## Motivation update quantization based on model quant_method
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/models/deepseek_v2.py`; associated commits `7257525ccea6`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[DeepSeek-V3.2][NSA] Enable MHA Pathway for Short Sequence Prefill on B200 (SM100)". The diff centers on `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation Follow up this PR: DeepSeek-V3.2: Add Adaptive MHA Attention Pathway for Short-Sequence Prefill. Enable and optimize MHA on B200 (SM100) in the NSA backend by TRT-...
...
- Status/date: merged / 2025-11-07
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/index_buf_accessor.py`; associated commits `125f76ea44d8`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Test] Add DeepSeekV3.2 NSA Indexer Test Suite". The diff centers on `python/sglang/srt/layers/attention/nsa/index_buf_accessor.py`. PR body context: ## Motivation Address the issue https://github.com/sgl-project/sglang/issues/12509 ## Modifications Added 11 tests: 1. test_forward_decode_mode 2. test_forward_extend_mode 3. te...
...
- Status/date: merged / 2025-11-07
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/models/deepseek_v2.py`; associated commits `bef37d6de86a`
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Deepseek V3.2] Only skip Indexer logits computation when is_extend_without_speculative". The diff centers on `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`. PR body context: ## Motivation Fixed a bug in https://github.com/sgl-project/sglang/pull/12788 ## Modifications We should only skip Indexer logits computation when forward_mode.is_extend_without...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `4a78031a71dd`
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "[ROCM] Optimized deepseek-r1 model with rmsnorm + fp8 quant fusion". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation Fusion of rmsnorm + group scale FP8 quantization (group_size=128) w/o fusion w/ fusion ## Modifications deepseek.py - LayerCommunicator (for hidden states) - Deeps...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`; associated commits `0d4a41842424`
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Deepseek V3.2] Fix accuracy bug in the Indexer". The diff centers on `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`. PR body context: ## Motivation Must merge after https://github.com/sgl-project/sglang/pull/12582 Part 2 of the fix for https://github.com/sgl-project/sglang/issues/11629 This PR also fixed the m...
...
- Status/date: merged / 2025-11-12
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa_backend.py`; associated commits `4eda9969e8b9`
...
- Motivation: For DeepSeek V3/R1, this PR extends deployment docs, tests, or CI coverage. Title: "[DeepseekV32]: use `_concat_mla_absorb_q_general` to replace `torch.cat`". The diff centers on `python/sglang/srt/layers/attention/nsa_backend.py`. PR body context: ## Motivation https://github.com/sgl-project/sglang/issues/11989 `torch.cat([q_nope, q_rope], dim=-1)` is heavily used in `nsa_backend`, which is less efficient. ## Modification...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/function_call/deepseekv31_detector.py`; associated commits `fc5da1e80b78`
...
- Motivation: For DeepSeek V3/R1, this PR changes model-related implementation. Title: "[Tool Call] Steamline function arguments when tool_choice="auto" for deepseekv31_detector". The diff centers on `python/sglang/srt/function_call/deepseekv31_detector.py`. PR body context: ## Motivation The current `deepseekv31_detector` only detects complete function arguments and immediately returns them while clearing the buffer. This approach can lead to a poo...
...
- Status/date: merged / 2025-11-14
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`; associated commits `a7002e614bbd`
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Deepseek V3.2] Clean up MTP". The diff centers on `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`. PR body context: ## Modifications - Fix an accuracy bug in MTP draft_extend stage. The `seqlens_int32_cpu` used in the calculations of `seqlens_expanded` is incorrect. We should use `seq_lens_cp...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "(1/n)support context parallel with deepseekv3.2-DSA". The diff centers on `python/sglang/srt/layers/attention/nsa/utils.py`, `python/sglang/srt/layers/communicator_nsa_cp.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`. PR body context: ## Motivation Currently, under deepseek3.2-DSA, prefill-ttft of long text sequences takes a long time. Introducing context parallel can reduce ttft. **Main design ideas：** Takin...
...
- Status/date: merged / 2025-11-17
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa_backend.py`; associated commits `a8fcbf6fe3a2`
...
- Motivation: For DeepSeek V3/R1, this PR changes model-related implementation. Title: "[Deepseek V3.2] Use torch.compile to speed up torch.cat in nsa". The diff centers on `python/sglang/srt/layers/attention/nsa_backend.py`. PR body context: ## Motivation Use torch.compile to speed up torch.cat used in prefill. ## Modifications Replace the cat ops for k in `forward_extend` in the nsa attention with the torch.compile...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `e389f91decda`
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[NVIDIA] Fix broken fp8 MoE of deepseek v3". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation This breaks the fp8 moe of the deepseek v3, causing: the root cause is now the routing_method_type becomes a default attribute of FusedMoE layer. So, we should exp...
...
- Trace source: preserved from an explicit existing history/skill citation
- Diff scope read: GitHub Pull Request files API returned 1 files, +1/-0, 8 readable patch lines; this card prioritizes model-related and high-change files.
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Fix] Fix DeepSeek V3 MTP on B200". The diff centers on `python/sglang/srt/models/deepseek_nextn.py`. PR body context: ## Motivation Fix the bug here https://github.com/sgl-project/sglang/actions/runs/19449452397/job/55665254802 ## Modifications ## Accuracy Tests Test with GSM8K ## Benchmarking...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `c8ede0e93c3a`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[ROCM] Optimized deepseek-r1 fp8 model with + triton_gemm_a8w8 + batch_gemm_a8w8 + fused set_mla_kv_buffer kernel". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation Co-author : @yichiche To optimize the deepseek-r1 model performance on ROCM. This PR improves the performance of a8w8 GEMM and enable batched_gemm and mla_kv_buffe...
...
- Status/date: merged / 2025-11-20
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/models/deepseek_v2.py`; associated commits `fa924410276d`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[DeepseekV3.2] Deepseek fp8 support for MHA path". The diff centers on `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/attention/nsa_backend.py`. PR body context: ## Motivation Enable FP8 KV cache support for MHA (Multi-Head Attention) path in DeepSeek NSA backend. Previously, MHA could only be used when KV cache dtype is BF16, limiting i...
...
- Status/date: merged / 2025-11-20
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/models/deepseek_v2.py`; associated commits `7291c72e575d`
...
- Motivation: For DeepSeek V3/R1, this PR extends deployment docs, tests, or CI coverage. Title: "[Deepseek V3.2] Change indexer weights_proj to fp32". The diff centers on `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation Following https://github.com/deepseek-ai/DeepSeek-V3.2-Exp/commit/8631a813356d39b09a5c1ee1cde8ed6015559eb3, change the indexer `weights_proj` precision from bf16 t...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `eff7df6d0a49`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[AMD] Enable fused shared expert append and flatten quant for fp8 deepseekR1 model". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation Co-author : @yctseng0211 This PR introduces two performance improvements for DeepseekR1-0528 (fp8): 1. Support aiter fused_flatten_fp8_group_quant for fp8 DeepSeek...
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "[Perf] Optimize DeepSeek-R1 w4afp8 glue kernels". The diff centers on `python/sglang/srt/layers/moe/ep_moe/kernels.py`, `python/sglang/srt/layers/moe/cutlass_w4a8_moe.py`, `python/sglang/srt/layers/quantization/w4afp8.py`. PR body context: ## Motivation This PR improves DeepSeek-R1 w4afp8 TP8/EP8 ITL performance. It is motivated by the following profiling trace, obtained by running DeepSeek-R1 w4afp8 TP8 with a co...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/models/deepseek_v2.py`; associated commits `5eed5fc0b091`
...
- Motivation: For DeepSeek V3/R1, this PR changes model-related implementation. Title: "[DeepSeekV3.2] Centralize NSA dispatch logic in NativeSparseAttnBackend". The diff centers on `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation NativeSparseAttnBackend currently spreads dispatch logic for NSA prefill/decode implementations and MHA vs. MLA selection across multiple places: - Global `NSA_PRE...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `13e5beeab499`
- Diff scope read: GitHub Pull Request files API returned 1 files, +1/-1, 9 readable patch lines; this card prioritizes model-related and high-change files.
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "Fix Deepseek v3.1 loading issue". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: Fix nightly test error in https://github.com/sgl-project/sglang/actions/runs/19689716521/job/56402800532
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`; associated commits `decb48965dd1`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[DeepSeekV3.2] Enable pure TP & Partial DP Attention". The diff centers on `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`. PR body context: ## Motivation DeepSeekV3.2 NSA currently has rough edges when running in **pure TP mode** (`dp_size Serving with one request: TP mode: DP Attention: ## Checklist - [x] Format yo...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[feature] implement dcp for deepseek_v2". The diff centers on `python/sglang/srt/layers/attention/utils.py`, `python/sglang/srt/model_executor/forward_batch_info.py`, `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla.py`. PR body context: ## Motivation Here's the first step to fully implement #12196 to support much longer context with TP 8 under 8xH20. Currently, it only works with attention backend flashinfer. I...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`; associated commits `7dfcc78155b6`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[DeepseekV3.2][NSA][Indexer] Fix PAGED top-k transform for NSA indexer chunked execution on H200". The diff centers on `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`. PR body context: ## Motivation In `extend` mode with NSA indexer enabled, H200 setups that select **PAGED** top-k transform (`flashmla_kv` + FP8 KV cache) may trigger chunked execution in `_get_...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `ea177372bd8c`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "support mtp with deepseek r1 nvfp4 model". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: collabrate with @trevor-m ## Motivation Support large scale EP deployment for the DS R1 fp4 model with eagle spec decoding. ## Modifications - add the custom moe a2a backend for...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/function_call/deepseekv32_detector.py`; associated commits `b7b7524e9560`
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Tool Call] Fix DeepSeekV32Detector skipping functions with no params in streaming mode". The diff centers on `python/sglang/srt/function_call/deepseekv32_detector.py`. PR body context: ## Motivation When using `--tool-call-parser deepseekv32` in streaming mode, tool calls for functions with no parameters (e.g., `get_date()`) were being silently skipped, while...
...
- Trace source: `git log --name-only -- <model-files>` found it through `examples/chat_template/tool_chat_template_deepseekv31.jinja`; associated commits `ef1ab2302ab2`
- Diff scope read: GitHub Pull Request files API returned 1 files, +5/-1, 13 readable patch lines; this card prioritizes model-related and high-change files.
- Motivation: For DeepSeek V3/R1, this PR changes model-related implementation. Title: "[Auto Sync] Update tool_chat_template_deepseekv31.jinja (20251210)". The diff centers on `examples/chat_template/tool_chat_template_deepseekv31.jinja`. PR body context: Sync changes from commit `abd3e13d`. **Files Changed:** - examples/chat_template/tool_chat_template_deepseekv31.jinja Author: Jue Wang --- *This is an automated PR created by sc...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[NPU]dsv3.2 cp for npu". The diff centers on `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa/utils.py`, `python/sglang/srt/layers/communicator_nsa_cp.py`. PR body context: ## Motivation Now, sglang already has the --enable-nsa-prefill-context-parallel option proposed to support CP parallelism, (1/n)support context parallel with deepseekv3.2-DSA ,...
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "Fix dsv3 dp accuracy issue when using bf16-kv". The diff centers on `python/sglang/srt/layers/attention/aiter_backend.py`. PR body context: ## Motivation cherry-pick "Fix dp issue dsv3 bf16 kv #14895" to `amd_mori` branch ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/hardware_backend/npu/modules/deepseek_v2_attention_mla_npu.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/models/deepseek_v2.py`; associated commits `c05d3afb5d8b`
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "[NPU] optimization for dsv3.2". The diff centers on `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/hardware_backend/npu/modules/deepseek_v2_attention_mla_npu.py`. PR body context: Co-author: @jiaming1130 ## Motivation In this PR, we optimized the performance of the DeepSeek-V3.2 on NPU to improve the performance of the network in the decode phase, and als...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/dequant_k_cache.py`, `python/sglang/srt/layers/attention/nsa_backend.py`; associated commits `c96903074c4e`
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[NSA] Fix NSA backend assertion error when running DeepSeek-V3.2 PP with radix-cache". The diff centers on `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/dequant_k_cache.py`. PR body context: ## Motivation Fix NSA backend assertion error when running DeepSeek-V3.2 triggered when running DeepSeek-V3.2 PP with radix-cache. The assertion `num_tokens <= total_num_tokens`...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `8ac350f335c6`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[AMD] Support fused_rms_mxfp4_quant in the prefill stage for DeepSeek-R1-MXFP4". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation Adopted the fused kernel fused_rms_mxfp4_quant to replace the existing RMSNorm + quant path, enabling more efficient preparation of activations for quantized proje...
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "[Performance] Optimize group gemm in DeepSeek-R1-W4AFP8 w4a8 moe". The diff centers on `python/sglang/srt/layers/moe/cutlass_w4a8_moe.py`, `sgl-kernel/csrc/moe/cutlass_moe/w4a8/w4a8_grouped_mm_c3x.cu`, `sgl-kernel/csrc/moe/cutlass_moe/w4a8/scaled_mm_entry.cu`. PR body context: ## Motivation While deploying and optimizing the w4a8 moe group gemm for DeepSeek-R1-W4AFP8, I found that when w4a8 is in deepep low_latency mode, only the final else branch of...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Fix the accuracy issue when running mxfp4 dsv3 model and enable ep". The diff centers on `python/sglang/srt/layers/quantization/mxfp4.py`, `python/sglang/srt/layers/quantization/quark/quark_moe.py`. PR body context: ## Motivation Fix the accuracy problem ## Modifications Add expert mask in moe function call ## Accuracy Tests `export SGLANG_AITER_MLA_PERSIST=0 python3 -m sglang.launch_server...
...
- Status/date: merged / 2025-12-17
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`; associated commits `d20699a33c50`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Deepseek V3.2] Support Overlap Spec + NSA". The diff centers on `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`. PR body context: ## Motivation Part of V3.2 Roadmap https://github.com/sgl-project/sglang/issues/15025 Enable overlap spec and EAGLE + NSA backend. ## Modifications In EAGLE V1, we had (with `py...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/function_call/deepseekv32_detector.py`; associated commits `41683536d394`
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "feat: DeepSeek-V3.2 Streaming tool call output". The diff centers on `python/sglang/srt/function_call/deepseekv32_detector.py`. PR body context: ## Motivation FIxes #14711 ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [...
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "[Performance] sgl-kernel, optimize group gemm in DeepSeek-R1-W4AFP8 w4a8 moe". The diff centers on `sgl-kernel/csrc/moe/cutlass_moe/w4a8/w4a8_grouped_mm_c3x.cu`, `sgl-kernel/csrc/moe/cutlass_moe/w4a8/scaled_mm_entry.cu`, `sgl-kernel/python/sgl_kernel/cutlass_moe.py`. PR body context: ## Motivation https://github.com/sgl-project/sglang/pull/15315 While deploying and optimizing the w4a8 moe group gemm for DeepSeek-R1-W4AFP8, I found that when w4a8 is in deepep...
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "[perf]optimize w4afp8 kernel on deepseek-v3-0324". The diff centers on `python/sglang/srt/layers/quantization/w4afp8.py`, `sgl-kernel/csrc/moe/cutlass_moe/w4a8/w4a8_grouped_mm_c3x.cu`, `sgl-kernel/csrc/moe/cutlass_moe/w4a8/w4a8_moe_data.cu`. PR body context: ## Motivation we use w4afp8 deepseekv3-0324 online, and we find its performance is not good enough when decode batch size < 32 ## Modifications fine-grained tiling config and ba...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa_backend.py`; associated commits `e88e75a9dfdd`
- Diff scope read: GitHub Pull Request files API returned 1 files, +1/-1, 9 readable patch lines; this card prioritizes model-related and high-change files.
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Deepseek V3.2] Fix Deepseek MTP in V1 mode". The diff centers on `python/sglang/srt/layers/attention/nsa_backend.py`. PR body context: ## Motivation ## Modifications Restore the earlier behaviour of Deepseek V3 + MTP. (Accidental change) Fix https://github.com/sgl-project/sglang/issues/15428
...
- Status/date: merged / 2025-12-19
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`; associated commits `6afc5d497bb3`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[DSv32] Move deep_gemm.get_paged_mqa_logits_metadata to init time as metadata". The diff centers on `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`. PR body context: ## Motivation https://github.com/sgl-project/sglang/issues/15025 ## Changes - Add `paged_mqa_schedule_metadata` to `NSAMetadata` (batch-level caching). - Compute once in `init_f...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Support piecewise cuda graph for dsv3 fp4". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation https://github.com/sgl-project/sglang/issues/11490 ## Accuracy Tests
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/function_call/deepseekv32_detector.py`; associated commits `01bd0d3e8b8c`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Tool Call][DSV32] Streamline function call parameters". The diff centers on `python/sglang/srt/function_call/deepseekv32_detector.py`. PR body context: ## Motivation As titled and Issue #14711. **IMPORTANT: This is an initial version aimed at streamlining function call parameters for `DeepSeek-V3.2`. I have tested it, and it pa...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `2ec6fa3c54a8`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "feat PD: add eagle3 support for DeepSeek V3 in EP mode". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: When try to enable EP MOE for DeepSeek V3 with EAGLE3, SGL_ENABLE_JIT_DEEPGEMM_BMM=1 SGL_ENABLE_JIT_DEEPGEMM=1 \ TORCHINDUCTOR_CACHE_DIR=/home/admin/inductor_root_cache /opt/con...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/function_call/deepseekv31_detector.py`; associated commits `2667c857a78f`
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "Fix DeepSeekV31's structural tag trigger". The diff centers on `python/sglang/srt/function_call/deepseekv31_detector.py`. PR body context: ## Motivation As titled. To prevent LLMs generating tool calls with wrong name Reference (can be seen in other models in the SGLang codebase as well): ## Modifications ## Checkl...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[DeepSeek v3.2] opt Context Parallelism: support fused moe, multi batch and fp8 kvcache". The diff centers on `python/sglang/srt/layers/attention/nsa/utils.py`, `python/sglang/srt/layers/communicator_nsa_cp.py`, `python/sglang/srt/layers/attention/nsa_backend.py`. PR body context: ## Motivation The original default token splitting scheme of cp does not support prefill multi-batch. A new token splitting method is introduced to enable multi-batch support, f...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/dequant_k_cache.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa/quant_k_cache.py`, `python/sglang/srt/layers/attention/nsa/utils.py`, `python/sglang/srt/layers/attention/nsa_backend.py`; associated commits `7d757d6f17ef`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Clean Some Environment Variables for DeepSeek V32". The diff centers on `python/sglang/srt/layers/attention/nsa/quant_k_cache.py`, `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/utils.py`. PR body context: ## Motivation ## Modifications - Move `SGLANG_NSA_FUSE_TOPK` and `SGLANG_NSA_ENABLE_MTP_PRECOMPUTE_METADATA` to environ.py - Deprecate `SGLANG_NSA_DUAL_STREAM`, `SGLANG_NSA_FLAS...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/models/deepseek_v2.py`; associated commits `20abaee26cd8`
...
- Motivation: For DeepSeek V3/R1, this PR changes model-related implementation. Title: "[DSv32] Overlap indexer weights_proj during dual_stream decode". The diff centers on `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`. PR body context: ## Motivation The weights_proj in the DSA indexer uses float type, which is slow on modern GPUs. Profiler traces show this accounts for ~20% of decode layer runtime. The traces...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Remove deepseek-r1 from THINKING_MODE_CHOICES in run_eval.py". The diff centers on `python/sglang/test/run_eval.py`. PR body context: ## Motivation deepseek-r1 is a pure reasoning model with no way to enable/disable reasoning, unless you modify the chat template directly. Starting from v3.1, deepseek introduce...
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "[DeepSeek V3.1/V3.2] Optimize fused moe configs for H20 & H20-3E based on swapab". The diff centers on `python/sglang/srt/layers/moe/fused_moe_triton/configs/triton_3_5_1/E=257,N=256,device_name=NVIDIA_H20,dtype=fp8_w8a8,block_shape=[128, 128]_down.json`, `python/sglang/srt/layers/moe/fused_moe_triton/configs/triton_3_5_1/E=257,N=256,device_name=NVIDIA_H20-3e,dtype=fp8_w8a8,block_shape=[128, 128]_down.json`, `python/sglang/srt/layers/moe/fused_moe_triton/configs/triton_3_5_1/E=257,N=256,device_name=NVIDIA_H20,dtype=fp8_w8a8,block_shape=[128, 128].json`. PR body context: ## Motivation 1. Performance tuning based on the code after fused moe swapab #16723. The optimal configuration of fused MoE changes when swapab is taken into consideration. 2. O...
...
- Motivation: For DeepSeek V3/R1, this PR changes model-related implementation. Title: "[Refactor] Split out deepseek v2 weight loader function into mixin". The diff centers on `python/sglang/srt/models/deepseek_common/deepseek_weight_loader.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/models/deepseek_common/utils.py`. PR body context: ## Motivation DeepseekV2 code has been developed fast and a lot of historical code and be more orgranized, including the weight loading part. Issue related: https://github.com/s...
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "Use dsv3 optimized routing `fused_topk_deepseek` instead of `moe_fused_gate`". The diff centers on `python/sglang/srt/layers/moe/topk.py`, `test/registered/kernels/test_fused_topk_deepseek.py`, `test/srt/test_deepseek_v3_mtp.py`. PR body context: ## Motivation flashinfer has an optimized routing kernel for DeepSeek V3: https://github.com/flashinfer-ai/flashinfer/pull/2099 The API was renamed to `fused_topk_deepseek` here...
...
- Status/date: merged / 2026-01-19
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa_backend.py`; associated commits `d2105d4abda6`
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "[DeepSeek v3.2] Opt MTP decode cuda batch sizes and nsa implementation". The diff centers on `python/sglang/srt/layers/attention/nsa_backend.py`. PR body context: ## Motivation 1、`draft_extend` and `target_verify` use `nsa_decode_backend` as backend implementation instead of `nsa_prefill_backend` In the MTP scenario, the NSA attention imp...
...
- Trace source: `git log --name-only -- <model-files>` found it through `test/registered/amd/accuracy/mi35x/test_deepseek_r1_mxfp4_eval_mi35x.py`, `test/registered/amd/perf/mi35x/test_deepseek_r1_mxfp4_perf_mi35x.py`; associated commits `a3addd6203ca`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[AMD] Add DeepSeek-V3.2 and VLMs model in nightly tests". The diff centers on `test/registered/amd/accuracy/mi35x/test_deepseek_r1_mxfp4_eval_mi35x.py`, `test/registered/amd/perf/mi35x/test_deepseek_r1_mxfp4_perf_mi35x.py`. PR body context: ## Motivation Add 5 unique test in AMD coverage: https://github.com/sgl-project/sglang/actions/runs/21153414554/attempts/1#summary-60833755716 - Add accuracy and perf tests for...
...
- Status/date: merged / 2026-01-20
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`; associated commits `612026ad2c82`
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "[OPT] DeepSeekV3.2: optimize indexer weight_proj-mma performance". The diff centers on `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`. PR body context: ## Motivation From these pr[https://github.com/sgl-project/sglang/pull/16637 and https://github.com/sgl-project/sglang/issues/16861 and https://github.com/sgl-project/sglang/pul...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/hardware_backend/npu/modules/deepseek_v2_attention_mla_npu.py`, `python/sglang/srt/models/deepseek_v2.py`; associated commits `c0b5a180febe`
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[NPU]bugfix: fix for dsv3.2 and dsvl2". The diff centers on `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/hardware_backend/npu/modules/deepseek_v2_attention_mla_npu.py`. PR body context: ## Motivation 1、There are some bugs for DS-Vl2 on rotary_embedding 2、DSV32 is not compatible with the scenario where m.alt_stream is not None. ## Modifications 1、Fix the ds-v12...
...
- Status/date: merged / 2026-01-23
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa_backend.py`; associated commits `2fb328109fb9`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[DeepSeek V3.2] Enable trtllm NSA with bf16 kvcache". The diff centers on `python/sglang/srt/layers/attention/nsa_backend.py`. PR body context: ## Motivation Enables NSA backend with trtllm kernels for sparse attention. This can be more efficient than FlashMLA when the head size isn't a multiple of 64 and hence requires...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa_backend.py`; associated commits `1674b9ef4494`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[DeepSeek-V3.2] Fix TRT-LLM NSA in target_verify/draft_extend". The diff centers on `python/sglang/srt/layers/attention/nsa_backend.py`. PR body context: ## Motivation Quick follow‑up to #16758: speculative decoding still fails with `--nsa-decode-backend trtllm`. ## Modifications - Add TRT‑LLM handling in the extend path for spec...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/hardware_backend/npu/modules/deepseek_v2_attention_mla_npu.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`; associated commits `b56366f8275a`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[NPU]DeepSeek-V3.2 support npu mlaprolog". The diff centers on `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/hardware_backend/npu/modules/deepseek_v2_attention_mla_npu.py`. PR body context: ## Motivation we enabled the DSV3.2 to support npu mla_prolog_v3 to improve the performance ## Modifications - Added support for the mla_prolog_v3 custom operator, which include...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`; associated commits `a8dda2aa5727`
...
- Motivation: For DeepSeek V3/R1, this PR changes model-related implementation. Title: "[DSv32] Overlap indexer qk projection and activation quant". The diff centers on `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`. PR body context: ## Motivation @humansand After https://github.com/sgl-project/sglang/pull/17205, the indexer weight projection is fully hidden and no longer exposes latency. ## Modifications ##...
...
- Status/date: merged / 2026-01-28
- Trace source: `git log --name-only -- <model-files>` found it through `test/registered/amd/accuracy/mi35x/test_deepseek_r1_eval_mi35x.py`, `test/registered/amd/accuracy/mi35x/test_deepseek_r1_mxfp4_eval_mi35x.py`; associated commits `f8636fbb253a`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[AMD] Add Kimi-K2, DeepSeek-V3.2 tests to nightly CI". The diff centers on `test/registered/amd/accuracy/mi35x/test_deepseek_r1_eval_mi35x.py`, `test/registered/amd/accuracy/mi35x/test_deepseek_r1_mxfp4_eval_mi35x.py`. PR body context: ## Motivation 1. Add **Kimi-K2**, **DeepSeek-V3.2** accuracy and performance tests for MI325 (MI30x) platform, update Mi35x tests, consolidate test jobs, and fix various CI fail...
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "Fix OOM in DeepSeek weight loading by deferring dict(weights) materialization". The diff centers on `python/sglang/srt/models/deepseek_common/deepseek_weight_loader.py`. PR body context: ## Summary - Fix OOM when loading DeepSeek models with streaming/remote weight loading - Defer `dict(weights)` call until quantization is actually needed - Return original weigh...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa/utils.py`, `python/sglang/srt/layers/attention/nsa_backend.py`, `test/registered/kernels/test_nsa_indexer.py`; associated commits `677f3c49dabf`
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[DeepSeek V3.2] [Bugfix] slice indexer and padding fa3 when can not run cuda graph". The diff centers on `python/sglang/srt/layers/attention/nsa/utils.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`. PR body context: ## Motivation This PR #15227 only resolves the issue occurring during `forward_idle`. If running in Eagle mode (MTP Spec2, batch size exceeding the CUDA maximum batch size, or w...
...
- Status/date: merged / 2026-02-02
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`; associated commits `b0a6d5244cef`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[NPU] support dsv32 radixcache on ascend". The diff centers on `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`. PR body context: ## Motivation Ascend currently does not support the radixcache feature on the DeepSeek-3.2 model ## Modifications 1. We have modified the inputs of `npu_sparse_flash_attention`...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/models/deepseek_v2.py`; associated commits `e8a2c133807c`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Deepseekv32 compatibility with transformers v5". The diff centers on `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`. PR body context: ## Motivation ## Modifications 1. Enhance Indexer and DeepseekV2AttentionMLA to support dynamic is_neox_style parameter. 2. Refactor rope parameter handling in DeepseekV2Decoder...
...
- Status/date: merged / 2026-02-10
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/tilelang_kernel.py`; associated commits `4262f5259b94`
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "Tilelang sparse decode fwd for dsv32 mi355". The diff centers on `python/sglang/srt/layers/attention/nsa/tilelang_kernel.py`. PR body context: cc @HaiShaw, @kkHuang-amd, @hubertlu-tw ## Motivation The current dsv32 attention implementation uses a single TileLang kernel for both prefill and decode. This kernel is optimi...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`; associated commits `2cc235e7952e`
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "Fix Bug on dsv3.2". The diff centers on `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`. PR body context: ## Motivation Fix two bugs on dsv32 when infering on npu ## Modifications 1. fix a bug: forward_npu method of nas_Indexer has a dual stream feature that can not be closed. May c...
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[AMD] Fix accuracy issue when running TP4 dsv3 model with mtp". The diff centers on `python/sglang/srt/layers/attention/aiter_backend.py`, `docker/rocm.Dockerfile`. PR body context: cc @HaiShaw, @kkHuang-amd ## Motivation Fixed dpsk-r1-fp4 accuracy issue on tp4 with mtp. ## Modifications - aiter backend change in sglang. - aiter update for tp4 fp8 prefill a...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `190fa8246fbc`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Fix model loading for DeepSeek-V3.2-AWQ". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation Got the following error when using DeepSeek-V3.2-AWQ (https://huggingface.co/QuantTrio/DeepSeek-V3.2-AWQ/) ## Modifications Add check of whether `weight_packed` ex...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/models/deepseek_v2.py`, `test/registered/kernels/test_nsa_indexer.py`; associated commits `0ffd0a3995e5`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Nsa trtllm mla sparse fp8 support with Deepseek v3.2 NVFP4". The diff centers on `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/models/deepseek_v2.py`, `test/registered/kernels/test_nsa_indexer.py`. PR body context: ## Motivation #17655 - support Deepseek v3.2 NVFP4 with trtllm mla sparse fp8 attention backend ## Modifications - update the nsa backend to support trtllm sparse fp8 attention...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Kernel] Add JIT support for DeepSeek V3 GEMM". The diff centers on `python/sglang/jit_kernel/csrc/gemm/dsv3_fused_a_gemm.cuh`, `python/sglang/jit_kernel/csrc/gemm/dsv3_router_gemm.cuh`, `python/sglang/jit_kernel/benchmark/bench_dsv3_router_gemm.py`. PR body context: ## Motivation This PR migrates DeepSeek V3 GEMM kernels to the JIT (Just-In-Time) compilation framework, including: - `dsv3_fused_a_gemm` - `dsv3_router_gemm` related roadmap: h...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa_backend.py`; associated commits `462267982bd8`
- Diff scope read: GitHub Pull Request files API returned 1 files, +1/-1, 9 readable patch lines; this card prioritizes model-related and high-change files.
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[AMD] Fix mi35x dsv32 mtp nightly". The diff centers on `python/sglang/srt/layers/attention/nsa_backend.py`. PR body context: ## Motivation PR #17554 introduced `fused_metadata_copy`, a JIT-compiled CUDA kernel enabled by default. This kernel cannot run on ROCm/HIP — it fails on every CUDA graph replay...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `test/registered/kernels/test_nsa_indexer.py`; associated commits `eddf193292d3`
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "[DSv32] [GLM5] Improve Model Quality by Avoiding FP32 Precision Loss in `weights_proj`". The diff centers on `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `test/registered/kernels/test_nsa_indexer.py`. PR body context: @humansand ## Motivation From recent GLM-5 tech report: > Compared with the non-deterministic CUDA-based top‑k implementation used in SGLang’s DSA Indexer, directly using the na...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_v2.py`; associated commits `c193a52fa263`
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "[AMD] DSR1/V3 use fp8 bmm in MLA for MI300X". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation Improve TPOT by using use fp8 bmm in MLA on MI300X for DSR1/V3 ## Modifications - [x] q@w_kc: bmm bf16 -> bmm_fp8 - [x] attn_output@w_vc: bmm bf16 -> bmm_fp8 ## Ac...
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "[ROCm] Optimize Deepseek R1 on MI300X". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation The DeepSeek-V3/R1 model on AMD MI300X (gfx942) was previously limited to fallback code paths for several MLA attention operations because the optimized kernels we...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/jit_kernel/csrc/nsa/fused_store_index_cache.cuh`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`; associated commits `4e843f121657`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[DeepSeek-V3.2][JIT-kernel] Support nsa fuse store indexer k cache". The diff centers on `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/jit_kernel/csrc/nsa/fused_store_index_cache.cuh`. PR body context: ## Motivation In DeepSeek v3.2, after the Indexer produces key in bf16 (roughly (N, 128)), it needs to populate NSA’s index_k_with_scale_buffer. The previous implementation used...
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[AMD] Fix weight load shape mismatch for amd dsr1 0528 mxfp4". The diff centers on `python/sglang/srt/models/deepseek_nextn.py`, `python/sglang/srt/layers/quantization/quark/quark.py`. PR body context: > Co-authored-by: Haoyang Li > Co-authored-by: billishyahao ## Motivation This patch is to fix tensor shape mismatch issue for amd dsr1 0528 mxfp4 https://huggingface.co/amd/Dee...
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "[3/n] deepseek_v2.py Refactor: Migrate MLA forward method in deepseek_v2.py". The diff centers on `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla.py`, `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla_fused_rope_rocm.py`. PR body context: ## Motivation Part of #16255 Split different MLA forward methods into separate files, including: - `forward_mla.py` for Cuda/HIP mla aborption - `forward_mla_fused_rope_cpu.py‎`...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/hardware_backend/npu/modules/deepseek_v2_attention_mla_npu.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/models/deepseek_v2.py`; associated commits `b3718982a1b4`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Feature] add feature mla_ag_after_qlora for dsv3.2". The diff centers on `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/hardware_backend/npu/modules/deepseek_v2_attention_mla_npu.py`. PR body context: ## Motivation In DeepSeek-V3.2, due to the large hidden dimension of the model, performing all-gather on hidden_states before attention introduces a high-dimensional communicati...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/function_call/deepseekv32_detector.py`; associated commits `6af0448cc9bf`
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Catch errors when DeepSeek-V3.2 generates malformed JSON". The diff centers on `python/sglang/srt/function_call/deepseekv32_detector.py`. PR body context: ## Motivation LLMs may generate tool call content that are not correct json format, we must catch this error for normal streaming output. ## Modifications ## Accuracy Tests ## B...
...
- Status/date: merged / 2026-03-03
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/function_call/deepseekv32_detector.py`; associated commits `666caaf9ce76`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Tool Call] Stream DeepSeek-V3.2 function call parameters in JSON format.". The diff centers on `python/sglang/srt/function_call/deepseekv32_detector.py`. PR body context: ## Motivation The previous implementation only supported streaming function call parameters in XML format. ## Modifications 1. Convert `_parse_parameters_from_xml` to return `st...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[AMD] CI - Add MI35x nightly/PR tests for kv-cache-fp8 and allreduce-fusion (DeepSeek)". The diff centers on `test/registered/amd/accuracy/mi35x/test_deepseek_r1_mxfp4_kv_fp8_eval_mi35x.py`, `test/registered/amd/accuracy/mi35x/test_deepseek_r1_mxfp4_ar_fusion_eval_mi35x.py`, `.github/workflows/nightly-test-amd-rocm720.yml`. PR body context: ## Motivation Track accuracy and performance regression for two new DeepSeek-R1-MXFP4 server configurations on MI35x: - `--kv-cache-dtype fp8_e4m3` - `--enable-aiter-allreduce-f...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[AMD] Use bfloat16 for correction_bias in AITER FP8 path to avoid runtime dtype conversion for dsv3". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation In the AITER (AMD MI355X) path with FP8 quantization, was initialized in float32, but at runtime in biased_grouped_topk it gets cast to match gating_output.dtype:...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/index_buf_accessor.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `test/manual/layers/attention/nsa/test_get_k_scale_triton_kernel.py`, `test/manual/layers/attention/nsa/test_index_buf_accessor.py`; associated commits `006bd44cf920`
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[deepseekv3.2] fix get_k_and_s_triton kenel for 128K seqlen case bug". The diff centers on `test/manual/layers/attention/nsa/test_get_k_scale_triton_kernel.py`, `python/sglang/srt/layers/attention/nsa/index_buf_accessor.py`, `test/manual/layers/attention/nsa/test_index_buf_accessor.py`. PR body context: ## Motivation fix get_k_and_s_triton kenel for 128K seqlen case bug ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [x] Format your code accordin...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/index_buf_accessor.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`, `test/registered/kernels/test_nsa_indexer.py`; associated commits `17031120b8f6`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[DeepSeek v3.2][Bugfix] get_index_k_scale_buffer support cp". The diff centers on `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/index_buf_accessor.py`. PR body context: ## Motivation Fix PR #16043 #19319 Prefill CP scenario bug. In the DeepSeek V3.2 prefill with CP (context parallel) scenario, `forward_batch.seq_lens` and `forward_batch.seq_len...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[CPU] Add FP8 Bmm support". The diff centers on `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla.py`, `python/sglang/srt/models/longcat_flash.py`, `python/sglang/srt/models/deepseek_common/deepseek_weight_loader.py`. PR body context: ## Motivation This PR is a follow-up on https://github.com/sgl-project/sglang/issues/8281 to add FP8 BMM support in MLA weight absorb. ## Modifications - The main change is the...
...
- Diff scope read: GitHub Pull Request files API returned 1 files, +5/-1, 13 readable patch lines; this card prioritizes model-related and high-change files.
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "[AMD] Use aiter_dsv3_router_gemm kernel if number of experts <= 256.". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: This change modified the router kernel selection with ROCm backend for models using DeepSeek model architecture. The Aiter kernel aiter_dsv3_router_gemm has been optimized for t...
...
- Trace source: preserved from an explicit existing history/skill citation
- Diff scope read: GitHub Pull Request files API returned 1 files, +1/-1, 9 readable patch lines; this card prioritizes model-related and high-change files.
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Fix gpu-fault issue when run deepseek-r1 and enable dp". The diff centers on `python/sglang/srt/layers/attention/aiter_backend.py`. PR body context: ## Motivation Fix the gpu cored-dump. Root cause: During CUDA graph capture, the captured data was fed with a KV sequence length smaller than the query (Q) length, which caused...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`; associated commits `649172879778`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Perf] Overlap NSA-CP key all-gather with query computation for DeepSeek-V3.2". The diff centers on `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`. PR body context: ## Motivation In the current Native Sparse Attention (NSA) implementation, specifically when Context Parallelism (CP) is enabled via `--enable-nsa-prefill-context-parallel`, the...
...
- Status/date: merged / 2026-03-24
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/layers/attention/nsa/tilelang_kernel.py`; associated commits `855d15adf657`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[AMD] Tilelang sparse fwd for dsv32 mi355/mi300". The diff centers on `python/sglang/srt/layers/attention/nsa/tilelang_kernel.py`. PR body context: ## Motivation - Enable the faster/new tilelang kernel on MI300. - Improve longer-context kernel performance on MI355. ## Modifications - Enable the new tilelang partial+combine...
...
- Trace source: preserved from an explicit existing history/skill citation
- Diff scope read: GitHub Pull Request files API returned 1 files, +1/-0, 8 readable patch lines; this card prioritizes model-related and high-change files.
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "Fix aiter router GEMM regression for non-DSR1 MoE models on ROCm gfx95". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: aiter_dsv3_router_gemm uses a Triton atomic kernel (_gemm_a16_w16_atomic) that is specifically tuned for DeepSeek-R1/V3's hidden_size=7168. On models with different hidden dimen...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Add MXFP4 (including Quark W4A4) quantization support for DeepSeek-architecture on ROCm". The diff centers on `python/sglang/srt/layers/quantization/quark/schemes/quark_w4a4_mxfp4_moe.py`, `python/sglang/srt/layers/moe/fused_moe_triton/layer.py`, `python/sglang/srt/models/deepseek_v2.py`. PR body context: Enable MXFP4 (including Quark W4A4) quantized models (e.g. GLM-5-MXFP4-Q8, Kimi-K2.5-MXFP4) to load and run on AMD MI355X with ROCm. These models use Quark's W4A4 MXFP4 scheme w...
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[ROCm] Fix fused MLA decode rope path for Kimi K2.5 and DeepSeek-variant models". The diff centers on `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla_fused_rope_rocm.py`, `python/sglang/srt/layers/attention/triton_ops/rocm_mla_decode_rope.py`. PR body context: The SGLANG_ROCM_FUSED_DECODE_MLA=1 path had several issues preventing it from working with models like Kimi K2.5 that use DeepseekScalingRotaryEmbedding or different ForwardMeta...
...
- Motivation: For DeepSeek V3/R1, this PR changes model-related implementation. Title: "[JIT Kernel] Migrate dsv3_router_gemm from AOT sgl-kernel to JIT kernel". The diff centers on `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/models/glm4_moe_lite.py`, `python/sglang/jit_kernel/csrc/gemm/dsv3_router_gemm.cuh`. PR body context: dsv3_router_gemm_bf16_out.cu → kOutFloat=false path dsv3_router_gemm_float_out.cu → kOutFloat=true path dsv3_router_gemm_entry.cu → RouterGemmDispatcher replace LoopUnroller ##...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Intel GPU] Enable DeepSeek R1 inference on XPU". The diff centers on `python/sglang/srt/models/deepseek_common/deepseek_weight_loader.py`, `python/sglang/srt/models/deepseek_common/utils.py`, `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation Enable DeepSeek-R1 model inference for XPU use FP8 precision through triton ## Modifications 1. run bmm in bf16 for absorb 2. Add XPU support for benchmarking 3. A...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "DeepSeek-R1-0528-w4a8: DeepEP Low Latency Dispatch Adopts FP8 Communication". The diff centers on `python/sglang/srt/layers/moe/ep_moe/kernels.py`, `python/sglang/srt/layers/moe/cutlass_w4a8_moe.py`, `python/sglang/srt/layers/moe/ep_moe/layer.py`. PR body context: ## Motivation profiling: deepseek-R1-0528: deepseek-R1-0528-w4afp8: 1.When DeepEP is enabled, the communication latency of the DeepSeek-R1-0508-W4AFP8 model is twice that of the...
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "Revert "DeepSeek-R1-0528-w4a8: DeepEP Low Latency Dispatch Adopts FP8 Communication"". The diff centers on `python/sglang/srt/layers/moe/ep_moe/kernels.py`, `python/sglang/srt/layers/moe/cutlass_w4a8_moe.py`, `python/sglang/srt/layers/moe/ep_moe/layer.py`. PR body context: Reverts sgl-project/sglang#14162
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "[AMD] Use tgemm.mm for MoEGate router gemm in deepseek_v2.py". The diff centers on `python/sglang/srt/layers/rocm_linear_utils.py`, `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation When running the GLM-5-fp8 model, the MoEGate router GEMM is a major bottleneck, taking about **~30%** of a single layer's time. ## Modifications Following the ATO...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[RL] Support mxfp8 DeepSeek V3". The diff centers on `python/sglang/srt/layers/moe/moe_runner/flashinfer_trtllm.py`, `python/sglang/srt/layers/moe/fused_moe_triton/layer.py`, `python/sglang/srt/layers/quantization/fp8.py`. PR body context: ## Motivation @humansand Support Blackwell mxfp8 DeepSeek RL. Since the `kv_b_proj` can have different contraction axis in absorbed vs non-absorbed MLA mode while mxfp8 is 1d qu...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Add dsv3 router gemm benchmark on blackwell". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation * Comparing dsv3_router_gemm performance between flashinfer and the current sglang kernel to optimize performance (https://github.com/sgl-project/sglang/issues/144...
...
- Trace source: `git log --name-only -- <model-files>` found it through `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla.py`, `python/sglang/srt/models/deepseek_v2.py`; associated commits `5a3531641735`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Enable IndexCache for DeepSeek V3.2". The diff centers on `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla.py`. PR body context: ## Motivation fix https://github.com/sgl-project/sglang/issues/21286 ## Modifications * Port https://github.com/THUDM/IndexCache * add ut for deepseek-v3.2 ## Accuracy Tests gsm...
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Fix prepare_qkv_latent bypassing LoRA adapters in DeepSeek V2/V3". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation When LoRA adapters are applied to `fused_qkv_a_proj_with_mqa` in DeepSeek V2/V3 models, the fused GEMM optimization path in `prepare_qkv_latent` reads `.weight` di...
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "feat: [1/2] [DeepEP] Fuse shared expert into MoE dispatch under EP". The diff centers on `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/moe/topk.py`, `python/sglang/srt/layers/moe/fused_moe_triton/layer.py`. PR body context: ## Motivation In DeepSeek V3/R1 with expert parallelism (EP), each rank currently computes the shared expert **separately** from routed experts. This means shared expert is not...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Lora] Lora quat info re-factor and support deepseekv3 mla lora". The diff centers on `python/sglang/srt/layers/quantization/fp8.py`, `python/sglang/srt/layers/quantization/moe_wna16.py`, `python/sglang/srt/layers/quantization/w8a8_int8.py`. PR body context: ## Motivation ## Modifications - MoE quant-info refactor Extracts get_triton_quant_info() into each quantization method (FP8, INT8, WNA16, etc.) so FusedMoEWithLoRA correctly re...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Reland] DeepSeek-R1-0528-w4a8: DeepEP Low Latency Dispatch Adopts FP8 Communication". The diff centers on `python/sglang/srt/layers/moe/ep_moe/kernels.py`, `python/sglang/srt/layers/moe/cutlass_w4a8_moe.py`, `python/sglang/srt/layers/moe/ep_moe/layer.py`. PR body context: ## Motivation ## Modifications ## Accuracy Tests ## Motivation profiling: deepseek-R1-0528: deepseek-R1-0528-w4afp8: 1.When DeepEP is enabled, the communication latency of the D...
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "[AMD][MI30X] Restore DeepSeek MLA MI300X paths after MLA refactor (#19122)". The diff centers on `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla.py`, `python/sglang/srt/models/deepseek_v2.py`. PR body context: ## Motivation Re-apply #18242 and #18624 behavior in forward_mla.py (logic moved out of deepseek_v2.py by #19122 but not fully carried over): ## Modifications - Import fused_qk_...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Allow piecewise CUDA graph with speculative decoding". The diff centers on `python/sglang/srt/model_executor/piecewise_cuda_graph_runner.py`, `python/sglang/srt/model_executor/model_runner.py`, `test/registered/piecewise_cuda_graph/test_pcg_with_speculative_decoding.py`. PR body context: ## Summary - Allow `--enable-piecewise-cuda-graph` to coexist with all speculative decoding algorithms (EAGLE/EAGLE3/NEXTN/STANDALONE/NGRAM) - Previously all speculative algorit...
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Guard .weight access in DeepseekV2AttentionMLA for AWQ / compressed-tensors". The diff centers on `test/registered/unit/models/test_deepseek_v2_attention_mla.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla_fused_rope_cpu.py`. PR body context: Hey all, this is my first PR, thought I'd try my hand a a recent bug. ---- ## Motivation Fixes #22855. When a DeepseekV2-architecture checkpoint is loaded with a `compressed_ten...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Fix double-reduce in DeepseekV2MoE with flashinfer_cutedsl + EP + DP-attention". The diff centers on `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/moe/moe_runner/flashinfer_cutedsl.py`. PR body context: ## Motivation Running `nvidia/DeepSeek-V3-0324-NVFP4` with `--moe-runner-backend flashinfer_cutedsl --enable-dp-attention --ep-size N --dp-size N` (N>1) hit two separate bugs. *...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[SPEC][1/N] feat: add adaptive speculative_num_steps for EAGLE topk=1". The diff centers on `python/sglang/srt/model_executor/cuda_graph_runner.py`, `benchmark/bench_adaptive_speculative.py`, `test/registered/unit/spec/test_adaptive_spec_params.py`. PR body context: ## Motivation One of the core parameters of speculative decoding is `speculative_num_steps`, which controls how many autoregressive draft-model steps are executed in each round....
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Opt-in strip of thinking tokens from radix cache". The diff centers on `test/registered/unit/mem_cache/test_unified_radix_cache_unittest.py`, `python/sglang/srt/managers/schedule_batch.py`, `python/sglang/srt/server_args.py`. PR body context: Fixes #22373. Closes #22617, #22950. Opt-in: enable with `--strip-thinking-cache`. Off by default. ## Why Reasoning-model requests (`--reasoning-parser `) insert all output toke...
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[fix] Parser-gated two-phase cache stripping for reasoning radix caches (fixes #22373)". The diff centers on `python/sglang/srt/parser/reasoning_parser.py`, `python/sglang/srt/configs/model_config.py`, `test/registered/unit/mem_cache/test_radix_cache_thinking.py`. PR body context: # Fix dead reasoning branches in radix cache for multi-turn separated-thinking models Fixes #22373. Related to #22617. --- ## Motivation ### What is the problem Reasoning models...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[SPEC V2][2/N] feat: adaptive spec support spec v2". The diff centers on `python/sglang/srt/speculative/eagle_worker_v2.py`, `python/sglang/srt/speculative/eagle_info_v2.py`, `python/sglang/srt/managers/scheduler_output_processor_mixin.py`. PR body context: ## Motivation adaptive spec support spec v2: 1. launch sgl+adaptive spec+spec v2 2. benchmark 3. result Models: - Target model: `/models/ZhipuAI/GLM-4.7-FP8` - Draft model: `/mo...
...
- Motivation: For DeepSeek V3/R1, this PR changes model-related implementation. Title: "[CPU] expand the interface of shared_expert without scaling factor". The diff centers on `sgl-kernel/csrc/cpu/moe_int4.cpp`, `sgl-kernel/csrc/cpu/moe.h`, `sgl-kernel/csrc/cpu/moe.cpp`. PR body context: ## Motivation This patch expands `shared_expert` kernel when scaling factor is None, which maps exactly the behavior of MoE when expert equals to one. Also simplifies moe kernel...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[MUSA][16/N] Add MUSA backend support for layers and DeepSeek models (V2/V3/R1)". The diff centers on `python/sglang/srt/layers/deep_gemm_wrapper/compile_utils.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/models/deepseek_common/deepseek_weight_loader.py`. PR body context: ## Motivation Enable SGLang to run DeepSeek models on Moore Threads MUSA GPUs. This PR adds MUSA backend support across the inference stack, including layers, quantization, MoE,...
