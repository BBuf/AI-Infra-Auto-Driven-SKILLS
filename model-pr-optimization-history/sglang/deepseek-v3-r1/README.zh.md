...
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「AMD DeepSeek_V3 FP8 Numerical fix」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Together with core changes from: https://github.com/sgl-project/sglang/pull/2637 ## Modifications Several places in deepseek_v2.py needs extra normalization to wei...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Feature/docs deepseek usage and add multi-node」，变更集中在 文件级 diff。PR 描述补充为：## Motivation People are using the Deepseek V3 model, our guidance are not centralised for the ease of usage. ## Modifications Update: docs\references\deepseek.md Update: docs\r...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补强部署文档、测试或 CI 验证面，标题为「refine deepseek_v3 launch server doc」，变更集中在 `benchmark/deepseek_v3/README.md`。PR 描述补充为：RT
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `70f894b810c0`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 12 个文件，+296/-132，可读 patch 737 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「feat: support flashinfer mla attention for deepseek v3」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Kudos to @yzh119 Throughout the integration process, we have identified and resolved numerous issues with the exceptional support from the FlashInfer team. Current...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Support NextN (MTP) speculative decoding for DeepSeek-V3/R1」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation We implemented NextN (MTP) speculative decoding for DeepSeek-V3/R1 based on EAGLE 2 on Triton backend (https://github.com/sgl-project/sglang/pull/3466) and achieve...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `b110084654a1`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+565/-19，可读 patch 635 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「Refactor flashinfer logic for deepseek v3 and fix accuracy bug」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation `flashinfer_backend.py` for attention is too complex, this PR extract the logic of MLA and creates a new `flashinfer_mla_backend.py` Also, #3716 #3751 reports an a...
...
- 状态/时间: merged / 2025-02-24
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `1a6e97577acb`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 9 个文件，+1092/-5，可读 patch 1339 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Feature DeepSeek V3/R1 INT8 Quantization (block-wise)」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Support block-wise INT8 quantization for DeepSeek V3/R1. INT8 is a friendly type for most hardware platforms. ## Modifications - Fork FP8 implementation and suppor...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `6ce9dbe82882`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+863/-1，可读 patch 894 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[ROCm] Enable Fused MLA Triton kernel for DeepSeekV3」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：This PR introduces the concept of fusing MLA rope into the grouped attention on ROCm. To use this feature use the env variable : SGLANG_ROCM_FUSED_DECODE_MLA=1. Triton Kernel au...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「add deepgemm and sglang fp8 block-wise gemm benchmark」，变更集中在 `benchmark/kernels/deepseek/benchmark_deepgemm_fp8_gemm.py`, `benchmark/kernels/deepseek/README.md`。PR 描述补充为：The script benchmark all FP8 w8a8 block-wise matrix multiplications involved in DeepSeek V3/R1 under the current tensor parallelism (TP) setting with DeepGemm and vLLM/SGLang. *...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `c7f254468fca`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 5 个文件，+369/-21，可读 patch 503 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[Feature] DeepSeek V3/R1 INT8 Quantization (channel-wise)」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Support channel-wise INT8 quantization for DeepSeek V3/R1. INT8 is a friendly type for most hardware platforms. ## Modifications Co-author: @yych0745 @sleepcoo @b0...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Support nextn for flashinfer mla attention backend」，变更集中在 `python/sglang/srt/layers/attention/flashinfer_mla_backend.py`, `python/sglang/srt/models/deepseek_v2.py`, `test/srt/test_mla_flashinfer.py`。PR 描述补充为：## Motivation Support the compatibility of nextn and flashinfer mla attention backend. Currently topk can only be set to 1 due to lack of custom mask support for flashinfer MLA...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「DeepGemm integrate to sgl-kernel」，变更集中在 `sgl-kernel/tests/test_deep_gemm.py`, `sgl-kernel/setup.py`, `sgl-kernel/build.sh`。PR 描述补充为：## Motivation Integrate DeepGemm in setup. Linear usage: #4199 . ## Modifications ## Checklist - [x] Format your code according to the Code Formatting with Pre-Commit. - [x] Add...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「linear support deepgemm」，变更集中在 `python/sglang/srt/layers/quantization/fp8_kernel.py`, `python/sglang/test/test_block_fp8.py`, `test/srt/test_fp8_kernel.py`。PR 描述补充为：## Motivation Use deepgemm linear gemm, dependent on https://github.com/sgl-project/sglang/pull/4165 ## Test case **MMLU Average accuracy: 0.878** ### performence test size | or...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「add INT8 example into dsv3 README」，变更集中在 `benchmark/deepseek_v3/README.md`。PR 描述补充为：## Motivation add INT8 example into benchmark/deepseek_v3/README.md ## Modifications ## Checklist - [x] Format your code according to the Code Formatting with Pre-Commit. - [x]...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Support FlashMLA backend」，变更集中在 `python/sglang/srt/layers/attention/flashmla_backend.py`, `python/sglang/srt/layers/attention/utils.py`, `python/sglang/srt/model_executor/model_runner.py`。PR 描述补充为：## Motivation Integrate flashmla for decoding, and the accuracy test is currently okay. The current implementation is quite simple, directly integrating flashmla as the backend....
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Support FlashMLA backend cuda graph」，变更集中在 `python/sglang/srt/layers/attention/flashmla_backend.py`, `python/sglang/srt/layers/attention/utils.py`, `python/sglang/srt/server_args.py`。PR 描述补充为：## Motivation Support FlashMLA backend cuda graph. Optimize index calculation, complete the calculation in init_forward ## Modifications * Optimize the FlashInfer block table ca...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「Set deepgemm to the default value in the hopper architecture.」，变更集中在 `python/sglang/srt/layers/quantization/fp8_kernel.py`, `python/sglang/srt/utils.py`。PR 描述补充为：## Motivation Set deepgemm to the default value in the hopper architecture. ## Modifications
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[ROCm] Enable MTP (NextN) on AMD GPU」，变更集中在 `sgl-kernel/csrc/speculative/pytorch_extension_utils_rocm.h`, `sgl-kernel/csrc/torch_extension_rocm.cc`, `python/sglang/srt/speculative/build_eagle_tree.py`。PR 描述补充为：## Motivation To enable MTP (NextN) on AMD GPU. ## Modifications Added a few kernels to `torch_extension_rocm.cc`. Enabled MTP test for AMD GPU. ## Benchmark result ## Correctne...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[Feature] Support FA3 backend for MLA」，变更集中在 `python/sglang/srt/layers/attention/flashattention_backend.py`, `python/sglang/srt/model_executor/model_runner.py`, `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Support FlashAttention 3 backend for MLA. FA3 official example of MLA can be found in this file. TODO: - [x] Cuda Graph - [x] Benchmark Results (page_size=1) Maybe...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Add deepseek style fused moe group gate selection kernel」，变更集中在 `sgl-kernel/csrc/moe/moe_fused_gate.cu`, `sgl-kernel/benchmark/bench_moe_fused_gate.py`, `sgl-kernel/tests/test_moe_fused_gate.py`。PR 描述补充为：## Motivation PR adapted and improved from #3191 Rewrite Macro. Extended to support all power of 2 `# expert` & `# expert group`, also all `# topk_group` & `# topk` use cases +...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `924ca7c92c86`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 14 个文件，+536/-36，可读 patch 939 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Add DeepSeek V3/R1 shared experts fusion」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation I gain idea mainly from https://github.com/vllm-project/vllm/pull/15502 , thanks for the author's work.I will add references in the modifications for `grouped_topk...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「reduce moe_align_block_size_kernel small batch mode overhead」，变更集中在 `python/sglang/srt/layers/moe/fused_moe_triton/fused_moe.py`, `sgl-kernel/csrc/moe/moe_align_kernel.cu`, `sgl-kernel/benchmark/bench_moe_align_block_size.py`。PR 描述补充为：## Motivation - [x] Fix `moe_align_block_size` kernel benchmark bug. - [x] Reduce moe_align_block_size_kernel small batch mode ovehead caused by `torch.zeros` . - [x] After this...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「fix: use deepgemm only on hopper」，变更集中在 `python/sglang/srt/layers/quantization/fp8_kernel.py`。PR 描述补充为：## Motivation ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests as outlined in the Running Unit Tests....
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「Apply deepseek cuda rope」，变更集中在 `python/sglang/srt/layers/rotary_embedding.py`。PR 描述补充为：## Motivation The torch.compile issue described in #4959 arises from overriding the `forward` method of `CustomOp`. When torch.compile is applied, the `_forward_method` of `Cust...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「apply fused moe gate in ds v3/r1」，变更集中在 `python/sglang/srt/layers/moe/topk.py`。PR 描述补充为：## torch profile ### main branch !图片 ### pr !图片 Only one kernel now. 36us->8us. ## fused_moe_gate gsm8k acc test ## fused moe gate performance(in H200) |qps|Input token throughp...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「[Fix] Turn off DeepGEMM by default」，变更集中在 `python/sglang/srt/layers/quantization/fp8_kernel.py`, `docs/references/deepseek.md`。PR 描述补充为：## Motivation Currently DeepGEMM is turned on by default for deepseek models, which caused confusions (#5223) on its performance due to warmup time. This PR makes DeepGEMM turne...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「kernel: support slightly faster merge_state_v2 cuda kernel」，变更集中在 `sgl-kernel/tests/test_merge_state_v2.py`, `sgl-kernel/csrc/attention/merge_attn_states.cu`, `sgl-kernel/python/sgl_kernel/attention.py`。PR 描述补充为：support slightly faster merge_state_v2 cuda kernel - [x] float32 - [x] float16 - [x] bfloat16 - [x] dispatch by scalar_t - [x] unit tests (performance & correctness) - [x] suppo...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Support MHA with chunked prefix cache for DeepSeek chunked prefill」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/model_executor/forward_batch_info.py`, `python/sglang/srt/layers/attention/flashattention_backend.py`。PR 描述补充为：## Motivation The current implementation of MLA is slow when when handling long prefix lengths, such as sequences with 32k input length, as highlighted in issues like #5031. Pro...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `53dcf3887639`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+31/-1，可读 patch 101 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Introduce moe_dense_tp_size to fix dense layer errors in DeepSeek V3 + 4x8xH100」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：This code depends on https://github.com/sgl-project/sglang/pull/5190 and https://github.com/sgl-project/sglang/pull/5205 and please subtract diff from there ## Speed tests 4x8xH...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「use sglang_per_token_group_quant_fp8 from sgl-kernel instead of trion kernel」，变更集中在 `python/sglang/srt/layers/quantization/fp8_kernel.py`, `python/sglang/srt/layers/quantization/fp8_utils.py`。PR 描述补充为：## Motivation This pr implemented a faster kernel for per_token_group_quant_fp8, which can replace triton kernel. https://github.com/sgl-project/sglang/pull/4817 I tested the ke...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「Remove one kernel in per_tensor_quant_mla_fp8」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/quantization/fp8_kernel.py`, `python/sglang/srt/models/deepseek_nextn.py`。PR 描述补充为：## Motivation Thanks @Alcanderian for discussing it is acceptable to change APIs in the caller site ### Accuracy PR: 93.3 (baseline: roughly 93 when I tested in https://github.c...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[perf] introduce deep gemm group_gemm_masked as bmm」，变更集中在 `python/sglang/srt/layers/quantization/fp8_kernel.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/test/test_block_fp8.py`。PR 描述补充为：## Motivation per-token-group quant+deep_gemm's grouped_gemm_masked is generally faster than per-tensor quant+bmm_fp8 NOTE: Multi-batch throughput is hardly comparable because e...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「enable DeepSeek V3 shared_experts_fusion in sm90」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## H200 Benchmark I tested the benchmark using the command provided by https://github.com/sgl-project/sglang/issues/5514. To avoid warmup, I turned off deepgemm. Below are the r...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「Remove extra copy in deepseek forward absorb」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/rotary_embedding.py`, `.github/workflows/pr-test-amd.yml`。PR 描述补充为：## Benchmark ### Performance main branch: this PR: 0.5% improvement for bs 1 and 1.5% improvement for bs 32. main branch: this PR: removed 3 copy with 2 cat added (can overlap o...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Turn on DeepGemm By Default and Update Doc」，变更集中在 `python/sglang/srt/layers/quantization/deep_gemm.py`, `docs/references/deepseek.md`。PR 描述补充为：## Motivation Followup of #5580 ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests as outlined in the Ru...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「Fuse q_a_proj and kv_a_proj for DeepSeek models」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation In self attention module of DeepSeek v3, the `q_a_proj` and `kv_a_proj` both takes hidden state as input, so they can be fused into one module, and one launching o...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「[BugFix] Fix combination of MTP and `--n-share-experts-fusion`with R1」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation We found bugs while using MTP and `-n-share-experts-fusion` together. 1. While using `--n-share-experts-fusion ${TP_SIZE}` with MTP, there will be weight loading e...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「update triton 3.2.0 h200 fused moe triton config and add warning about triton fused_moe_kernel performance degradation due to different Triton versions.」，变更集中在 `python/sglang/srt/layers/moe/fused_moe_triton/configs/E=264,N=256,device_name=NVIDIA_H200,dtype=fp8_w8a8,block_shape=[128, 128].json`, `python/sglang/srt/layers/moe/fused_moe_triton/fused_moe.py`。PR 描述补充为：## Motivation Reproduction commands : ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests as outlined in...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「perf: update H20 fused_moe_triton kernel config to get higher throughput during prefilling」，变更集中在 `python/sglang/srt/layers/moe/fused_moe_triton/configs/E=272,N=128,device_name=NVIDIA_H20,dtype=fp8_w8a8,block_shape=[128, 128].json`。PR 描述补充为：## Motivation We have provided fused_moe_triton config in PR(https://github.com/sgl-project/sglang/pull/5641) to improve model performance. However, we found that after using th...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Fuse MLA set kv cache kernel」，变更集中在 `python/sglang/srt/layers/attention/flashattention_backend.py`, `python/sglang/srt/layers/radix_attention.py`, `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Fuse MLA set kv cache kernel and remove k concat operation. Currently only support FA3 backend. Can be applied to other backend with subsequent verification. ## Be...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Add Cutlass MLA attention backend」，变更集中在 `python/sglang/srt/layers/attention/cutlass_mla_backend.py`, `python/sglang/srt/model_executor/model_runner.py`, `python/sglang/srt/layers/attention/utils.py`。PR 描述补充为：## Motivation Enables use of the blackwell cutlass MLA decode kernel with deepseek models. ## Modifications Adds "cutlass_mla" option for attention backend. ## Deepseek-R1 Bench...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 调整模型相关实现，标题为「Auto set draft model path for MTP」，变更集中在 `python/sglang/srt/models/deepseek_nextn.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/model_executor/model_runner.py`。PR 描述补充为：## Motivation - load nextn layer weights directly from target model - refactor to reuse common weight loading logic for draft model and target model - auto set draft model path...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补强部署文档、测试或 CI 验证面，标题为「Update ci test and doc for MTP api change」，变更集中在 `test/srt/test_mla_deepseek_v3.py`, `python/sglang/srt/server_args.py`, `docs/references/deepseek.md`。PR 描述补充为：## Motivation Update ci test and doc for MTP api change.
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `examples/chat_template/tool_chat_template_deepseekv3.jinja`；关联提交 `170d1f218a4e`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+95/-29，可读 patch 155 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 调整模型相关实现，标题为「feat: Refactor DeepSeekV3 function call」，变更集中在 `examples/chat_template/tool_chat_template_deepseekv3.jinja`。PR 描述补充为：## Motivation Instead of keeping editing the prompt engineering for `deepseekv3` in `adapter.py`, this PR provides a chat template instead. Implements: https://github.com/sgl-pr...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「Overlap qk norm with two streams」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Overlap qk rms norm with two streams in cuda graph. ### Accuracy ### Performance main branch: this PR: ### Profile main branch: this PR: 23.9μm -> 21.5μm
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「feat: flashinfer_mla speculative decoding with custom mask」，变更集中在 `python/sglang/srt/layers/attention/flashinfer_mla_backend.py`, `test/srt/test_mla_flashinfer.py`, `python/sglang/srt/speculative/eagle_worker.py`。PR 描述补充为：status: not ready. dependent on https://github.com/flashinfer-ai/flashinfer/pull/1017. ## Motivation Support top_k > 1 Eagle speculative decoding with flashinfer MLA backend on...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Update doc for MLA attention backends」，变更集中在 `docs/references/deepseek.md`, `docs/backend/server_arguments.md`。PR 描述补充为：## Motivation ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests as outlined in the Running Unit Tests....
...
- 状态/时间: merged / 2025-05-08
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `73600673bb1d`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 7 个文件，+61/-39，可读 patch 231 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 调整模型相关实现，标题为「Clean logs for DeepSeek-V3 launching」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation When launching Deepseek-V3 model, some of the logs only need to be printed once (on tp_rank=0). ## Modifications This PR adds a `should_rank` util function. And pr...
...
- 状态/时间: merged / 2025-05-08
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `5e02330137a1`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+48/-3，可读 patch 76 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「[perf] dsv3 bmm fallback to bf16」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Align to official impl according to released profile data: https://github.com/deepseek-ai/profile-data/blob/ad8f61ed5e590c4984bb6579f370f9689c427416/decode.json Is...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[Feat] Support FlashMLA backend with MTP and FP8 KV cache」，变更集中在 `python/sglang/srt/layers/attention/flashmla_backend.py`, `python/sglang/srt/layers/attention/flashinfer_mla_backend.py`, `python/sglang/srt/model_executor/cuda_graph_runner.py`。PR 描述补充为：## Motivation This PR improves flashmla backend by accelerating decode stage with mtp. The implementation utilizes the feature that flashmla can handle `seq_len_q > 1`. To use f...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「[Feat] optimize Qwen3 on H20 by hybrid Attention Backend」，变更集中在 `python/sglang/srt/model_executor/model_runner.py`, `python/sglang/srt/model_executor/cuda_graph_runner.py`, `python/sglang/srt/server_args.py`。PR 描述补充为：## Motivation ### FA3 decode performance is significantly lower than flashinfer on H20 #5630 https://github.com/Dao-AILab/flash-attention/issues/1572 I have profiled the Qwen3-2...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/function_call/deepseekv3_detector.py`；关联提交 `461a73028070`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+52/-4，可读 patch 100 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「fix(deepseekv3): Fix DeepSeekV3Detector tool_index assignment and multi-tool call streaming support」，变更集中在 `python/sglang/srt/function_call/deepseekv3_detector.py`。PR 描述补充为：## Motivation This PR focuses to fix the current two issues in `DeepSeekV3Detector`. 1. tool_index: Current the tool_index of each ToolCallItem is assigned based on the index of...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Partially supports using MHA kernels in MLA forward when page-size > 1.」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation add Partially support to use MHA kernels when page-size > 1. related to #6411 ## Modifications add AttnForwardMethod.MHA_PAGED_PREFILL, and when any(forward_batch....
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `51cdd81f9720`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+47/-14，可读 patch 114 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「[fix][RL] Fix DeepSeekV3ForCausalLM.post_load_weights for multiple update weight」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation When doing RL, which will call `post_load_weights` a second time apart from the intialization, the origin `post_load_weights` will somehow corrupt the weight when...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「Use deepgemm instead of triton for fused_qkv_a_proj_with_mqa」，变更集中在 `python/sglang/srt/layers/quantization/fp8_utils.py`。PR 描述补充为：## Motivation ~~do not merge now, I will test and check more first~~ Test: * triton: 41us * change to DeepGEMM: 12us thus more than 3x speedup for this kernel change in my scena...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「Fuse routed scaling factor in topk_reduce kernel」，变更集中在 `python/sglang/srt/layers/moe/fused_moe_triton/fused_moe.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/moe/fused_moe_triton/layer.py`。PR 描述补充为：## Motivation ### gsm8k acc in h200 ### benchmark in H200 command: ### Micro benchmark in h200 micro benchmark in h200: And I use the below code to select triton or `torch.compi...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `c2c4f57f6311`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+386/-13，可读 patch 506 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[DeepseekR1-FP4] Add Support for nvidia/DeepSeekR1-FP4 model」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Adds Model support for DeepSeek R1 FP4 Model. (Functional enablement - kernel optimizations underway) ## Modifications Adds ModelOptFP4FusedMoEMethod and CutlassMo...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Fuse routed scaling factor in deepseek」，变更集中在 `python/sglang/srt/layers/moe/fused_moe_triton/fused_moe.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/moe/fused_moe_triton/layer.py`。PR 描述补充为：## Motivation ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests as outlined in the Running Unit Tests....
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「FlowMLA: zero-overhead DP MLA throughput boost via memory optimization」，变更集中在 `python/sglang/srt/layers/linear.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/model_executor/cuda_graph_runner.py`。PR 描述补充为：## Motivation This PR is part of our upcoming work FlowMLA @antgroup. Targeting MLA in Data Parallel scenarios, we enable serving DeepSeek-like models on fewer devices through s...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Support new DeepGEMM format in per token group quant」，变更集中在 `sgl-kernel/csrc/gemm/per_token_group_quant_8bit.cu`, `sgl-kernel/tests/test_per_token_group_quant_8bit.py`, `sgl-kernel/include/sgl_kernel_ops.h`。PR 描述补充为：## Motivation ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests as outlined in the Running Unit Tests....
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Support new DeepGEMM format in per token group quant (part 2: srt)」，变更集中在 `python/sglang/srt/layers/quantization/fp8_kernel.py`, `python/sglang/srt/entrypoints/engine.py`, `python/pyproject.toml`。PR 描述补充为：## Motivation ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests as outlined in the Running Unit Tests....
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Re-quantize DeepSeek model weights to support DeepGEMM new input format」，变更集中在 `python/sglang/srt/layers/quantization/fp8_utils.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/math_utils.py`。PR 描述补充为：## Motivation ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests as outlined in the Running Unit Tests....
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Refactor DeepGEMM integration」，变更集中在 `python/sglang/srt/layers/quantization/deep_gemm_wrapper/compile_utils.py`, `python/sglang/srt/layers/quantization/deep_gemm_wrapper/entrypoint.py`, `python/sglang/srt/layers/moe/ep_moe/layer.py`。PR 描述补充为：## Motivation ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests as outlined in the Running Unit Tests....
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Support new DeepGEMM」，变更集中在 `python/sglang/srt/layers/quantization/deep_gemm_wrapper/entrypoint.py`, `python/sglang/srt/layers/quantization/fp8_kernel.py`, `python/sglang/srt/layers/moe/ep_moe/layer.py`。PR 描述补充为：## Motivation ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests as outlined in the Running Unit Tests....
...
- 状态/时间: merged / 2025-06-14
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `ed54bf9d19f1`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+11/-96，可读 patch 131 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「[fix] fix dsv3 weight loader tqdm and simplify shared experts fusion」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Using `weights = [w for w in weights_list if w[0] not in names_to_remove]` to update `weights` will change it from async weight iterator to a list. Thats will make...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「feat: mtp support dp-attention」，变更集中在 `python/sglang/srt/models/deepseek_nextn.py`, `python/sglang/srt/model_executor/cuda_graph_runner.py`, `python/sglang/srt/layers/attention/triton_backend.py`。PR 描述补充为：## Motivation mtp support dp-attention - implemented MTP for DP-attention, also fixed related bugs #4783 #4847 . - Enabled CUDA Graph support for both target and draft models at...
...
- 状态/时间: merged / 2025-06-17
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `8c16da334e93`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+10/-7，可读 patch 45 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「Fix Deepseek R1 0528 FP4 tensor name mismatch issue during weights loading.」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation This PR tries to fix a weights loading issue for Deepseek R1 0528 Fp4 from Nvidia. To reproduce the issue: (i'm using blackwell image) If we look into the weight f...
...
- 状态/时间: merged / 2025-06-17
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `4f204db57cf9`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+2/-0，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「fix: resolve b200 dsv3 mtp issue」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests as outlined in the Running Unit Tests....
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `a06912ad8b35`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+3/-3，可读 patch 24 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「Fix judgment condition for enabling Deepseek V3/R1 shared expert fusion optimization」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation ref: https://github.com/sgl-project/sglang/pull/7180 ## Modifications Modify the compatibility version judgment conditions. ## Benchmark The following results were...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `bdbb8d009ae1`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+4/-2，可读 patch 17 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[perf] slightly imporve DeepSeek-R1-FP4 TP8」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Tuned dual stream kernel order, 79->81 ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「Fix MTP with Deepseek R1 Fp4」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation https://github.com/sgl-project/sglang/issues/7365 ## Modifications Update quant config to None for MTP module in Deepseek modelopt fp4. It's not quantized. See htt...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `00aec6ad6c34`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+18/-2，可读 patch 48 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「Apply dsv3_fused_a_gemm kernel」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Apply `dsv3_fused_a_gemm kernel` in https://github.com/sgl-project/sglang/pull/7630. Profile and accuracy test are already listed in the previous PR. Dependency: r...
...
- 状态/时间: merged / 2025-07-02
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `88f484ce4c73`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+21/-3，可读 patch 47 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「Apply dsv3 router gemm kernel for deepseek-r1 fp4」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Apply kernel added in #7627 to dpsk-r1 fp4 model. Currently we have to convert the dtype of router logits back to bf16, otherwise there will be cuda illegal memory...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `84f2e4a0f847`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+15/-2，可读 patch 47 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「fix awq and dsv3 fused gemm compatible」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation added a check to avoid compatibility issues when loading AWQ weights with dsv3 fused gemm enabled related issue https://github.com/sgl-project/sglang/issues/7728 #...
...
- 状态/时间: merged / 2025-07-03
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `b58226510f7d`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+15/-14，可读 patch 61 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「fix dsv3 fused proj check」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation fix dsv3 fused proj check in https://github.com/sgl-project/sglang/pull/7735 ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting...
...
- 状态/时间: merged / 2025-07-03
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `ac49dac00946`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+2/-1，可读 patch 10 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「[fix] fix dsv3_router_gemm filter」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation fix ci on main: https://github.com/sgl-project/sglang/actions/runs/16045971205/job/45277302233#step:5:3827 ## Modifications ## Checklist - [ ] Format your code acc...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `54411f6afa27`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+9/-1，可读 patch 49 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「fix: disable dsv3_router_gemm in dsv3_nextn」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation It has been confirmed that using dsv3_router_gemm leads to a degradation in the acc_len of the dsr1-fp4 nextn module.: https://github.com/sgl-project/sglang/pull/7...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「feat: support DeepSeek-R1-W4AFP8 model with ep-moe mode」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation This PR supports running DeepSeek-R1-W4AFP8 model with ep-moe mode(deepep mode support is on the way~) Due to the reduced space required for model weights and decr...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/function_call/deepseekv3_detector.py`；关联提交 `624a3b8d1f10`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+90/-1，可读 patch 104 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Fix incomplete tool call capture issue in streaming response of DeepSeek-V3 when enable MTP」，变更集中在 `python/sglang/srt/function_call/deepseekv3_detector.py`。PR 描述补充为：## Motivation When using DeepSeeker V3 and enabling MTP and tool call parsing, we found that there are have some unexpected string in the streaming reponse like " \n ". The prob...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `e7e5a3050a64`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-2，可读 patch 10 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 调整模型相关实现，标题为「Update batch size limitation of dsv3_router_gemm kernel to 16」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Raise the batch size limitation to from 4 to 16 for dsv3_router_gemm kernel ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. -...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `examples/chat_template/tool_chat_template_deepseekv3.jinja`, `python/sglang/srt/function_call/deepseekv3_detector.py`；关联提交 `c9dd70fbde92`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+17/-17，可读 patch 98 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补强部署文档、测试或 CI 验证面，标题为「tool-call(dsv3): Improve deepseek-v3 chat template and tool_choice = `required`」，变更集中在 `python/sglang/srt/function_call/deepseekv3_detector.py`, `examples/chat_template/tool_chat_template_deepseekv3.jinja`。PR 描述补充为：## Motivation Improve deepseek-v3 chat template, following #8576 ## Modifications - Improve deepseek-v3 chat template - Remove spaces in the `build_ebnf` ## Accuracy Tests ### A...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `examples/chat_template/tool_chat_template_deepseekv31.jinja`, `python/sglang/srt/function_call/deepseekv31_detector.py`；关联提交 `b9683be6538e`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+315/-0，可读 patch 331 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Support DeepSeek-V3.1 tool call」，变更集中在 `python/sglang/srt/function_call/deepseekv31_detector.py`, `examples/chat_template/tool_chat_template_deepseekv31.jinja`。PR 描述补充为：## Motivation Support tool call for DeepSeek-V3.1 The tool call format of DeepSeek-V3.1 is different from DeepSeek-V3/R1: DeepSeek-V3.1: tool_call_name tool_call_arguments DeepS...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `8abe8deae6cd`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+12/-4，可读 patch 31 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「fix: dsv3 lite q_lora_rank none」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[feat] Support tp mode for DeepSeek-R1-W4AFP8」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Support tp mode for DeepSeek w4a8 model, which has a better performace than ep mode. ## Modifications 1. Add W4AFp8MoEMethod and associated `create_weights`, `proc...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `examples/chat_template/tool_chat_template_deepseekv31.jinja`；关联提交 `cc9a31c66226`
...
- 状态/时间: merged / 2025-09-03
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `0dfd54d11d06`, `1b2ff4fb7f05`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 7 个文件，+458/-62，可读 patch 763 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「Optimized deepseek-v3/r1 model performance on mxfp4 run」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation In order to decrease the activation tensor quantized overhead, fused the quantized behavior into the different ops (activation, layernorm, gemm, flatten) ## Modifi...
...
- 状态/时间: merged / 2025-09-03
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `1b2ff4fb7f05`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 7 个文件，+62/-458，可读 patch 763 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「Revert "Optimized deepseek-v3/r1 model performance on mxfp4 run (#9671)"」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：This reverts commit 0dfd54d11d06eb8363bc7fb3cf9a1f464368caf8. Because #9671 breaks on B200 ## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Chec...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `e96973742c32`
...
- 状态/时间: merged / 2025-09-04
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `918e3d4c27c3`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+100/-69，可读 patch 210 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Fix accuracy drop of dsv3 run in dp enablement」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Got an accuracy issue of dsv3 model run on aiter backend with dp enablement. Issue link: https://github.com/sgl-project/sglang/issues/7692 ## Modifications For ext...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `012584ecd539`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+13/-4，可读 patch 52 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「perf: Avoid unnecessary data type conversions for DeepSeek-V3 on Blackwell」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：Optimize the performance of DeepSeek-V3 on Blackwell for small batch sizes by - Change the output data type of router GEMM to FP32 - Convert the data type of `correction_bias` i...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `400d3b97aebc`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+5/-1，可读 patch 13 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「Fix run time error in dsv3-fp8 model on mi35x」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Got run-time error in running dsv3-fp8 model on mi35x. The error is caused by the activation tensor is changed to tuple of tensors type for the next mxfp4 linear c...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `examples/chat_template/tool_chat_template_deepseekv3.jinja`；关联提交 `2cd94dd07eea`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-1，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「tool-call(dsv3): Fixed a parse problem when there are multiple function definitions in tool_calls」，变更集中在 `examples/chat_template/tool_chat_template_deepseekv3.jinja`。PR 描述补充为：## Motivation When there are multiple function definitions in tool_calls, an error will be reported **TypeError: can only concatenate str (not "dict") to str** ## Modifications...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Fix GPU fault issue when run dsv3 with dp mode and enable torch-compile」，变更集中在 `python/sglang/srt/layers/dp_attention.py`, `python/sglang/srt/layers/logits_processor.py`。PR 描述补充为：## Motivation Got GPU fault when running "python3 -m sglang.launch_server --model-path DeepSeek-V3.1 --tp 8 --trust-remote-code --chunked-prefill-size 131072 --dp-size 8 --enabl...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `examples/chat_template/tool_chat_template_deepseekv32.jinja`；关联提交 `85c1f7937781`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+100/-0，可读 patch 101 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Add DeepSeek-V3.2 Tool Call Template」，变更集中在 `examples/chat_template/tool_chat_template_deepseekv32.jinja`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/dequant_k_cache.py`, `python/sglang/srt/layers/attention/nsa/index_buf_accessor.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa/quant_k_cache.py`, `python/sglang/srt/layers/attention/nsa/tilelang_kernel.py` 等 9 个文件；关联提交 `efbc687c2817`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 29 个文件，+4542/-141，可读 patch 5365 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Support DeepSeek V3.2 Exp」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/tilelang_kernel.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：We now support DeepSeek V3.2. Try this model in the tracking issue #11060 --- Authors: General: @fzyzcjy @DarkSharpness @hnyls2002 @hebiao064 @Fridge003; AMD: @HaiShaw @kkHuang-...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Update DeepSeek-R1-FP4 default config on blackwell」，变更集中在 `python/sglang/srt/server_args.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`；关联提交 `384733639a91`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-0，可读 patch 8 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「[DSv32] Use torch.compile for _get_logits_head_gate」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation fuse 2 unary mul + 1 binary mul in _get_logits_head_gate. ## Modifications ## Accuracy Tests ## Benchmarking and Profiling 1.8% e2e total throughput With torch.com...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[1/N]Support DeepSeek-R1 w4a8 normal deepep」，变更集中在 `python/sglang/srt/layers/moe/cutlass_w4a8_moe.py`, `python/sglang/srt/layers/quantization/w4afp8.py`, `python/sglang/srt/layers/moe/ep_moe/layer.py`。PR 描述补充为：## Motivation Support deepep normal mode for DeepSeek w4a8 model with @yangsijia-serena @rainj-me ## Modifications add forward_cutlass_w4a8 for deepep normal mode ## Command `SG...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `3cceaa381ad3`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 12 个文件，+102/-33，可读 patch 359 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[Bugfix] Fix Qwen3/DSV3/DSV3.2 model support」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation This pr fixes very models that failed to start on npu after dsv3.2 support pr was merged. ci and image release infra was also improved. ## Modifications - Fix mla...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa/utils.py`；关联提交 `20b8d2306c3d`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+3/-66，可读 patch 122 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补强部署文档、测试或 CI 验证面，标题为「Cleaning indexer for DeepSeek V3.2」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa/utils.py`。PR 描述补充为：## Motivation - Fake indexer is for testing accuracy during early development, so can be deprecated now. - Cleaning forward logic. ## Modifications ## Accuracy Tests ## Benchmar...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`；关联提交 `efa473348bc9`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 6 个文件，+445/-79，可读 patch 814 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[Spec Decoding] Support MTP for dsv3.2」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation Based on https://github.com/sgl-project/sglang/pull/11109 We have implemented MTP support for DS v3.2 and ***cuda graph*** in our in-house maintained version of sg...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[1/2] deepseek deterministic: support deterministic inference for deepseek arch models on a single GPU」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/batch_invariant_ops/batch_invariant_ops.py`, `python/sglang/srt/server_args.py`。PR 描述补充为：## Motivation Part of this Issue: https://github.com/sgl-project/sglang/issues/10278 As part of deepseek deterministic inference support, this change ensures deterministic infer...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[2/N]Support DeepSeek-R1 w4a8 low latency deepep」，变更集中在 `python/sglang/srt/layers/moe/ep_moe/kernels.py`, `python/sglang/srt/layers/moe/cutlass_w4a8_moe.py`, `python/sglang/srt/layers/quantization/w4afp8.py`。PR 描述补充为：## Motivation Follow https://github.com/sgl-project/sglang/pull/8247 https://github.com/sgl-project/sglang/pull/7762. Based on https://github.com/sgl-project/sglang/pull/8311. S...
...
- 状态/时间: merged / 2025-10-25
- 反查来源: `git log --name-only -- <model-files>` 反查到 `docs/references/multi_node_deployment/rbg_pd/deepseekv32_pd.md`；关联提交 `729b242934cb`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+723/-3，可读 patch 749 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[Doc] Add documentation for DeepSeek V3.2」，变更集中在 `docs/references/multi_node_deployment/rbg_pd/deepseekv32_pd.md`。PR 描述补充为：Depends on #11844 and #11876 Also waiting for @whybeyoung on some PD updates ## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Fo...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[doc] add example of using w4fp8 for Deepseek」，变更集中在 `benchmark/deepseek_v3/README.md`。PR 描述补充为：## Motivation This PR adds a basic example of deploying w4fp8 Deepseek using SGLang. Below are the evaluation of accuracy. | Model | MMLU | |-------|------| | novita/Deepseek-V3...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Support running FP4 Deepseek on SM120.」，变更集中在 `python/sglang/srt/layers/quantization/modelopt_quant.py`, `python/sglang/srt/models/gpt_oss.py`, `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation sm120 is not runnable for deepseek because of sm check and invalid kernels. ## Modifications - add `is_blackwell_supported` as an extend to `is_sm100_supported` -...
...
- 状态/时间: merged / 2025-10-28
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/dequant_k_cache.py`, `python/sglang/srt/layers/attention/nsa/quant_k_cache.py`, `python/sglang/srt/layers/attention/nsa_backend.py`；关联提交 `81a632ace647`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+367/-44，可读 patch 626 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[DeepseekV32] Enable flashmla_prefill kernel with fp8 kvcache」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/dequant_k_cache.py`, `python/sglang/srt/layers/attention/nsa/quant_k_cache.py`。PR 描述补充为：## Motivation Add logics to dequant the kvcache from fp8 to bf16 in a separate kernel and use `flashmla_prefill` kernel with fp8 kvcache. - [x] Verify accuracy and perf on both...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa_backend.py`；关联提交 `42e1a72efb78`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-3，可读 patch 13 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[Deepseek V3.2] Enable flashmla_auto with MTP」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`。PR 描述补充为：## Motivation Follow up to https://github.com/sgl-project/sglang/pull/11655 ## Modifications With fp8 kvcache on B200, flashmla_sparse can be used in the extend (not draft_exten...
...
- 状态/时间: merged / 2025-10-29
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `e39628fd07b3`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+19/-0，可读 patch 54 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[2/2] Deepseek deterministic: support deepseek v3 deterministic inference on 8 x H200」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation part of roadmap Previous PR supported deepseek arch model's deterministic inference on a single Hopper GPU. This PR is to further support full deepseek v3 model's...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/models/deepseek_v2.py`；关联提交 `9ff9fa7f95be`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+110/-22，可读 patch 231 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「Fuse wk and weight_proj in Indexer for DeepSeekV3.2-FP4」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Since wk and weight_proj in Indexer have the same input, they can be fused into a single gemm by concatenating weights. For the base DeepSeekV3.2-Exp model, wk is...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/transform_index.py`；关联提交 `662725b936a2`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-1，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「[DeepSeekV32] Bug fix to ensure `page_table` and `result` in same type」，变更集中在 `python/sglang/srt/layers/attention/nsa/transform_index.py`。PR 描述补充为：Running transform_index.py in NSA attention path fails at torch.gather(...) with a dtype mismatch: the index/out tensor is int (int32) while PyTorch requires Long (int64) for ga...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/models/deepseek_v2.py`；关联提交 `f235498eca7a`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+188/-4，可读 patch 255 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「DeepSeek-V3.2: Add Adaptive MHA Attention Pathway for Short-Sequence Prefill」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation For DeepSeek-V3.2 models, using MLA (Multi-Latent Attention) uniformly across all sequence lengths during prefill is suboptimal. For short sequences, the overhead...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 调整模型相关实现，标题为「Update dsv3 quantization auto setting for sm100」，变更集中在 `python/sglang/srt/server_args.py`。PR 描述补充为：## Motivation update quantization based on model quant_method
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/models/deepseek_v2.py`；关联提交 `7257525ccea6`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+53/-6，可读 patch 96 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[DeepSeek-V3.2][NSA] Enable MHA Pathway for Short Sequence Prefill on B200 (SM100)」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Follow up this PR: DeepSeek-V3.2: Add Adaptive MHA Attention Pathway for Short-Sequence Prefill. Enable and optimize MHA on B200 (SM100) in the NSA backend by TRT-...
...
- 状态/时间: merged / 2025-11-07
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/index_buf_accessor.py`；关联提交 `125f76ea44d8`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+617/-1，可读 patch 633 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[Test] Add DeepSeekV3.2 NSA Indexer Test Suite」，变更集中在 `python/sglang/srt/layers/attention/nsa/index_buf_accessor.py`。PR 描述补充为：## Motivation Address the issue https://github.com/sgl-project/sglang/issues/12509 ## Modifications Added 11 tests: 1. test_forward_decode_mode 2. test_forward_extend_mode 3. te...
...
- 状态/时间: merged / 2025-11-07
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/models/deepseek_v2.py`；关联提交 `bef37d6de86a`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+20/-18，可读 patch 112 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「[Deepseek V3.2] Only skip Indexer logits computation when is_extend_without_speculative」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation Fixed a bug in https://github.com/sgl-project/sglang/pull/12788 ## Modifications We should only skip Indexer logits computation when forward_mode.is_extend_without...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `4a78031a71dd`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 6 个文件，+331/-22，可读 patch 474 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「[ROCM] Optimized deepseek-r1 model with rmsnorm + fp8 quant fusion」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Fusion of rmsnorm + group scale FP8 quantization (group_size=128) w/o fusion w/ fusion ## Modifications deepseek.py - LayerCommunicator (for hidden states) - Deeps...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`；关联提交 `0d4a41842424`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 6 个文件，+96/-17，可读 patch 231 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「[Deepseek V3.2] Fix accuracy bug in the Indexer」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`。PR 描述补充为：## Motivation Must merge after https://github.com/sgl-project/sglang/pull/12582 Part 2 of the fix for https://github.com/sgl-project/sglang/issues/11629 This PR also fixed the m...
...
- 状态/时间: merged / 2025-11-12
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa_backend.py`；关联提交 `4eda9969e8b9`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+7/-6，可读 patch 62 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补强部署文档、测试或 CI 验证面，标题为「[DeepseekV32]: use `_concat_mla_absorb_q_general` to replace `torch.cat`」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`。PR 描述补充为：## Motivation https://github.com/sgl-project/sglang/issues/11989 `torch.cat([q_nope, q_rope], dim=-1)` is heavily used in `nsa_backend`, which is less efficient. ## Modification...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/function_call/deepseekv31_detector.py`；关联提交 `fc5da1e80b78`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+4/-9，可读 patch 34 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 调整模型相关实现，标题为「[Tool Call] Steamline function arguments when tool_choice="auto" for deepseekv31_detector」，变更集中在 `python/sglang/srt/function_call/deepseekv31_detector.py`。PR 描述补充为：## Motivation The current `deepseekv31_detector` only detects complete function arguments and immediately returns them while clearing the buffer. This approach can lead to a poo...
...
- 状态/时间: merged / 2025-11-14
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`；关联提交 `a7002e614bbd`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+68/-76，可读 patch 248 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「[Deepseek V3.2] Clean up MTP」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Modifications - Fix an accuracy bug in MTP draft_extend stage. The `seqlens_int32_cpu` used in the calculations of `seqlens_expanded` is incorrect. We should use `seq_lens_cp...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa/utils.py`, `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/communicator_nsa_cp.py`, `python/sglang/srt/models/deepseek_v2.py`；关联提交 `d368c7451a48`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 17 个文件，+1247/-54，可读 patch 1729 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「(1/n)support context parallel with deepseekv3.2-DSA」，变更集中在 `python/sglang/srt/layers/attention/nsa/utils.py`, `python/sglang/srt/layers/communicator_nsa_cp.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation Currently, under deepseek3.2-DSA, prefill-ttft of long text sequences takes a long time. Introducing context parallel can reduce ttft. **Main design ideas：** Takin...
...
- 状态/时间: merged / 2025-11-17
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa_backend.py`；关联提交 `a8fcbf6fe3a2`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+22/-1，可读 patch 37 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 调整模型相关实现，标题为「[Deepseek V3.2] Use torch.compile to speed up torch.cat in nsa」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`。PR 描述补充为：## Motivation Use torch.compile to speed up torch.cat used in prefill. ## Modifications Replace the cat ops for k in `forward_extend` in the nsa attention with the torch.compile...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `e389f91decda`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 5 个文件，+117/-4，可读 patch 171 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「[NVIDIA] Fix broken fp8 MoE of deepseek v3」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation This breaks the fp8 moe of the deepseek v3, causing: the root cause is now the routing_method_type becomes a default attribute of FusedMoE layer. So, we should exp...
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-0，可读 patch 8 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「[Fix] Fix DeepSeek V3 MTP on B200」，变更集中在 `python/sglang/srt/models/deepseek_nextn.py`。PR 描述补充为：## Motivation Fix the bug here https://github.com/sgl-project/sglang/actions/runs/19449452397/job/55665254802 ## Modifications ## Accuracy Tests Test with GSM8K ## Benchmarking...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `c8ede0e93c3a`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+61/-15，可读 patch 125 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[ROCM] Optimized deepseek-r1 fp8 model with + triton_gemm_a8w8 + batch_gemm_a8w8 + fused set_mla_kv_buffer kernel」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Co-author : @yichiche To optimize the deepseek-r1 model performance on ROCM. This PR improves the performance of a8w8 GEMM and enable batched_gemm and mla_kv_buffe...
...
- 状态/时间: merged / 2025-11-20
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/models/deepseek_v2.py`；关联提交 `fa924410276d`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+55/-9，可读 patch 114 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[DeepseekV3.2] Deepseek fp8 support for MHA path」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/attention/nsa_backend.py`。PR 描述补充为：## Motivation Enable FP8 KV cache support for MHA (Multi-Head Attention) path in DeepSeek NSA backend. Previously, MHA could only be used when KV cache dtype is BF16, limiting i...
...
- 状态/时间: merged / 2025-11-20
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/models/deepseek_v2.py`；关联提交 `7291c72e575d`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+92/-124，可读 patch 345 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补强部署文档、测试或 CI 验证面，标题为「[Deepseek V3.2] Change indexer weights_proj to fp32」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Following https://github.com/deepseek-ai/DeepSeek-V3.2-Exp/commit/8631a813356d39b09a5c1ee1cde8ed6015559eb3, change the indexer `weights_proj` precision from bf16 t...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `eff7df6d0a49`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+92/-26，可读 patch 162 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[AMD] Enable fused shared expert append and flatten quant for fp8 deepseekR1 model」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Co-author : @yctseng0211 This PR introduces two performance improvements for DeepseekR1-0528 (fp8): 1. Support aiter fused_flatten_fp8_group_quant for fp8 DeepSeek...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「[Perf] Optimize DeepSeek-R1 w4afp8 glue kernels」，变更集中在 `python/sglang/srt/layers/moe/ep_moe/kernels.py`, `python/sglang/srt/layers/moe/cutlass_w4a8_moe.py`, `python/sglang/srt/layers/quantization/w4afp8.py`。PR 描述补充为：## Motivation This PR improves DeepSeek-R1 w4afp8 TP8/EP8 ITL performance. It is motivated by the following profiling trace, obtained by running DeepSeek-R1 w4afp8 TP8 with a co...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/models/deepseek_v2.py`；关联提交 `5eed5fc0b091`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+74/-78，可读 patch 342 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 调整模型相关实现，标题为「[DeepSeekV3.2] Centralize NSA dispatch logic in NativeSparseAttnBackend」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation NativeSparseAttnBackend currently spreads dispatch logic for NSA prefill/decode implementations and MHA vs. MLA selection across multiple places: - Global `NSA_PRE...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `13e5beeab499`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-1，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「Fix Deepseek v3.1 loading issue」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：Fix nightly test error in https://github.com/sgl-project/sglang/actions/runs/19689716521/job/56402800532
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`；关联提交 `decb48965dd1`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 7 个文件，+286/-24，可读 patch 460 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[DeepSeekV3.2] Enable pure TP & Partial DP Attention」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`。PR 描述补充为：## Motivation DeepSeekV3.2 NSA currently has rough edges when running in **pure TP mode** (`dp_size Serving with one request: TP mode: DP Attention: ## Checklist - [x] Format yo...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[feature] implement dcp for deepseek_v2」，变更集中在 `python/sglang/srt/layers/attention/utils.py`, `python/sglang/srt/model_executor/forward_batch_info.py`, `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla.py`。PR 描述补充为：## Motivation Here's the first step to fully implement #12196 to support much longer context with TP 8 under 8xH20. Currently, it only works with attention backend flashinfer. I...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`；关联提交 `7dfcc78155b6`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+195/-63，可读 patch 300 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[DeepseekV3.2][NSA][Indexer] Fix PAGED top-k transform for NSA indexer chunked execution on H200」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation In `extend` mode with NSA indexer enabled, H200 setups that select **PAGED** top-k transform (`flashmla_kv` + FP8 KV cache) may trigger chunked execution in `_get_...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `ea177372bd8c`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 11 个文件，+123/-59，可读 patch 459 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「support mtp with deepseek r1 nvfp4 model」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：collabrate with @trevor-m ## Motivation Support large scale EP deployment for the DS R1 fp4 model with eagle spec decoding. ## Modifications - add the custom moe a2a backend for...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/function_call/deepseekv32_detector.py`；关联提交 `b7b7524e9560`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+144/-7，可读 patch 165 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「[Tool Call] Fix DeepSeekV32Detector skipping functions with no params in streaming mode」，变更集中在 `python/sglang/srt/function_call/deepseekv32_detector.py`。PR 描述补充为：## Motivation When using `--tool-call-parser deepseekv32` in streaming mode, tool calls for functions with no parameters (e.g., `get_date()`) were being silently skipped, while...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `examples/chat_template/tool_chat_template_deepseekv31.jinja`；关联提交 `ef1ab2302ab2`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+5/-1，可读 patch 13 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 调整模型相关实现，标题为「[Auto Sync] Update tool_chat_template_deepseekv31.jinja (20251210)」，变更集中在 `examples/chat_template/tool_chat_template_deepseekv31.jinja`。PR 描述补充为：Sync changes from commit `abd3e13d`. **Files Changed:** - examples/chat_template/tool_chat_template_deepseekv31.jinja Author: Jue Wang --- *This is an automated PR created by sc...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/hardware_backend/npu/modules/deepseek_v2_attention_mla_npu.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa/utils.py`, `python/sglang/srt/layers/communicator_nsa_cp.py`；关联提交 `388018a5bd41`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 8 个文件，+281/-134，可读 patch 587 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[NPU]dsv3.2 cp for npu」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa/utils.py`, `python/sglang/srt/layers/communicator_nsa_cp.py`。PR 描述补充为：## Motivation Now, sglang already has the --enable-nsa-prefill-context-parallel option proposed to support CP parallelism, (1/n)support context parallel with deepseekv3.2-DSA ,...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「Fix dsv3 dp accuracy issue when using bf16-kv」，变更集中在 `python/sglang/srt/layers/attention/aiter_backend.py`。PR 描述补充为：## Motivation cherry-pick "Fix dp issue dsv3 bf16 kv #14895" to `amd_mori` branch ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/hardware_backend/npu/modules/deepseek_v2_attention_mla_npu.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/models/deepseek_v2.py`；关联提交 `c05d3afb5d8b`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 11 个文件，+141/-68，可读 patch 393 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「[NPU] optimization for dsv3.2」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/hardware_backend/npu/modules/deepseek_v2_attention_mla_npu.py`。PR 描述补充为：Co-author: @jiaming1130 ## Motivation In this PR, we optimized the performance of the DeepSeek-V3.2 on NPU to improve the performance of the network in the decode phase, and als...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/dequant_k_cache.py`, `python/sglang/srt/layers/attention/nsa_backend.py`；关联提交 `c96903074c4e`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+19/-5，可读 patch 52 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「[NSA] Fix NSA backend assertion error when running DeepSeek-V3.2 PP with radix-cache」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/dequant_k_cache.py`。PR 描述补充为：## Motivation Fix NSA backend assertion error when running DeepSeek-V3.2 triggered when running DeepSeek-V3.2 PP with radix-cache. The assertion `num_tokens <= total_num_tokens`...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `8ac350f335c6`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+11/-1，可读 patch 19 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[AMD] Support fused_rms_mxfp4_quant in the prefill stage for DeepSeek-R1-MXFP4」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Adopted the fused kernel fused_rms_mxfp4_quant to replace the existing RMSNorm + quant path, enabling more efficient preparation of activations for quantized proje...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「[Performance] Optimize group gemm in DeepSeek-R1-W4AFP8 w4a8 moe」，变更集中在 `python/sglang/srt/layers/moe/cutlass_w4a8_moe.py`, `sgl-kernel/csrc/moe/cutlass_moe/w4a8/w4a8_grouped_mm_c3x.cu`, `sgl-kernel/csrc/moe/cutlass_moe/w4a8/scaled_mm_entry.cu`。PR 描述补充为：## Motivation While deploying and optimizing the w4a8 moe group gemm for DeepSeek-R1-W4AFP8, I found that when w4a8 is in deepep low_latency mode, only the final else branch of...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Fix the accuracy issue when running mxfp4 dsv3 model and enable ep」，变更集中在 `python/sglang/srt/layers/quantization/mxfp4.py`, `python/sglang/srt/layers/quantization/quark/quark_moe.py`。PR 描述补充为：## Motivation Fix the accuracy problem ## Modifications Add expert mask in moe function call ## Accuracy Tests `export SGLANG_AITER_MLA_PERSIST=0 python3 -m sglang.launch_server...
...
- 状态/时间: merged / 2025-12-17
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`；关联提交 `d20699a33c50`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+25/-8，可读 patch 82 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[Deepseek V3.2] Support Overlap Spec + NSA」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation Part of V3.2 Roadmap https://github.com/sgl-project/sglang/issues/15025 Enable overlap spec and EAGLE + NSA backend. ## Modifications In EAGLE V1, we had (with `py...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/function_call/deepseekv32_detector.py`；关联提交 `41683536d394`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+111/-69，可读 patch 328 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「feat: DeepSeek-V3.2 Streaming tool call output」，变更集中在 `python/sglang/srt/function_call/deepseekv32_detector.py`。PR 描述补充为：## Motivation FIxes #14711 ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「[Performance] sgl-kernel, optimize group gemm in DeepSeek-R1-W4AFP8 w4a8 moe」，变更集中在 `sgl-kernel/csrc/moe/cutlass_moe/w4a8/w4a8_grouped_mm_c3x.cu`, `sgl-kernel/csrc/moe/cutlass_moe/w4a8/scaled_mm_entry.cu`, `sgl-kernel/python/sgl_kernel/cutlass_moe.py`。PR 描述补充为：## Motivation https://github.com/sgl-project/sglang/pull/15315 While deploying and optimizing the w4a8 moe group gemm for DeepSeek-R1-W4AFP8, I found that when w4a8 is in deepep...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「[perf]optimize w4afp8 kernel on deepseek-v3-0324」，变更集中在 `python/sglang/srt/layers/quantization/w4afp8.py`, `sgl-kernel/csrc/moe/cutlass_moe/w4a8/w4a8_grouped_mm_c3x.cu`, `sgl-kernel/csrc/moe/cutlass_moe/w4a8/w4a8_moe_data.cu`。PR 描述补充为：## Motivation we use w4afp8 deepseekv3-0324 online, and we find its performance is not good enough when decode batch size < 32 ## Modifications fine-grained tiling config and ba...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa_backend.py`；关联提交 `e88e75a9dfdd`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-1，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「[Deepseek V3.2] Fix Deepseek MTP in V1 mode」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`。PR 描述补充为：## Motivation ## Modifications Restore the earlier behaviour of Deepseek V3 + MTP. (Accidental change) Fix https://github.com/sgl-project/sglang/issues/15428
...
- 状态/时间: merged / 2025-12-19
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`；关联提交 `6afc5d497bb3`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+91/-5，可读 patch 166 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[DSv32] Move deep_gemm.get_paged_mqa_logits_metadata to init time as metadata」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation https://github.com/sgl-project/sglang/issues/15025 ## Changes - Add `paged_mqa_schedule_metadata` to `NSAMetadata` (batch-level caching). - Compute once in `init_f...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Support piecewise cuda graph for dsv3 fp4」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation https://github.com/sgl-project/sglang/issues/11490 ## Accuracy Tests
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/function_call/deepseekv32_detector.py`；关联提交 `01bd0d3e8b8c`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+60/-29，可读 patch 200 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[Tool Call][DSV32] Streamline function call parameters」，变更集中在 `python/sglang/srt/function_call/deepseekv32_detector.py`。PR 描述补充为：## Motivation As titled and Issue #14711. **IMPORTANT: This is an initial version aimed at streamlining function call parameters for `DeepSeek-V3.2`. I have tested it, and it pa...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `2ec6fa3c54a8`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+12/-1，可读 patch 34 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「feat PD: add eagle3 support for DeepSeek V3 in EP mode」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：When try to enable EP MOE for DeepSeek V3 with EAGLE3, SGL_ENABLE_JIT_DEEPGEMM_BMM=1 SGL_ENABLE_JIT_DEEPGEMM=1 \ TORCHINDUCTOR_CACHE_DIR=/home/admin/inductor_root_cache /opt/con...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Clean Some Environment Variables for DeepSeek V32」，变更集中在 `python/sglang/srt/layers/attention/nsa/quant_k_cache.py`, `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/utils.py`。PR 描述补充为：## Motivation ## Modifications - Move `SGLANG_NSA_FUSE_TOPK` and `SGLANG_NSA_ENABLE_MTP_PRECOMPUTE_METADATA` to environ.py - Deprecate `SGLANG_NSA_DUAL_STREAM`, `SGLANG_NSA_FLAS...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/models/deepseek_v2.py`；关联提交 `20abaee26cd8`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+64/-22，可读 patch 134 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 调整模型相关实现，标题为「[DSv32] Overlap indexer weights_proj during dual_stream decode」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation The weights_proj in the DSA indexer uses float type, which is slow on modern GPUs. Profiler traces show this accounts for ~20% of decode layer runtime. The traces...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 调整模型相关实现，标题为「[Refactor] Split out deepseek v2 weight loader function into mixin」，变更集中在 `python/sglang/srt/models/deepseek_common/deepseek_weight_loader.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/models/deepseek_common/utils.py`。PR 描述补充为：## Motivation DeepseekV2 code has been developed fast and a lot of historical code and be more orgranized, including the weight loading part. Issue related: https://github.com/s...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「Use dsv3 optimized routing `fused_topk_deepseek` instead of `moe_fused_gate`」，变更集中在 `python/sglang/srt/layers/moe/topk.py`, `test/registered/kernels/test_fused_topk_deepseek.py`, `test/srt/test_deepseek_v3_mtp.py`。PR 描述补充为：## Motivation flashinfer has an optimized routing kernel for DeepSeek V3: https://github.com/flashinfer-ai/flashinfer/pull/2099 The API was renamed to `fused_topk_deepseek` here...
...
- 状态/时间: merged / 2026-01-19
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa_backend.py`；关联提交 `d2105d4abda6`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+26/-12，可读 patch 105 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「[DeepSeek v3.2] Opt MTP decode cuda batch sizes and nsa implementation」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`。PR 描述补充为：## Motivation 1、`draft_extend` and `target_verify` use `nsa_decode_backend` as backend implementation instead of `nsa_prefill_backend` In the MTP scenario, the NSA attention imp...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `test/registered/amd/accuracy/mi35x/test_deepseek_r1_mxfp4_eval_mi35x.py`, `test/registered/amd/perf/mi35x/test_deepseek_r1_mxfp4_perf_mi35x.py`；关联提交 `a3addd6203ca`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 18 个文件，+1140/-99，可读 patch 1539 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[AMD] Add DeepSeek-V3.2 and VLMs model in nightly tests」，变更集中在 `test/registered/amd/accuracy/mi35x/test_deepseek_r1_mxfp4_eval_mi35x.py`, `test/registered/amd/perf/mi35x/test_deepseek_r1_mxfp4_perf_mi35x.py`。PR 描述补充为：## Motivation Add 5 unique test in AMD coverage: https://github.com/sgl-project/sglang/actions/runs/21153414554/attempts/1#summary-60833755716 - Add accuracy and perf tests for...
...
- 状态/时间: merged / 2026-01-20
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`；关联提交 `612026ad2c82`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+5/-4，可读 patch 32 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「[OPT] DeepSeekV3.2: optimize indexer weight_proj-mma performance」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation From these pr[https://github.com/sgl-project/sglang/pull/16637 and https://github.com/sgl-project/sglang/issues/16861 and https://github.com/sgl-project/sglang/pul...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/hardware_backend/npu/modules/deepseek_v2_attention_mla_npu.py`, `python/sglang/srt/models/deepseek_v2.py`；关联提交 `c0b5a180febe`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 5 个文件，+129/-46，可读 patch 224 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「[NPU]bugfix: fix for dsv3.2 and dsvl2」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/hardware_backend/npu/modules/deepseek_v2_attention_mla_npu.py`。PR 描述补充为：## Motivation 1、There are some bugs for DS-Vl2 on rotary_embedding 2、DSV32 is not compatible with the scenario where m.alt_stream is not None. ## Modifications 1、Fix the ds-v12...
...
- 状态/时间: merged / 2026-01-23
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa_backend.py`；关联提交 `2fb328109fb9`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+118/-31，可读 patch 228 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[DeepSeek V3.2] Enable trtllm NSA with bf16 kvcache」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`。PR 描述补充为：## Motivation Enables NSA backend with trtllm kernels for sparse attention. This can be more efficient than FlashMLA when the head size isn't a multiple of 64 and hence requires...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa_backend.py`；关联提交 `1674b9ef4494`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+18/-1，可读 patch 47 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[DeepSeek-V3.2] Fix TRT-LLM NSA in target_verify/draft_extend」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`。PR 描述补充为：## Motivation Quick follow‑up to #16758: speculative decoding still fails with `--nsa-decode-backend trtllm`. ## Modifications - Add TRT‑LLM handling in the extend path for spec...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/hardware_backend/npu/modules/deepseek_v2_attention_mla_npu.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`；关联提交 `b56366f8275a`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 5 个文件，+195/-61，可读 patch 364 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[NPU]DeepSeek-V3.2 support npu mlaprolog」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/hardware_backend/npu/modules/deepseek_v2_attention_mla_npu.py`。PR 描述补充为：## Motivation we enabled the DSV3.2 to support npu mla_prolog_v3 to improve the performance ## Modifications - Added support for the mla_prolog_v3 custom operator, which include...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`；关联提交 `a8dda2aa5727`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+4/-4，可读 patch 16 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 调整模型相关实现，标题为「[DSv32] Overlap indexer qk projection and activation quant」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation @humansand After https://github.com/sgl-project/sglang/pull/17205, the indexer weight projection is fully hidden and no longer exposes latency. ## Modifications ##...
...
- 状态/时间: merged / 2026-01-28
- 反查来源: `git log --name-only -- <model-files>` 反查到 `test/registered/amd/accuracy/mi35x/test_deepseek_r1_eval_mi35x.py`, `test/registered/amd/accuracy/mi35x/test_deepseek_r1_mxfp4_eval_mi35x.py`；关联提交 `f8636fbb253a`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 27 个文件，+1540/-43，可读 patch 1823 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[AMD] Add Kimi-K2, DeepSeek-V3.2 tests to nightly CI」，变更集中在 `test/registered/amd/accuracy/mi35x/test_deepseek_r1_eval_mi35x.py`, `test/registered/amd/accuracy/mi35x/test_deepseek_r1_mxfp4_eval_mi35x.py`。PR 描述补充为：## Motivation 1. Add **Kimi-K2**, **DeepSeek-V3.2** accuracy and performance tests for MI325 (MI30x) platform, update Mi35x tests, consolidate test jobs, and fix various CI fail...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「Fix OOM in DeepSeek weight loading by deferring dict(weights) materialization」，变更集中在 `python/sglang/srt/models/deepseek_common/deepseek_weight_loader.py`。PR 描述补充为：## Summary - Fix OOM when loading DeepSeek models with streaming/remote weight loading - Defer `dict(weights)` call until quantization is actually needed - Return original weigh...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa/utils.py`, `python/sglang/srt/layers/attention/nsa_backend.py`, `test/registered/kernels/test_nsa_indexer.py`；关联提交 `677f3c49dabf`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+58/-7，可读 patch 135 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「[DeepSeek V3.2] [Bugfix] slice indexer and padding fa3 when can not run cuda graph」，变更集中在 `python/sglang/srt/layers/attention/nsa/utils.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`。PR 描述补充为：## Motivation This PR #15227 only resolves the issue occurring during `forward_idle`. If running in Eagle mode (MTP Spec2, batch size exceeding the CUDA maximum batch size, or w...
...
- 状态/时间: merged / 2026-02-02
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`；关联提交 `b0a6d5244cef`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+16/-2，可读 patch 60 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[NPU] support dsv32 radixcache on ascend」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation Ascend currently does not support the radixcache feature on the DeepSeek-3.2 model ## Modifications 1. We have modified the inputs of `npu_sparse_flash_attention`...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/models/deepseek_v2.py`；关联提交 `e8a2c133807c`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 5 个文件，+33/-19，可读 patch 131 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Deepseekv32 compatibility with transformers v5」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation ## Modifications 1. Enhance Indexer and DeepseekV2AttentionMLA to support dynamic is_neox_style parameter. 2. Refactor rope parameter handling in DeepseekV2Decoder...
...
- 状态/时间: merged / 2026-02-10
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/tilelang_kernel.py`；关联提交 `4262f5259b94`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+257/-0，可读 patch 271 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「Tilelang sparse decode fwd for dsv32 mi355」，变更集中在 `python/sglang/srt/layers/attention/nsa/tilelang_kernel.py`。PR 描述补充为：cc @HaiShaw, @kkHuang-amd, @hubertlu-tw ## Motivation The current dsv32 attention implementation uses a single TileLang kernel for both prefill and decode. This kernel is optimi...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`；关联提交 `2cc235e7952e`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+16/-8，可读 patch 62 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「Fix Bug on dsv3.2」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation Fix two bugs on dsv32 when infering on npu ## Modifications 1. fix a bug: forward_npu method of nas_Indexer has a dual stream feature that can not be closed. May c...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「[AMD] Fix accuracy issue when running TP4 dsv3 model with mtp」，变更集中在 `python/sglang/srt/layers/attention/aiter_backend.py`, `docker/rocm.Dockerfile`。PR 描述补充为：cc @HaiShaw, @kkHuang-amd ## Motivation Fixed dpsk-r1-fp4 accuracy issue on tp4 with mtp. ## Modifications - aiter backend change in sglang. - aiter update for tp4 fp8 prefill a...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `190fa8246fbc`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+8/-4，可读 patch 19 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Fix model loading for DeepSeek-V3.2-AWQ」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Got the following error when using DeepSeek-V3.2-AWQ (https://huggingface.co/QuantTrio/DeepSeek-V3.2-AWQ/) ## Modifications Add check of whether `weight_packed` ex...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/models/deepseek_v2.py`, `test/registered/kernels/test_nsa_indexer.py`；关联提交 `0ffd0a3995e5`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 10 个文件，+352/-183，可读 patch 914 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Nsa trtllm mla sparse fp8 support with Deepseek v3.2 NVFP4」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/models/deepseek_v2.py`, `test/registered/kernels/test_nsa_indexer.py`。PR 描述补充为：## Motivation #17655 - support Deepseek v3.2 NVFP4 with trtllm mla sparse fp8 attention backend ## Modifications - update the nsa backend to support trtllm sparse fp8 attention...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[Kernel] Add JIT support for DeepSeek V3 GEMM」，变更集中在 `python/sglang/jit_kernel/csrc/gemm/dsv3_fused_a_gemm.cuh`, `python/sglang/jit_kernel/csrc/gemm/dsv3_router_gemm.cuh`, `python/sglang/jit_kernel/benchmark/bench_dsv3_router_gemm.py`。PR 描述补充为：## Motivation This PR migrates DeepSeek V3 GEMM kernels to the JIT (Just-In-Time) compilation framework, including: - `dsv3_fused_a_gemm` - `dsv3_router_gemm` related roadmap: h...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa_backend.py`；关联提交 `462267982bd8`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-1，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[AMD] Fix mi35x dsv32 mtp nightly」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`。PR 描述补充为：## Motivation PR #17554 introduced `fused_metadata_copy`, a JIT-compiled CUDA kernel enabled by default. This kernel cannot run on ROCm/HIP — it fails on every CUDA graph replay...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `test/registered/kernels/test_nsa_indexer.py`；关联提交 `eddf193292d3`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+48/-9，可读 patch 121 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「[DSv32] [GLM5] Improve Model Quality by Avoiding FP32 Precision Loss in `weights_proj`」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `test/registered/kernels/test_nsa_indexer.py`。PR 描述补充为：@humansand ## Motivation From recent GLM-5 tech report: > Compared with the non-deterministic CUDA-based top‑k implementation used in SGLang’s DSA Indexer, directly using the na...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `c193a52fa263`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+13/-9，可读 patch 67 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「[AMD] DSR1/V3 use fp8 bmm in MLA for MI300X」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Improve TPOT by using use fp8 bmm in MLA on MI300X for DSR1/V3 ## Modifications - [x] q@w_kc: bmm bf16 -> bmm_fp8 - [x] attn_output@w_vc: bmm bf16 -> bmm_fp8 ## Ac...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「[ROCm] Optimize Deepseek R1 on MI300X」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation The DeepSeek-V3/R1 model on AMD MI300X (gfx942) was previously limited to fallback code paths for several MLA attention operations because the optimized kernels we...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/jit_kernel/csrc/nsa/fused_store_index_cache.cuh`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`；关联提交 `4e843f121657`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+307/-21，可读 patch 386 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[DeepSeek-V3.2][JIT-kernel] Support nsa fuse store indexer k cache」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/jit_kernel/csrc/nsa/fused_store_index_cache.cuh`。PR 描述补充为：## Motivation In DeepSeek v3.2, after the Indexer produces key in bf16 (roughly (N, 128)), it needs to populate NSA’s index_k_with_scale_buffer. The previous implementation used...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「[AMD] Fix weight load shape mismatch for amd dsr1 0528 mxfp4」，变更集中在 `python/sglang/srt/models/deepseek_nextn.py`, `python/sglang/srt/layers/quantization/quark/quark.py`。PR 描述补充为：> Co-authored-by: Haoyang Li > Co-authored-by: billishyahao ## Motivation This patch is to fix tensor shape mismatch issue for amd dsr1 0528 mxfp4 https://huggingface.co/amd/Dee...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「[3/n] deepseek_v2.py Refactor: Migrate MLA forward method in deepseek_v2.py」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla.py`, `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla_fused_rope_rocm.py`。PR 描述补充为：## Motivation Part of #16255 Split different MLA forward methods into separate files, including: - `forward_mla.py` for Cuda/HIP mla aborption - `forward_mla_fused_rope_cpu.py‎`...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/hardware_backend/npu/modules/deepseek_v2_attention_mla_npu.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/models/deepseek_v2.py`；关联提交 `b3718982a1b4`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 5 个文件，+101/-9，可读 patch 316 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[Feature] add feature mla_ag_after_qlora for dsv3.2」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/hardware_backend/npu/modules/deepseek_v2_attention_mla_npu.py`。PR 描述补充为：## Motivation In DeepSeek-V3.2, due to the large hidden dimension of the model, performing all-gather on hidden_states before attention introduces a high-dimensional communicati...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/function_call/deepseekv32_detector.py`；关联提交 `6af0448cc9bf`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+6/-3，可读 patch 16 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] Catch errors when DeepSeek-V3.2 generates malformed JSON」，变更集中在 `python/sglang/srt/function_call/deepseekv32_detector.py`。PR 描述补充为：## Motivation LLMs may generate tool call content that are not correct json format, we must catch this error for normal streaming output. ## Modifications ## Accuracy Tests ## B...
...
- 状态/时间: merged / 2026-03-03
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/function_call/deepseekv32_detector.py`；关联提交 `666caaf9ce76`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+31/-22，可读 patch 151 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[Tool Call] Stream DeepSeek-V3.2 function call parameters in JSON format.」，变更集中在 `python/sglang/srt/function_call/deepseekv32_detector.py`。PR 描述补充为：## Motivation The previous implementation only supported streaming function call parameters in XML format. ## Modifications 1. Convert `_parse_parameters_from_xml` to return `st...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[AMD] CI - Add MI35x nightly/PR tests for kv-cache-fp8 and allreduce-fusion (DeepSeek)」，变更集中在 `test/registered/amd/accuracy/mi35x/test_deepseek_r1_mxfp4_kv_fp8_eval_mi35x.py`, `test/registered/amd/accuracy/mi35x/test_deepseek_r1_mxfp4_ar_fusion_eval_mi35x.py`, `.github/workflows/nightly-test-amd-rocm720.yml`。PR 描述补充为：## Motivation Track accuracy and performance regression for two new DeepSeek-R1-MXFP4 server configurations on MI35x: - `--kv-cache-dtype fp8_e4m3` - `--enable-aiter-allreduce-f...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[AMD] Use bfloat16 for correction_bias in AITER FP8 path to avoid runtime dtype conversion for dsv3」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation In the AITER (AMD MI355X) path with FP8 quantization, was initialized in float32, but at runtime in biased_grouped_topk it gets cast to match gating_output.dtype:...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/index_buf_accessor.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `test/manual/layers/attention/nsa/test_get_k_scale_triton_kernel.py`, `test/manual/layers/attention/nsa/test_index_buf_accessor.py`；关联提交 `006bd44cf920`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 5 个文件，+380/-81，可读 patch 740 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「[deepseekv3.2] fix get_k_and_s_triton kenel for 128K seqlen case bug」，变更集中在 `test/manual/layers/attention/nsa/test_get_k_scale_triton_kernel.py`, `python/sglang/srt/layers/attention/nsa/index_buf_accessor.py`, `test/manual/layers/attention/nsa/test_index_buf_accessor.py`。PR 描述补充为：## Motivation fix get_k_and_s_triton kenel for 128K seqlen case bug ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [x] Format your code accordin...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/index_buf_accessor.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`, `test/registered/kernels/test_nsa_indexer.py`；关联提交 `17031120b8f6`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+22/-4，可读 patch 91 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[DeepSeek v3.2][Bugfix] get_index_k_scale_buffer support cp」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/index_buf_accessor.py`。PR 描述补充为：## Motivation Fix PR #16043 #19319 Prefill CP scenario bug. In the DeepSeek V3.2 prefill with CP (context parallel) scenario, `forward_batch.seq_lens` and `forward_batch.seq_len...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[CPU] Add FP8 Bmm support」，变更集中在 `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla.py`, `python/sglang/srt/models/longcat_flash.py`, `python/sglang/srt/models/deepseek_common/deepseek_weight_loader.py`。PR 描述补充为：## Motivation This PR is a follow-up on https://github.com/sgl-project/sglang/issues/8281 to add FP8 BMM support in MLA weight absorb. ## Modifications - The main change is the...
...
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+5/-1，可读 patch 13 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「[AMD] Use aiter_dsv3_router_gemm kernel if number of experts <= 256.」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：This change modified the router kernel selection with ROCm backend for models using DeepSeek model architecture. The Aiter kernel aiter_dsv3_router_gemm has been optimized for t...
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-1，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Fix gpu-fault issue when run deepseek-r1 and enable dp」，变更集中在 `python/sglang/srt/layers/attention/aiter_backend.py`。PR 描述补充为：## Motivation Fix the gpu cored-dump. Root cause: During CUDA graph capture, the captured data was fed with a KV sequence length smaller than the query (Q) length, which caused...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`；关联提交 `649172879778`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+19/-0，可读 patch 26 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[Perf] Overlap NSA-CP key all-gather with query computation for DeepSeek-V3.2」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation In the current Native Sparse Attention (NSA) implementation, specifically when Context Parallelism (CP) is enabled via `--enable-nsa-prefill-context-parallel`, the...
...
- 状态/时间: merged / 2026-03-24
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/tilelang_kernel.py`；关联提交 `855d15adf657`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+141/-95，可读 patch 314 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[AMD] Tilelang sparse fwd for dsv32 mi355/mi300」，变更集中在 `python/sglang/srt/layers/attention/nsa/tilelang_kernel.py`。PR 描述补充为：## Motivation - Enable the faster/new tilelang kernel on MI300. - Improve longer-context kernel performance on MI355. ## Modifications - Enable the new tilelang partial+combine...
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-0，可读 patch 8 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「Fix aiter router GEMM regression for non-DSR1 MoE models on ROCm gfx95」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：aiter_dsv3_router_gemm uses a Triton atomic kernel (_gemm_a16_w16_atomic) that is specifically tuned for DeepSeek-R1/V3's hidden_size=7168. On models with different hidden dimen...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Add MXFP4 (including Quark W4A4) quantization support for DeepSeek-architecture on ROCm」，变更集中在 `python/sglang/srt/layers/quantization/quark/schemes/quark_w4a4_mxfp4_moe.py`, `python/sglang/srt/layers/moe/fused_moe_triton/layer.py`, `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：Enable MXFP4 (including Quark W4A4) quantized models (e.g. GLM-5-MXFP4-Q8, Kimi-K2.5-MXFP4) to load and run on AMD MI355X with ROCm. These models use Quark's W4A4 MXFP4 scheme w...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「[ROCm] Fix fused MLA decode rope path for Kimi K2.5 and DeepSeek-variant models」，变更集中在 `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla_fused_rope_rocm.py`, `python/sglang/srt/layers/attention/triton_ops/rocm_mla_decode_rope.py`。PR 描述补充为：The SGLANG_ROCM_FUSED_DECODE_MLA=1 path had several issues preventing it from working with models like Kimi K2.5 that use DeepseekScalingRotaryEmbedding or different ForwardMeta...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 调整模型相关实现，标题为「[JIT Kernel] Migrate dsv3_router_gemm from AOT sgl-kernel to JIT kernel」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/models/glm4_moe_lite.py`, `python/sglang/jit_kernel/csrc/gemm/dsv3_router_gemm.cuh`。PR 描述补充为：dsv3_router_gemm_bf16_out.cu → kOutFloat=false path dsv3_router_gemm_float_out.cu → kOutFloat=true path dsv3_router_gemm_entry.cu → RouterGemmDispatcher replace LoopUnroller ##...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[Intel GPU] Enable DeepSeek R1 inference on XPU」，变更集中在 `python/sglang/srt/models/deepseek_common/deepseek_weight_loader.py`, `python/sglang/srt/models/deepseek_common/utils.py`, `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Enable DeepSeek-R1 model inference for XPU use FP8 precision through triton ## Modifications 1. run bmm in bf16 for absorb 2. Add XPU support for benchmarking 3. A...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「DeepSeek-R1-0528-w4a8: DeepEP Low Latency Dispatch Adopts FP8 Communication」，变更集中在 `python/sglang/srt/layers/moe/ep_moe/kernels.py`, `python/sglang/srt/layers/moe/cutlass_w4a8_moe.py`, `python/sglang/srt/layers/moe/ep_moe/layer.py`。PR 描述补充为：## Motivation profiling: deepseek-R1-0528: deepseek-R1-0528-w4afp8: 1.When DeepEP is enabled, the communication latency of the DeepSeek-R1-0508-W4AFP8 model is twice that of the...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「Revert "DeepSeek-R1-0528-w4a8: DeepEP Low Latency Dispatch Adopts FP8 Communication"」，变更集中在 `python/sglang/srt/layers/moe/ep_moe/kernels.py`, `python/sglang/srt/layers/moe/cutlass_w4a8_moe.py`, `python/sglang/srt/layers/moe/ep_moe/layer.py`。PR 描述补充为：Reverts sgl-project/sglang#14162
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「[AMD] Use tgemm.mm for MoEGate router gemm in deepseek_v2.py」，变更集中在 `python/sglang/srt/layers/rocm_linear_utils.py`, `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation When running the GLM-5-fp8 model, the MoEGate router GEMM is a major bottleneck, taking about **~30%** of a single layer's time. ## Modifications Following the ATO...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[RL] Support mxfp8 DeepSeek V3」，变更集中在 `python/sglang/srt/layers/moe/moe_runner/flashinfer_trtllm.py`, `python/sglang/srt/layers/moe/fused_moe_triton/layer.py`, `python/sglang/srt/layers/quantization/fp8.py`。PR 描述补充为：## Motivation @humansand Support Blackwell mxfp8 DeepSeek RL. Since the `kv_b_proj` can have different contraction axis in absorbed vs non-absorbed MLA mode while mxfp8 is 1d qu...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Add dsv3 router gemm benchmark on blackwell」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation * Comparing dsv3_router_gemm performance between flashinfer and the current sglang kernel to optimize performance (https://github.com/sgl-project/sglang/issues/144...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla.py`, `python/sglang/srt/models/deepseek_v2.py`；关联提交 `5a3531641735`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+196/-20，可读 patch 358 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Enable IndexCache for DeepSeek V3.2」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla.py`。PR 描述补充为：## Motivation fix https://github.com/sgl-project/sglang/issues/21286 ## Modifications * Port https://github.com/THUDM/IndexCache * add ut for deepseek-v3.2 ## Accuracy Tests gsm...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] Fix prepare_qkv_latent bypassing LoRA adapters in DeepSeek V2/V3」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation When LoRA adapters are applied to `fused_qkv_a_proj_with_mqa` in DeepSeek V2/V3 models, the fused GEMM optimization path in `prepare_qkv_latent` reads `.weight` di...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「feat: [1/2] [DeepEP] Fuse shared expert into MoE dispatch under EP」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/moe/topk.py`, `python/sglang/srt/layers/moe/fused_moe_triton/layer.py`。PR 描述补充为：## Motivation In DeepSeek V3/R1 with expert parallelism (EP), each rank currently computes the shared expert **separately** from routed experts. This means shared expert is not...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[Lora] Lora quat info re-factor and support deepseekv3 mla lora」，变更集中在 `python/sglang/srt/layers/quantization/fp8.py`, `python/sglang/srt/layers/quantization/moe_wna16.py`, `python/sglang/srt/layers/quantization/w8a8_int8.py`。PR 描述补充为：## Motivation ## Modifications - MoE quant-info refactor Extracts get_triton_quant_info() into each quantization method (FP8, INT8, WNA16, etc.) so FusedMoEWithLoRA correctly re...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[Reland] DeepSeek-R1-0528-w4a8: DeepEP Low Latency Dispatch Adopts FP8 Communication」，变更集中在 `python/sglang/srt/layers/moe/ep_moe/kernels.py`, `python/sglang/srt/layers/moe/cutlass_w4a8_moe.py`, `python/sglang/srt/layers/moe/ep_moe/layer.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Motivation profiling: deepseek-R1-0528: deepseek-R1-0528-w4afp8: 1.When DeepEP is enabled, the communication latency of the D...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 优化关键推理路径或后端选择，标题为「[AMD][MI30X] Restore DeepSeek MLA MI300X paths after MLA refactor (#19122)」，变更集中在 `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla.py`, `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Re-apply #18242 and #18624 behavior in forward_mla.py (logic moved out of deepseek_v2.py by #19122 but not fully carried over): ## Modifications - Import fused_qk_...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] Guard .weight access in DeepseekV2AttentionMLA for AWQ / compressed-tensors」，变更集中在 `test/registered/unit/models/test_deepseek_v2_attention_mla.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla_fused_rope_cpu.py`。PR 描述补充为：Hey all, this is my first PR, thought I'd try my hand a a recent bug. ---- ## Motivation Fixes #22855. When a DeepseekV2-architecture checkpoint is loaded with a `compressed_ten...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「Fix double-reduce in DeepseekV2MoE with flashinfer_cutedsl + EP + DP-attention」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/moe/moe_runner/flashinfer_cutedsl.py`。PR 描述补充为：## Motivation Running `nvidia/DeepSeek-V3-0324-NVFP4` with `--moe-runner-backend flashinfer_cutedsl --enable-dp-attention --ep-size N --dp-size N` (N>1) hit two separate bugs. *...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 调整模型相关实现，标题为「[CPU] expand the interface of shared_expert without scaling factor」，变更集中在 `sgl-kernel/csrc/cpu/moe_int4.cpp`, `sgl-kernel/csrc/cpu/moe.h`, `sgl-kernel/csrc/cpu/moe.cpp`。PR 描述补充为：## Motivation This patch expands `shared_expert` kernel when scaling factor is None, which maps exactly the behavior of MoE when expert equals to one. Also simplifies moe kernel...
...
- 动机: 该 PR 围绕 DeepSeek V3/R1 补齐模型支持入口或运行时能力，标题为「[MUSA][16/N] Add MUSA backend support for layers and DeepSeek models (V2/V3/R1)」，变更集中在 `python/sglang/srt/layers/deep_gemm_wrapper/compile_utils.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/models/deepseek_common/deepseek_weight_loader.py`。PR 描述补充为：## Motivation Enable SGLang to run DeepSeek models on Moore Threads MUSA GPUs. This PR adds MUSA backend support across the inference stack, including layers, quantization, MoE,...
