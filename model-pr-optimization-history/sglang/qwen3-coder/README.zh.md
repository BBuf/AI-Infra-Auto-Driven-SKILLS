...
- 动机: 该 PR 围绕 Qwen3 Coder 补齐模型支持入口或运行时能力，标题为「Preliminary Support for Qwen3XMLDetector」，变更集中在 `python/sglang/srt/function_call/qwen3_detector.py`, `python/sglang/srt/function_call/function_call_parser.py`, `python/sglang/srt/server_args.py`。PR 描述补充为：## Motivation This is a very basic, preliminary implementation of the Qwen3XMLDetector, intended to provide initial support. Please note that the related functionality still req...
...
- 动机: 该 PR 围绕 Qwen3 Coder 补齐模型支持入口或运行时能力，标题为「[Bugfix][Feat] Add XML-ish grammar in EBNFComposer and fix misc bugs in Qwen3 detector」，变更集中在 `test/srt/test_function_call_parser.py`, `python/sglang/srt/function_call/ebnf_composer.py`, `python/sglang/srt/function_call/qwen3_coder_detector.py`。PR 描述补充为：## Motivation Follow up to #8260 Here are some issues of `Qwen3XMLDetector`: 1. **EBNFComposer XML Support:** `tool_choice="required"` and specific tool selection fail because E...
...
- 动机: 该 PR 围绕 Qwen3 Coder 调整模型相关实现，标题为「Update qwen3_coder_detector.py for streaming」，变更集中在 `python/sglang/srt/function_call/qwen3_coder_detector.py`。PR 描述补充为：## Motivation This PR is to improve the streaming tool call parser from reading all values to incremental flow. ## Modifications Update the tool call parser for qwen3_coder_dete...
...
- 动机: 该 PR 围绕 Qwen3 Coder 调整模型相关实现，标题为「Forward unknown tool calls instead of dropping」，变更集中在 `python/sglang/srt/function_call/qwen3_coder_detector.py`, `test/srt/function_call/test_unknown_tool_name.py`, `python/sglang/srt/function_call/base_format_detector.py`。PR 描述补充为：Closes #12223 ## Motivation - Avoid silent data loss when models emit unknown tool names. - Make behavior consistent with paths that already forward malformed arguments (no sche...
...
- 动机: 该 PR 围绕 Qwen3 Coder 调整模型相关实现，标题为「Remove EBNF Composer」，变更集中在 `test/srt/test_function_call_parser.py`, `python/sglang/srt/function_call/ebnf_composer.py`, `test/srt/function_call/test_json_schema_constraint.py`。PR 描述补充为：## Motivation Resolve issue: https://github.com/sgl-project/sglang/issues/11078 We are now using json schema to constrain model output for required and named tool_choice cases,...
...
- 动机: 该 PR 围绕 Qwen3 Coder 调整模型相关实现，标题为「Improve Qwen3CoderDetector with schema-aware parameter type conversion」，变更集中在 `python/sglang/srt/function_call/qwen3_coder_detector.py`, `test/per_commit/function_call/test_function_call_parser.py`。PR 描述补充为：## Motivation In the original implementation of Qwen3CoderDetector, there is a function called `_safe_val` to help convert the parameter type of function calls: However, due to...
...
- 动机: 该 PR 围绕 Qwen3 Coder 补齐模型支持入口或运行时能力，标题为「Add Qwen3-Coder-480B to nightly tests」，变更集中在 `.github/workflows/nightly-test-nvidia.yml`, `test/nightly/test_qwen3_coder_480b_perf.py`, `test/nightly/nightly_utils.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- 动机: 该 PR 围绕 Qwen3 Coder 补齐模型支持入口或运行时能力，标题为「support new qwen3_coder_detector」，变更集中在 `python/sglang/srt/function_call/qwen3_coder_detector.py`。PR 描述补充为：# UT !39a93fe0305fa84ec404a08211d84853 # E2E Test !76e2386d0e007fb9c295f7f8b61f2904 The test results, provided by Zeyu Cui from the Qwen Team @cyente, have been confirmed to mee...
...
- 动机: 该 PR 围绕 Qwen3 Coder 补齐模型支持入口或运行时能力，标题为「[Fix] Triton TP MoE Dpsk V3/Qwen3 Coder with SwapAB」，变更集中在 `python/sglang/srt/layers/moe/fused_moe_triton/configs/triton_3_5_1/E=80,N=640,device_name=NVIDIA_H200,dtype=fp8_w8a8,block_shape=[128, 128]_down.json`, `python/sglang/srt/layers/moe/fused_moe_triton/configs/triton_3_5_1/E=80,N=640,device_name=NVIDIA_H200,dtype=fp8_w8a8,block_shape=[128, 128].json`, `python/sglang/srt/layers/moe/fused_moe_triton/configs/triton_3_5_1/E=257,N=256,device_name=NVIDIA_H200,dtype=fp8_w8a8,block_shape=[128, 128]_down.json`。PR 描述补充为：## Motivation Enable SwapAB on H200, which has been verified in this PR. And also retune the configuration of TP=8 EP=1 DeepseekV3 and TP=8 EP=2 Qwen3 Coder, in the latency scen...
...
- 动机: 该 PR 围绕 Qwen3 Coder 补齐模型支持入口或运行时能力，标题为「Add MoE fused config for Qwen3-Coder-Next-FP8 on H100 TP=2」，变更集中在 `python/sglang/srt/layers/moe/fused_moe_triton/configs/triton_3_5_1/E=512,N=256,device_name=NVIDIA_H100_80GB_HBM3,dtype=fp8_w8a8,block_shape=[128, 128].json`。PR 描述补充为：## Motivation Add optimized Triton MoE kernel configuration for Qwen3-Coder-Next-FP8 on H100 with TP=2. ## Modifications ## Accuracy Tests ## Benchmarking and Profiling `python...
...
- 动机: 该 PR 围绕 Qwen3 Coder 补齐模型支持入口或运行时能力，标题为「[ModelOPT] Support Qwen 3 Next Coder NVFP4」，变更集中在 `python/sglang/srt/models/qwen3_next.py`。PR 描述补充为：## Motivation This branch include important bugfix for qwen 3 coder next nvfp4 # B300 `sglang serve --model vincentzed-hf/Qwen3-Coder-Next-NVFP4 --quantization modelopt_fp4` **W...
...
- 动机: 该 PR 围绕 Qwen3 Coder 修复已暴露的启动、加载、解析或数值问题，标题为「[NPU] bugfix for model Qwen3-Coder-Next at weight shape transpose for npu.」，变更集中在 `python/sglang/srt/layers/attention/hybrid_linear_attn_backend.py`, `python/sglang/srt/hardware_backend/npu/quantization/fused_moe_method_npu.py`。PR 描述补充为：## Motivation 1. The original code contained an import error that affected the proper loading of weight-related modules. 2. Duplicate dimension transformations were applied to w...
...
- 动机: 该 PR 围绕 Qwen3 Coder 补齐模型支持入口或运行时能力，标题为「[AMD] Add Qwen3-Coder-Next accuracy and functionality test scripts for MI35x 8-GPU」，变更集中在 `test/registered/amd/accuracy/mi35x/test_qwen3_coder_next_eval_mi35x.py`, `test/registered/amd/test_qwen3_coder_next_8gpu.py`。PR 描述补充为：## Motivation Add CI test coverage for the Qwen3-Coder-Next model on AMD MI35x (gfx950) 8-GPU systems. This model features a hybrid architecture combining full attention (GQA) w...
...
- 动机: 该 PR 围绕 Qwen3 Coder 补齐模型支持入口或运行时能力，标题为「feat: Add FP8 KV cache support for Triton attention backend」，变更集中在 `python/sglang/srt/layers/attention/triton_backend.py`, `python/sglang/srt/layers/attention/triton_ops/decode_attention.py`, `python/sglang/srt/layers/attention/triton_ops/extend_attention.py`。PR 描述补充为：## Motivation The Triton attention backend does not correctly handle FP8 KV cache. It does not pass k_scale/v_scale to set_kv_buffer, causing scales to default to 1.0. In additi...
...
- 动机: 该 PR 围绕 Qwen3 Coder 补齐模型支持入口或运行时能力，标题为「[AMD] Fix Qwen3-Coder-Next: Add missing k_scale/v_scale args to extend_attention_fwd in aiter_backend」，变更集中在 `python/sglang/srt/layers/attention/aiter_backend.py`。PR 描述补充为：## Summary - Fix `TypeError: extend_attention_fwd() missing 1 required positional argument: 'v_scale'` crash in aiter_backend when running non-MLA speculative decoding (target_v...
...
- 动机: 该 PR 围绕 Qwen3 Coder 补齐模型支持入口或运行时能力，标题为「[Feature] Support incremental streaming for tool_call arguments in Qwen3CoderDetector」，变更集中在 `python/sglang/srt/function_call/qwen3_coder_detector.py`。PR 描述补充为：## Motivation When using --tool-call-parser qwen3_coder with streaming enabled, the arguments field of toolcall deltas is not streamed incrementally. Instead, the entire paramet...
