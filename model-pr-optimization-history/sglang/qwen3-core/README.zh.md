...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[Model] Adding Qwen3 and Qwen3MoE」，变更集中在 `python/sglang/srt/models/qwen3_moe.py`, `python/sglang/srt/models/qwen3.py`。PR 描述补充为：This PR provides initial support for the Qwen3 and Qwen3MoE. Once the model file becomes available, I will further refine the integration. ref https://github.com/huggingface/tra...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[qwen3] support qwen3 ep moe」，变更集中在 `python/sglang/srt/models/qwen3_moe.py`。PR 描述补充为：## Motivation ## Modifications ## Performance Test in 8xH200 Server launch and benchmarking command line: ## Checklist - [x] Format your code according to the Code Formatting wi...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「feat: add dp attention support for Qwen 2/3 MoE models, fixes #6088」，变更集中在 `python/sglang/srt/models/qwen2_moe.py`, `python/sglang/srt/models/qwen3_moe.py`, `python/sglang/srt/layers/dp_attention.py`。PR 描述补充为：This is the prerequisites of EP, which is introduced in PR #5917 ## Motivation As described in #6088, DP attention is not supported for Qwen MoE models, but #5917 introduces EP...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「Support qwen3 deepep」，变更集中在 `python/sglang/srt/models/qwen3_moe.py`。PR 描述补充为：## Motivation Support qwen3's deepep. For now, we've simply copied the deepep code from DS, and the accuracy test has passed. ## TODO * test bf16 compatibility ## Test Command *...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「support eplb for qwen3」，变更集中在 `python/sglang/srt/models/qwen3_moe.py`。PR 描述补充为：## Motivation support eplb for qwen3moe, ~~need merge #6120 first, then do other modification. (add ExpertLocationDispatchInfo)~~ simple test ## Modifications ## Checklist - [ ]...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「added support for tied weights in qwen pipeline parallelism」，变更集中在 `python/sglang/srt/models/qwen3.py`, `python/sglang/srt/models/qwen2.py`, `test/srt/test_pp_single_node.py`。PR 描述补充为：## Motivation Some Qwen variants have `tie_word_embeddings = true`, however, the current code only caters to case where its value is false. If this value is true, the last rank...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「qwen3moe support two batch overlap」，变更集中在 `python/sglang/srt/models/qwen3_moe.py`。PR 描述补充为：## Motivation Support two batch overlap for Qwen3, need merge #6581 first. Current overlap strategy is not stable, maybe we need change during tests. ## Modifications ## Checkli...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「Fix qwen3 tbo/dp-lm-head」，变更集中在 `python/sglang/srt/models/qwen3_moe.py`。PR 描述补充为：## Motivation 1. fix qwen3 tbo parameter 2. add dp_lm_head for qwen2/qwen3 ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit....
...
- 动机: 该 PR 围绕 Qwen3 Core 修复已暴露的启动、加载、解析或数值问题，标题为「Fix PP for Qwen3 MoE」，变更集中在 `python/sglang/srt/models/qwen3_moe.py`。PR 描述补充为：## Motivation Try to fix qwen3_moe PP failure issue mentioned by @libratiger in https://github.com/sgl-project/sglang/pull/6533#issuecomment-2911923181 ## Modifications Traverse...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「support qwen3 emebedding」，变更集中在 `python/sglang/srt/models/qwen3.py`。PR 描述补充为：## Motivation #6917 Support Qwen3 Embedding model. ## Modifications Add "model" prefix to model weight name. Test with `python3 -m unittest models/test_embedding_models.py`. ##...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「Support both approximate and exact expert distribution collection」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/models/qwen3_moe.py`, `python/sglang/srt/managers/expert_distribution.py`。PR 描述补充为：## Motivation ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests as outlined in the Running Unit Tests....
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「Move files related to EPLB」，变更集中在 `python/sglang/srt/model_executor/model_runner.py`, `python/sglang/srt/models/qwen2_moe.py`, `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation suggested by @merrymercy ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests as outlined in...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「support qwen3 dense model dp attention」，变更集中在 `python/sglang/srt/models/qwen3.py`。PR 描述补充为：## Motivation support qwen3 dense model use dp attention for special use @ShangmingCai ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[optimize] add two stream norm for qwen3」，变更集中在 `python/sglang/srt/models/qwen3.py`, `python/sglang/srt/models/qwen3_moe.py`。PR 描述补充为：## Motivation overlap RMSNorm by two stream Co-authored-by: @ispobock before | max_concurrency | input_throughput | output_throughput | mean_ttft_ms | median_ttft_ms | p99_ttft_...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[Bug] add flashinfer bool check for fusedmoe in Qwen moe models」，变更集中在 `python/sglang/srt/models/qwen2_moe.py`, `python/sglang/srt/models/qwen3_moe.py`。PR 描述补充为：## Motivation The current Qwen MoE models (`qwen2_moe.py` and `qwen3_moe.py`) do not pass the `enable_flashinfer_moe` flag, causing `FuseMoE` to default to `enable_flashinfer_mo...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[feat] Support EAGLE3 for Qwen」，变更集中在 `python/sglang/srt/models/qwen3.py`, `python/sglang/srt/models/qwen3_moe.py`, `python/sglang/srt/models/qwen2_moe.py`。PR 描述补充为：## Motivation refer to https://huggingface.co/Tengyunw/qwen3_8b_eagle3 and https://huggingface.co/Tengyunw/qwen3_30b_moe_eagle3 ## Modifications ## Checklist - [ ] Format your c...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「DP Attention with Auto DeepEP Dispatch」，变更集中在 `python/sglang/srt/layers/moe/ep_moe/token_dispatcher.py`, `python/sglang/srt/models/qwen3_moe.py`, `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation This PR enables auto DeepEP dispatch for DP attention. Integration with TBO will be supported in future PRs. ## Modifications ## Checklist - [ ] Format your code a...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[Feature] Layer-wise Prefill」，变更集中在 `python/sglang/srt/models/gemma3_causal.py`, `python/sglang/srt/models/gemma2.py`, `python/sglang/srt/models/gemma.py`。PR 描述补充为：This PR implements layer-wise prefill functionality by adding a new `forward_split_prefill` method to all supported model architectures. This allows launching several decoder la...
...
- 动机: 该 PR 围绕 Qwen3 Core 补强部署文档、测试或 CI 验证面，标题为「DP Enhancement」，变更集中在 `python/sglang/srt/model_executor/forward_batch_info.py`, `python/sglang/srt/layers/dp_attention.py`, `python/sglang/srt/model_executor/cuda_graph_runner.py`。PR 描述补充为：## Motivation This PR is for CI test. All changes are separated in #8276, #8277, #8278, and #8279. ## Modifications ## Checklist - [ ] Format your code according to the Code For...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[NVIDIA] Add Flashinfer MoE blockscale fp8 backend」，变更集中在 `python/sglang/srt/layers/moe/ep_moe/layer.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/moe/fused_moe_triton/layer.py`。PR 描述补充为：Enable flashinfer moe blockscale fp8 backend for low latency scenario. The e2e perf shows up to 3x improvement (see here). cc. @kushanam @pavanimajety
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[3/N] MoE Refactor: Simplify DeepEP Output」，变更集中在 `python/sglang/srt/layers/moe/ep_moe/token_dispatcher.py`, `python/sglang/srt/layers/moe/ep_moe/layer.py`, `python/sglang/srt/models/qwen3_moe.py`。PR 描述补充为：- Introduce `DispatchOutput` to maintain dispatcher's results. - Move DeepEP's `dispatch` and `combine` operations from model files the moe layer file. After this PR, all forwar...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「Support EPLB in FusedMoE」，变更集中在 `python/sglang/srt/layers/moe/fused_moe_triton/layer.py`, `python/sglang/srt/layers/moe/ep_moe/layer.py`, `python/sglang/srt/models/glm4_moe.py`。PR 描述补充为：## Motivation Fix #8398 ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests as outlined in the Running Un...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[NVIDIA] Enable Flashinfer MoE blockscale fp8 backend for TP MoE」，变更集中在 `python/sglang/srt/layers/moe/fused_moe_triton/layer.py`, `python/sglang/srt/layers/moe/ep_moe/layer.py`, `python/sglang/srt/layers/quantization/fp8.py`。PR 描述补充为：A followup PR to enable Flashinfer MoE blockscale fp8 backend for TP MoE. The previous PR is doing the same but for the EP MoE. cc. @kushanam
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[5/N] MoE Refactor: Update MoE parallelism arguments」，变更集中在 `python/sglang/srt/layers/moe/ep_moe/layer.py`, `python/sglang/srt/layers/moe/utils.py`, `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation This PR introduces `--moe-a2a-backend` and deprecates `--enable-ep-moe` and `--enable-deepep-moe`. ## Modifications ## Accuracy Test ## Benchmark & Profiling ## Ch...
...
- 动机: 该 PR 围绕 Qwen3 Core 优化关键推理路径或后端选择，标题为「[1/3] Optimize Slime Update Weights: Remove QWen3MOE Load Weight Overhead」，变更集中在 `python/sglang/srt/models/qwen3_moe.py`。PR 描述补充为：## Motivation - 1/3: https://github.com/sgl-project/sglang/pull/8751 （23s => 22s) - 2/3: https://github.com/sgl-project/sglang/pull/8753 (22s => 12s) - 3/3: https://github.com/s...
...
- 动机: 该 PR 围绕 Qwen3 Core 修复已暴露的启动、加载、解析或数值问题，标题为「Fix incorrect default get_hidden_dim logic」，变更集中在 `python/sglang/srt/models/gemma2.py`, `python/sglang/srt/models/granite.py`, `python/sglang/srt/models/llama.py`。PR 描述补充为：## Motivation See #8939 ## Modifications 1. Correct previously wrong calculation logic. 2. Clean-up unnecessary model-wise overrides ## Accuracy Test ## Benchmark & Profiling ##...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「Feature: support qwen and llama4 reducescatter for dp attention padding」，变更集中在 `python/sglang/srt/models/qwen3_moe.py`, `python/sglang/srt/models/qwen2_moe.py`, `python/sglang/srt/models/llama4.py`。PR 描述补充为：## Motivation Similar to #8280 and #8539 , this PR adds support for using reduce-scatter instead of all-reduce after MoE/MLP layers in Qwen2 MoE, Qwen3 MoE, and Llama4 when DP a...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「Qwen FP8/NVFP4 ModelOPT Quantization support」，变更集中在 `python/sglang/srt/layers/quantization/modelopt_quant.py`, `python/sglang/srt/models/qwen3.py`。PR 描述补充为：## Motivation 1. Add the quantization support for Qwen models from modelopt. so user can use it by one line of simple code: `llm = sgl.Engine(model_path="Qwen3-30B-A3B", modelop...
...
- 动机: 该 PR 围绕 Qwen3 Core 优化关键推理路径或后端选择，标题为「Optimize Qwen3-moe model by using flashinfer fused allreduce」，变更集中在 `python/sglang/srt/models/qwen3_moe.py`。PR 描述补充为：## Motivation This PR is to make qwen moe model leverage FlashInfer fused_allreduce to fuse allreduce+rmsnorm+residual_add. The E2E input throughput improved 2.2%. Currently use...
...
- 动机: 该 PR 围绕 Qwen3 Core 修复已暴露的启动、加载、解析或数值问题，标题为「Refactor TopK to ensure readability and extensibility」，变更集中在 `python/sglang/srt/layers/moe/topk.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/moe/fused_moe_triton/layer.py`。PR 描述补充为：## Motivation Some recent fixes and optimizations are hardcoded in `deepseek_v2.py`. This PR slightly adjust the code structure and rename some variables to ensure readability a...
...
- 动机: 该 PR 围绕 Qwen3 Core 优化关键推理路径或后端选择，标题为「[Ascend]optimize Qwen3 on Ascend」，变更集中在 `python/sglang/srt/models/qwen3.py`。PR 描述补充为：## Motivation related to #10337 ## Modifications 1. ~~use high-performance Attention Ops named `torch_npu._npu_paged_attention` in ACLGraph~~. The internal testing is ready. How...
...
- 动机: 该 PR 围绕 Qwen3 Core 优化关键推理路径或后端选择，标题为「Fuse write kv buffer into rope for qwen3 moe & bailing moe」，变更集中在 `python/sglang/srt/models/qwen3_moe.py`。PR 描述补充为：## Motivation Fused write kv buffer into rope for qwen3 moe and bailing moe models. Got minor e2e speedup. Inspired by https://github.com/sgl-project/sglang/pull/9014 ## Modific...
...
- 动机: 该 PR 围绕 Qwen3 Core 调整模型相关实现，标题为「Use more general heuristics to set the default value of --mem-fraction-static」，变更集中在 `python/sglang/srt/models/qwen3.py`, `python/sglang/srt/server_args.py`, `python/sglang/srt/managers/io_struct.py`。PR 描述补充为：- Clean up the settings of chunked prefill size, mem fraction static in server_args.py. Make the heuristics more general. - We only want to check the `gpu_mem` once with a singl...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/qwen3_moe.py`；关联提交 `61305291430a`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+14/-2，可读 patch 58 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3 Core 修复已暴露的启动、加载、解析或数值问题，标题为「Quick Fix: fix Qwen3-VL launch failure caused by MRotaryEmbedding arg」，变更集中在 `python/sglang/srt/models/qwen3_moe.py`。PR 描述补充为：## Motivation https://github.com/sgl-project/sglang/pull/10749 This PR fixes an issue in Qwen3-MOE where `fused_set_kv_buffer_arg` was passed to `q, k = self.rotary_emb`, but on...
...
- 动机: 该 PR 围绕 Qwen3 Core 补强部署文档、测试或 CI 验证面，标题为「model: qwen3-omni (thinker-only)」，变更集中在 `python/sglang/srt/models/qwen3_moe.py`。PR 描述补充为：## Motivation solve #11343 ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「Eagle3 DP attention for Qwen3 MoE」，变更集中在 `python/sglang/srt/models/qwen3_moe.py`。PR 描述补充为：## Motivation Add eagle3 dp attention for Qwen3 MoE model for large scale EP deployment. ## Modifications ## Accuracy Tests ## Benchmarking and Profiling We test DP attention fo...
...
- 动机: 该 PR 围绕 Qwen3 Core 优化关键推理路径或后端选择，标题为「Flashinfer TRTLLM-GEN-MoE + Qwen3」，变更集中在 `python/sglang/srt/models/qwen3_moe.py`。PR 描述补充为：This cmd needs to work, https://github.com/sgl-project/sglang/pull/12543 only make it work for Qwen2 MoE (qwen3 next) Also set better default, st: Uses `flashinfer_trtllm` autom...
...
- 动机: 该 PR 围绕 Qwen3 Core 修复已暴露的启动、加载、解析或数值问题，标题为「[Ascend] qwen optimization」，变更集中在 `python/sglang/srt/layers/moe/ep_moe/layer.py`, `python/sglang/srt/layers/attention/ascend_backend.py`, `python/sglang/srt/layers/moe/token_dispatcher/fuseep.py`。PR 描述补充为：## Motivation related to #10337 ## Modifications -bugfix: 1.memory bugfix(w8a8_int8.py): in previous code, both layer.w13_weight and layer.w2_weight occupied double memory. now...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `test/srt/cpu/test_qwen3.py`；关联提交 `974c562a254e`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+218/-0，可读 patch 241 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[CPU] add fused_qkvzba_split_reshape_cat kernel for Qwen3-next」，变更集中在 `test/srt/cpu/test_qwen3.py`。PR 描述补充为：## Motivation This pr adds `fused_qkvzba_split_reshape_cat` kernel for Qwen3-next on CPU. Reference: https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/models/qwe...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「Add fused FP8 KV cache write kernel for TRTLLM MHA backend」，变更集中在 `python/sglang/srt/layers/attention/trtllm_fp8_kv_kernel.py`, `python/sglang/srt/layers/attention/trtllm_mha_backend.py`, `python/sglang/srt/models/qwen3_moe.py`。PR 描述补充为：## Summary This PR introduces a fused FP8 KV cache write kernel for the TRTLLM MHA backend. It combines FP8 quantization and paged KV cache writes into a single Triton kernel (p...
...
- 动机: 该 PR 围绕 Qwen3 Core 优化关键推理路径或后端选择，标题为「[apply][2/2] Fused qk_norm_rope for Qwen3-MoE」，变更集中在 `python/sglang/srt/models/qwen3_moe.py`。PR 描述补充为：## Motivation Qwen3-MoE qk_norm and rope ops account for a non-trivial proportion in model inference. See below: Take Qwen3-30B-A3B for example, it has 48 Decode Layers. Each la...
...
- 动机: 该 PR 围绕 Qwen3 Core 修复已暴露的启动、加载、解析或数值问题，标题为「[bug fix][pp] fix qwen3 model load」，变更集中在 `python/sglang/srt/models/qwen3.py`。PR 描述补充为：## Motivation Now when launch Qwen3-0.6b with tp 2 pp4, the startup failed with below error: > [2025-12-16 10:15:15 PP0 TP1] Scheduler hit an exception: Traceback (most recent c...
...
- 动机: 该 PR 围绕 Qwen3 Core 修复已暴露的启动、加载、解析或数值问题，标题为「[NPU]qwen3 pp bugfix」，变更集中在 `python/sglang/srt/models/qwen3.py`, `python/sglang/srt/models/qwen3_moe.py`。PR 描述补充为：## Motivation Fix bug for qwen3 model when pp-size > 1 ## Modifications ## Accuracy Tests qwen32b, pp-size=4 ## Benchmarking and Profiling ## Checklist - [ ] Format your code ac...
...
- 动机: 该 PR 围绕 Qwen3 Core 优化关键推理路径或后端选择，标题为「[Feature] JIT Fused QK norm + qk norm clean up」，变更集中在 `python/sglang/srt/models/utils.py`, `python/sglang/srt/models/qwen3_moe.py`, `python/sglang/srt/models/qwen3.py`。PR 描述补充为：## Motivation 1. QK Norm kernel is not efficient enough. Around 2 months ago, flashinfer has released a newer version of QK-norm, which has not been integrated into sgl-kernel....
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[NPU][Bugfix] Fix qwen3 error when enable-dp-lm-head」，变更集中在 `python/sglang/srt/models/qwen3.py`, `python/sglang/srt/models/qwen3_moe.py`。PR 描述补充为：## Motivation 1、--enable-dp-lm-head causes Qwen3 to report an error. 2、split_qkv_rmsnorm_rope op parameters are not aligned. ## Modifications 1、When dp and enable-dp-lm-head are...
...
- 动机: 该 PR 围绕 Qwen3 Core 修复已暴露的启动、加载、解析或数值问题，标题为「Fix EPLB + FP4 Quantization Compatibility Issue」，变更集中在 `python/sglang/srt/layers/moe/utils.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/models/qwen2_moe.py`。PR 描述补充为：----------------------------------------------------------------------------------------- Note: The following description was generated by AI and may not be accurate. ----------...
...
- 动机: 该 PR 围绕 Qwen3 Core 修复已暴露的启动、加载、解析或数值问题，标题为「[PP] fix wrong weight logic for tie_word_embeddings model」，变更集中在 `python/sglang/srt/models/qwen3.py`, `python/sglang/srt/models/qwen2.py`。PR 描述补充为：## Motivation see #15843 This PR fixes a bug where models with tie_word_embeddings=True (e.g., Qwen3-4B) output garbage text (random characters) when running with Pipeline Paral...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/qwen3_moe.py`；关联提交 `b77b0ffd6021`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 7 个文件，+111/-49，可读 patch 296 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3 Core 修复已暴露的启动、加载、解析或数值问题，标题为「[NPU] NZ for non-quantized MOE, Qwen3 MOE double memory consumption fix」，变更集中在 `python/sglang/srt/models/qwen3_moe.py`。PR 描述补充为：## Motivation The part of closed PR https://github.com/sgl-project/sglang/pull/11984. Adding weight conversion from ND to FRACTAL_NZ speeds up the GroupedMatmul kernel ## Modifi...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[NPU] support GPTQ quantization on npu」，变更集中在 `python/sglang/srt/layers/quantization/gptq.py`, `python/sglang/srt/models/qwen3.py`, `python/sglang/srt/layers/linear.py`。PR 描述补充为：## Motivation This PR follows https://github.com/sgl-project/sglang/issues/15202 and Roadmap of NPU support https://github.com/sgl-project/sglang/issues/13664. ## Modifications...
...
- 动机: 该 PR 围绕 Qwen3 Core 调整模型相关实现，标题为「Update weight rename check for Qwen3 Embeddings」，变更集中在 `python/sglang/srt/models/qwen3.py`。PR 描述补充为：## Motivation Update weight rename check for Qwen3 Embeddings Fine-tuned models may not have "Embedding" in their name causing loading issues ## Modifications Since SGLang alway...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[ModelOpt] Fix broken Qwen3-235B-A22B-Instruct-2507-NVFP4 launch」，变更集中在 `python/sglang/srt/models/qwen3_moe.py`。PR 描述补充为：## Motivation Support https://huggingface.co/nvidia/Qwen3-235B-A22B-Instruct-2507-NVFP4 Previously it failed to launch on SGLang. However, the 30B NVFP4 always worked. https://h...
...
- 动机: 该 PR 围绕 Qwen3 Core 修复已暴露的启动、加载、解析或数值问题，标题为「[NPU] bugs fix: fix a condition bug when using speculative inference on Qwen3 and Qwen3 moe」，变更集中在 `python/sglang/srt/models/qwen3.py`, `python/sglang/srt/models/qwen3_moe.py`。PR 描述补充为：## Motivation The previous PR 17511 added a condition ( forward_mode.is_extend() ) to avoid using fused Triton kernel (split_qkv_rmsnorm_rope) during prefill stage. However, whe...
...
- 动机: 该 PR 围绕 Qwen3 Core 修复已暴露的启动、加载、解析或数值问题，标题为「[Qwen] Handle tie_word_embeddings for Qwen MoE and Qwen3Next」，变更集中在 `python/sglang/srt/models/qwen3_moe.py`, `python/sglang/srt/models/qwen2_moe.py`, `python/sglang/srt/models/qwen3_next.py`。PR 描述补充为：## Motivation Fixes #19908 Qwen3MoE, Qwen2MoE, and Qwen3Next always create a separate `ParallelLMHead`, ignoring `config.tie_word_embeddings`. When `True`, the checkpoint has no...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「Intel XPU: Qwen3 support (layernorm/MRoPE) + test_qwen3」，变更集中在 `python/sglang/srt/layers/rotary_embedding/mrope.py`, `python/sglang/srt/layers/attention/fla/layernorm_gated.py`, `test/srt/xpu/test_qwen3.py`。PR 描述补充为：## Motivation This PR integrates Qwen3 support on Intel XPU. It includes: 1. **Merged changes from contributor_fork/qwen3** (commit by Yang Wang): Fixes to run Qwen3 on XPU by a...
...
- 动机: 该 PR 围绕 Qwen3 Core 调整模型相关实现，标题为「Upgrade transformers==5.3.0」，变更集中在 `python/sglang/srt/models/gemma3_causal.py`, `python/sglang/srt/layers/rotary_embedding/factory.py`, `python/sglang/srt/configs/model_config.py`。PR 描述补充为：## Motivation Address #17779 — Upgrade `transformers` to `5.3.0`. ## Changes - Bump `transformers>=5.2.0`, `huggingface_hub>=1.0.0`; remove `hf_transfer` - `get_rope_config()` u...
...
- 动机: 该 PR 围绕 Qwen3 Core 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugifx] qwen3 rope parameter compatibility」，变更集中在 `python/sglang/srt/models/qwen3_moe.py`。PR 描述补充为：## Motivation To fix #20932 SGLang fails to load some Qwen3 MoE checkpoints whose HF config uses top-level `rope_theta` (v4-style) but does not define `rope_parameters` (v5-styl...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「Support Qwen3 MoE context parallel」，变更集中在 `python/sglang/srt/models/qwen3_moe.py`。PR 描述补充为：## Motivation Context parallelism is essential in long context LLM inference. It splits a long input sequence across multiple GPUs so attention can be computed in parallel, dras...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「Enable the qwen3 test」，变更集中在 `python/sglang/srt/models/qwen3_moe.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [x] Format your code according to the Format code with pre-commit. - [x] Add unit t...
...
- 动机: 该 PR 围绕 Qwen3 Core 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] Fix Qwen3 RoPE config compatibility for old-style checkpoints」，变更集中在 `python/sglang/srt/models/qwen3.py`。PR 描述补充为：## Motivation Fixes the non-MoE counterpart of #20932, which fixed the same issue in `qwen3_moe.py`. SGLang fails to load Qwen3 (non-MoE) checkpoints whose HF config uses top-le...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[jit_kernel] Add fused_qknorm_rope JIT kernel」，变更集中在 `python/sglang/srt/models/qwen3_moe.py`, `python/sglang/jit_kernel/tests/test_fused_qknorm_rope.py`, `python/sglang/jit_kernel/csrc/elementwise/fused_qknorm_rope.cuh`。PR 描述补充为：## Motivation Part of tracking issue #17865 — migrate sgl-kernel AOT kernels to the lightweight python/sglang/jit_kernel/ system. This PR ports sgl-kernel/csrc/moe/fused_qknorm_...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[Apple][MLX][Test] Add Qwen3 correctness and accuracy tests for Apple Silicon」，变更集中在 `test/registered/models/test_qwen3_mlx_correctness.py`, `test/registered/models/test_qwen3_mlx_accuracy.py`。PR 描述补充为：## Summary This PR adds initial correctness coverage for the Apple Silicon MLX path using `Qwen/Qwen3-0.6B`. ## What’s included - `test/registered/models/test_qwen3_mlx_correctn...
...
- 动机: 该 PR 围绕 Qwen3 Core 优化关键推理路径或后端选择，标题为「[jit_kernel] Optimize fused_qknorm_rope: deduplicate sincosf for interleave RoPE」，变更集中在 `python/sglang/srt/models/qwen3_moe.py`, `python/sglang/jit_kernel/csrc/elementwise/fused_qknorm_rope.cuh`, `python/sglang/jit_kernel/benchmark/bench_fused_qknorm_rope.py`。PR 描述补充为：## Motivation **Five optimizations to fusedQKNormRopeKernel:** **1. __sincosf deduplication (interleave RoPE)** In interleave (GPT-J) style RoPE, element pairs (2k, 2k+1) share...
...
- 状态/时间: merged / 2026-04-01
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/qwen3.py`；关联提交 `912494f59665`, `a188208e9a03`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+101/-3，可读 patch 152 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3 Core 优化关键推理路径或后端选择，标题为「[AMD] Optimize Qwen3-VL decode - fuse QK-norm + 3D mRoPE + KV cache write」，变更集中在 `python/sglang/srt/models/qwen3.py`。PR 描述补充为：## Motivation Use aiter's fused_qk_norm_mrope_3d_cache_pts_quant_shuffle kernel to replace 4 separate kernel launches (QKV split, QK RMSNorm, 3D mRoPE, KV cache write) with a si...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[NPU]add Qwen3-32b and Qwen3-8b low latency md」，变更集中在 `docs/platforms/ascend/ascend_npu_best_practice.md`。PR 描述补充为：## Motivation [NPU]add Qwen3-32b and Qwen3-8b low latency md ## Modifications [NPU]add Qwen3-32b and Qwen3-8b low latency md ## Accuracy Tests None ## Speed Tests and Profiling...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[NPU] Add Qwen3-14B low latency doc」，变更集中在 `docs/platforms/ascend/ascend_npu_best_practice.md`。PR 描述补充为：## Motivation Add Qwen3-14B low latency example. ## Modifications docs/platforms/ascend/ascend_npu_best_practice.md ## Accuracy Tests ## Speed Tests and Profiling ## Checklist -...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「Enable DFLASH support for additional model backends」，变更集中在 `python/sglang/srt/models/qwen3_5.py`, `python/sglang/srt/models/kimi_k25.py`, `python/sglang/srt/models/qwen3_next.py`。PR 描述补充为：## Summary Enable DFLASH for additional supported models from the z-lab collection: https://huggingface.co/collections/z-lab/dflash Based on #20547, landing this early to enable...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[Model] Support sliding window attention for Qwen3」，变更集中在 `python/sglang/srt/models/qwen3.py`。PR 描述补充为：## Summary - Add interleaved sliding window attention (SWA) support for Qwen3 model - Driven by HF config fields `layer_types` (primary) and `max_window_layers` (fallback), cons...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[NPU] add qwen3-30b-a3b low latency example」，变更集中在 `docs/platforms/ascend/ascend_npu_best_practice.md`。PR 描述补充为：## Motivation add qwen3-30b-a3b low latency example ## Modifications docs/platforms/ascend/ascend_npu_best_practice.md ## Accuracy Tests ## Speed Tests and Profiling ## Checklis...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[NPU] Support Qwen3.5-MoE and Qwen3-Next quantization」，变更集中在 `python/sglang/srt/model_loader/loader.py`。PR 描述补充为：## Motivation Qwen3.5 and Qwen3Next introduces GDN (Gated Dual Attention) linear attention layers that fuse separate projections into combined modules at load time: in_proj_qkv...
...
- 动机: 该 PR 围绕 Qwen3 Core 修复已暴露的启动、加载、解析或数值问题，标题为「[NPU]qwen3-8b and 32b md bugfix」，变更集中在 `docs/platforms/ascend/ascend_npu_best_practice.md`。PR 描述补充为：## Motivation [NPU]qwen3-8b and 32b md bugfix ## Modifications [NPU]qwen3-8b and 32b md bugfix ## Accuracy Tests None ## Speed Tests and Profiling None ## Checklist - [ ] Format...
...
- 动机: 该 PR 围绕 Qwen3 Core 调整模型相关实现，标题为「Restore Qwen3 rope config fallback」，变更集中在 `python/sglang/srt/models/qwen3.py`。PR 描述补充为：## Repro This happens when Qwen3 is launched with a JSON model override that provides `rope_scaling`. That makes `config.rope_parameters` exist, but it may not contain `rope_the...
...
- 动机: 该 PR 围绕 Qwen3 Core 修复已暴露的启动、加载、解析或数值问题，标题为「[Bug] Qwen3 reasoning detector silently swallows tool_call when is missing」，变更集中在 `test/registered/unit/parser/test_reasoning_parser.py`, `python/sglang/srt/parser/reasoning_parser.py`。PR 描述补充为：## Motivation `Qwen3Detector` forwards every constructor argument to the base class **except** `tool_start_token`. The base `BaseReasoningFormatDetector` already contains a fall...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[NPU] Add CI tests for Speculative Decoding」，变更集中在 `test/registered/ascend/basic_function/speculative_inference/test_npu_speculative_attention_mode.py`, `test/registered/ascend/basic_function/speculative_inference/test_npu_speculative_multi_npu.py`, `test/registered/ascend/basic_function/speculative_inference/test_npu_speculative_token_map.py`。PR 描述补充为：## Motivation This PR adds a comprehensive suite of NPU-specific CI tests for speculative decoding and expert parallelism features on Ascend hardware. The goal is to validate th...
...
- 动机: 该 PR 围绕 Qwen3 Core 调整模型相关实现，标题为「[alignment-sglang] PR3: Dense Deterministic Math」，变更集中在 `python/sglang/srt/layers/on_policy_utils.py`, `python/sglang/srt/model_executor/cuda_graph_runner.py`, `python/sglang/srt/models/qwen3.py`。PR 描述补充为：## Summary - Make dense Qwen3 rollout numerically compatible with Megatron true-on-policy scoring - Stacked on PR2: #23350 ## Changes - `layernorm.py`: RMSNorm `forward_native`...
...
- 动机: 该 PR 围绕 Qwen3 Core 调整模型相关实现，标题为「[Model] Qwen3ForPooledOutput: forward get_input_embeddings to inner model」，变更集中在 `python/sglang/srt/models/qwen3_classification.py`。PR 描述补充为：## Motivation `Qwen3ForCausalLM` forwards `get_input_embeddings()` to its inner `Qwen3Model`, but `Qwen3ForPooledOutput` — the shared base for `Qwen3ForSequenceClassification` a...
