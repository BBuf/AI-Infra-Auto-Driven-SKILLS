...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/configs/kimi_vl.py`, `python/sglang/srt/configs/kimi_vl_moonvit.py`, `python/sglang/srt/models/kimi_vl.py`, `python/sglang/srt/models/kimi_vl_moonvit.py`；关联提交 `8fefdd32c7c3`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 13 个文件，+1189/-11，可读 patch 1316 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「[Feature] add support kimi vl model」，变更集中在 `python/sglang/srt/models/kimi_vl_moonvit.py`, `python/sglang/srt/models/kimi_vl.py`, `python/sglang/srt/managers/multimodal_processors/kimi_vl.py`。PR 描述补充为：## Motivation add support kimi vl model：https://github.com/sgl-project/sglang/issues/5314 ## Modifications i just tested the code locally. it works fine! Prompt: What's in this...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/function_call/kimik2_detector.py`；关联提交 `615553079dc1`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 7 个文件，+480/-3，可读 patch 568 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「Support Kimi K2」，变更集中在 `python/sglang/srt/function_call/kimik2_detector.py`。PR 描述补充为：## Motivation Support Kimi K2 ## Modifications add Kimi style tool call parser read generation_config.json and update eos_ids ## Checklist - [ ] Format your code according to th...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「perf: add kimi k2 fused_moe tuning config for h30_3e」，变更集中在 `python/sglang/srt/layers/moe/fused_moe_triton/configs/triton_3_3_1/E=384,N=256,device_name=NVIDIA_H20-3e,dtype=fp8_w8a8,block_shape=[128, 128].json`。PR 描述补充为：## Motivation Add h20_3e fused MoE tuning config for Kimi K2 (E=384) ## Modifications ## Checklist - [x] Format your code according to the Code Formatting with Pre-Commit. - [ ]...
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+146/-0，可读 patch 147 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「H20 tune config for Kimi」，变更集中在 `python/sglang/srt/layers/moe/fused_moe_triton/configs/triton_3_3_1/E=384,N=128,device_name=NVIDIA_H20,dtype=fp8_w8a8,block_shape=[128, 128].json`。PR 描述补充为：add tune config for kimi on H20 , performance improvement will be update soon.. ## Performance * origin Kimi Performance * Performance with our config ## Checklist - [ ] Format...
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+146/-0，可读 patch 147 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「feat: add h200 tp 16 kimi k2 moe config」，变更集中在 `python/sglang/srt/layers/moe/fused_moe_triton/configs/triton_3_3_1/E=385,N=128,device_name=NVIDIA_H200,dtype=fp8_w8a8,block_shape=[128, 128].json`。PR 描述补充为：## Motivation ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests as outlined in the Running Unit Tests....
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+146/-0，可读 patch 147 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「feat: add b200 tp 16 kimi k2 moe config」，变更集中在 `python/sglang/srt/layers/moe/fused_moe_triton/configs/triton_3_3_1/E=385,N=128,device_name=NVIDIA_B200,dtype=fp8_w8a8,block_shape=[128, 128].json`。PR 描述补充为：## Motivation ## Modifications ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tests as outlined in the Running Unit Tests....
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补强部署文档、测试或 CI 验证面，标题为「feat(function call): complete utility method for KimiK2Detector and enhance documentation」，变更集中在 `python/sglang/srt/function_call/kimik2_detector.py`。PR 描述补充为：## Motivation This PR tries to implement the missing utility methods for `KimiK2Detector` and improves the the overall function call module by adding thorough documentation and...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「[Kimi K2] dsv3_router_gemm supports NUM_EXPERTS == 384」，变更集中在 `sgl-kernel/csrc/gemm/dsv3_router_gemm_entry.cu`, `sgl-kernel/csrc/gemm/dsv3_router_gemm_bf16_out.cu`, `sgl-kernel/csrc/gemm/dsv3_router_gemm_float_out.cu`。PR 描述补充为：Follow up of PR https://github.com/sgl-project/sglang/pull/8007 @yhyang201 mentioned that the `router_gemm` should be modified to support Kimi K2 in DeepSeek architecture but nu...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/function_call/kimik2_detector.py`；关联提交 `91e2f902db0e`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+3/-3，可读 patch 26 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 修复已暴露的启动、加载、解析或数值问题，标题为「Fix kimi k2 function call format」，变更集中在 `python/sglang/srt/function_call/kimik2_detector.py`。PR 描述补充为：Remove the space after `functions.{func_name}:{index}` in the template. Otherwise, xgrammar will mask all tokens as invalid, then the probability becomes NaN, and the engine wil...
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+146/-0，可读 patch 147 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「[perf] add kimi-k2 b200 fused moe config」，变更集中在 `python/sglang/srt/layers/moe/fused_moe_triton/configs/triton_3_4_0/E=384,N=256,device_name=NVIDIA_B200,dtype=fp8_w8a8,block_shape=[128, 128].json`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Test ## Benchmark & Profiling ## Checklist - [ ] Format your code according to the Code Formatting with Pre-Commit. - [ ] Add unit tes...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 修复已暴露的启动、加载、解析或数值问题，标题为「Fix kimi k2 function calling format」，变更集中在 `python/sglang/srt/entrypoints/openai/serving_chat.py`, `test/srt/openai_server/basic/test_serving_chat.py`。PR 描述补充为：https://github.com/sgl-project/sglang/issues/9575
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 调整模型相关实现，标题为「Replace the Kimi-K2 generated tool call idx with history tool call count」，变更集中在 `python/sglang/srt/entrypoints/openai/serving_chat.py`, `test/srt/openai_server/basic/test_serving_chat.py`。PR 描述补充为：## Motivation Related to #10600 Replace the Kimi-K2 generated tool call idx with history tool call count, make sure the tool calls indexes in history are ordered. ## Modificatio...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 修复已暴露的启动、加载、解析或数值问题，标题为「fix: KimiK2Detector Improve tool call ID parsing with regex」，变更集中在 `python/sglang/srt/function_call/kimik2_detector.py`。PR 描述补充为：## Motivation Enhance the tool_call_id parser, make it more robust. For random scenarios, like samples provide in https://github.com/MoonshotAI/K2-Vendor-Verfier , this change c...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/configs/kimi_linear.py`, `python/sglang/srt/models/kimi_linear.py`；关联提交 `a4bf5c6ad25d`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 18 个文件，+2847/-112，可读 patch 3404 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「Support Kimi Linear」，变更集中在 `python/sglang/srt/models/kimi_linear.py`, `python/sglang/srt/configs/kimi_linear.py`。PR 描述补充为：## Motivation Support Kimi Linear model (https://huggingface.co/moonshotai/Kimi-Linear-48B-A3B-Instruct). Major work is done by @yizhang2077 . Thanks @zhiyuan1i for valuable dis...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/kimi_linear.py`；关联提交 `cc2e36c352e8`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+37/-5，可读 patch 101 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补强部署文档、测试或 CI 验证面，标题为「overlap shared + routed expert computation in kimi linear」，变更集中在 `python/sglang/srt/models/kimi_linear.py`。PR 描述补充为：Before: `python3 -m sglang.test.send_one` After: Around 6%.
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 优化关键推理路径或后端选择，标题为「Opt kimi_k2_thinking biased topk module」，变更集中在 `python/sglang/srt/layers/moe/topk.py`。PR 描述补充为：### launch server ### Acc ### profiler main: pr: **33us->15us** . ### End2End benchmark
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「[opt kimi k2 1 / n] Add kimi k2 moe fused gate」，变更集中在 `sgl-kernel/csrc/moe/kimi_k2_moe_fused_gate.cu`, `sgl-kernel/tests/test_kimi_k2_moe_fused_gate.py`, `sgl-kernel/benchmark/bench_kimi_k2_moe_fused_gate.py`。PR 描述补充为：## Kimi K2 Acc test ### main branch ### pr ## Kernel benchmark ## Kimi K2 Profile main: pr: 14us->9us. ### End2End benchmark
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 优化关键推理路径或后端选择，标题为「[opt kimi k2 2/n] apply kimi k2 thinking moe_fused_gate」，变更集中在 `python/sglang/srt/layers/moe/topk.py`。PR 描述补充为：## Motivation Follow https://github.com/sgl-project/sglang/pull/13287 ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code accord...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「[opt kimi k2 3/n] opt kimi_k2 moe_fused_gate kernel」，变更集中在 `sgl-kernel/csrc/moe/kimi_k2_moe_fused_gate.cu`。PR 描述补充为：## Motivation - [x] Simplified dispatch logic and only support fp32 dtype - [x] Optimize `kimi_k2_moe_fused_gate_kernel` with vectorize(`float4`) - [x] Optimize `kimi_k2_moe_fus...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「[kimi k2 thinking] Avoid useless torch.zeros_」，变更集中在 `python/sglang/srt/layers/moe/fused_moe_triton/fused_marlin_moe.py`, `python/sglang/srt/layers/quantization/compressed_tensors/compressed_tensors_moe.py`, `python/sglang/srt/layers/quantization/awq.py`。PR 描述补充为：## Motivation - [x] remove useless `expert_map=torch.empty(1, device=x.device),` to avoid a ` torch.zeros_` kernel because we haven't support ep for kimi-k2-thinking now. EP sup...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 优化关键推理路径或后端选择，标题为「[opt kimi k2 4 / n] Delete useless pad kernel in sgl_moe_align_block_size」，变更集中在 `python/sglang/srt/layers/moe/fused_moe_triton/moe_align_block_size.py`。PR 描述补充为：## Motivation main: pr: 7.8us->6.5us. ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「[Piecewise CUDA Graph] Support Kimi-K2 (non-Thinking)」，变更集中在 `python/sglang/srt/layers/moe/topk.py`。PR 描述补充为：Currently, this only supports `moonshotai/Kimi-K2-Instruct-0905` (there are some additional complication w/ some torch.cuda non tensor ops in the prefill stage that I will remov...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「[Ascend] support Kimi-K2-Thinking」，变更集中在 `python/sglang/srt/layers/quantization/w8a8_int8.py`, `python/sglang/srt/layers/moe/ep_moe/layer.py`, `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Kimi-K2-Think model Day 0 support on SGLang on Ascend NPU backend. Ascend has completed the adaptation of the INT4 Weight (A16W4, pergroup=32) quantization format...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/kimi_linear.py`；关联提交 `6d5d76ad97dd`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+0/-8，可读 patch 50 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 调整模型相关实现，标题为「remove unecessary dual stream token threshold from the rest of models (qwen moe, kimi linear, etc.)」，变更集中在 `python/sglang/srt/models/kimi_linear.py`。PR 描述补充为：in an earlier pr of llama 4, i found the same conclusion as https://github.com/sgl-project/sglang/pull/9405 (it is also done in glm now). TODO there are still some in GDN and NS...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「Add Expert Parallelism (EP) support for kimi-k2-thinking」，变更集中在 `python/sglang/srt/layers/quantization/compressed_tensors/compressed_tensors_moe.py`。PR 描述补充为：Result:
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「Support piecewise cuda graph for fused marlin moe」，变更集中在 `python/sglang/srt/layers/quantization/gptq.py`, `python/sglang/srt/layers/moe/fused_moe_triton/fused_marlin_moe.py`, `python/sglang/srt/layers/moe/moe_runner/marlin.py`。PR 描述补充为：## Motivation Support piecewise cuda graph for models using `fused_marlin_moe` like moe models with gptq/awq quantization, kimi-k2-thinking model. ## Accuracy Tests
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「Fix warp illegal instruction in kimi k2 thinking PCG」，变更集中在 `sgl-kernel/csrc/moe/kimi_k2_moe_fused_gate.cu`。PR 描述补充为：## Motivation The `kimi_k2_moe_fused_gate` kernels fail to initialize `indices_ptr` when no valid expert is found (when `expert_id` is out of bounds or `max_expert == -1`), leav...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 优化关键推理路径或后端选择，标题为「Use dsv3 optimized routing `fused_topk_deepseek` instead of `moe_fused_gate`」，变更集中在 `python/sglang/srt/layers/moe/topk.py`, `test/registered/kernels/test_fused_topk_deepseek.py`, `test/srt/test_deepseek_v3_mtp.py`。PR 描述补充为：## Motivation flashinfer has an optimized routing kernel for DeepSeek V3: https://github.com/flashinfer-ai/flashinfer/pull/2099 The API was renamed to `fused_topk_deepseek` here...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「Fix kernel selection in biased_grouped_topk_gpu」，变更集中在 `python/sglang/srt/layers/moe/topk.py`。PR 描述补充为：## Motivation The has supported the case since PR https://github.com/sgl-project/sglang/pull/5440, but PR https://github.com/sgl-project/sglang/pull/15347 incorrectly added a ch...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/kimi_linear.py`；关联提交 `e6b7c04947ee`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+18/-42，可读 patch 129 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 优化关键推理路径或后端选择，标题为「[Kimi-Linear] Refactor kimi-linear gate calculation to avoid duplicated code」，变更集中在 `python/sglang/srt/models/kimi_linear.py`。PR 描述补充为：## Motivation Refactor Kimi-Linear gate ReplicatedLinear / ColumnParallelLinear project and fused_kda_gate out of attention forward in order to avoid duplicated code in both KDA...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/kimi_linear.py`；关联提交 `0c8165ffbd1b`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+95/-90，可读 patch 345 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「[Kimi-Linear] Refactor Kimi-Linear to support RadixLinearAttention」，变更集中在 `python/sglang/srt/models/kimi_linear.py`。PR 描述补充为：## Motivation Refactor Kimi-Linear based on new RadixLinearAttention. Result is correct. gsm8k Server: Client: ## Modifications ## Accuracy Tests ## Benchmarking and Profiling #...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/kimi_linear.py`；关联提交 `1e8db1829096`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+0/-1，可读 patch 8 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补强部署文档、测试或 CI 验证面，标题为「[Kimi-Linear] Remove duplicated code in kimi-linear」，变更集中在 `python/sglang/srt/models/kimi_linear.py`。PR 描述补充为：## Motivation There's a duplicated code in Kimi-Linear due to PR rebase. Removed it. ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format y...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「[AMD CI] Add moonshotai/Kimi-K2-Instruct-0905 testcases」，变更集中在 `test/registered/amd/test_kimi_k2_instruct.py`。PR 描述补充为：## Motivation Add kimi_k2_instruct model testcases to increase the test coverage on AMD GPUs. Change CI job partition from 2 to 3. ## Modifications Add 1 test script : test/regi...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「Support Kimi-K2.5 model」，变更集中在 `python/sglang/srt/models/kimi_k25.py`, `python/sglang/srt/configs/kimi_k25.py`, `python/sglang/srt/multimodal/processors/kimi_k25.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「[AMD] Add Kimi-K2, DeepSeek-V3.2 tests to nightly CI」，变更集中在 `test/registered/amd/accuracy/test_deepseek_v32_eval_amd.py`, `.github/workflows/nightly-test-amd.yml`, `test/registered/amd/perf/test_deepseek_v32_mtp_perf_amd.py`。PR 描述补充为：## Motivation 1. Add **Kimi-K2**, **DeepSeek-V3.2** accuracy and performance tests for MI325 (MI30x) platform, update Mi35x tests, consolidate test jobs, and fix various CI fail...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 修复已暴露的启动、加载、解析或数值问题，标题为「Fix: Avoid Double Reduce in VLM DP Attention」，变更集中在 `python/sglang/srt/layers/attention/vision.py`, `python/sglang/srt/models/kimi_k25.py`, `test/registered/distributed/test_dp_attention_large.py`。PR 描述补充为：## Motivation This PR fixes a redundant reduce operation in VLM DP Attention. Here’s the context: 1. After PR1 (https://github.com/sgl-project/sglang/pull/17624/changes) was ope...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `test/registered/amd/accuracy/mi30x/test_kimi_k2_eval_amd.py`, `test/registered/amd/accuracy/mi35x/test_kimi_k2_eval_mi35x.py`；关联提交 `6fd878b41df0`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 34 个文件，+184/-14，可读 patch 414 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「[AMD] Add kimi mi35x nightly test, folder organization and several stability fixes」，变更集中在 `test/registered/amd/accuracy/mi35x/test_kimi_k2_eval_mi35x.py`, `test/registered/amd/accuracy/mi30x/test_kimi_k2_eval_amd.py`。PR 描述补充为：## Motivation Add kimi mi35x nightly test, Test Suite Restructuring and fix gpt-oss accuracy issue. Please help to review: @yctseng0211 @bingxche. Nightly: https://github.com/sg...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 修复已暴露的启动、加载、解析或数值问题，标题为「fix kimi k2.5's moe gemm config init」，变更集中在 `python/sglang/srt/managers/scheduler.py`。PR 描述补充为：## Motivation ` --moe-runner-backend flashinfer_trtllm` doesn't work for kimi k2.5 ## Modifications ## Accuracy Tests ## Benchmarking and Profiling - b200 tp8 before this: : ~80...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 修复已暴露的启动、加载、解析或数值问题，标题为「[Kimi-K2.5] Fix NVFP4 Kimi-K2.5 weight mapping and exclude list」，变更集中在 `python/sglang/srt/models/kimi_k25.py`。PR 描述补充为：## Motivation Make `nvidia/Kimi-K2.5-NVFP4` load cleanly in SGLang. The checkpoint’s naming and `hf_quant_config.json` exclude list use `language_model.layers.*`, which doesn’t...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 修复已暴露的启动、加载、解析或数值问题，标题为「[Kimi-K2.5] Fix missing `quant_config` in `KimiK25`」，变更集中在 `python/sglang/srt/models/kimi_k25.py`。PR 描述补充为：## Motivation Fix missing `quant_config` in `KimiK25` causing `nvidia/Kimi-K2.5-NVFP4` to use bf16 KV cache instead of fp8. ## Modifications ## Accuracy Tests ## Benchmarking an...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「[AMD] Fix Janus-Pro crash and add Kimi-K2.5 nightly test」，变更集中在 `test/registered/amd/accuracy/mi35x/test_kimi_k25_eval_mi35x.py`, `test/registered/amd/accuracy/mi30x/test_kimi_k25_eval_amd.py`。PR 描述补充为：## Motivation **1. Fix Janus-Pro crash (regression from #18503)**, 2/9 nightly: ERROR: test_mmmu_vlm_models (__main__.TestNightlyVLMMmmuEvalAMD) (model='deepseek-ai/Janus-Pro-7B...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/kimi_linear.py`；关联提交 `bf5238835459`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+157/-71，可读 patch 423 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「[PCG] support piecewise cuda graph for kimi-linear model」，变更集中在 `python/sglang/srt/models/kimi_linear.py`。PR 描述补充为：## Motivation 1. refactor `kimi-linear` to do fused `qkv_proj` and unify `ralix_linear_attention` interface for mixed_qkv input. 2. support piecewise cuda graph for `kimi-linear...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「Add DP ViT support for Kimi K2.5」，变更集中在 `python/sglang/srt/models/kimi_k25.py`。PR 描述补充为：## Motivation Add DP ViT support for Kimi K2.5 launch: eval: Results (OCRBench): ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 修复已暴露的启动、加载、解析或数值问题，标题为「fix KimiK2Detector regex patterns with re.DOTALL」，变更集中在 `python/sglang/srt/function_call/kimik2_detector.py`。PR 描述补充为：## Motivation Enhance the tool call detection for kimi_k2 tool call parser to parse the multi-line tool arguments that contains `\n`. Contribute to #18086 ## Modifications ## Ac...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「[Fix] Kimi K2.5 support pp」，变更集中在 `python/sglang/srt/models/kimi_k25.py`。PR 描述补充为：## Motivation fix 18433 ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ]...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 调整模型相关实现，标题为「[Kernel Slimming] Migrate marlin moe kernel to JIT」，变更集中在 `python/sglang/srt/layers/moe/fused_moe_triton/fused_marlin_moe.py`, `python/sglang/jit_kernel/csrc/gemm/marlin_moe/marlin_template.h`, `python/sglang/jit_kernel/csrc/gemm/marlin_moe/moe_wna16_marlin.cuh`。PR 描述补充为：## Motivation See https://github.com/sgl-project/sglang/issues/17865 ## Modifications New files: - `python/sglang/jit_kernel/csrc/gemm/marlin_moe/moe_wna16_marlin.cuh` — JIT-com...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「[NPU] support Kimi-K2.5 on NPU」，变更集中在 `python/sglang/srt/models/kimi_k25.py`。PR 描述补充为：## Motivation support Kimi-K2.5 on Ascend ## Modifications ## Accuracy Tests gsm8k test: image example: ## Benchmarking and Profiling ## Checklist - [ ] Format your code accordi...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 修复已暴露的启动、加载、解析或数值问题，标题为「[AMD] optimize Kimi K2.5 fused_moe_triton performance by tuning」，变更集中在 `python/sglang/srt/layers/moe/fused_moe_triton/configs/triton_3_4_0/E=384,N=128,device_name=,dtype=int4_w4a16.json`, `python/sglang/srt/layers/moe/fused_moe_triton/configs/triton_3_4_0/E=384,N=128,device_name=,dtype=int4_w4a16_down.json`, `benchmark/kernels/fused_moe_triton/tuning_fused_moe_triton_sep.py`。PR 描述补充为：## Motivation Kimi K2.5 fused_moe_triton use default config so the performance is poor. ## Modifications 1. optimize Kimi K2.5 fused_moe_triton performance by tuning 2. fix fuse...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 优化关键推理路径或后端选择，标题为「[JIT Kernel] Migrate kimi_k2_moe_fused_gate to JIT」，变更集中在 `python/sglang/srt/layers/moe/topk.py`, `python/sglang/jit_kernel/csrc/moe/kimi_k2_moe_fused_gate.cuh`, `python/sglang/jit_kernel/benchmark/bench_kimi_k2_moe_fused_gate.py`。PR 描述补充为：## Motivation Migrate `kimi_k2_moe_fused_gate` kernel to JIT compilation (#17865). ## Modifications Under `python/sglang/jit_kernel/`: - `csrc/moe/kimi_k2_moe_fused_gate.cuh` —...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「feat: support Kimi K2.5 for Eagle3」，变更集中在 `python/sglang/srt/models/kimi_k25.py`。PR 描述补充为：feat: Integrate Kimi K2.5 with Eagle3 and benchmark performance on H200 This commit introduces initial support for the Kimi K2.5 model on the Eagle3 platform. The primary goal i...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「Fix Kimi K2.5 PP layer range exposure for PD disaggregation」，变更集中在 `python/sglang/srt/models/kimi_k25.py`。PR 描述补充为：## Motivation - Kimi K2.5 shows repeated outputs in **PP>1 + PD**. Root cause: the wrapper does not expose `start_layer/end_layer`, so each PP rank initializes KV cache with `st...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「[Nightly] Add Kimi K2.5 nightly test (base + Eagle3 MTP), replace Kimi K2」，变更集中在 `test/registered/8-gpu-models/test_kimi_k25.py`。PR 描述补充为：## Summary - Add unified nightly test for Kimi K2.5 with two variants using `run_combined_tests` framework: - **TP8**: Base Kimi K2.5 with tool/reasoning parsers (from #19218) -...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「fix piecewise cuda graph support for Kimi-K2.5 model」，变更集中在 `python/sglang/srt/models/kimi_k25.py`。PR 描述补充为：## Motivation ## Modifications - Add `self.model` alias pointing to `self.language_model.model` in `KimiK25ForConditionalGeneration`, following the same pattern as InternVL. Thi...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 修复已暴露的启动、加载、解析或数值问题，标题为「[feat] Enhance Kimi-K2/K2.5 function call and reasoning detection」，变更集中在 `test/registered/function_call/test_kimik2_detector.py`, `python/sglang/srt/function_call/kimik2_detector.py`。PR 描述补充为：## Motivation Related: #18086 fix 18086 Kimi-K2.5 models exhibit a special behavior where they output tool call markers ( ) directly inside blocks without first closing with . S...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/kimi_linear.py`；关联提交 `db995fba4790`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+10/-10，可读 patch 56 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 优化关键推理路径或后端选择，标题为「perf(kimi_linear): replace einops rearrange with native torch ops in Kimi-Linear KDA path」，变更集中在 `python/sglang/srt/models/kimi_linear.py`。PR 描述补充为：## Motivation `einops.rearrange` adds Python-level overhead (pattern parsing, backend dispatch, shape validation) on every call. In the Kimi-Linear-48B model's KimiDeltaAttentio...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「[Fix] Add EPLB rebalance support for Kimi K2.5」，变更集中在 `python/sglang/srt/models/kimi_k25.py`。PR 描述补充为：## Motivation Add routed_experts_weights_of_layer property to KimiK25ForConditionalGeneration to enable EPLB (Expert Parallel Load Balancing) rebalance support for Kimi K2.5 mod...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 修复已暴露的启动、加载、解析或数值问题，标题为「Fix Kimi K2.5 dp attention+ spec decoding launch crash」，变更集中在 `test/registered/8-gpu-models/test_kimi_k25.py`。PR 描述补充为：## Motivation Closes #21336 This issue is if there are mm input, draft embedding cannot handle it (will have out of boundary issue) ## Modifications Follows the same pattern in...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「[1/N] feat: support compressed-tensors w4afp8 MoE」，变更集中在 `python/sglang/srt/layers/quantization/compressed_tensors/schemes/compressed_tensors_w4a8_fp8_moe.py`, `python/sglang/srt/layers/moe/ep_moe/kernels.py`, `python/sglang/srt/layers/quantization/compressed_tensors/compressed_tensors.py`。PR 描述补充为：## Motivation Add support for loading compressed-tensors W4AFP8 MoE checkpoints in SGLang's `compressed_tensors` path. This enables INT4 group-quantized MoE weights with dynamic...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `test/registered/8-gpu-models/test_kimi_k25.py`；关联提交 `648632b6c41f`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+4/-23，可读 patch 53 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「[CI] Remove crashing Kimi K2.5 EAGLE3/MTP variants, keep TP8 and TP8+DP8」，变更集中在 `test/registered/8-gpu-models/test_kimi_k25.py`。PR 描述补充为：## Summary - Remove Kimi K2.5 MTP variants (TP8+MTP and TP8+DP8+MTP) that crash in nightly tests - Keep TP8 base variant and add TP8+DP8 variant (without EAGLE3 speculative deco...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `test/registered/amd/accuracy/mi35x/test_kimi_k25_mxfp4_eval_mi35x.py`, `test/registered/amd/test_kimi_k25_mxfp4.py`；关联提交 `dd49127fe612`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+81/-83，可读 patch 319 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「[AMD]: Support MLA with nhead<16 and FP8 KV cache for TP=8 (Kimi K2.5…」，变更集中在 `test/registered/amd/accuracy/mi35x/test_kimi_k25_mxfp4_eval_mi35x.py`, `test/registered/amd/test_kimi_k25_mxfp4.py`。PR 描述补充为：## Motivation ## Modifications 1. Support AITER MLA for num_heads < 16 (e.g., TP=8 with Kimi K2.5 giving 8 heads/rank). Uses head-repeat to expand to 16 heads before calling the...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 优化关键推理路径或后端选择，标题为「[AMD] Optimize fused MoE kernel config for small-M decode on gfx950」，变更集中在 `python/sglang/srt/layers/moe/fused_moe_triton/fused_moe_triton_config.py`。PR 描述补充为：## Summary - Adds an AMD-optimized Triton fused MoE kernel config for very small M (batch=1-2 decode) on gfx950 (MI300X/MI355X). - When `_is_hip and M 2, NVIDIA, Marlin). ## Mot...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `test/registered/amd/test_kimi_k25_mxfp4.py`；关联提交 `e14876742a08`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+3/-0，可读 patch 17 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 修复已暴露的启动、加载、解析或数值问题，标题为「[AMD] Fix test_kimi_k25_mxfp4.py : stage-c-test-large-8-gpu-amd-mi35x (linux-mi35x-gpu-8, 1)」，变更集中在 `test/registered/amd/test_kimi_k25_mxfp4.py`。PR 描述补充为：## Motivation Pin amd/Kimi-K2.5-MXFP4 model revision in test_kimi_k25_mxfp4.py to fix CI weight loading failure. The HuggingFace model was updated (commit 94d8c1bd) to quantize...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/kimi_k25.py`, `python/sglang/srt/multimodal/processors/kimi_k25.py`；关联提交 `42ffb168b311`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+166/-42，可读 patch 348 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「[EPD][VLM] Support Kimi K25 EPD」，变更集中在 `python/sglang/srt/models/kimi_k25.py`, `python/sglang/srt/multimodal/processors/kimi_k25.py`。PR 描述补充为：## Motivation Encoder–Prefill–Decode (EPD) disaggregation lets vision encoding run on dedicated encoder instances while language prefill/decode runs elsewhere. **Kimi K2.5** (`K...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「Extend kimi2 fused moe gate kernel to support GLM-5 (256 experts) via JIT compilation」，变更集中在 `python/sglang/srt/layers/moe/topk.py`, `python/sglang/jit_kernel/csrc/moe/moe_fused_gate_ungrouped.cu`, `python/sglang/jit_kernel/tests/test_moe_fused_gate_ungrouped.py`。PR 描述补充为：## Motivation GLM-5 uses a MoE architecture with 256 experts and `num_expert_group=1`, which shares the same ungrouped gating pattern as Kimi K2 (384 experts). However, the prev...
...
- 状态/时间: merged / 2026-04-10
- 反查来源: `git log --name-only -- <model-files>` 反查到 `test/registered/lora/test_lora_kimi_k25_logprob_diff.py`；关联提交 `6d79c6099545`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 5 个文件，+188/-12，可读 patch 248 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「[Lora] Lora kimi support」，变更集中在 `test/registered/lora/test_lora_kimi_k25_logprob_diff.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Speed Tests and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit te...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「[Feature] kimi k25 w4a16 support deepep low latency」，变更集中在 `python/sglang/srt/layers/quantization/compressed_tensors/schemes/compressed_tensors_wNa16_moe.py`, `python/sglang/srt/layers/moe/token_dispatcher/deepep.py`, `python/sglang/srt/layers/moe/ep_moe/layer.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Speed Tests and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit te...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/multimodal/processors/kimi_k25.py`；关联提交 `16f306fd85b6`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+344/-48，可读 patch 438 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 优化关键推理路径或后端选择，标题为「[VLM] GPU Image Preprocessing for Kimi-K2.5」，变更集中在 `python/sglang/srt/multimodal/processors/kimi_k25.py`。PR 描述补充为：## Motivation H200x8 std is 0.747 from https://www.kimi.com/blog/kimi-vendor-verifier SGLANG_USE_CUDA_IPC_TRANSPORT=1 python -m sglang.launch_server --model-path moonshotai/Kimi...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「feat(w4afp8): add KimiW4AFp8Config for Kimi K2.5 W4AFP8 model loading」，变更集中在 `python/sglang/srt/layers/quantization/w4afp8.py`, `python/sglang/srt/models/deepseek_common/deepseek_weight_loader.py`, `python/sglang/srt/layers/moe/fused_moe_triton/layer.py`。PR 描述补充为：## Motivation The existing `W4AFp8Config` is designed for DeepSeek-style W4AFP8 checkpoints with hardcoded defaults (e.g., `moe_activation_scheme="static"`, empty `ignored_layer...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/kimi_k25.py`, `python/sglang/srt/multimodal/processors/kimi_k25.py`；关联提交 `8686f42acb3e`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+11/-64，可读 patch 113 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「[VLM] Enable per-image ViT cache and avoid TP CUDA context creation for Kimi-K2.5」，变更集中在 `python/sglang/srt/models/kimi_k25.py`, `python/sglang/srt/multimodal/processors/kimi_k25.py`。PR 描述补充为：…DA context on device 0 in every TP rank ## Motivation Fixes redundant CUDA context allocation on device 0 across all TP ranks when running Kimi-K2.5 with the GPU image preproce...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/kimi_vl.py`, `python/sglang/srt/multimodal/processors/kimi_common.py`, `python/sglang/srt/multimodal/processors/kimi_k25.py`, `python/sglang/srt/multimodal/processors/kimi_vl.py`；关联提交 `e7ad7c587a35`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 6 个文件，+268/-102，可读 patch 520 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「[EPD][VLM] Support Kimi VL EPD」，变更集中在 `python/sglang/srt/multimodal/processors/kimi_common.py`, `python/sglang/srt/multimodal/processors/kimi_k25.py`, `python/sglang/srt/models/kimi_vl.py`。PR 描述补充为：## Motivation Kimi VL (kimi-vl-a3b) shares the same MoonViT vision tower as Kimi K2.5 but was not yet supported under the EPD (Encode-Prefill-Decode) disaggregation pipeline. Th...
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+6/-1，可读 patch 14 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 修复已暴露的启动、加载、解析或数值问题，标题为「[fix][Kimi] fix KimiGPUProcessorWrapper _cpu_call output」，变更集中在 `python/sglang/srt/multimodal/processors/kimi_k25.py`。PR 描述补充为：## Motivation When I run Kimi-K2.5 and curl with image, I get error `AttributeError: 'MultimodalDataItem' object has no attribute 'image_grid_thw'`. ## Modifications In this PR#...
...
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「[DeepEP Support] Support kimi-k2-thinking deepep」，变更集中在 `python/sglang/srt/layers/moe/fused_moe_triton/fused_marlin_moe.py`, `python/sglang/srt/layers/quantization/compressed_tensors/compressed_tensors_moe.py`, `python/sglang/srt/layers/moe/fused_moe_triton/moe_align_block_size.py`。PR 描述补充为：## Motivation It will crash with IMA when launch server now, need more debug or advice. ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Forma...
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+12/-0，可读 patch 26 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 优化关键推理路径或后端选择，标题为「[AMD] Fused qk rmsnorm bf16 for amd/Kimi-K2.5-MXFP4」，变更集中在 `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla.py`。PR 描述补充为：## Motivation On ROCm targets with a BF16 DeepSeek model, the MLA QK layernorm path falls through to the PyTorch sequential fallback instead of using aiter's fused kernel. The B...
...
- 状态/时间: merged / 2026-04-21
- 反查来源: `git log --name-only -- <model-files>` 反查到 `docs_new/cookbook/autoregressive/Moonshotai/Kimi-K2.6.mdx`；关联提交 `d20ae9ceaa14`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+34/-2，可读 patch 45 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Kimi K2/K2.5/Linear/VL 补齐模型支持入口或运行时能力，标题为「[docs] sync kimi-k2.6 from sgl-cookbook」，变更集中在 `docs_new/cookbook/autoregressive/Moonshotai/Kimi-K2.6.mdx`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Speed Tests and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit te...
