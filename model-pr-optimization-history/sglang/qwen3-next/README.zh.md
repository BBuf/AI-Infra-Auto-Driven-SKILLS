...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「Qwen3-Next support」，变更集中在 `python/sglang/srt/models/qwen3_next.py`, `python/sglang/srt/configs/qwen3_next.py`, `python/sglang/srt/models/qwen3_next_mtp.py`。PR 描述补充为：## Motivation ref #10306 support qwen3-next/qwen3-next-mtp ## Modifications 1. add `MambaPool` / `HybridReqTokenPool` to allocate mamba cache 2. add `HybridLinearKVPool` to avoi...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「[bugfix] fix norm type error in qwen3_next model」，变更集中在 `python/sglang/srt/models/qwen3_next.py`, `python/sglang/srt/models/qwen3_next_mtp.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「Support Qwen3-Next on Ascend NPU」，变更集中在 `python/sglang/srt/models/qwen3_next.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [x] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「[Fix] Support qwen3-next MTP+DP」，变更集中在 `python/sglang/srt/models/qwen3_next_mtp.py`。PR 描述补充为：## Motivation When MTP/SpecDecoding and DP are both enabled in qwen3-next, it will fail at cuda graph capture & forward: ## Modifications - Fix `set_dp_buffer_len` logic in logi...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「feat: update support for qwen3next model」，变更集中在 `python/sglang/srt/layers/attention/fla/fused_recurrent.py`, `python/sglang/srt/models/qwen3_next.py`, `python/sglang/srt/layers/attention/fla/fused_sigmoid_gating_recurrent.py`。PR 描述补充为：## Motivation 1、fix the l2norm accuracy issue 2、add support for the upcoming FP8 Qwen3Next model Note: DeepGEMM is currently not supported for Qwen3Next FP8, please set `SGL_ENA...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「support qwen3-next-fp8 deepep」，变更集中在 `python/sglang/srt/models/qwen3_next.py`。PR 描述补充为：## Motivation need merge https://github.com/sgl-project/sglang/pull/10624 first ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your c...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/configs/qwen3_next.py`；关联提交 `6b143d62a297`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+1/-15，可读 patch 47 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3 Next 补强部署文档、测试或 CI 验证面，标题为「Clean up some Qwen3-Next and deterministic code」，变更集中在 `python/sglang/srt/configs/qwen3_next.py`。PR 描述补充为：## Motivation Clean up Qwen3-Next and deterministic code ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Fo...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「init support for KTransformers Heterogeneous Computing」，变更集中在 `python/sglang/srt/layers/quantization/compressed_tensors/compressed_tensors_moe.py`, `python/sglang/srt/layers/moe/fused_moe_triton/layer.py`, `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation KTransformers Integration to Support CPU/GPU Hybrid Inference for MoE Models Only support CompressedTensor format. Will support other formats later. ## Modificatio...
...
- 动机: 该 PR 围绕 Qwen3 Next 修复已暴露的启动、加载、解析或数值问题，标题为「[NPU] bugfix for Qwen3-Next and performance update」，变更集中在 `python/sglang/srt/models/qwen3_next.py`。PR 描述补充为：## Motivation This PR aims to solve model issues and improve model performance and throughput on Ascend NPUs. We have reached a better performance result on a single Altas 800I...
...
- 动机: 该 PR 围绕 Qwen3 Next 优化关键推理路径或后端选择，标题为「[GDN] Fuse b.sigmoid(), fused_gdn_gating and unsqueeze into one kernel: up to 0.85% e2e speedup」，变更集中在 `python/sglang/srt/layers/attention/fla/fused_gdn_gating.py`, `python/sglang/srt/models/qwen3_next.py`, `python/sglang/srt/layers/attention/hybrid_linear_attn_backend.py`。PR 描述补充为：## Motivation Target Extend: 89us -> 79us: 10us saving -> Verify: 3.5us -> 1.4us: 2.1us saving -> ## Accuracy Tests Accuracy: 0.950 ## Benchmarking and Profiling H100 x 4 from 3...
...
- 动机: 该 PR 围绕 Qwen3 Next 调整模型相关实现，标题为「[GDN/Qwen3-Next] Avoid SSM and conv state copy for speculative decoding - up to 9.47% e2e speedup」，变更集中在 `python/sglang/srt/layers/attention/mamba/causal_conv1d_triton.py`, `python/sglang/srt/layers/attention/hybrid_linear_attn_backend.py`, `python/sglang/srt/layers/attention/fla/fused_recurrent.py`。PR 描述补充为：## Motivation Replace intermediate state buffers with last_steps for speculative decoding. All conv and SSM states are now lazily updated inside their kernels based on last_step...
...
- 动机: 该 PR 围绕 Qwen3 Next 修复已暴露的启动、加载、解析或数值问题，标题为「[Fix] Qwen3Next lmhead dtype」，变更集中在 `python/sglang/srt/models/qwen3_next.py`。PR 描述补充为：## Motivation The default dtype of `Qwen3-Next-80B-A3B` lmhead should be bf16? See lmhead dtype in vllm: ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Chec...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「Support piecewise cuda graph for Qwen3-next」，变更集中在 `python/sglang/srt/models/qwen3_next.py`。PR 描述补充为：## Motivation Support piecewise cuda graph for Qwen3-next https://github.com/sgl-project/sglang/issues/11490 ## Modifications 1. Temporarily split the entire GDN attention due t...
...
- 动机: 该 PR 围绕 Qwen3 Next 优化关键推理路径或后端选择，标题为「[Performance]Qwen3 Next kernel performance optimize」，变更集中在 `python/sglang/srt/layers/attention/fla/fused_sigmoid_gating_recurrent.py`。PR 描述补充为：## Motivation Update from: https://github.com/sgl-project/sglang/pull/13924 Improve the fused_sigmoid_gating_delta_rule_update_kernel performance. ## Modifications 1. autotune a...
...
- 动机: 该 PR 围绕 Qwen3 Next 补强部署文档、测试或 CI 验证面，标题为「Clean up GDN Init」，变更集中在 `python/sglang/srt/models/qwen3_next.py`。PR 描述补充为：## Motivation torch.log is very confusing to developer and we can actually just delete it ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] For...
...
- 动机: 该 PR 围绕 Qwen3 Next 修复已暴露的启动、加载、解析或数值问题，标题为「[NPU] Adapt qwen3-next W8A8 on NPU」，变更集中在 `python/sglang/srt/models/qwen3_next.py`。PR 描述补充为：## Motivation Fix the bugs on NPU when running TP+EP on the qwen3-next model, improve the performance, and adapt to W8A8 quantization. ## Modifications The qwen3-next model was...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「Two-Batch Overlap (TBO) support to Qwen3-Next Models」，变更集中在 `python/sglang/srt/models/qwen3_next.py`, `python/sglang/srt/models/qwen2_moe.py`, `python/sglang/srt/layers/attention/hybrid_linear_attn_backend.py`。PR 描述补充为：## Motivation This PR adds Two-Batch Overlap (TBO) support to Qwen3Next models. The optimization takes effect when `--enable-piecewise-cuda-graph` is not enabled, providing perf...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「tiny refactor pcg split op registration」，变更集中在 `python/sglang/srt/layers/radix_attention.py`, `python/sglang/srt/models/qwen3_next.py`, `python/sglang/srt/compilation/compilation_config.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- 动机: 该 PR 围绕 Qwen3 Next 修复已暴露的启动、加载、解析或数值问题，标题为「[bugfix] fix qwen3-next alt_stream none issue」，变更集中在 `python/sglang/srt/models/qwen3_next.py`。PR 描述补充为：## Motivation This patch is to solve the alt_stream None issue for qwen3-next on other non-cuda platform The command for how to reproduce on AMD MI300X: The error trace: The roo...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「[jit-kernel] Add CuTe DSL GDN Decode Kernel」，变更集中在 `python/sglang/srt/layers/attention/hybrid_linear_attn_backend.py`, `python/sglang/jit_kernel/cutedsl_gdn.py`, `python/sglang/jit_kernel/tests/test_cutedsl_gdn.py`。PR 描述补充为：Co-author: @zhou9402, @HongliMi, @xutizhou ## Motivation ## Modifications Add CuTe DSL GDN Decode Kernel - hybrid_linear_attn_backend.py - Add CuTe DSL decode branch (env: SGLAN...
...
- 动机: 该 PR 围绕 Qwen3 Next 调整模型相关实现，标题为「refactor Qwen3-Next with a new RadixLinearAttention」，变更集中在 `python/sglang/srt/models/qwen3_next.py`。PR 描述补充为：## Motivation Refactor: Abstract linear attention backend calls through `RadixLinearAttention` ## Modifications - `qwen3_next.py`: Replaced direct forward_batch.attn_backend.for...
...
- 动机: 该 PR 围绕 Qwen3 Next 补强部署文档、测试或 CI 验证面，标题为「Use attn tp group in embedding for more models」，变更集中在 `python/sglang/srt/models/bailing_moe.py`, `python/sglang/srt/models/bailing_moe_nextn.py`, `python/sglang/srt/models/falcon_h1.py`。PR 描述补充为：## Motivation Follow up of https://github.com/sgl-project/sglang/pull/17403. cc: @ch-wan ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Form...
...
- 动机: 该 PR 围绕 Qwen3 Next 补强部署文档、测试或 CI 验证面，标题为「[hybrid-model] clean up and consolidate redundant fields in RadixLinearAttention」，变更集中在 `python/sglang/srt/layers/attention/hybrid_linear_attn_backend.py`, `python/sglang/srt/layers/radix_linear_attention.py`, `python/sglang/srt/models/kimi_linear.py`。PR 描述补充为：## Motivation clean up and consolidate redundant fields in `RadixLinearAttention` ## Modifications ## Accuracy Tests Qwen3-Next Kimi Linear ## Benchmarking and Profiling ## Chec...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「[CPU] Optimize Qwen3-next model on CPU」，变更集中在 `python/sglang/srt/models/qwen3_next.py`, `python/sglang/srt/configs/qwen3_next.py`。PR 描述补充为：This PR adds unified CPU optimizations for Qwen3-next models, including: 1. Add CPU paths to call optimized kernels, which is depending on below sgl-kernels: a. chunk_gated_delt...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「[Qwen3-Next] Add cutedsl decode/mtp kernel with transposed ssm_state and prefill gluon kernel for blackwell.」，变更集中在 `python/sglang/srt/layers/attention/linear/kernels/gdn_cutedsl_transpose.py`, `python/sglang/srt/layers/attention/fla/chunk_delta_h.py`, `python/sglang/srt/layers/attention/linear/gdn_backend.py`。PR 描述补充为：## Motivation For Qwen3-Next model, we observed current preill/decode kernels cannot fully utilize the hardware efficiency of Blackwell. Thus, we implement gluon based prefill k...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「[Qwen3-Next] Optimize Prefill Kernel, add GDN Gluon kernel and optimize cumsum kernel」，变更集中在 `python/sglang/srt/layers/attention/fla/gluon/chunk_delta_h_gluon.py`, `python/sglang/srt/layers/attention/fla/gluon/wy_fast_gluon.py`, `python/sglang/srt/layers/attention/fla/gluon/chunk_o_gluon.py`。PR 描述补充为：## Motivation This PR optimizes the prefill kernel for Qwen3-Next (Gated Delta Rule) models, focusing on key improvements: - Blackwell GPU Performance: Add Gluon kernels that le...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「support qwen3-next eagle3」，变更集中在 `python/sglang/srt/models/qwen3_next.py`。PR 描述补充为：## Motivation support qwen3 next eagle3,https://huggingface.co/lukeysong/qwen3-next-draft/tree/main ## Benchmarking and Profiling use Specfoge tp4 h20 |**Benchmark**|**Latency (...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「[ModelOPT] Support Qwen 3 Next Coder NVFP4」，变更集中在 `python/sglang/srt/models/qwen3_next.py`。PR 描述补充为：## Motivation This branch include important bugfix for qwen 3 coder next nvfp4 # B300 `sglang serve --model vincentzed-hf/Qwen3-Coder-Next-NVFP4 --quantization modelopt_fp4` **W...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「[MODEL] Adding Support for Qwen3.5 Models」，变更集中在 `python/sglang/srt/models/qwen3_5.py`, `python/sglang/srt/models/qwen3_5_mtp.py`, `python/sglang/srt/configs/qwen3_5.py`。PR 描述补充为：## Purpose This PR adds model support for the upcoming Qwen3.5 models, including both dense and MoE variants. Special thanks to @cao1zhg, @yizhang2077, and @attack204 for their...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「[Perf] refactor piecewise cuda graph support of Qwen3-Next」，变更集中在 `python/sglang/srt/models/qwen3_next.py`。PR 描述补充为：## Motivation Currently in Qwen3Next, we hide all ops inside `Qwen3GatedDeltaNet.forward` in a custom fake op, so that even with piecewise cuda graph, the whole forward of Qwen3...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「[Qwen3-Next] Enable fused_qkvzba_split_reshape_cat also for prefill」，变更集中在 `python/sglang/srt/models/qwen3_next.py`。PR 描述补充为：## Motivation In the GDN (Gated Delta Net) prefill path of Qwen3-Next, the `fix_query_key_value_ordering` function uses a chain of PyTorch `view` / `split` / `reshape` / `cat` o...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「[AMD] Support Qwen3-Coder-Next on AMD platform」，变更集中在 `python/sglang/srt/layers/attention/aiter_backend.py`, `python/sglang/srt/models/qwen3_next.py`。PR 描述补充为：## Motivation Enable Qwen3-Coder-Next model on AMD GPU platform. With this PR, we are able to support non-MTP (fp8 kv cache) and MTP on Qwen3-Coder-Next. ## Modifications - aite...
...
- 动机: 该 PR 围绕 Qwen3 Next 修复已暴露的启动、加载、解析或数值问题，标题为「[PCG] fix piecewise cuda graph for Qwen3.5」，变更集中在 `python/sglang/srt/models/qwen3_next.py`, `python/sglang/srt/models/qwen3_5.py`, `python/sglang/srt/layers/quantization/fp8_utils.py`。PR 描述补充为：## Motivation fix piecewise cuda graph for Qwen3.5 ## Modifications 1. fix piecewise cuda graph for Qwen3.5 2. clean up legacy code `gdn_with_output` as it's not used anymore. #...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「[Qwen3-Next] Support gdn fused_rms_norm_gated」，变更集中在 `python/sglang/srt/models/qwen3_next.py`。PR 描述补充为：## Motivation This PR is to refactor Qwen3-Next GDN fused_rms_norm_gate. The ground is in attention / state-space style architectures, each layer and each step does a norm + gat...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「[feat] Support nvfp4 quantized model of Qwen3-Next」，变更集中在 `test/registered/models/test_qwen3_next_models_fp4.py`, `python/sglang/srt/models/qwen3_next.py`。PR 描述补充为：## Motivation Support https://huggingface.co/nvidia/Qwen3-Next-80B-A3B-Instruct-NVFP4 ## Modifications - The `qkv_proj` is not quantized in the NVFP4 quantized version, so is sp...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「Fix Qwen3.5/Qwen3Next MTP EPLB compatibility」，变更集中在 `python/sglang/srt/models/qwen3_5.py`, `python/sglang/srt/models/qwen2_moe.py`。PR 描述补充为：## Summary - Fix three bugs when EPLB is enabled with Qwen3.5/Qwen3Next MTP (speculative decoding): - MTP layers created `ExpertLocationDispatchInfo` with wrong layer IDs (MTP h...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「Fix qwen3.5 mtp eplb related issues」，变更集中在 `python/sglang/srt/models/qwen3_5.py`, `python/sglang/srt/models/qwen3_5_mtp.py`, `python/sglang/srt/models/qwen3_next_mtp.py`。PR 描述补充为：## Motivation support eplb with mtp for Qwen3.5 & Qwen3-Next ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to th...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「[NPU] Qwen3 next Ascend Support MTP」，变更集中在 `python/sglang/srt/layers/attention/mamba/mamba_state_scatter_triton.py`, `python/sglang/srt/layers/attention/linear/gdn_backend.py`, `python/sglang/srt/layers/attention/hybrid_linear_attn_backend.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- 动机: 该 PR 围绕 Qwen3 Next 优化关键推理路径或后端选择，标题为「[Qwen3-Next] Fuse Qwen3-Next GDN's qkvz_proj and ba_proj」，变更集中在 `python/sglang/srt/models/qwen3_next.py`。PR 描述补充为：## Motivation This PR is to fuse Qwen3-Next GDN's qkvz_proj and ba_proj with MergedColumnParallelLinear in order to improve performance. TTFT speedup 2.6%. (Stably) E2E throughp...
...
- 动机: 该 PR 围绕 Qwen3 Next 优化关键推理路径或后端选择，标题为「[Qwen3.5] Fuse split/reshape/cat ops in GDN projection with Triton kernel」，变更集中在 `python/sglang/srt/models/qwen3_5.py`, `python/sglang/srt/models/qwen3_next.py`, `python/sglang/jit_kernel/triton/gdn_fused_proj.py`。PR 描述补充为：## Motivation In PR https://github.com/sgl-project/sglang/pull/19321 we fused Qwen3-Next GDN's qkvz_proj and ba_proj. This PR is a follow up. The background that Qwen3-Next and...
...
- 动机: 该 PR 围绕 Qwen3 Next 修复已暴露的启动、加载、解析或数值问题，标题为「bugfix for weight loading for qwen3-next」，变更集中在 `python/sglang/srt/models/qwen3_next.py`。PR 描述补充为：## Motivation previously, When loading quantization weight such as w8a8, as weight_loader of self.in_proj_qkvz.weight is denoted by @property, which means it doesn't have a sett...
...
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+2/-2，可读 patch 13 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3 Next 修复已暴露的启动、加载、解析或数值问题，标题为「Revert "bugfix for weight loading for qwen3-next"」，变更集中在 `python/sglang/srt/models/qwen3_next.py`。PR 描述补充为：Reverts sgl-project/sglang#21313
- 实现要点: `python/sglang/srt/models/qwen3_next.py` modified +2/-2 (4 lines); hunks: -135,10 +135,10 @@ def __init__(; symbols: __init__，涉及 `__init__`。
- 代码 diff 细节:
  - `python/sglang/srt/models/qwen3_next.py` modified +2/-2 (4 lines); hunks: -135,10 +135,10 @@ def __init__(; symbols: __init__
- 关键代码摘录:
...
- 动机: 该 PR 围绕 Qwen3 Next 修复已暴露的启动、加载、解析或数值问题，标题为「[Fix] Fix weight_loader property assignment for qwen3-next FP8 models」，变更集中在 `python/sglang/srt/models/qwen3_next.py`。PR 描述补充为：## Summary - Fix `AttributeError: property 'weight_loader' of 'ModelWeightParameter' object has no setter` when loading Qwen3-Coder-Next-FP8 weights - `BasevLLMParameter.weight_...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「[bugfix] fix Qwen3-next memory leak」，变更集中在 `python/sglang/srt/mem_cache/allocator.py`, `python/sglang/srt/mem_cache/memory_pool.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Speed Tests and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit te...
...
- 动机: 该 PR 围绕 Qwen3 Next 修复已暴露的启动、加载、解析或数值问题，标题为「[npu]fix: qwen3-next w8a8 precision bugs」，变更集中在 `python/sglang/srt/models/qwen3_next.py`。PR 描述补充为：## Motivation The change at https://github.com/sgl-project/sglang/pull/19321 caused qwen3-next to fail in running the w8a8 model on NPU due to incorrect loading of the in_proj_q...
...
- 动机: 该 PR 围绕 Qwen3 Next 修复已暴露的启动、加载、解析或数值问题，标题为「Fix NCCL AllGather hanging issue for Qwen3 Next MTP」，变更集中在 `python/sglang/srt/speculative/eagle_info.py`, `python/sglang/srt/speculative/eagle_info_v2.py`。PR 描述补充为：## Motivation Fix hanging issue (AllGather size mismatch) in EAGLE speculative decoding (both V1 and V2) when running with TP>1 and non-greedy sampling. (https://github.com/sgl-...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「Fix: Raise ValueError when --enable-mixed-chunk and --mamba-scheduler-strategy extra_buffer cause ac」，变更集中在 `test/registered/unit/server_args/test_server_args.py`, `python/sglang/srt/server_args.py`。PR 描述补充为：## Motivation In recent tests, it was found that when both --enable-mixed-chunk and --mamba-scheduler-strategy extra_buffer are enabled, the model exhibits a drop in accuracy un...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「[Fix] Mixed chunk query_start_loc and mamba_cache_indices to the prefill-only prefix so that the tracking helpers see a consistent, prefill-only view.」，变更集中在 `python/sglang/srt/layers/attention/mamba/mamba2_metadata.py`, `python/sglang/srt/layers/attention/hybrid_linear_attn_backend.py`, `python/sglang/srt/managers/schedule_batch.py`。PR 描述补充为：## Motivation In recent tests, it was found that when both --enable-mixed-chunk and --mamba-scheduler-strategy extra_buffer are enabled, the model exhibits a drop in accuracy un...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「Qwen3next flashinfer allreduce auto enable」，变更集中在 `python/sglang/srt/server_args.py`。PR 描述补充为：Made with @Codex ## Summary Enable FlashInfer allreduce fusion by default for `Qwen3NextForCausalLM` on supported single-node SM90/SM100 TP runs. ## Why `Qwen/Qwen3-Coder-Next`...
...
- 动机: 该 PR 围绕 Qwen3 Next 补齐模型支持入口或运行时能力，标题为「[NVIDIA] [GDN] Enable FlashInfer MTP verify on SM100+ (Blackwell)」，变更集中在 `python/sglang/srt/layers/attention/linear/kernels/gdn_flashinfer.py`, `python/sglang/srt/server_args.py`。PR 描述补充为：## [GDN] Enable FlashInfer MTP verify on SM100+ (Blackwell) co-authored by @YAMY1234 (main contributor) ### Summary Enables FlashInfer GDN MTP (speculative decoding) verify on S...
...
- 动机: 该 PR 围绕 Qwen3 Next 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] Try to fix --cpu-offload-gb on hybrid linear-attn models」，变更集中在 `test/registered/unit/utils/test_offloader_tied_params.py`, `python/sglang/srt/utils/offloader.py`。PR 描述补充为：## Motivation Fixes #23150. `--cpu-offload-gb > 0` was broken on hybrid linear-attention models (Qwen3-Next, Qwen3.5, Kimi-Linear): the first `/v1/chat/completions` request rais...
