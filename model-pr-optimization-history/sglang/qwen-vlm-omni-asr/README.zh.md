...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/qwen2_vl.py`；关联提交 `f6f713797bcb`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+39/-12，可读 patch 114 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「Add support for Qwen2-VL-based embedding models」，变更集中在 `python/sglang/srt/models/qwen2_vl.py`。PR 描述补充为：## Motivation Add support for Qwen2-VL based embedding models (e.g. mcdse-2b-v1), as requested in https://github.com/sgl-project/sglang/issues/2032. ## Modifications ## Checklis...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/qwen2_5_vl.py`, `python/sglang/srt/models/qwen2_vl.py`；关联提交 `c998d04b4692`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 26 个文件，+435/-337，可读 patch 1363 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「vlm: enable radix cache for qwen-vl models」，变更集中在 `python/sglang/srt/models/qwen2_vl.py`, `python/sglang/srt/models/qwen2_5_vl.py`。PR 描述补充为：## Motivation 1. support qwen2vl/qwen2.5vl radix cache 2. fix bugs about mrope calculation in qwen2vl/qwen2.5vl ## Background **mrope** is a special 3D position_id for qwen-vl m...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/qwen2_vl.py`；关联提交 `e97e57e699e5`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+0/-12，可读 patch 19 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 调整模型相关实现，标题为「Remove unused method `calculate_num_image_tokens` from qwen2_vl.py」，变更集中在 `python/sglang/srt/models/qwen2_vl.py`。PR 描述补充为：## Motivation `calculate_num_image_tokens` is not used anywhere, just remove it. ## Modifications ## Checklist - [x] Format your code according to the Code Formatting with Pre-C...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/qwen2_5_vl.py`, `python/sglang/srt/models/qwen2_vl.py`；关联提交 `f19a9204cdbd`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 14 个文件，+592/-125，可读 patch 1118 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「Support precomputed multimodal features for Qwen-VL and Gemma3 models.」，变更集中在 `python/sglang/srt/models/qwen2_5_vl.py`, `python/sglang/srt/models/qwen2_vl.py`。PR 描述补充为：## Motivation When running a VLM via SGLang, it's useful to be able to supply image embeddings directly. For example maybe you've frozen your vision encoder and want to precompu...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/qwen2_5_vl.py`；关联提交 `37d83c6e6d8a`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 9 个文件，+114/-5，可读 patch 260 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「Qwen2.5-VL eagle3 infer」，变更集中在 `python/sglang/srt/models/qwen2_5_vl.py`。PR 描述补充为：## Motivation support qwen2.5-vl eagle3 infer ## Modifications 1. add set_eagle3_layers_to_capture in qwen2 and qwen2.5_vl 2. change raw_bs to raw_num_token for mrope when targe...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/configs/qwen3_vl.py`, `python/sglang/srt/models/qwen3_vl.py`, `python/sglang/srt/models/qwen3_vl_moe.py`, `python/sglang/srt/multimodal/processors/qwen_vl.py`；关联提交 `4f564b9e8378`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 7 个文件，+1898/-8，可读 patch 2006 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「model: support qwen3-vl series」，变更集中在 `python/sglang/srt/models/qwen3_vl.py`, `python/sglang/srt/configs/qwen3_vl.py`, `python/sglang/srt/models/qwen3_vl_moe.py`。PR 描述补充为：This PR introduces support for the upcoming **Qwen3-VL** models — including both dense and MoE variants, as well as Instruct and Thinking editions. As the next generation of the...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 修复已暴露的启动、加载、解析或数值问题，标题为「Quick Fix: fix Qwen3-VL launch failure caused by MRotaryEmbedding arg」，变更集中在 `python/sglang/srt/models/qwen3_moe.py`, `python/sglang/srt/layers/rotary_embedding.py`。PR 描述补充为：## Motivation https://github.com/sgl-project/sglang/pull/10749 This PR fixes an issue in Qwen3-MOE where `fused_set_kv_buffer_arg` was passed to `q, k = self.rotary_emb`, but on...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/qwen3_vl.py`, `python/sglang/srt/models/qwen3_vl_moe.py`；关联提交 `be740acdb0ad`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+7/-11，可读 patch 55 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 优化关键推理路径或后端选择，标题为「[smol] [perf] Qwen3-VL in place op.」，变更集中在 `python/sglang/srt/models/qwen3_vl.py`, `python/sglang/srt/models/qwen3_vl_moe.py`。PR 描述补充为：## Motivation Because it is not touched by `compile` ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/qwen3_vl.py`；关联提交 `b2c856692092`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+5/-3，可读 patch 19 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 修复已暴露的启动、加载、解析或数值问题，标题为「[BugFix][Qwen3-VL]: fix cu_seqlens in qwen3-vl」，变更集中在 `python/sglang/srt/models/qwen3_vl.py`。PR 描述补充为：## Motivation The implementation of cur_seqlens is inconsistent with the HF implementation, resulting in accuracy issues with multi-frame/multi-patch inputs. ## Modifications Fi...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/multimodal/processors/qwen_vl.py`；关联提交 `fde2decf8b59`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+26/-8，可读 patch 49 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[BugFix][Qwen3-VL]: add metadata for video in qwen3-vl」，变更集中在 `python/sglang/srt/multimodal/processors/qwen_vl.py`。PR 描述补充为：## Motivation In Qwen3's pre-processing pipeline within the transformers repository, `do_sample_frames=True` is enabled by default, which has already been handled by sglang itse...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 修复已暴露的启动、加载、解析或数值问题，标题为「[BugFix][Qwen2.5-VL]: fix cu_seqlens in qwen2.5-vl」，变更集中在 `python/sglang/srt/models/qwen2_5_vl.py`。PR 描述补充为：## Motivation The implementation of cu_seqlens is incorrect and leads to accuracy issues with multi-frame/multi-patch inputs. This exact issue was already solved for Qwen3-VL in...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[CPU] Add support for Qwen3-vl and Qwen3-omni」，变更集中在 `python/sglang/srt/layers/attention/vision.py`, `python/sglang/srt/configs/update_config.py`, `python/sglang/srt/multimodal/processors/qwen_vl.py`。PR 描述补充为：## Motivation This PR added frontend support for Qwen3-VL and Qwen3-Omni on CPU. - Fixed the tp=3/6 padding issue in the vision encoder. - Replaced nn.Linear with ReplicatedLine...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「add qwen3-omni docs」，变更集中在 `docs/basic_usage/qwen3_omni.md`, `docs/index.rst`。PR 描述补充为：## Motivation When I was fixing bugs related to qwen3-omni, I couldn't find any user manuals for qwen3-omni. The official qwen3-omni website only provides a vllm version. I also...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/multimodal/processors/qwen_vl.py`；关联提交 `fd3034da7515`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+43/-10，可读 patch 131 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 优化关键推理路径或后端选择，标题为「[VLM] Optimize qwen_vl preprocess_video」，变更集中在 `python/sglang/srt/multimodal/processors/qwen_vl.py`。PR 描述补充为：## Motivation The preprocess_video function can be divided into several stages. Through a combination of optimizations, each stage achieved varying degrees of performance improv...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[Docs] Add docs for Qwen3-VL image and video support」，变更集中在 `docs/basic_usage/qwen3_vl.md`。PR 描述补充为：Added standalone page for Qwen3 VL model family. cc @mickqian
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/multimodal/processors/qwen_vl.py`；关联提交 `ffeb28ba6fb7`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+1/-67，可读 patch 136 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 修复已暴露的启动、加载、解析或数值问题，标题为「fix: duplicate resize images logic of qwen-vl series models」，变更集中在 `python/sglang/srt/multimodal/processors/qwen_vl.py`。PR 描述补充为：## Motivation fix https://github.com/sgl-project/sglang/issues/12390 and https://github.com/sgl-project/sglang/issues/11896 ## Modifications 1. set `MIN_PIXELS`, `MAX_PIXELS`, `...
...
- 状态/时间: merged / 2025-11-12
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/qwen2_5_vl.py`；关联提交 `706502ff6cff`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+88/-54，可读 patch 244 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[VLM] Support PP for Qwen2.5-VL」，变更集中在 `python/sglang/srt/models/qwen2_5_vl.py`。PR 描述补充为：## Motivation This PR is to support PP for Qwen2.5-VL model. ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to th...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/qwen2_5_vl.py`；关联提交 `ac81db66c2d8`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 6 个文件，+595/-6，可读 patch 790 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[VLM][feat] Support encoder DP for Qwen2.5-VL」，变更集中在 `python/sglang/srt/models/qwen2_5_vl.py`。PR 描述补充为：## Motivation Based on this PR https://github.com/vllm-project/vllm/pull/22742, vLLM has introduced support for data parallelism (DP) in the vision transformer (ViT) component w...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/qwen2_5_vl.py`, `python/sglang/srt/models/qwen2_vl.py`；关联提交 `af6bcadcf723`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 10 个文件，+710/-29，可读 patch 970 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[VLM] Support Piecewise CUDA Graph for Qwen2.5-VL」，变更集中在 `python/sglang/srt/models/qwen2_5_vl.py`, `python/sglang/srt/models/qwen2_vl.py`。PR 描述补充为：## Motivation Address https://github.com/sgl-project/sglang/issues/12838 This PR cooperates closely with @yhyang201 . Per comparing the SGLang and vLLM performance on VLM, one o...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[VLM] support qwen3-vl eagle infer」，变更集中在 `python/sglang/srt/models/qwen3_vl.py`, `python/sglang/srt/models/llama_eagle3.py`。PR 描述补充为：## Motivation Support qwen3-vl dense model eagle infer. use qwen3-vl-8b eagle model for test. ## Modifications ## Accuracy Tests server: acc_bench: result: - with eagle: | Tasks...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/qwen3_vl_moe.py`；关联提交 `15ff6982b94f`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+9/-0，可读 patch 22 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「Support KTransformers for Qwen3-VL moe」，变更集中在 `python/sglang/srt/models/qwen3_vl_moe.py`。PR 描述补充为：## Motivation This pr adds KTransformers Support for qwen3-vl moe models ## Modifications add `get_model_config_for_expert_location` in `Qwen3VLMoeForConditionalGeneration` ## A...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/qwen2_5_vl.py`；关联提交 `f6e37d3edb94`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-0，可读 patch 8 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] qwen2.5-vl spec decode accept_len low」，变更集中在 `python/sglang/srt/models/qwen2_5_vl.py`。PR 描述补充为：## Motivation After this pr, qwen2.5-vl eagle3 infer draft accept_len is low. Because `aux_hidden_states` is miss in logits_processor. Releated issue: https://github.com/sgl-pro...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「support qwen3_vl vision model dp」，变更集中在 `python/sglang/srt/models/qwen3_vl.py`。PR 描述补充为：## Motivation Based on PR 13126, add support for the Qwen3_VL vision model DP. ## Modifications ## Accuracy Tests ### Qwen/Qwen3-VL-32B-Instruct ## Benchmarking and Profiling se...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[VLM] Introduce Cache for positional embedding ids for Qwen-VL family」，变更集中在 `python/sglang/srt/models/qwen2_5_vl.py`, `python/sglang/srt/models/qwen3_vl.py`。PR 描述补充为：## Motivation Introduce a cache for rot_pos_emb index computation to boost the calculation. Introduce a mixin class for broader reuse for this mechanism. For cached rotary posit...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/qwen3_omni_moe.py`；关联提交 `2ac5b9839508`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-2，可读 patch 17 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 修复已暴露的启动、加载、解析或数值问题，标题为「fix rmsnorm -> layernorm in qwen3 omni」，变更集中在 `python/sglang/srt/models/qwen3_omni_moe.py`。PR 描述补充为：## Motivation If you look at the transformer's modeling code, then you will notice that it's actually supposed to be layer norm instead of RMS norm. You can transform RMS norm t...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「Support qwen3-omni with DP Encoder」，变更集中在 `python/sglang/srt/models/qwen3_omni_moe.py`, `test/nightly/test_encoder_dp.py`。PR 描述补充为：## Motivation Add support for Qwen3-Omni with vision model DP based on #13724 ## Modifications ## Accuracy Tests Tested Qwen/Qwen3-Omni-30B-A3B-Instruct with dp | Tasks |Version...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[VLM] Support chunked vit attention」，变更集中在 `python/sglang/srt/models/qwen3_vl.py`, `python/sglang/srt/managers/mm_utils.py`。PR 描述补充为：## Motivation Inspired by @merrymercy @mickqian . One request with 500 images (or a video with 500 frames) on Qwen3-VL-235B-A22B-Instruct-FP8 will have OOM. The PR have two part...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[PP] Add pp support for Qwen3-VL」，变更集中在 `python/sglang/srt/models/qwen3_vl.py`, `python/sglang/srt/models/qwen3_omni_moe.py`。PR 描述补充为：## Motivation see issue 11947 test with command : `python3 -m sglang.launch_server --model Qwen/Qwen3-VL-8B-Thinking --tp 2 --pp-size 2` ## Checklist - [ ] Format your code acco...
...
- 状态/时间: merged / 2025-12-17
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/qwen2_5_vl.py`；关联提交 `0071fe9c407a`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+10/-4，可读 patch 28 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 修复已暴露的启动、加载、解析或数值问题，标题为「[bug fix][pp] fix weight load for qwen2.5-vl」，变更集中在 `python/sglang/srt/models/qwen2_5_vl.py`。PR 描述补充为：## Motivation Now， There are two bug for Qwen2.5-vl 1. when loading weights for Qwen2.5-vl, errors ouucrs(brought by EPD feature): File "/root/leipi/sglang/python/sglang/srt/man...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[VLM] Support cos sin cache for Qwen3-VL & GLM-4.1V」，变更集中在 `python/sglang/srt/models/qwen3_vl.py`。PR 描述补充为：## Motivation Support cos sin cache for Qwen3-VL & GLM-4.1V. This PR refactors the rotary positional embedding (RoPE) implementation to expose an explicit cosine/sine cache inte...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[VLM] Support ViT Piecewise CUDA Graph for Qwen3-VL」，变更集中在 `python/sglang/srt/models/qwen3_vl.py`, `python/sglang/srt/models/qwen2_5_vl.py`。PR 描述补充为：## Motivation This PR is to enable ViT Piecewise CUDA Graph for Qwen3-VL. Building logic upon ViTCudaGraphRunner to support both Qwen2.5-VL and Qwen3-VL. TP>1 is supported. Benc...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/glmasr.py`, `python/sglang/srt/multimodal/processors/glmasr.py`；关联提交 `82f1d6157f8e`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+226/-1，可读 patch 250 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[GLM-ASR] GLM-ASR Support」，变更集中在 `python/sglang/srt/models/glmasr.py`, `python/sglang/srt/multimodal/processors/glmasr.py`。PR 描述补充为：Note that this PR needs to wait for support from the transformers PR PR
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/glmasr.py`, `python/sglang/srt/multimodal/processors/glmasr.py`；关联提交 `f3ba71166262`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 5 个文件，+19/-15，可读 patch 103 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 调整模型相关实现，标题为「Change GLM-ASR class name」，变更集中在 `python/sglang/srt/models/glmasr.py`, `python/sglang/srt/multimodal/processors/glmasr.py`。PR 描述补充为：The class name for GLM-ASR was changed in the transformers PR, so the original code will be unable to import and find the processor.
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 修复已暴露的启动、加载、解析或数值问题，标题为「[Qwen3-VL][PP] Skip loading expert weights not on this rank」，变更集中在 `python/sglang/srt/models/qwen3_vl_moe.py`。PR 描述补充为：## Motivation Failed to run Qwen3-vl-235B-A22B-FP8 with pp=2, tp=4 ## Modifications Fix expert weights loading logic, skip loading expert weights not on this PP rank. ## Accurac...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[Feature] [ROCM] Support Add & LayerNorm fused for Qwen3-VL VIT」，变更集中在 `python/sglang/srt/models/qwen3_vl.py`, `python/sglang/srt/layers/layernorm.py`。PR 描述补充为：## Motivation Support Add & LayerNorm fused for Qwen3.5-Next VIT before： after： ## Modifications **layernorm.py :** - In LayerNorm.__init__: when _use_aiter is True, set _forwar...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「feat: Support 'use_audio_in_video' option for qwen3omnimoe model」，变更集中在 `python/sglang/srt/multimodal/processors/base_processor.py`, `python/sglang/srt/multimodal/processors/qwen_vl.py`, `python/sglang/srt/entrypoints/openai/protocol.py`。PR 描述补充为：## Motivation This PR adds support for the `use_audio_in_video` configuration parameter in the `Qwen3-Omni-Moe` model. ## Modifications - Passed this parameter to the input of '...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 调整模型相关实现，标题为「[Feat] Accelerate qwen3vl by remove cpu op」，变更集中在 `python/sglang/srt/layers/attention/vision.py`, `python/sglang/srt/managers/mm_utils.py`。PR 描述补充为：# Motivation Accelerate qwen3vl by delete contiguous in vision_attention && delete D2D in torch.where and index_put During the forward pass of the ViT in the Qwen3VL multimodal...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「Add Qwen3VL Eagle3 Inference Support」，变更集中在 `python/sglang/srt/models/qwen3_vl.py`。PR 描述补充为：## Motivation Add support for EAGLE3 for Qwen3VL. I have not extensively tried to optimize benchmark performance, but the accuracy/performance numbers at the bottom are there to...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 修复已暴露的启动、加载、解析或数值问题，标题为「Optimize Qwen3-VL video memory usage」，变更集中在 `python/sglang/srt/models/qwen3_vl.py`。PR 描述补充为：## Motivation We observed that when launching the Qwen3-Omni-30B-A3B-Instruct model with sglang, an OOM (Out-of-Memory) error occurs under high-concurrency inference. After anal...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[BUGFIX] Fix dp size > 1 for qwen3 vl model」，变更集中在 `python/sglang/srt/models/qwen3_vl.py`, `python/sglang/srt/multimodal/mm_utils.py`, `python/sglang/srt/layers/linear.py`。PR 描述补充为：## Question In my local tests, the Qwen3-VL service fails to start when both `--mm-enable-dp-encoder` and `--enable-dp-attention` are enabled. I came across the related PR: pr17...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 修复已暴露的启动、加载、解析或数值问题，标题为「fix: correct weight loading prefix mapping for Qwen3-VL」，变更集中在 `python/sglang/srt/models/qwen3_vl.py`。PR 描述补充为：## Summary Fix Qwen3-VL-8B model producing garbage output due to incorrect weight loading. Fixes #17887 ## Problem The weight loading code unconditionally copies `embed_tokens.w...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/qwen3_vl.py`；关联提交 `6a4b81e2d9fc`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+126/-25，可读 patch 175 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 优化关键推理路径或后端选择，标题为「Refactor(qwen3-vl) optimize position encoding interpolation」，变更集中在 `python/sglang/srt/models/qwen3_vl.py`。PR 描述补充为：## Motivation Sync Slime patches. Optimize fast_pos_embed_interpolate for Qwen3-VL by vectorizing core embedding lookups and interpolation math. This reduces GPU kernel launch o...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[BUG] fix mm_enable_dp_encoder hang for Qwen3-VL models」，变更集中在 `python/sglang/srt/layers/vocab_parallel_embedding.py`, `python/sglang/srt/models/qwen3_vl.py`。PR 描述补充为：## Motivation Launch server with --mm-enable-dp-encoder option for Qwen3-VL model( for example LLM parts use tp2 and vision encoder part use DP2) will hang currently. Root cause...
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-0，可读 patch 8 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「Add Qwen3-Omni to Qwen MoE architecture handling in fused_moe_triton」，变更集中在 `benchmark/kernels/fused_moe_triton/common_utils.py`。PR 描述补充为：### What This PR Does This PR simply adds: "Qwen3OmniMoeForConditionalGeneration" to the existing Qwen MoE architecture list. Since Qwen3-Omni follows the same configuration sch...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[feat] feat: add Qwen3-ASR support like whisper」，变更集中在 `python/sglang/srt/multimodal/processors/qwen3_asr.py`, `python/sglang/srt/configs/qwen3_asr.py`, `python/sglang/srt/configs/__init__.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[VLM] Introduce FlashInfer CUDNN Prefill as ViT Backend」，变更集中在 `python/sglang/srt/models/qwen3_vl.py`, `python/sglang/srt/layers/attention/vision.py`, `test/manual/nightly/test_vlms_vit_flashinfer_cudnn.py`。PR 描述补充为：## Motivation FlashInfer CUDNN Prefill demonstrates strong performance. This PR is to introduce it to SGLang as one of VLM ViT attention backends. A new "flashinfer" mm attentio...
...
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-0，可读 patch 8 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 修复已暴露的启动、加载、解析或数值问题，标题为「fix qwen3_vl visual module loading」，变更集中在 `python/sglang/srt/models/qwen3_vl.py`。PR 描述补充为：## Motivation Fix https://github.com/sgl-project/sglang/issues/19335 Qwen3_vl's visual model weight loading is broken. Somehow the name mapping logic was deleted. ## Modificatio...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[NPU] Fix Qwen3-VL-8B Accuracy for NPU」，变更集中在 `python/sglang/srt/models/qwen3_vl.py`, `python/sglang/srt/models/llama.py`, `python/sglang/srt/layers/rotary_embedding.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-0，可读 patch 8 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 修复已暴露的启动、加载、解析或数值问题，标题为「[Qwen3.5] Fix missing `quant_config` in `Qwen3VL`」，变更集中在 `python/sglang/srt/models/qwen3_vl.py`。PR 描述补充为：## Motivation Fix missing `quant_config` in `Qwen3VL` causing Qwen3.5 NVFP4 versions to use bf16 KV cache instead of fp8. ## Modifications ## Accuracy Tests ## Benchmarking and...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[Omni] Optimize AudioEncoder for Qwen3_Omni_Thinker」，变更集中在 `python/sglang/srt/models/qwen3_omni_moe.py`。PR 描述补充为：## Motivation Optimize Qwen3_Omni_Thinker with: 1. Support audio encoder layer's FFN TP>1 2. AudioEncoder vectorized mask and conv fast path in order to reduce CPU small tensor...
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-0，可读 patch 8 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[DP encoder] Fix `pos_emb `layer TP issue when DP encoder enabled for Qwen3 VL」，变更集中在 `python/sglang/srt/models/qwen3_vl.py`。PR 描述补充为：Issue description: Enable DP encoder for qwen3-vl model and TP size = 2: Where DP size shall also be equal to TP size, depending image distribution. However, in current init cod...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「add EVS support for Qwen3-VL」，变更集中在 `python/sglang/srt/multimodal/processors/qwen_vl.py`, `python/sglang/srt/layers/rotary_embedding/mrope_rope_index.py`, `python/sglang/srt/models/qwen3_vl.py`。PR 描述补充为：## Motivation We support EVS(Efficient Video Sampling) for Qwen3-VL. Long video token can be pruned while keeping accuracy ## Modifications * add pruning functions in Qwen-VL's...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[Bugfix] fix qwen3vl hang when --mm-enable-dp-encoder is enable」，变更集中在 `python/sglang/srt/models/qwen3_vl.py`。PR 描述补充为：## Motivation The Qwen3-VL type model hangs during forward when --mm-enable-dp-encoder is enabled. This is because the VocabParallelEmbedding part remains as TP8 after enabling...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[3/n] lora moe - Support Qwen3-VL-30B-A3B-Instruct」，变更集中在 `python/sglang/srt/models/qwen3_vl_moe.py`, `test/registered/lora/test_lora_qwen3_vl_30b_a3b_instruct_logprob_diff.py`。PR 描述补充为：## Motivation - Support Qwen3-VL-30B-A3B-Instruct ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [x] Format your code according to the Format co...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 优化关键推理路径或后端选择，标题为「[AMD] Optimize Qwen3-VL decode - fuse QK-norm + 3D mRoPE + KV cache write」，变更集中在 `python/sglang/srt/models/qwen3.py`。PR 描述补充为：## Motivation Use aiter's fused_qk_norm_mrope_3d_cache_pts_quant_shuffle kernel to replace 4 separate kernel launches (QKV split, QK RMSNorm, 3D mRoPE, KV cache write) with a si...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[Fix] Enable precise embedding interpolation by default for Qwen3-VL」，变更集中在 `python/sglang/srt/models/qwen3_vl.py`, `python/sglang/srt/server_args.py`, `docs/advanced_features/server_arguments.md`。PR 描述补充为：The previous default (enable_precise_embedding_interpolation=False) uses an align_corners=False interpolation scheme that diverges from the HuggingFace transformers reference im...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 调整模型相关实现，标题为「[VLM] Chunk-aware ViT encoding with per-image cache and lazy device transfer」，变更集中在 `python/sglang/srt/models/qwen3_vl.py`, `python/sglang/srt/mem_cache/multimodal_cache.py`, `python/sglang/srt/models/deepseek_vl2.py`。PR 描述补充为：## Motivation - Per-image embedding cache: Switch multimodal embedding cache granularity from per-request (combine_hashes(all_items)) to per-image (item.hash), improving cache r...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[VLM]: allow Qwen3.5 models for encoder disaggregation」，变更集中在 `python/sglang/srt/multimodal/processors/qwen_vl.py`, `test/registered/distributed/test_epd_disaggregation.py`, `python/sglang/srt/disaggregation/encode_server.py`。PR 描述补充为：## Motivation Fixes #21805. SGLang already supports Qwen3.5 multimodal models in the runtime, but encoder disaggregation rejected them during servervstartup. This blocked valid...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[Feature] Adding Qwen3-asr Model Support」，变更集中在 `python/sglang/srt/models/qwen3_asr.py`, `python/sglang/srt/configs/qwen3_asr.py`, `python/sglang/srt/multimodal/processors/qwen3_asr.py`。PR 描述补充为：## Motivation Issue : https://github.com/sgl-project/sglang/issues/22025 This PR adds support so users can serve Qwen3-ASR via the existing `/v1/audio/transcriptions` endpoint....
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[NPU] fix qwen3.5 video processor」，变更集中在 `python/sglang/srt/hardware_backend/npu/modules/qwen_vl_processor.py`。PR 描述补充为：## Motivation In the Qwen3VLVideoProcessor, there is a permute operation with more than 8 dimensions which is not supported on the NPU. Following PR #20189 , this PR applies a p...
...
- 状态/时间: merged / 2026-04-08
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/configs/qwen3_asr.py`, `python/sglang/srt/entrypoints/openai/transcription_adapters/qwen3_asr.py`, `python/sglang/srt/multimodal/processors/qwen3_asr.py`, `test/manual/models/test_qwen3_asr.py`；关联提交 `a5ed507a1639`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 8 个文件，+473/-223，可读 patch 809 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[refactor] [asr] Add transcription adapter for extensible ASR models support」，变更集中在 `python/sglang/srt/configs/qwen3_asr.py`, `test/manual/models/test_qwen3_asr.py`, `python/sglang/srt/entrypoints/openai/transcription_adapters/qwen3_asr.py`。PR 描述补充为：## Motivation Follow up of #22073. To remove hardcoded model family detection and branching. See reviews in #22073 and #22089. ## Modifications Mainly scoped under python/sglang...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[Feature] Support eagle3 for qwen3-vl」，变更集中在 `python/sglang/srt/models/qwen3_vl.py`。PR 描述补充为：## Motivation Adapt Eagle3 capture for the Qwen3-VL model ## Modifications Add `set_eagle3_layers_to_capture` function on `qwen3_vl.py`. Like: qwen2_5_vl.py ## Launch Script ##...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[Feature] Add chunk-based streaming ASR for Qwen3-ASR」，变更集中在 `python/sglang/srt/entrypoints/openai/transcription_adapters/qwen3_asr.py`, `python/sglang/srt/multimodal/processors/qwen3_asr.py`。PR 描述补充为：Motivation Issue: #22025 (streaming input design and implementation) This PR adds chunk-based streaming transcription support for Qwen3-ASR via the existing /v1/audio/transcript...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「fix(config): Add from_dict() for Qwen3VL config classes」，变更集中在 `test/registered/unit/configs/test_qwen3_vl_config.py`, `python/sglang/srt/configs/qwen3_5.py`, `python/sglang/srt/configs/qwen3_vl.py`。PR 描述补充为：## Motivation Fix Qwen3-VL and Qwen3.5 model loading with transformers 5.5.0+. Transformers 5.5.0+ natively supports Qwen3-VL, causing `AutoConfig.from_pretrained()` to skip sgl...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 调整模型相关实现，标题为「[Feature] WebSocket streaming audio input for ASR」，变更集中在 `test/manual/models/test_qwen3_asr.py`, `python/sglang/srt/entrypoints/openai/serving_transcription_websocket.py`, `python/sglang/srt/entrypoints/openai/streaming_asr.py`。PR 描述补充为：## Motivation Implements M1 of the RFC in sgl-project/sglang#22474. PR #22089 shipped chunked streaming **output** for Qwen3-ASR via `POST /v1/audio/transcriptions?stream=true`...
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-0，可读 patch 8 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 修复已暴露的启动、加载、解析或数值问题，标题为「fix: guard self.model access in Qwen3VLMoeForConditionalGeneration.load_weights」，变更集中在 `python/sglang/srt/models/qwen3_vl_moe.py`。PR 描述补充为：Fixes #23063 ## Problem When launching a Qwen3-VL-MoE model in `--encoder-only` mode, the server crashes during weight loading with `AttributeError: 'Qwen3VLMoeForConditionalGen...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 修复已暴露的启动、加载、解析或数值问题，标题为「Fix Qwen3.5 video processing when passing video_data in "processor_output" format」，变更集中在 `python/sglang/srt/multimodal/processors/qwen_vl.py`。PR 描述补充为：## Motivation Currently, the video preprocessing function of Qwen3.5 requires two return values: https://github.com/sgl-project/sglang/blob/ef6bfc1197ab45290e33941881f23c39fbf30...
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-0，可读 patch 8 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 修复已暴露的启动、加载、解析或数值问题，标题为「Bugfix: Qwen3-VL-MoE adapt encoder_only」，变更集中在 `python/sglang/srt/models/qwen3_vl_moe.py`。PR 描述补充为：## Motivation This bug is caused by PR https://github.com/sgl-project/sglang/pull/19135, `self.model` is created in the parent class and is only created when `--encoder-only` is...
- 实现要点: `python/sglang/srt/models/qwen3_vl_moe.py` modified +1/-0 (1 lines); hunks: -235,6 +235,7 @@ def load_weights(self, weights: Iterable[Tuple[str, torch.Te...; symbols: load_weights，涉及 `load_weights`。
- 代码 diff 细节:
  - `python/sglang/srt/models/qwen3_vl_moe.py` modified +1/-0 (1 lines); hunks: -235,6 +235,7 @@ def load_weights(self, weights: Iterable[Tuple[str, torch.Te...; symbols: load_weights
- 关键代码摘录:
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[Bugfix] Fix Qwen3-VL rope config compatibility」，变更集中在 `python/sglang/srt/models/qwen3.py`。PR 描述补充为：## Motivation `Qwen3-VL` can fail during model initialization because `qwen3.py` assumes `config.rope_parameters["rope_theta"]` is always available. In practice, Qwen3-VL series...
...
- 动机: 该 PR 围绕 Qwen VLM/Omni/ASR 补齐模型支持入口或运行时能力，标题为「[NPU] adapt the Qwen3-ASR model for deployment on NPU」，变更集中在 `python/sglang/srt/utils/common.py`。PR 描述补充为：## Motivation Enable Qwen3-ASR model deployment on NPU devices. The current audio loading implementation relies on torchaudio with CUDA dependencies, which is not compatible wit...
