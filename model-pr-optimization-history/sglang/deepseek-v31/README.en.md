...
- Motivation: For DeepSeek V3.1, this PR adds or enables a model support/runtime surface. Title: "Add deepseek v3.1 thinking parser support and update docs". The diff centers on `python/sglang/srt/entrypoints/openai/serving_chat.py`, `python/sglang/srt/reasoning_parser.py`. PR body context: Adds documentation and examples for enabling model reasoning capabilities in the OpenAI API completions endpoint. This includes support for different model families like DeepSee...
...
- Motivation: For DeepSeek V3.1, this PR adds or enables a model support/runtime surface. Title: "[doc] deepseekv31 support". The diff centers on `benchmark/deepseek_v3/README.md`, `docs/basic_usage/deepseek.md`. PR body context: Update docs for deepseekv3.1 DeepSeek-V3.1 is natively supported by SGLang - Fast and Efficient in Large Scale Serving, enjoy all the existing optimization introduced in SGLang...
...
- Motivation: For DeepSeek V3.1, this PR adds or enables a model support/runtime surface. Title: "Support DeepSeek-V3.1 tool call". The diff centers on `python/sglang/srt/function_call/deepseekv31_detector.py`, `examples/chat_template/tool_chat_template_deepseekv31.jinja`. PR body context: ## Motivation Support tool call for DeepSeek-V3.1 The tool call format of DeepSeek-V3.1 is different from DeepSeek-V3/R1: DeepSeek-V3.1: tool_call_name tool_call_arguments DeepS...
...
- Motivation: For DeepSeek V3.1, this PR extends deployment docs, tests, or CI coverage. Title: "Use jsonschema to constrain required or specific tool choice". The diff centers on `test/srt/function_call/test_json_schema_constraint.py`, `test/srt/openai_server/function_call/test_tool_choice.py`, `test/srt/test_function_call_parser.py`. PR body context: ## Motivation EBNF constraints for tool choice are hard to read and extend. They must be individually created for each model's tool call format. Instead, we can use a much more...
...
- Motivation: For DeepSeek V3.1, this PR adds or enables a model support/runtime surface. Title: "feat(reasoning): improve enable thinking from request". The diff centers on `python/sglang/srt/entrypoints/openai/serving_chat.py`. PR body context: ## Motivation read from request's "enable_thinking" or "thinking", should also filtered with `reasoning_parser` ---- there is a real case: to compat dpsk and qwen, both `chat_te...
...
- Motivation: For DeepSeek V3.1, this PR adds or enables a model support/runtime surface. Title: "Add --thinking-mode to run_eval". The diff centers on `python/sglang/test/run_eval.py`, `python/sglang/test/simple_eval_common.py`. PR body context: ## Motivation Enabling thinking mode improves the accuracy score in GPQA for Deepseek V3.2. ## Modifications Add `--thinking-mode` to run_eval for models with think/non-think mo...
...
- Motivation: For DeepSeek V3.1, this PR adds or enables a model support/runtime surface. Title: "Update tool parser and related documentation". The diff centers on `python/sglang/srt/function_call/function_call_parser.py`, `python/sglang/srt/server_args.py`. PR body context: - Updated supported parsers table to include Qwen, DeepSeek, GLM, and Mistral models. - Refactored tool call parser mappings to use simplified names. - Implemented deprecation w...
...
- Motivation: For DeepSeek V3.1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "Fix DeepSeek chat templates to handle tool call arguments type checking (#11700)". The diff centers on `test/srt/test_deepseek_chat_templates.py`, `examples/chat_template/tool_chat_template_deepseekv3.jinja`, `examples/chat_template/tool_chat_template_deepseekv31.jinja`. PR body context: This commit fixes the double JSON encoding issue in DeepSeek chat templates (v3, v3.1, v3.2) that was causing incorrect tool call formatting in multi-round function calling scen...
...
- Motivation: For DeepSeek V3.1, this PR adds or enables a model support/runtime surface. Title: "Remove enable_dp_attention in deepseek nightly tests". The diff centers on `test/srt/nightly/test_deepseek_v32_perf.py`, `test/srt/nightly/test_deepseek_v31_perf.py`. PR body context: ## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- Motivation: For DeepSeek V3.1, this PR changes model-related implementation. Title: "[Tool Call] Steamline function arguments when tool_choice="auto" for deepseekv31_detector". The diff centers on `python/sglang/srt/function_call/deepseekv31_detector.py`. PR body context: ## Motivation The current `deepseekv31_detector` only detects complete function arguments and immediately returns them while clearing the buffer. This approach can lead to a poo...
...
- Motivation: For DeepSeek V3.1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "Fix Deepseek v3.1 loading issue". The diff centers on `python/sglang/srt/models/deepseek_v2.py`. PR body context: Fix nightly test error in https://github.com/sgl-project/sglang/actions/runs/19689716521/job/56402800532
...
- Motivation: For DeepSeek V3.1, this PR changes model-related implementation. Title: "[Auto Sync] Update tool_chat_template_deepseekv31.jinja (20251210)". The diff centers on `examples/chat_template/tool_chat_template_deepseekv31.jinja`. PR body context: Sync changes from commit `abd3e13d`. **Files Changed:** - examples/chat_template/tool_chat_template_deepseekv31.jinja Author: Jue Wang --- *This is an automated PR created by sc...
...
- Motivation: For DeepSeek V3.1, this PR adds or enables a model support/runtime surface. Title: "[CI] Enable dpsk v31 test on nightly H200". The diff centers on `test/registered/8-gpu-models/test_deepseek_v31.py`. PR body context: ## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- Motivation: For DeepSeek V3.1, this PR adds or enables a model support/runtime surface. Title: "Remove deepseek-r1 from THINKING_MODE_CHOICES in run_eval.py". The diff centers on `python/sglang/test/run_eval.py`. PR body context: ## Motivation deepseek-r1 is a pure reasoning model with no way to enable/disable reasoning, unless you modify the chat template directly. Starting from v3.1, deepseek introduce...
...
- Motivation: For DeepSeek V3.1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "fix: Fixed the issue where "finish_reason":"stop" appeared when calling the tool and the tool was in the content.". The diff centers on `python/sglang/srt/function_call/deepseekv32_detector.py`, `examples/chat_template/tool_chat_template_deepseekv32.jinja`. PR body context: Motivation The DeepSeek-V3.2 model exhibits an issue with tool call parsing: tool call information is always output as plain text in the content field instead of being correctly...
...
- Trace source: preserved from an explicit existing history/skill citation
- Diff scope read: GitHub Pull Request files API returned 2 files, +14/-16, 56 readable patch lines; this card prioritizes model-related and high-change files.
- Motivation: For DeepSeek V3.1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "fix: Fixed the issue where "finish_reason":"stop" appeared when calling the tool and the tool was in the content.". The diff centers on `python/sglang/srt/function_call/deepseekv32_detector.py`, `examples/chat_template/tool_chat_template_deepseekv32.jinja`. PR body context: # 1. Motivation The DeepSeek-V3.2 model exhibits an issue with tool call parsing: tool call information is always output as plain text in the content field instead of being corr...
- Key implementation: `python/sglang/srt/function_call/deepseekv32_detector.py` modified +8/-4 (12 lines); hunks: -195,12 +195,16 @@ def detect_and_parse(self, text: str, tools: list[Tool]) -...; symbols: detect_and_parse, touching `detect_and_parse`; `examples/chat_template/tool_chat_template_deepseekv32.jinja` modified +6/-12 (18 lines); hunks: -22,7 +22,7; -41,20 +41,14.
- Code diff details:
  - `python/sglang/srt/function_call/deepseekv32_detector.py` modified +8/-4 (12 lines); hunks: -195,12 +195,16 @@ def detect_and_parse(self, text: str, tools: list[Tool]) -...; symbols: detect_and_parse
  - `examples/chat_template/tool_chat_template_deepseekv32.jinja` modified +6/-12 (18 lines); hunks: -22,7 +22,7; -41,20 +41,14
...
- Motivation: For DeepSeek V3.1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "fix: missing Assistant token after tool output in DeepSeek v3.1/v3.2 chat templates". The diff centers on `test/manual/test_deepseek_chat_templates.py`, `examples/chat_template/tool_chat_template_deepseekv31.jinja`, `examples/chat_template/tool_chat_template_deepseekv32.jinja`. PR body context: ## Motivation Fix missing ` ` token after tool output in DeepSeek v3.1 and v3.2 chat templates. When an assistant message follows a tool output, the ` ` token was not being adde...
...
- Motivation: For DeepSeek V3.1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "Fix function call arguments missing in streaming mode for DeepSeek V3.1". The diff centers on `python/sglang/srt/function_call/deepseekv31_detector.py`. PR body context: ## Problem When using DeepSeek V3/V3.1 models with function calling in streaming mode, the tool call arguments are returned as empty `{}` even when the model generates valid par...
...
- Motivation: For DeepSeek V3.1, this PR extends deployment docs, tests, or CI coverage. Title: "[NPU] Update DeepSeek-V3.1 and DeepSeek-V3.2 model deployment instructions in documentation". The diff centers on `docs/platforms/ascend/ascend_npu_best_practice.md`. PR body context: ## Motivation Updates the documentation for deploying the DeepSeek-V3.1 and DeepSeek-V3.2 model on Ascend NPU ## Modifications Based on the main branch of the sglang-project, we...
...
- Motivation: For DeepSeek V3.1, this PR adds or enables a model support/runtime surface. Title: "[Test] Add unit tests for DeepSeekV31Detector". The diff centers on `test/registered/unit/function_call/test_deepseekv31_detector.py`. PR body context: ## Motivation Part of #20865 (improve unit test coverage). `srt/function_call/deepseekv31_detector.py` previously had no direct test coverage — unlike its siblings `DeepSeekV3De...
...
- Motivation: For DeepSeek V3.1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "Fix tool call constrained decoding and parsing for models with native formats". The diff centers on `test/registered/unit/function_call/test_function_call_parser.py`, `python/sglang/srt/entrypoints/openai/serving_chat.py`, `python/sglang/srt/function_call/function_call_parser.py`. PR body context: ## Summary When a model-specific `--tool-call-parser` is configured (e.g. `kimi_k2`, `deepseekv3`, `qwen25`), `tool_choice="required"` previously: 1. Forced a **generic JSON sch...
...
- Motivation: For DeepSeek V3.1, this PR adds or enables a model support/runtime surface. Title: "[Test] Add unit tests for 7 missing function call detectors". The diff centers on `test/registered/unit/function_call/test_function_call_parser.py`, `test/registered/openai_server/function_call/test_tool_choice.py`, `test/registered/unit/function_call/test_kimik2_detector.py`. PR body context: ## Summary - Add unit tests for 7 previously uncovered detector classes: DeepSeekV31, GptOss, Internlm, Step3, Trinity, MiMo, MinimaxM2 - Strengthen DeepSeekV3 detector coverage...
...
- Motivation: For DeepSeek V3.1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[fix] Parser-gated two-phase cache stripping for reasoning radix caches (fixes #22373)". The diff centers on `python/sglang/srt/parser/reasoning_parser.py`, `python/sglang/srt/configs/model_config.py`, `test/registered/unit/mem_cache/test_radix_cache_thinking.py`. PR body context: # Fix dead reasoning branches in radix cache for multi-turn separated-thinking models Fixes #22373. Related to #22617. --- ## Motivation ### What is the problem Reasoning models...
