...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[Feature] Support Sparse Attention and KV cache scheduling between CPU and GPU for GQA/DSA.」，变更集中在 `python/sglang/srt/layers/attention/flashattention_backend.py`, `python/sglang/srt/model_executor/model_runner.py`, `python/sglang/srt/sparse_attention/kernels/attention/flash_fwd_sm100.py`。PR 描述补充为：The main implementation is currently in a private repository. I will migrate the code to this branch within a week. ## Motivation Support sparse attention computation for LLMs u...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「Add DeepSeek-V3.2 Tool Call Template」，变更集中在 `examples/chat_template/tool_chat_template_deepseekv32.jinja`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[Feature] Add a fast-topk to sgl-kernel for DeepSeek v3.2」，变更集中在 `sgl-kernel/csrc/elementwise/topk.cu`, `sgl-kernel/tests/test_topk.py`, `sgl-kernel/python/sgl_kernel/top_k.py`。PR 描述补充为：## Motivation This kernel is written for DeepSeek v3.2 indexer. Indexer of Deepseek Sparse Attention requires a variable-length topk kernel, which is extremely crucial for ## Mo...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 调整模型相关实现，标题为「[DeepSeek-V3.2] Include indexer kv cache when estimating kv cache size」，变更集中在 `python/sglang/srt/model_executor/model_runner.py`, `python/sglang/srt/mem_cache/memory_pool.py`, `python/sglang/srt/server_args.py`。PR 描述补充为：## Motivation For DS v3.2 to avoid OOM. ## Results On 8xB200: With this PR: With main (hardcoded mem-fraction-static=0.8) - KV size including index is actually 65.49 GB ## Modif...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 优化关键推理路径或后端选择，标题为「[DPSKv3.2] Rewrite nsa tilelang act_quant kernel to triton」，变更集中在 `python/sglang/srt/layers/attention/nsa/triton_kernel.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation We want to remove the tilelang dependency because it relies on tvm, but the tvm version required by tilelang is incompatible with the one used by flashinfer. This...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「Fix DeepSeek-v3.2 default config (ValueError: not enough values to unpack (expected 4, got 3))」，变更集中在 `python/sglang/srt/server_args.py`。PR 描述补充为：## Motivation Changes to DeepSeek-R1-FP4 default config in #11512 are accidently being applied to DeepSeek-V3.2-Exp. On blackwell, this causes the attention backend for 3.2 to b...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[CI] Add Basic Test for DeepSeek V3.2」，变更集中在 `test/srt/test_deepseek_v32_basic.py`, `.github/workflows/pr-test.yml`, `scripts/ci/ci_install_dependency.sh`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[PD] Add PD support for hybrid model (Qwen3-Next, DeepSeek V3.2 Exp)」，变更集中在 `python/sglang/srt/model_executor/model_runner.py`, `python/sglang/srt/mem_cache/memory_pool.py`, `python/sglang/srt/disaggregation/mooncake/conn.py`。PR 描述补充为：## Motivation and Modifications To support hybrid attention models like Qwen3-Next with mamba state or DeepSeek V3.2 Exp with indexer cache, this PR extends the current PD disag...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[Draft] Support MTP for DeepSeek-V3.2」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/configs/model_config.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[CI] Add CI test for DeepSeek V3.2 MTP」，变更集中在 `test/srt/test_deepseek_v32_mtp.py`, `test/srt/test_deepseek_v32_basic.py`, `python/sglang/srt/server_args.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa_backend.py`；关联提交 `ef4a8097b8e1`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+31/-31，可读 patch 206 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「Rename flashmla kernel options of nsa backend for better readability」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`。PR 描述补充为：## Motivation - Rename `--nsa-prefill` and `--nsa-decode` to `--nsa-prefill-backend` and `--nsa-decode-backend` - Add documents for `--nsa-prefill-backend` and `--nsa-decode-bac...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「(beta)support context parallel with deepseekv3.2-DSA」，变更集中在 文件级 diff。PR 描述补充为：## Motivation Currently, under deepseek3.2-DSA, prefill-ttft of long text sequences takes a long time. Introducing context parallel can reduce ttft. Current description： 1. Curr...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[Doc] Add documentation for DeepSeek V3.2」，变更集中在 `docs/references/multi_node_deployment/rbg_pd/deepseekv32_pd.md`, `docs/basic_usage/deepseek_v32.md`。PR 描述补充为：Depends on #11844 and #11876 Also waiting for @whybeyoung on some PD updates ## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Fo...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「Fix Illegal Instruction/IMA errors when using DP attention with DeepSeek-V3.2 models」，变更集中在 `python/sglang/srt/layers/dp_attention.py`。PR 描述补充为：## Motivation Fix illegal memory access (IMA) errors when using DP attention with DeepSeek-V3.2 models. **Issue**: When running with `--enable-dp-attention`, the DP gather opera...
...
- 状态/时间: merged / 2025-10-25
- 反查来源: `git log --name-only -- <model-files>` 反查到 `docs/basic_usage/deepseek_v32.md`；关联提交 `bcecf27e7ca2`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+3/-3，可读 patch 22 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[Doc] Fix format for deepseek v3.2 document」，变更集中在 `docs/basic_usage/deepseek_v32.md`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[Test] Add dsv3.2 nsa backend testing」，变更集中在 `test/srt/test_deepseek_v32_nsabackend.py`, `test/srt/run_suite.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- 状态/时间: merged / 2025-10-26
- 反查来源: `git log --name-only -- <model-files>` 反查到 `docs/basic_usage/deepseek_v32.md`；关联提交 `97828878d833`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-1，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「[Doc] Small update of DeepSeek v3.2 document」，变更集中在 `docs/basic_usage/deepseek_v32.md`。PR 描述补充为：## Motivation Fixing a wrong sentence ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 优化关键推理路径或后端选择，标题为「Update deepseek_v32.md」，变更集中在 `docs/basic_usage/deepseek_v32.md`。PR 描述补充为：## Modifications Update the configuration tips of Deepseek V3.2 after the optimizations in https://github.com/sgl-project/sglang/pull/11655 ## Checklist - [ ] Format your code a...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[Deepseek V3.2] Enable flashmla_auto with MTP」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`。PR 描述补充为：## Motivation Follow up to https://github.com/sgl-project/sglang/pull/11655 ## Modifications With fp8 kvcache on B200, flashmla_sparse can be used in the extend (not draft_exten...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「Enable mixed type LayerNorm kernel for NSA indexer」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation Currently we cast input to layernorm to fp32 and use native Torch layernorm, then cast back. Instead we pull in a more efficient TRT-LLM kernel (via flashinfer) th...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「[Bug] Fix NSA Backend KV-Buffer Shape Mismatch in DeepSeek-V3.2」，变更集中在 `python/sglang/srt/mem_cache/memory_pool.py`。PR 描述补充为：## Motivation Fix this issue https://github.com/sgl-project/sglang/issues/12643 Root Cause: The override_kv_cache_dim calculation in NSATokenToKVPool.__init__() was using the wr...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「DeepSeek-V3.2: Add Adaptive MHA Attention Pathway for Short-Sequence Prefill」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation For DeepSeek-V3.2 models, using MLA (Multi-Latent Attention) uniformly across all sequence lengths during prefill is suboptimal. For short sequences, the overhead...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[DeepSeek-V3.2][NSA] Enable MHA Pathway for Short Sequence Prefill on B200 (SM100)」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Follow up this PR: DeepSeek-V3.2: Add Adaptive MHA Attention Pathway for Short-Sequence Prefill. Enable and optimize MHA on B200 (SM100) in the NSA backend by TRT-...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[Test] Add DeepSeekV3.2 NSA Indexer Test Suite」，变更集中在 `python/sglang/srt/layers/attention/nsa/index_buf_accessor.py`。PR 描述补充为：## Motivation Address the issue https://github.com/sgl-project/sglang/issues/12509 ## Modifications Added 11 tests: 1. test_forward_decode_mode 2. test_forward_extend_mode 3. te...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[WIP][Feature] support tp-sp on qwen2/3 & deepseek v2/3/3.2」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/communicator.py`, `python/sglang/srt/models/qwen3.py`。PR 描述补充为：## Motivation For the classic dense decode layer structure (self-attention + MLP), in the pure TP case, tensors are parallelized in the attention layer and the MLP layer. Since...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[sgl-kernel][Deepseek V3.2] Add row_starts to topk kernel」，变更集中在 `sgl-kernel/tests/test_topk.py`, `sgl-kernel/csrc/elementwise/topk.cu`, `sgl-kernel/python/sgl_kernel/top_k.py`。PR 描述补充为：## Motivation Part1 of the fix for bug in https://github.com/sgl-project/sglang/issues/11629 ## Modifications In the topk kernel for prefill, the q and k inputs are both ragged....
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补强部署文档、测试或 CI 验证面，标题为「[Docs][DeepseekV3.2] Update deepseekv3.2 docs for mha short seq prefill」，变更集中在 `docs/basic_usage/deepseek_v32.md`。PR 描述补充为：## Motivation Update DeepSeek V3.2 documentation to document the adaptive MHA short-sequence prefill mechanism, helping users understand the new attention pathway selection logi...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 调整模型相关实现，标题为「[Deepseek V3.2] Use torch.compile to speed up torch.cat in nsa」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`。PR 描述补充为：## Motivation Use torch.compile to speed up torch.cat used in prefill. ## Modifications Replace the cat ops for k in `forward_extend` in the nsa attention with the torch.compile...
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-1，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「DeepSeek V3.2 indexer RoPE fix」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation https://github.com/sgl-project/sglang/issues/13530 ## Modifications Optimized the rotary position embedding configuration in the sparse-attention indexer to improv...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 调整模型相关实现，标题为「[DeepSeekV3.2] Centralize NSA dispatch logic in NativeSparseAttnBackend」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation NativeSparseAttnBackend currently spreads dispatch logic for NSA prefill/decode implementations and MHA vs. MLA selection across multiple places: - Global `NSA_PRE...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`；关联提交 `846ba3c62c22`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+5/-4，可读 patch 24 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「Fix nightly test failures: NSA indexer dtype and CPP radix cache init」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Summary Fixes two nightly test failures: 1. **test_nsa_indexer.py** - dtype mismatch error 2. **test_cpp_radix_cache.py** - server crash during initialization ## Changes ###...
...
- 状态/时间: merged / 2025-11-26
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`；关联提交 `8a9b8b8457d0`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+4/-5，可读 patch 24 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「Revert "Fix nightly test failures: NSA indexer dtype and CPP radix cache init"」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：Reverts sgl-project/sglang#13958 This change caused dpsk v32 to break https://github.com/sgl-project/sglang/actions/runs/19694975166/job/56476095353?pr=13151 @alisonshao Please...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa_backend.py`；关联提交 `1f2b84d28d63`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+3/-2，可读 patch 12 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「Fix NSA Bug in Centralize NSA Dispatch Logic」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`。PR 描述补充为：## Motivation Fix operator precedence bug in `set_nsa_prefill_impl` that caused `use_mha` to always be True on H200 (SM90), bypassing the `max_kv_len B`). Added parentheses to e...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/entrypoints/openai/encoding_dsv32.py`, `python/sglang/srt/function_call/deepseekv32_detector.py`；关联提交 `7c38eca1e4a7`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 6 个文件，+1153/-89，可读 patch 1359 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 补强部署文档、测试或 CI 验证面，标题为「feat: DeepSeek new v3.2 encoding」，变更集中在 `python/sglang/srt/entrypoints/openai/encoding_dsv32.py`, `python/sglang/srt/function_call/deepseekv32_detector.py`。PR 描述补充为：## Motivation https://github.com/sgl-project/sglang/issues/14227 DeepSeek official release a new encoding func to replace chat_template, and I made one workable version(though i...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `docs/basic_usage/deepseek_v32.md`；关联提交 `922054079c29`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+76/-13，可读 patch 170 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 补强部署文档、测试或 CI 验证面，标题为「[Doc] Update DeepSeek-V3.2 document」，变更集中在 `docs/basic_usage/deepseek_v32.md`。PR 描述补充为：## Motivation Following #14249 ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit....
...
- 动机: 该 PR 围绕 DeepSeek V3.2 优化关键推理路径或后端选择，标题为「[Performance] Optimize NSA Indexer K/S Buffer Access with Fused Triton Kernels」，变更集中在 `test/manual/layers/attention/nsa/test_index_buf_accessor.py`, `python/sglang/srt/layers/attention/nsa/index_buf_accessor.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation https://github.com/sgl-project/sglang/issues/13811 ## Modifications Implement fused Triton kernels that: 1. Retrieve both K and S data in a single kernel call 2. I...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 调整模型相关实现，标题为「feat: V32 tool call parsing for no-dsml tag」，变更集中在 `test/registered/function_call/test_function_call_parser.py`, `python/sglang/srt/function_call/deepseekv32_detector.py`。PR 描述补充为：## Motivation my last pr for new v32 encoding and tool-call parsing is merged. but with openrouter's help, we found out there's a low possibility that v32 model could output too...
...
- 状态/时间: merged / 2025-12-03
- 反查来源: `git log --name-only -- <model-files>` 反查到 `docs/basic_usage/deepseek_v32.md`；关联提交 `4bcc5879af61`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+2/-2，可读 patch 18 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[Doc] Fix DeepSeek V32 Doc」，变更集中在 `docs/basic_usage/deepseek_v32.md`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- 状态/时间: merged / 2025-12-03
- 反查来源: `git log --name-only -- <model-files>` 反查到 `docs/basic_usage/deepseek_v32.md`；关联提交 `7e78825d5af5`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+4/-2，可读 patch 18 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[Tiny]Small fixes in deepseek v32 doc」，变更集中在 `docs/basic_usage/deepseek_v32.md`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[DeepseekV3.2][NSA][Indexer] Fix PAGED top-k transform for NSA indexer chunked execution on H200」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation In `extend` mode with NSA indexer enabled, H200 setups that select **PAGED** top-k transform (`flashmla_kv` + FP8 KV cache) may trigger chunked execution in `_get_...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[Test] Add test suite for NSA backend」，变更集中在 `python/sglang/test/attention/test_nsa_backend.py`。PR 描述补充为：## Motivation https://github.com/sgl-project/sglang/issues/14523 ## Modifications 20+ test cases covering: 1. Core forward operations - test_forward_extend - Standard extend ope...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[SMG][DS32][fix] support dsv32, add role developer」，变更集中在 `sgl-model-gateway/src/protocols/chat.rs`, `sgl-model-gateway/src/routers/grpc/harmony/builder.rs`, `sgl-model-gateway/src/routers/http/pd_router.rs`。PR 描述补充为：Hi from novita.ai team 👋 same as #14304 add role developer for ChatMessage
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[FIX][DS32]openai protocol: support openai message role: developer」，变更集中在 `python/sglang/srt/entrypoints/openai/protocol.py`。PR 描述补充为：Hi from novita.ai team 👋 ## Motivation support for deepseeek v3.2. example test_input_search_w_data.json got error ## Checklist - [x] Format your code according to the Format co...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 调整模型相关实现，标题为「[DeepSeek V3.2] Proper drop_thinking logic」，变更集中在 `python/sglang/srt/entrypoints/openai/serving_chat.py`。PR 描述补充为：## Motivation DeepSeek's technical report describes the context management logic, which describes a mechanism for discarding thinking blocks when receiving a new message with th...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「fix: dpskv32 chat history processing, default drop_thinking to true」，变更集中在 `python/sglang/srt/entrypoints/openai/serving_chat.py`。PR 描述补充为：## Motivation Co-authored by: @vladnosiv As stated in #14904, the DeepSeek tech report shows a specific way for history thinking processing, so the current implementation, sets...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「feat(ds32): support tag for deepseek 3.2 tool call」，变更集中在 `python/sglang/srt/function_call/deepseekv32_detector.py`。PR 描述补充为：## Motivation #15042 ## Modifications add more match tag
...
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「fix(DeepSeek-V3.2 function_call): fix streaming content loss in DeepSeekV32Detector」，变更集中在 `python/sglang/srt/function_call/deepseekv32_detector.py`。PR 描述补充为：## Motivation When the buffer contains accumulated content from previous chunks (e.g., when a chunk ends with " `: - Chunk 1: "... ..." (returns only new_text, discards " ` bein...
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-1，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 优化关键推理路径或后端选择，标题为「[sgl-kernel] Update flashmla to include fp8 sparse_mla optimizations」，变更集中在 `sgl-kernel/cmake/flashmla.cmake`。PR 描述补充为：## Motivation https://github.com/sgl-project/sglang/issues/15211 ## Accuracy Tests gsm8k 20shots: Accuracy: 0.955 Invalid: 0.000 Latency: 175.612 s Output throughput: 727.763 to...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[DeepSeekV3.2] Add pure TP+MTP test」，变更集中在 `docs/basic_usage/deepseek_v32.md`。PR 描述补充为：## Motivation Add test coverage for DeepSeek V3.2 with pure tensor parallel mode and MTP (EAGLE speculative decoding) without DP attention. This addresses the "Support pure TP+M...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「dsv32 support o_proj tp」，变更集中在 `python/sglang/srt/layers/communicator.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/linear.py`。PR 描述补充为：## Motivation In order to improve the performance of DeepSeek-V3.2, we enabled the `o_proj` layers to support tensor parallel and tensor parallel independently of `attn_tp`. ##...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「feat: DeepSeek-V3.2 Streaming tool call output」，变更集中在 `python/sglang/srt/function_call/deepseekv32_detector.py`。PR 描述补充为：## Motivation FIxes #14711 ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 优化关键推理路径或后端选择，标题为「[Performance] optimize NSA backend metadata computation for multi-step speculative decoding」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_backend_mtp_precompute.py`, `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/utils.py`。PR 描述补充为：The NSA backend for multi-step speculative decoding. The optimization implements a precompute-once-copy-many strategy: - Old approach: Compute metadata N times (one per backend)...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「feat(dsv32): better error handling for DeepSeek-v3.2 encoder」，变更集中在 `python/sglang/srt/entrypoints/openai/encoding_dsv32.py`。PR 描述补充为：Hi from novita.ai team 👋 ## Motivation when users' input illegal and cause error in deepseek3.2 encoder, return 400 instead of 500 ## Modifications add a kind of exception for d...
...
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-1，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「[Deepseek V3.2] Fix Deepseek MTP in V1 mode」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`。PR 描述补充为：## Motivation ## Modifications Restore the earlier behaviour of Deepseek V3 + MTP. (Accidental change) Fix https://github.com/sgl-project/sglang/issues/15428
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「fix ds3.2 nsa backend prefill TBO」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation After applying the fixes, tbo can be enabled on ds3.2. And resolving • https://github.com/sgl-project/sglang/issues/12698 • https://github.com/sgl-project/sglang/i...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[1/N][Sparse With Hicache]: Add Sparse Interface」，变更集中在 `python/sglang/srt/mem_cache/sparsity/algorithms/base_algorithm.py`, `python/sglang/srt/mem_cache/sparsity/algorithms/quest_algorithm.py`, `python/sglang/srt/mem_cache/sparsity/algorithms/deepseek_nsa.py`。PR 描述补充为：## Motivation This PR primarily introduces a generic Retrievable Sparse Algorithm interface. Upstream PR：https://github.com/sgl-project/sglang/pull/14619 ## Modifications ## Acc...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[Tool Call][DSV32] Streamline function call parameters」，变更集中在 `python/sglang/srt/function_call/deepseekv32_detector.py`。PR 描述补充为：## Motivation As titled and Issue #14711. **IMPORTANT: This is an initial version aimed at streamlining function call parameters for `DeepSeek-V3.2`. I have tested it, and it pa...
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-1，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[cp] bug fix for dsv3.2 cp」，变更集中在 `python/sglang/srt/hardware_backend/npu/modules/deepseek_v2_attention_mla_npu.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「[Fix] DSV3.2 W4AFP8 MTP use FP8 draft model」，变更集中在 `python/sglang/srt/layers/moe/token_dispatcher/deepep.py`, `python/sglang/srt/layers/quantization/w4afp8.py`。PR 描述补充为：## Motivation DeepSeekV3.2 W4A8 MTP acceptance rate is very low use this commad: `SGLANG_DEEPEP_BF16_DISPATCH=1 SGLANG_DISAGGREGATION_BOOTSTRAP_TIMEOUT=600 SGLANG_DISAGGREGATION...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[cp] assert dsv3.2 cp in pd decode mode」，变更集中在 `python/sglang/srt/server_args.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[DeepSeek v3.2] opt Context Parallelism: support fused moe, multi batch and fp8 kvcache」，变更集中在 `python/sglang/srt/layers/attention/nsa/utils.py`, `python/sglang/srt/layers/communicator_nsa_cp.py`, `python/sglang/srt/layers/attention/nsa_backend.py`。PR 描述补充为：## Motivation The original default token splitting scheme of cp does not support prefill multi-batch. A new token splitting method is introduced to enable multi-batch support, f...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「Multiple updates of DeepSeek V32 and context parallel」，变更集中在 `docs/basic_usage/deepseek_v32.md`。PR 描述补充为：## Motivation - Refine the handler in `server_args.py` - Move dpsk v32 cp test to pr-test, and add pure tp tests - Update the document for better readibility ## Modifications ##...
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-0，可读 patch 8 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[Deepseek V3.2] Enable TRTLLM Allreduce Fusion」，变更集中在 `python/sglang/srt/server_args.py`。PR 描述补充为：This should close some of the gap in TP MoE (around 3-4%). TODO: provide perf numbers
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「fix: unimplemented methods in BaseIndexerMetadata」，变更集中在 `test/registered/kernels/test_nsa_indexer.py`。PR 描述补充为：## Motivation On Jan 2, commit 0d244116d introduced a breaking change as part of the Context Parallelism optimization for DeepSeek V3.2. This commit added 3 new non-abstract met...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「Clean Some Environment Variables for DeepSeek V32」，变更集中在 `python/sglang/srt/layers/attention/nsa/quant_k_cache.py`, `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/utils.py`。PR 描述补充为：## Motivation ## Modifications - Move `SGLANG_NSA_FUSE_TOPK` and `SGLANG_NSA_ENABLE_MTP_PRECOMPUTE_METADATA` to environ.py - Deprecate `SGLANG_NSA_DUAL_STREAM`, `SGLANG_NSA_FLAS...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[1/n]deepseek_v2.py Refactor: attention backend handlers and forward method definition」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/models/deepseek_common/attention_backend_handler.py`, `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_methods.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[DeepSeek 3.2] Support and optimize pipeline parallelis when context pipeline enabled」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/managers/scheduler_pp_mixin.py`。PR 描述补充为：## Motivation For issue #15358. CC @whybeyoung Support pipeline parallelism for the Prefill CP scenario (`--enable-nsa-prefill-context-parallel`). In this scenario, the linear l...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[DSv32] Add returning DSA topk indices」，变更集中在 `python/sglang/srt/layers/moe/routed_experts_capturer.py`, `python/sglang/srt/managers/detokenizer_manager.py`, `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation https://github.com/sgl-project/sglang/issues/16856 ## Modifications Incrementally add DSA topk indices capturing on top of https://github.com/sgl-project/sglang/pu...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「add doc for dsv32 cp+pp」，变更集中在 `docs/basic_usage/deepseek_v32.md`。PR 描述补充为：## Motivation add doc for dsv32 cp+pp
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+3/-2，可读 patch 19 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[Ascend] fix dsv3.2 weight cast bug」，变更集中在 `python/sglang/srt/layers/quantization/unquant.py`。PR 描述补充为：Add NPU format casting for layer weights in processing. ## Motivation fix the bug `NPU function error: call aclnnMatmulWeightNz failed` when loading `deepseekv3.2 `weight. ## Mo...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[AMD] enable CUDA graph for NSA backend and fix NSA FP8 fused RMSNorm group quant」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/attention/nsa/index_buf_accessor.py`。PR 描述补充为：Co-author: @wufann ## Motivation This PR enables cuda graph (`--cuda-graph-max-bs 64`) and fixes crashes/memory faults observed when running DeepSeek-V3.2-Exp with NSA (tilelang...
...
- 状态/时间: merged / 2026-01-14
- 反查来源: `git log --name-only -- <model-files>` 反查到 `docs/basic_usage/deepseek_v32.md`；关联提交 `2122fea3c408`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+2/-2，可读 patch 18 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「Update deepseekV32 Cp doc」，变更集中在 `docs/basic_usage/deepseek_v32.md`。PR 描述补充为：fix doc typo
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[DeepSeek V3.2] [Feat] add tensor parallel o_proj linear in context parallel nsa」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/attention/nsa/utils.py`, `python/sglang/srt/server_args.py`。PR 描述补充为：## Motivation The current NSA context parallel partitioning causes the o_proj to be copied across all cards, resulting in each card using 10GB more VRAM compared to DeepSeek v3....
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `test/registered/amd/test_deepseek_v32_basic.py`, `test/registered/amd/test_deepseek_v32_mtp.py`；关联提交 `968c4f55b194`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+379/-1，可读 patch 403 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[AMD] Enable DeepseekV3.2 test for AMD CI」，变更集中在 `test/registered/amd/test_deepseek_v32_mtp.py`, `test/registered/amd/test_deepseek_v32_basic.py`。PR 描述补充为：## Motivation To enable DeepseekV3.2 test for AMD CI ## Modifications ### test/registered/amd/test_deepseek_v32_basic.py from test/srt/test_deepseek_v32_basic.py ### test/regist...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 优化关键推理路径或后端选择，标题为「[DeepSeek v3.2] Opt MTP decode cuda batch sizes and nsa implementation」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`。PR 描述补充为：## Motivation 1、`draft_extend` and `target_verify` use `nsa_decode_backend` as backend implementation instead of `nsa_prefill_backend` In the MTP scenario, the NSA attention imp...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「[Fix]: correctly fetch ds32 config in tuning_fused_moe_triton」，变更集中在 `benchmark/kernels/fused_moe_triton/common_utils.py`。PR 描述补充为：## Motivation fix "deepseek_v32" KeyError ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 优化关键推理路径或后端选择，标题为「[OPT] DeepSeekV3.2: optimize indexer weight_proj-mma performance」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation From these pr[https://github.com/sgl-project/sglang/pull/16637 and https://github.com/sgl-project/sglang/issues/16861 and https://github.com/sgl-project/sglang/pul...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `test/registered/kernels/test_nsa_indexer.py`；关联提交 `be5121b45219`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+16/-0，可读 patch 23 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「Fix NSA indexer in the nightly test」，变更集中在 `test/registered/kernels/test_nsa_indexer.py`。PR 描述补充为：https://github.com/sgl-project/sglang/actions/runs/21194510000
...
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-1，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[HotFix]Fix dtype mismatch in nsa indexer on AMD device」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation Fix this CI failure https://github.com/sgl-project/sglang/actions/runs/21192748451/job/60966055534 Introduced in https://github.com/sgl-project/sglang/pull/17205/f...
...
- 状态/时间: merged / 2026-01-22
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `test/registered/amd/test_deepseek_v32_basic.py`, `test/registered/amd/test_deepseek_v32_mtp.py`；关联提交 `17807caf82c8`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 5 个文件，+16/-7，可读 patch 96 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「[AMD] fix amd ci dpskv32」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `test/registered/amd/test_deepseek_v32_basic.py`, `test/registered/amd/test_deepseek_v32_mtp.py`。PR 描述补充为：## Motivation Fix the runtime error from PR-17205 : https://github.com/sgl-project/sglang/actions/runs/21157007917/job/60858903195?pr=17205#step:6:17583 ## Modifications ## Accu...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[DeepSeek V3.2] Enable trtllm NSA with bf16 kvcache」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`。PR 描述补充为：## Motivation Enables NSA backend with trtllm kernels for sparse attention. This can be more efficient than FlashMLA when the head size isn't a multiple of 64 and hence requires...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `test/registered/kernels/test_nsa_indexer.py`；关联提交 `137eb5b95cb1`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+3/-1，可读 patch 25 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「Fix NSA indexer test and move it to pre commit test」，变更集中在 `test/registered/kernels/test_nsa_indexer.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「fix: missing Assistant token after tool output in DeepSeek v3.1/v3.2 chat templates」，变更集中在 `test/manual/test_deepseek_chat_templates.py`, `examples/chat_template/tool_chat_template_deepseekv31.jinja`, `examples/chat_template/tool_chat_template_deepseekv32.jinja`。PR 描述补充为：## Motivation Fix missing ` ` token after tool output in DeepSeek v3.1 and v3.2 chat templates. When an assistant message follows a tool output, the ` ` token was not being adde...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补强部署文档、测试或 CI 验证面，标题为「[AMD] Update dsv3.2 AMD GPU docs and unify ROCm TileLang build」，变更集中在 `docs/basic_usage/deepseek_v32.md`。PR 描述补充为：## Motivation Improve the AMD GPU user experience for DeepSeek V3.2 by documenting correct ROCm images and launch flags, while also consolidating the TileLang build to use the u...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 优化关键推理路径或后端选择，标题为「[DeepSeek] Update tests and document for DeepSeek V3.2 NVFP4 checkpoint」，变更集中在 `docs/basic_usage/deepseek_v32.md`。PR 描述补充为：## Accuracy Tests Some accuracy results: GSM8K 20shot GPQA-Diamond TODO：Update Cookbook ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[AMD] CI - enable deepseekv3.2 on MI325-8gpu and merge perf/accuracy test suites into stage-b suites」，变更集中在 `test/registered/amd/test_deepseek_v32_basic.py`, `test/registered/amd/test_deepseek_v32_mtp.py`。PR 描述补充为：## Motivation Enable/move 2 CI cases to mi325-8gpu runner - deepseek v3.2 - Kimi-K2-Instruct-0905 Update AMD CI config to align with https://github.com/sgl-project/sglang/pull/1...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 调整模型相关实现，标题为「[DSv32] Overlap indexer qk projection and activation quant」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation @humansand After https://github.com/sgl-project/sglang/pull/17205, the indexer weight projection is fully hidden and no longer exposes latency. ## Modifications ##...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[AMD] Add Kimi-K2, DeepSeek-V3.2 tests to nightly CI」，变更集中在 `test/registered/amd/accuracy/mi35x/test_deepseek_v32_mtp_eval_mi35x.py`, `test/registered/amd/accuracy/mi35x/test_deepseek_v32_dp_eval_mi35x.py`, `test/registered/amd/perf/mi35x/test_deepseek_v32_mtp_perf_mi35x.py`。PR 描述补充为：## Motivation 1. Add **Kimi-K2**, **DeepSeek-V3.2** accuracy and performance tests for MI325 (MI30x) platform, update Mi35x tests, consolidate test jobs, and fix various CI fail...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「Add tool call tests for DeepSeek V3.2 in nightly CI」，变更集中在 `test/registered/8-gpu-models/test_deepseek_v32.py`。PR 描述补充为：## Summary Add tool call tests for DeepSeek V3.2 to nightly 8-GPU CI. - New `tool_call_test_runner.py`: reusable runner with 9 test cases (basic format, streaming, tool_choice r...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[Feature] Add DCP support for DeepSeek v3.2」，变更集中在 `python/sglang/srt/layers/attention/utils.py`, `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Following PR https://github.com/sgl-project/sglang/pull/14982, add decode context parallel (DCP) support for DeepSeek v3.2. The KV cache redundancy issue in DeepSe...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「Deepseekv32 compatibility with transformers v5」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation ## Modifications 1. Enhance Indexer and DeepseekV2AttentionMLA to support dynamic is_neox_style parameter. 2. Refactor rope parameter handling in DeepseekV2Decoder...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「fix: fixed aux hidden state index out of range when using eagle3 with nsa cp」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation `python3 -m sglang.launch_server --model-path $MODEL_PATH --trust-remote-code \ --port 8000 --host 0.0.0.0 --attention-backend nsa \ --enable-metrics --mem-fractio...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「Fix Bug on dsv3.2」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation Fix two bugs on dsv32 when infering on npu ## Modifications 1. fix a bug: forward_npu method of nas_Indexer has a dual stream feature that can not be closed. May c...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补强部署文档、测试或 CI 验证面，标题为「[V3.2] Change default CP token split method to `--round-robin-split`」，变更集中在 `docs/basic_usage/deepseek_v32.md`。PR 描述补充为：## Motivation Since the `--in-seq-split` one is out of maintenance ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according...
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+100/-0，可读 patch 101 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「Add DeepSeek V32 PD disaggregation test」，变更集中在 `test/registered/distributed/test_disaggregation_deepseek_v32.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 优化关键推理路径或后端选择，标题为「Kernel: optimize decoding metadata in NSA multi-spec backend with fused kernels」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_mtp_verification.py`, `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/nsa_backend_mtp_precompute.py`。PR 描述补充为：## Motivation Implement fused CUDA kernels to eliminate redundant metadata copies in Native Sparse Attention (NSA) backend during CUDA graph replay for speculative decoding. Thi...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「Fix dsv32 encode_messages」，变更集中在 `python/sglang/srt/entrypoints/openai/serving_chat.py`, `python/sglang/srt/parser/jinja_template_utils.py`。PR 描述补充为：Fix #18125
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「Fix model loading for DeepSeek-V3.2-AWQ」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Got the following error when using DeepSeek-V3.2-AWQ (https://huggingface.co/QuantTrio/DeepSeek-V3.2-AWQ/) ## Modifications Add check of whether `weight_packed` ex...
...
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-1，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[AMD] Fix mi35x dsv32 mtp nightly」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`。PR 描述补充为：## Motivation PR #17554 introduced `fused_metadata_copy`, a JIT-compiled CUDA kernel enabled by default. This kernel cannot run on ROCm/HIP — it fails on every CUDA graph replay...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「Fix NSA FP8 KV cache path for both-trtllm MHA one-shot」，变更集中在 `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mha.py`。PR 描述补充为：## Motivation This is a follow-up to #18389 for the NSA FP8 KV cache path case when both prefill and decode backends are trtllm. In this setup, the MHA one-shot prefix path coul...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[DSv32] Fix MTP and CP compatability」，变更集中在 `python/sglang/srt/models/deepseek_nextn.py`。PR 描述补充为：## Motivation PR https://github.com/sgl-project/sglang/pull/17213 introduced separate `get_attention_cp_size()` / `get_attention_cp_rank()` APIs and migrated the main model in `...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/utils.py`；关联提交 `8cf003c44b1b`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+5/-1，可读 patch 13 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「Fix spec v2+dp attention in nsa backend」，变更集中在 `python/sglang/srt/layers/attention/nsa/utils.py`。PR 描述补充为：## Motivation Close #18943 This bug is introduced in #17213 ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补强部署文档、测试或 CI 验证面，标题为「[Refactor][DeepSeek-V3.2] Extract V3.2/NSA logic into `DeepseekV32Mixin`」，变更集中在 `python/sglang/srt/models/deepseek_common/v32_mixin.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/models/deepseek_nextn.py`。PR 描述补充为：## Motivation #16815 - part of #16255 cc @Fridge003 ## Accuracy Tests ### GSMK ### GPQA ### AIME ## Benchmark
...
- 动机: 该 PR 围绕 DeepSeek V3.2 优化关键推理路径或后端选择，标题为「[Perf] O(1) expert weight matching in DeepSeek weight loader」，变更集中在 `python/sglang/srt/models/deepseek_common/deepseek_weight_loader.py`, `test/unit/test_deepseek_weight_loader.py`。PR 描述补充为：## Motivation Large MoE models like GLM-5 with 256 routed experts generate hundreds of thousands of weight tensors. The original weight loading code used an O(N*M) algorithm (li...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「Fix NSA CP positions mismatch in eagle NextN model」，变更集中在 `python/sglang/srt/models/deepseek_nextn.py`。PR 描述补充为：## Summary - When context parallelism (CP) is enabled, the eagle NextN model (`deepseek_nextn.py`) splits `hidden_states` across CP ranks but does **not** split `positions`, cau...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[Feature] add feature mla_ag_after_qlora for dsv3.2」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/communicator.py`。PR 描述补充为：## Motivation In DeepSeek-V3.2, due to the large hidden dimension of the model, performing all-gather on hidden_states before attention introduces a high-dimensional communicati...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[AMD] Use `tilelang` as default NSA attention backend dispatch on AMD Instinct」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`。PR 描述补充为：As per title. This PR adds a default on Instinct to `tilelang` for NSA attention backend. "flashmla_sparse" and "fa3" are not supported on ROCm afaik. Seems like part of this wa...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[AMD] Enable cudagraph for aiter nsa backend and add aiter impl for nsa pr…」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/triton_kernel.py`。PR 描述补充为：## Motivation Bool mask produced dynamic shape that could not be captured into cuda graph. Capture NSA decode implemented by aiter backend into cuda graph for better performance...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补强部署文档、测试或 CI 验证面，标题为「TP indexer weight in NSA attention」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation Tested at BS = 4, ISL=2048 OSL=1024 Before: After: Only seems to be around 2%. Currently, the all-reduce in TP=4 will take the same time as the size of the indexer...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 优化关键推理路径或后端选择，标题为「[Perf] Optimize NSA backend metadata under MTP」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`。PR 描述补充为：## Motivation This PR is authored by @Baidu-AIAK in https://github.com/sgl-project/sglang/pull/17647. I just tested the code, since the author hasn't replied in a while. ## Accu...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/utils.py`；关联提交 `b7f7df7ee6c2`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+6/-1，可读 patch 14 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「[NSA] Fix line-too-long lint in `can_nsa_prefill_cp_round_robin_split`」，变更集中在 `python/sglang/srt/layers/attention/nsa/utils.py`。PR 描述补充为：- Reformat long boolean return expression in `can_nsa_prefill_cp_round_robin_split` to comply with line-length lint rule Made with Cursor
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[AMD] Support context parallel for DeepSeek-V3.2 on AMD GPUs and add its test to nightly CI」，变更集中在 `test/registered/8-gpu-models/test_deepseek_v32_cp_single_node.py`, `python/sglang/srt/server_args.py`, `.github/workflows/nightly-test-amd-rocm720.yml`。PR 描述补充为：## Motivation This PR adds and validates context parallel support for DeepSeek-V3.2 on AMD GPUs, focusing on the `round-robin-split` CP mode with AITER NSA backends. The goal is...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「[AMD] Fix nightly GLM-5 failures: Fix NSA indexer tensor aliasing on ROCm during CUDA graph capture」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation PR #19263 removed the `forward_native` override for rotary embeddings in DeepSeek V3.2 (`self.rotary_emb.forward = self.rotary_emb.forward_native`). Previously, `f...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[FIX] NSA backend page_table overflow in speculative decoding target_verify」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`。PR 描述补充为：## Motivation This PR may address #18980 . When speculative decoding is enabled with the NSA attention backend, the decode server crashes with: Root cause: - init_cuda_graph_sta...
...
- 状态/时间: merged / 2026-03-06
- 反查来源: `git log --name-only -- <model-files>` 反查到 `test/registered/quant/test_deepseek_v32_fp4_4gpu.py`, `test/registered/quant/test_deepseek_v32_fp4_mtp_4gpu.py`；关联提交 `04e364d538bb`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+477/-12，可读 patch 572 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[V32] Enhance deepseek v32 related tests」，变更集中在 `test/registered/quant/test_deepseek_v32_fp4_mtp_4gpu.py`, `test/registered/quant/test_deepseek_v32_fp4_4gpu.py`。PR 描述补充为：## Motivation - Add spec v2 tests - Add mtp tests for fp4 model ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[V32/GLM5] Change default setting of V32 nvfp4 on TP4」，变更集中在 `python/sglang/srt/server_args.py`。PR 描述补充为：## Motivation After the flashmla is rebased to latest version, DeepSeekV32 fp4+tp4 will break on flashmla decode kernel, since the latest code doesn't support q_head=32 error lo...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa_backend.py`, `test/registered/quant/test_deepseek_v32_fp4_4gpu.py`, `test/registered/quant/test_deepseek_v32_fp4_mtp_4gpu.py`；关联提交 `be63f982b7b7`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 6 个文件，+32/-59，可读 patch 200 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[V32/GLM5] Control the threshold of applying dense attention with an environ」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`, `test/registered/quant/test_deepseek_v32_fp4_4gpu.py`, `test/registered/quant/test_deepseek_v32_fp4_mtp_4gpu.py`。PR 描述补充为：## Motivation - Add an environ `SGLANG_NSA_DENSE_ATTN_KV_LEN_THRESHOLD`, for controlling whether to use dense MHA or sparse MLA kernel. It's set to index.topk by default, thus n...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「Add DeepSeek3.2 and GlmMoeDsa into moe tune」，变更集中在 `benchmark/kernels/fused_moe_triton/common_utils.py`。PR 描述补充为：## Motivation As title. tuning_fused_moe_triton_sep.py uses `get_model_config` in common_utils to choose the moe model. This PR is to count DeepSeek V3.2 and GLM-5(which uses Gl...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[AMD][Bug fix] Fix NSA context parallelism (round-robin-split) producing garbage output」，变更集中在 `python/sglang/srt/layers/communicator_nsa_cp.py`。PR 描述补充为：## Summary Fix garbage output when using `--enable-nsa-prefill-context-parallel` with `--attn-cp-size 2` on multi-GPU setups (observed on 8xMI300X with GLM-5-FP8 / DeepSeek-V3.2...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[Doc] Add DSA/NSA attention backend to support matrix」，变更集中在 `docs/advanced_features/attention_backend.md`。PR 描述补充为：## Summary - Adds NSA (DSA) row to the MLA backends table with its capabilities (FP8 KV cache, speculative decoding topk=1, chunked prefix cache) - Adds a new "DSA Attention Bac...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[bugfix] Fix NSA indexer ragged gather batch-view mismatch in CP round-robin split」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation This PR fixes a crash in the NSA indexer prefill path when context parallelism is enabled with round-robin-split. The issue was caused by mixing two different batc...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 优化关键推理路径或后端选择，标题为「Transfer FP8 K/K_scale for CP indexer prefill gather」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation This PR optimizes the NSA indexer CP prefill path in round-robin-split mode. Instead of: - all-gathering reordered bf16 indexer keys - then quantizing and storing...
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-0，可读 patch 8 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[Bugfix] Add DeepseekV32ForCausalLM to MTP draft model mapping」，变更集中在 `python/sglang/srt/configs/model_config.py`。PR 描述补充为：## Motivation Speculative decoding (EAGLE/NEXTN MTP) does not work with DeepSeek-V3.2 models (`DeepseekV32ForCausalLM`). The `_config_draft_model()` function only maps `Deepseek...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「Reject HiCache L3 storage backend for NSA models at init time」，变更集中在 `python/sglang/srt/mem_cache/hiradix_cache.py`。PR 描述补充为：## Summary - Raise `ValueError` at startup when `--hicache-storage-backend` is used with NSA (Native Sparse Attention) models (e.g. GLM-5, DeepSeek-V3.2) - The L3 storage layer...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「[BugFix] bug fix for DeepSeek eagle3 in Attn-DP mode」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation The current DeepSeek Eagle3 implementation has an issue in DP scenarios when using tensor_model_parallel_all_gather. The gather should be performed across the atte...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「[PD] Fix DeepSeek V3.2 indexer cache transfer」，变更集中在 `python/sglang/srt/disaggregation/prefill.py`。PR 描述补充为：## Motivation A bugfix by @whybeyoung , we will verify it together tomorrow. ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「Fix DeepSeek V32 FP4 test」，变更集中在 `test/registered/quant/test_deepseek_v32_fp4_4gpu.py`, `test/registered/quant/test_deepseek_v32_fp4_mtp_4gpu.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「Revert "Fix DeepSeek V32 FP4 test"」，变更集中在 `test/registered/quant/test_deepseek_v32_fp4_4gpu.py`, `test/registered/quant/test_deepseek_v32_fp4_mtp_4gpu.py`。PR 描述补充为：Reverts sgl-project/sglang#20984 because it breaks ci
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[Bug] Preserve DeepSeek-V3.2 tool-call markers in reasoning parsing」，变更集中在 `test/registered/unit/parser/test_reasoning_parser.py`, `python/sglang/srt/parser/reasoning_parser.py`。PR 描述补充为：## Summary - preserve DeepSeek-V3.2 DSML tool-call markers when `deepseek-v3` reasoning output switches into tool calls - add DeepSeek-V3-specific reasoning detector coverage fo...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「HiSparse for Sparse Attention」，变更集中在 `python/sglang/srt/model_executor/model_runner_kv_cache_mixin.py`, `python/sglang/srt/model_executor/model_runner.py`, `python/sglang/srt/layers/attention/nsa_backend.py`。PR 描述补充为：## Motivation This PR introduces HiSparse, which leverages CPU memory to store idle KV cache during decoding, thereby increasing batch size and improving throughput for models t...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[bugfix][AMD] Fix PPMissingLayer AttributeError for deepseek v2/v3 in aiter_gfx95 code path」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation When running DeepSeek-V2/V3 with Pipeline Parallelism (PP) on AMD gfx950 GPUs (MI300/MI350), the server crashes during model initialization with: `AttributeError:...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[2/N][Sparse With Hicache]: Support separating nsa memory management for KV cache and index_k in decode side.」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/model_executor/model_runner.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation In the previous implementation, NSA's indexer_k pool and KV cache pool shared a single set of index mappings (req_to_token_pool, token_to_kv_pool_allocator). In up...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[Sparse & HICache]: Enables hierarchical sparse KV cache management and scheduling for DeepSeek V32.」，变更集中在 `python/sglang/srt/model_executor/model_runner.py`, `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/flashattention_backend.py`。PR 描述补充为：## Motivation Previously, Sparse Attention was introduced in DeepSeekV32 Mode, which selects only the top-2048 tokens to participate in attention computation, significantly impr...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「Fix CP in-seq-split method for DeepSeek V32 and update related tests」，变更集中在 `test/registered/cp/test_deepseek_v32_cp_single_node.py`, `test/registered/8-gpu-models/test_deepseek_v32_cp_single_node.py`, `test/manual/nightly/test_deepseek_v32_perf.py`。PR 描述补充为：## Motivation - Fix in-seq-split for CP of V32 - Remove all the V32-Exp models in CI testcases - Move V32 CP tests to cp folder, and mark it as pr-test ## Modifications ## Accur...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[Perf] Overlap NSA-CP key all-gather with query computation for DeepSeek-V3.2」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation In the current Native Sparse Attention (NSA) implementation, specifically when Context Parallelism (CP) is enabled via `--enable-nsa-prefill-context-parallel`, the...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「Workaround of DSA performance drop on B200 + DP」，变更集中在 `python/sglang/srt/server_args.py`。PR 描述补充为：## Motivation #21291 Route the config of glmfp8+B200+DP to bf16 kvcache+flashmla sparse prefill+trtllm decode Note that this is just a workaround, not root fix. #21011 can be th...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 优化关键推理路径或后端选择，标题为「[Performance] Change sparse MLA and dense MHA switching threshold DSv3.2」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`。PR 描述补充为：## Command `SGLANG_DISAGGREGATION_BOOTSTRAP_TIMEOUT=600 SGLANG_DISAGGREGATION_THREAD_POOL_SIZE=128 SGLANG_DISAGGREGATION_QUEUE_SIZE=128 SGLANG_PP_LAYER_PARTITION="8,8,8,8,7,7,7,...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「FIX: (NSA) Compute topk_indices_offset when NSA prefill flashmla_sparse is used with FP8 KV cache」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`。PR 描述补充为：## Motivation When using the flashmla_sparse NSA prefill backend with FP8 KV cache, topk_indices_offset is never computed outside the normal EXTEND forward mode, causing a crash...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[WIP][NPU] DeepSeek-V3.2 adapt enable-torch-compile」，变更集中在 `python/sglang/srt/layers/moe/token_dispatcher/fuseep.py`, `python/sglang/srt/layers/quantization/compressed_tensors/schemes/compressed_tensors_w8a8_int8_moe.py`, `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「[ROCm] Fix fused MLA decode rope path for Kimi K2.5 and DeepSeek-variant models」，变更集中在 `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla_fused_rope_rocm.py`, `python/sglang/srt/layers/attention/triton_ops/rocm_mla_decode_rope.py`。PR 描述补充为：The SGLANG_ROCM_FUSED_DECODE_MLA=1 path had several issues preventing it from working with models like Kimi K2.5 that use DeepseekScalingRotaryEmbedding or different ForwardMeta...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「[Fix] Catch MalformedJSON exception for DeepSeek-V3.2 function call partial parsing」，变更集中在 `python/sglang/srt/function_call/deepseekv32_detector.py`。PR 描述补充为：## Motivation During partial JSON parsing, a MalformedJSON exception may be thrown if the format is invalid. DeepSeek-V3.2 does not catch this exception, causing the parsing pro...
...
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-1，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[CI] Move v32 cp test to deepep running suite」，变更集中在 `test/registered/cp/test_deepseek_v32_cp_single_node.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Speed Tests and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit te...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[Test] Add unit tests for encoding_dsv32.py」，变更集中在 `test/registered/unit/entrypoints/openai/test_encoding_dsv32.py`。PR 描述补充为：## Summary Part of #20865 (Improve Unit Test Coverage) Adds 72 CPU-only unit tests for `python/sglang/srt/entrypoints/openai/encoding_dsv32.py` — the DSML encoding/decoding modu...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[NPU] Update DeepSeek-V3.2 model deployment instructions in documentation」，变更集中在 `docs/platforms/ascend/ascend_npu_best_practice.md`。PR 描述补充为：## Motivation Updates the documentation for deploying the DeepSeek-V3.2 model on Ascend NPU ## Modifications Based on the main branch of the sglang-project, we enabled the atten...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[DSA] Support trtllm sparse mla kernel for prefill batches」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/server_args.py`, `python/sglang/test/run_eval.py`。PR 描述补充为：## Motivation Depends on flashinfer v0.6.7 #21422 ## Modifications ## Accuracy Tests ## Speed Tests and Profiling ## Checklist - [ ] Format your code according to the Format cod...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[AMD] Enable FP4 (E2M1) KV cache quantization for NSA with TileLang backend」，变更集中在 `python/sglang/srt/layers/attention/nsa/dequant_fp4_to_fp8.py`, `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/model_executor/model_runner_kv_cache_mixin.py`。PR 描述补充为：## Motivation Enable FP4 KV cache for NSA on Tilelang backend, based on FP8 attention https://github.com/sgl-project/sglang/pull/21511. This PR should be rebased onto main after...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[DSA] Set trtllm kernels as default for Blackwell」，变更集中在 `python/sglang/srt/server_args.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Speed Tests and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit te...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[HiSparse]: Optimize server args checking-HiSparse is temporarily only available for DSA models.」，变更集中在 `python/sglang/srt/server_args.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Speed Tests and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit te...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「Enable IndexCache for DeepSeek V3.2」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla.py`, `test/registered/8-gpu-models/test_deepseek_v32_indexcache.py`。PR 描述补充为：## Motivation fix https://github.com/sgl-project/sglang/issues/21286 ## Modifications * Port https://github.com/THUDM/IndexCache * add ut for deepseek-v3.2 ## Accuracy Tests gsm...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[HiSparse]: Add readme docs for HiSparse Feature」，变更集中在 `docs/advanced_features/hisparse_guide.md`, `docs/basic_usage/deepseek_v32.md`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Speed Tests and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit te...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 优化关键推理路径或后端选择，标题为「[HiSparse] Optimize the scheduling of decode backup.」，变更集中在 `python/sglang/srt/model_executor/model_runner.py`, `python/sglang/srt/managers/hisparse_coordinator.py`。PR 描述补充为：## Motivation In overlap scheduling, the backup of decode tokens within the `prepare_for_decode` method currently requires waiting for the forward stream to complete, resulting...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 调整模型相关实现，标题为「Reduce unnecessary kernels and copies in the NSA indexer」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：cc @Jacob0226 ## Motivation The NSA indexer currently launches several unnecessary kernels: - `_project_and_scale_head_gates` and `_get_logits_head_gate` use multiple element-wi...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[HiSparse]: Add HiSpares-DSA Model's nightly CI」，变更集中在 `test/registered/8-gpu-models/test_dsa_models_hisparse.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Speed Tests and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit te...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 优化关键推理路径或后端选择，标题为「[AMD] Use aiter CK layernorm2d for LayerNorm to reduce NSA indexer kernel launches」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：cc @Jacob0226 ## Motivation The current `LayerNorm` on HIP uses the torch implementation and triggers an extra dtype cast at both entry and exit. This results in 3 kernels per `...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[DSA] Enable all reduce fusion for DSA models」，变更集中在 `python/sglang/srt/server_args.py`。PR 描述补充为：## Motivation Including DeepSeek V3.2 and GLM-5 ## Modifications ## Accuracy Tests ## Speed Tests and Profiling ## Checklist - [ ] Format your code according to the Format code...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 修复已暴露的启动、加载、解析或数值问题，标题为「[Fix] Fix several bugs on DSA models」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/server_args.py`。PR 描述补充为：## Motivation Fix #22401 - Avoid hardcoding of nsa backends on sm100 - Fix topk transform method for draft model ## Modifications ## Accuracy Tests ## Speed Tests and Profiling...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[DSA] Add dense MLA decode fallback for short sequences」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/server_args.py`。PR 描述补充为：## Summary - When all sequences in a decode batch have `seq_len 2048) sequences - [ ] Verify `SGLANG_NSA_DECODE_DENSE_ATTN_KV_LEN_THRESHOLD=0` disables the optimization - [ ] Be...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[Deepseek V3.2] Optimize use of dual_stream in nsa_indexer/attention」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 优化关键推理路径或后端选择，标题为「[AMD][HIP] NSA: bf16 passthrough from RMSNorm to eliminate FP8 dequantization」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation When running GLM-5-FP8 on MI355X with FP8 quantization, the NSA indexer's head gate path (`weights_proj`) requires bf16 input. However, the upstream `fused_rms_fp8...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[DSA] Hopper FP8 FlashMLA KV padding」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`, `docs/basic_usage/deepseek_v32.md`, `python/sglang/srt/server_args.py`。PR 描述补充为：## Summary Adds q-head padding for `flashmla_kv` to support pure-TP configurations. FlashMLA requires q heads to be 64 or 128, but with tensor parallelism the per-GPU head count...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[HiCache & HybridModel] mooncake backend support DSA & mamba model」，变更集中在 `python/sglang/srt/mem_cache/memory_pool_host.py`, `python/sglang/srt/mem_cache/storage/mooncake_store/mooncake_store.py`, `python/sglang/srt/mem_cache/hybrid_cache/hybrid_pool_assembler.py`。PR 描述补充为：## Motivation Added support for the Mooncake backend. Supports both Mamba and DSA models. Roadmap：https://github.com/sgl-project/sglang/issues/21846 ## Modifications ## Accuracy...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 优化关键推理路径或后端选择，标题为「nsa indexer: use aiter indexer_k_quant_and_cache」，变更集中在 `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Motivation
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[RL] [DSv32] [GLM-5] Add `--nsa-topk-backend` and integrate FlashInfer and pytorch topk」，变更集中在 `python/sglang/srt/layers/attention/nsa_backend.py`, `test/registered/kernels/test_nsa_indexer.py`, `python/sglang/srt/server_args.py`。PR 描述补充为：## Motivation @humansand Add `--nsa-topk-backend` for configurable topk backend implementation selection. `torch.topk` is used by GLM-5 for RL. FlashInfer topk has determinism a...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[sparsity] extend framework to support non-NSA sparse algorithms」，变更集中在 `python/sglang/srt/model_executor/model_runner.py`, `python/sglang/srt/layers/attention/flashattention_backend.py`, `python/sglang/srt/model_executor/forward_batch_info.py`。PR 描述补充为：- wire up sparse_coordinator, handling non-NSA sparsity ## Motivation Enable the use of `SparseCoordinator`. This allows for the use of sparse attention algorithms (e.g. Quest,...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 优化关键推理路径或后端选择，标题为「[AMD][MI30X] Restore DeepSeek MLA MI300X paths after MLA refactor (#19122)」，变更集中在 `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla.py`, `python/sglang/srt/models/deepseek_v2.py`。PR 描述补充为：## Motivation Re-apply #18242 and #18624 behavior in forward_mla.py (logic moved out of deepseek_v2.py by #19122 but not fully carried over): ## Modifications - Import fused_qk_...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「Support allreduce fusion with cp」，变更集中在 `python/sglang/srt/layers/flashinfer_comm_fusion.py`, `python/sglang/srt/model_executor/model_runner.py`, `python/sglang/srt/layers/communicator.py`。PR 描述补充为：## Motivation Enable the allreduce_fusion with context parallel. This requires the allreduce_fusion can work with any sub comm group. ## Modifications This PR can only work afte...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 调整模型相关实现，标题为「[Refactor] Deduplicate NSA utils.py into cp_utils.py for context parallel」，变更集中在 `python/sglang/srt/layers/attention/nsa/utils.py`, `python/sglang/srt/layers/utils/cp_utils.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`。PR 描述补充为：## Summary - Removed ~270 lines of duplicated context-parallel utility functions from `layers/attention/nsa/utils.py`, consolidating them into `layers/utils/cp_utils.py` - Unifi...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「【NPU】【bugfix】accuracy fix when enable both nsa cp and prefixcache」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/hardware_backend/npu/modules/deepseek_v2_attention_mla_npu.py`。PR 描述补充为：## Motivation when enable both nsa cp and prefixcache, inference accuracy is abnormal. ## Modifications python/sglang/srt/hardware_backend/npu/modules/deepseek_v2_attention_mla_...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「Support moe_dp_size = 1 for various attention_cp_size」，变更集中在 `python/sglang/srt/layers/communicator.py`, `python/sglang/srt/layers/dp_attention.py`, `python/sglang/srt/models/qwen3_moe.py`。PR 描述补充为：## Motivation Previously, we can only support attention_cp_size == moe_dp_size which is too restricted. In the real world case, we should let the MoE part unchanged and only app...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[AMD] Enable MTP for GLM-5-mxfp4 model」，变更集中在 `python/sglang/srt/models/deepseek_nextn.py`。PR 描述补充为：## Motivation Fix https://github.com/sgl-project/sglang/issues/23142. Quark-quantized GLM-5-MXFP4 checkpoints store MTP (NextN) weights — including `eh_proj` — in FP4-packed for...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「Support piecewise CUDA graph with NSA」，变更集中在 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/layernorm.py`, `python/sglang/srt/layers/radix_attention.py`。PR 描述补充为：## Motivation GLM-5/DSV3.2 currently doesn't allow piecewise CUDA graph due to incompatibilities in NSA attention backend and NSA indexer. This commit fixes the incompatibilitie...
...
- 动机: 该 PR 围绕 DeepSeek V3.2 补齐模型支持入口或运行时能力，标题为「[HiCache & HybridModel] 3FS backend support DSA & mamba model」，变更集中在 `python/sglang/srt/mem_cache/storage/hf3fs/storage_hf3fs.py`, `python/sglang/srt/mem_cache/storage/hf3fs/mini_3fs_metadata_server.py`, `python/sglang/srt/mem_cache/hi_mamba_radix_cache.py`。PR 描述补充为：## Motivation Added support for the 3FS backend. Supports both Mamba and DSA models. Roadmap：#21846 ## Modifications ## Accuracy Tests 3FS Config **mamba** **DSA （DeepSeek-V3.2-...
