...
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「Mixtral 8x7B support」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：Adding support for `mistralai/Mixtral-8x7B-v0.1` and `mistralai/Mixtral-8x7B-Instruct-v0.1` models as described in our blogpost. This is joint work between @zhuohan123, @WoosukK...
...
- 状态/时间: merged / 2023-12-11
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `4ff0203987ff`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+5/-6，可读 patch 46 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「Minor fixes for Mixtral」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：This PR 1) adds Mixtral to supported model list doc, and 2) did minor code cleaning for `mixtral.py`.
...
- 状态/时间: merged / 2023-12-11
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `81ce2a4b26c7`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-1，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 修复已暴露的启动、加载、解析或数值问题，标题为「[Minor] Fix type annotation in Mixtral」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：The following error is raised for users who didn't install `megablocks`. This PR removes this misleading error so that users can see the warning messages on the import statement...
...
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补强部署文档、测试或 CI 验证面，标题为「Mixtral expert parallelism」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：This PR implements a more efficient parallelism scheme for the Mixtral model. Instead of sharding the layers of each expert by rank, we instead shard whole experts across ranks....
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `ba4f82673884`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+5/-26，可读 patch 59 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[BugFix] Fix weight loading for Mixtral with TP」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：Fixes #2202 Currently, the Mixtral model does not support quantization with TP > 1 because `DummyModule` does not use quantized linear methods. This PR removes `DummyModule` and...
...
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「Fused MOE for Mixtral」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：This builds on https://github.com/vllm-project/vllm/pull/2453 and https://github.com/vllm-project/vllm/pull/2293 to fuse the MOE kernel for the Mixtral model. It seems to give a...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `d0d93b92b190`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 5 个文件，+119/-55，可读 patch 209 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「Add unit test for Mixtral MoE layer」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：This is PR adds a unit test for the Mixtral MoE layer to vLLM. It is based on @casper-hansen 's test in https://github.com/casper-hansen/AutoAWQ/blob/mixtral_fused/tests/test_fu...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `f0d4e145575b`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 9 个文件，+591/-50，可读 patch 772 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「Add fused top-K softmax kernel for MoE」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：This PR ports a fused topk-softmax kernel from TensorRT-LLM v0.7.1. TODO: 1. Port more MoE-related kernels 2. Use CUTLASS-based grouped GEMM kernels with appropriate tuning (if...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `tests/lora/test_mixtral.py`, `vllm/model_executor/models/mixtral.py`；关联提交 `2a543d6efecc`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 10 个文件，+251/-121，可读 patch 751 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「Add LoRA support for Mixtral」，变更集中在 `vllm/model_executor/models/mixtral.py`, `tests/lora/test_mixtral.py`。PR 描述补充为：**Problem**: We don't have LoRA support for Mixtral. **Solution**: Add LoRA configurations for Mixtral and refactor relevant parts to allow this. **Testing**: added correctness...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `31348dff03d6`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+10/-4，可读 patch 34 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 修复已暴露的启动、加载、解析或数值问题，标题为「Align LoRA code between Mistral and Mixtral (fixes #2875)」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：This aligns the LoRA code between Mistral and Mixtral and fixes https://github.com/vllm-project/vllm/pull/2875 as a result.
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `31348dff03d6`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-1，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 修复已暴露的启动、加载、解析或数值问题，标题为「Fix AttributeError: MixtralModel object has no attribute org_vocab_size.」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：Mixtral cannot start because of this error: Related PR: #2831
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `41deac4a3d78`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+15/-15，可读 patch 93 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 修复已暴露的启动、加载、解析或数值问题，标题为「[BugFix] 1D query fix for MoE models」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：MoE models were broken by #3236.
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `eace8bf0b911`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 10 个文件，+385/-21，可读 patch 626 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[Kernel] FP8 support for MoE kernel / Mixtral」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：This PR is the first step towards fixing https://github.com/vllm-project/vllm/pull/3208 It implements dynamic per-tensor scaling (see https://github.com/vllm-project/vllm/pull/4...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `12628d3c787e`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 7 个文件，+95/-18，可读 patch 233 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[Kernel] Optimize FP8 support for MoE kernel / Mixtral via static scales」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：This PR contains the performance improvements mentioned in https://github.com/vllm-project/vllm/pull/4244 The performance numbers are as follows (TP2, H100) -- with mistralai/Mi...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `c9d852d601ce`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+4/-8，可读 patch 41 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[Misc] Remove Mixtral device="cuda" declarations」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：Remove the `device="cuda"` declarations in mixtral as promised in https://github.com/vllm-project/vllm/pull/4343 --- section, markdown rendering does not work, so we use raw htm...
...
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[Kernel] Support MoE Fp8 Checkpoints for Mixtral (Static Weights with Dynamic/Static Activations)」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：Follow on to https://github.com/vllm-project/vllm/pull/4332 to enable FP8 checkpoint loading for Mixtral and supersedes https://github.com/vllm-project/vllm/pull/4436. This PR e...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `33d3914b1e6d`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-1，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[Bugfix] Fix dynamic FP8 quantization for Mixtral」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：If the checkpoint is not an FP8 checkpoint, we first load the weights as FP16 and then quantize them after they are loaded. A small bug was introduced in https://github.com/vllm...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `a3a73ab0696b`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 40 个文件，+284/-158，可读 patch 869 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[Misc] Load FP8 kv-cache scaling factors from checkpoints」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：The 2nd PR for #4532. This PR supports loading FP8 kv-cache scaling factors from a FP8 checkpoint (with `.kv_scale` parameter). Specifically, 1. We now support `--kv-cache-dtype...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `e9899fb7a4d9`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 8 个文件，+711/-114，可读 patch 1000 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[Model] Enable FP8 QKV in MoE and refine kernel tuning script」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：In this PR, we enable QKV in FP8 for Mixtral models. Gate linear is still in FP16. Since most MoE models now have 8 experts, the shape of gate weights is not divisible by 16, wh...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `5563a4dea86e`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+80/-35，可读 patch 182 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 修复已暴露的启动、加载、解析或数值问题，标题为「[Model] Correct Mixtral FP8 checkpoint loading」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：This PR fixes the issue the the weight scaling factors of w1 and w3 are not properly loaded. Since we merge w1 and w3 in vLLM for better performance, their weights and scaling f...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `c09dade2a263`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+23/-23，可读 patch 127 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[Misc][Breaking] Change FP8 checkpoint format from act_scale -> input_scale」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：In tandem with https://github.com/neuralmagic/AutoFP8/pull/11. BREAKING CHANGE: Because there can be input and output scales (`kv_scale` is an example of an "output" scale for k...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `7c008c51a9aa`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 10 个文件，+537/-306，可读 patch 1033 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[ Misc ] Refactor MoE to isolate Fp8 From Mixtral」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：MOTIVATION: * Prior to this PR, MoE layer + weight loading logic is repeated for all models * Prior to this PR, fp8 logic for MoE is coupled with Mixtral and unsupported for oth...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `e72ae80b0640`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+7/-4，可读 patch 36 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[Bugfix] Support 2D input shape in MoE layer」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：The MoE layer current assumes 1D input shape, i.e., `[num_tokens, hidden_size]`. This PR enhances this to support both 1D and 2D (`[batch_size, seq_len, hidden_size]`) input sha...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `fb6af8bc0863`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 9 个文件，+223/-137，可读 patch 564 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[ Misc ] Apply MoE Refactor to Qwen2 + Deepseekv2 To Support Fp8」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：SUMMARY: * apply MoE refactor to DeepSeekv2 to support `fp8` * * apply MoE refactor to Qwen2Moe to support `fp8` * in combination with #6415, this will support fused AWQ as well...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `b5af8c223c3d`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+60/-19，可读 patch 207 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[Model] Pipeline parallel support for Mixtral」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：Take from #6403. Co-authored by @binxuan cc @youkaichao
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `d3bdfd3ab9ba`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 6 个文件，+264/-201，可读 patch 614 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[Misc] Update Fused MoE weight loading」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：# Summary: - Splits up #6422 into two separate PRs. This is the first of the two. The second will leverage the weight loading changes introduced in this PR while adding the AWQ...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `8678a69ab519`, `aae74ef95c37`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 15 个文件，+2374/-84，可读 patch 2645 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[Kernel] Expand MoE weight loading + Add Fused Marlin MoE Kernel」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：# Summary - Expands weight loading to support grouped and per channel weight quantization. Cleans-up fp8 MoE to use the updated weight loading - Adds Marlin Fused MoE Kernel for...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `aae74ef95c37`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 15 个文件，+84/-2374，可读 patch 2645 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「Revert "[Kernel] Expand MoE weight loading + Add Fused Marlin MoE Kernel (#7527)"」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：This reverts commit 8678a69ab51956031e3bb70bdf1a781a8652e67d from PR https://github.com/vllm-project/vllm/pull/7527 since it causes a failure in the AMD build
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `fc911880cc50`
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `6cd5e5b07e44`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 19 个文件，+912/-204，可读 patch 1442 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[Misc] Fused MoE Marlin support for GPTQ」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：# Summary - Add GPTQ Marlin MoE Support; marlin MoE kernels currently support int4 - Update/add optional testing for large MoE models for GPTQ and llm-compressor Co-authored by...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `tests/lora/test_mixtral.py`, `vllm/model_executor/models/mixtral.py`；关联提交 `9ade8bbc8dc6`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+69/-20，可读 patch 123 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[Model] add a bunch of supported lora modules for mixtral」，变更集中在 `vllm/model_executor/models/mixtral.py`, `tests/lora/test_mixtral.py`。PR 描述补充为：`all-linear` option for mixtral produces these `target modules`: This PR adds the missing modules (`w1`, `w2`, `w3` and `gate`) to the supported_lora_modules in `mixtral`. Updat...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `aa0addb39726`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+8/-0，可读 patch 64 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补强部署文档、测试或 CI 验证面，标题为「[torch.compile] Adding "torch compile" annotations to some models」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：This PR is to complete #9589 and #9632, adding "torch compile" annotations to some moe models and testing whether they can pass the compilation.
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `tests/lora/test_mixtral.py`；关联提交 `f1d1bf6288ab`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+5/-2，可读 patch 30 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[Bugfix] Fix fully sharded LoRAs with Mixtral」，变更集中在 `tests/lora/test_mixtral.py`。PR 描述补充为：Fixes a regression introduced by #9008 , which leads to an assertion error when `--fully-sharded-loras` is enabled with an adaptor that includes a gate target: This occurs becau...
...
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 调整模型相关实现，标题为「[Misc][Quark] Upstream Quark format to VLLM」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark.py`, `vllm/model_executor/layers/quantization/quark/quark_moe.py`, `vllm/model_executor/layers/quantization/quark/schemes/quark_w8a8_fp8.py`。PR 描述补充为：This PR is related to the issue discussed in this https://github.com/vllm-project/vllm/issues/10294.
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `eb5cb5e5280c`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 8 个文件，+243/-148，可读 patch 756 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 修复已暴露的启动、加载、解析或数值问题，标题为「[BugFix] Fix parameter names and `process_after_weight_loading` for W4A16 MoE Group Act Order」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：Summary - Fix parameter names to ensure proper weight loading - Fix `process_after_weight_loading` if running group act order - Don't shard w2 weight_scales when running actorde...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `1e15aaef562d`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 6 个文件，+22/-22，可读 patch 138 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix][Quantization] Fix FP8 + EP」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：`Fp8MoEMethod` is reaching into the `layer` to get the number of experts during `process_weights_after_loading`, which isn't right when using expert parallelism. Would hit the f...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `72c62eae5f01`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 17 个文件，+250/-75，可读 patch 778 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[V1] EP/TP MoE + DP Attention」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：Based on https://github.com/vllm-project/vllm/pull/13591 DP+EP implemented via collective ops in the fused_moe layer's forward pass.
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/schemes/quark_w8a8_fp8.py`；关联提交 `e1744502c21f`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 11 个文件，+268/-242，可读 patch 760 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[FP8] Refactor apply_fp8_linear and apply_fp8_linear_generic into an object」，变更集中在 `vllm/model_executor/layers/quantization/quark/schemes/quark_w8a8_fp8.py`。PR 描述补充为：This PR replaces `apply_fp8_linear` and `apply_fp8_linear_generic` with objects so that VllmConfig can be accessed in their `__init__` method as opposed to the `forward` method.
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`, `vllm/model_executor/layers/quantization/quark/schemes/quark_w8a8_fp8.py`；关联提交 `a1c8f3796c89`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 25 个文件，+293/-159，可读 patch 944 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「dynamic distpatch of fp8 kernels」，变更集中在 `vllm/model_executor/layers/quantization/quark/schemes/quark_w8a8_fp8.py`, `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：This mostly affects ROCm with hardware that can support one or the other FP8 type. All fp8 kernels are now templated on fp8_type instead of assuming a single fp8_type via `using...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `aaaec52ad907`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+10/-2，可读 patch 54 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix][Model] Mixtral: use unused head_dim config argument」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：`MixtralConfig` has a a `head_dim` argument that will never be used as `MixtralAttention` always uses `hidden_size // self.total_num_heads` as its value. This change brings `Mix...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/schemes/quark_w8a8_fp8.py`；关联提交 `4d0ec37267af`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 5 个文件，+41/-9，可读 patch 155 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[Quantization][FP8] Adding support for fp8 gemm layer input in fp8」，变更集中在 `vllm/model_executor/layers/quantization/quark/schemes/quark_w8a8_fp8.py`。PR 描述补充为：Adding support for the case where both inputs to the FP8 GEMM are in FP8 datatype and not only weights (in preparation for attention with fused FP8 conversion) Functionality por...
...
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「Add support to modelopt quantization of Mixtral model」，变更集中在 `vllm/model_executor/models/mixtral_quant.py`。PR 描述补充为：Add vllm support to modelopt fp8 quantization of Mixtral8x7b model
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `a564797151a0`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 5 个文件，+353/-323，可读 patch 836 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 修复已暴露的启动、加载、解析或数值问题，标题为「[Model] use AutoWeightsLoader for granite, granitemoe, granitemoeshared, grok1, mixtral」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：fix (partially) #15697 * GraniteForCausalLM * GraniteMoeForCausalLM * GraniteMoeShardedForCausalLM * Grok1ForCausalLM * MixtralForCausalLM
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `tests/quantization/test_quark.py`, `vllm/model_executor/layers/quantization/quark/schemes/quark_w8a8_fp8.py`, `vllm/model_executor/layers/quantization/quark/schemes/quark_w8a8_int8.py`；关联提交 `9e90c9f73f94`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+67/-22，可读 patch 183 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] Fix bugs of running Quark quantized models」，变更集中在 `vllm/model_executor/layers/quantization/quark/schemes/quark_w8a8_int8.py`, `vllm/model_executor/layers/quantization/quark/schemes/quark_w8a8_fp8.py`, `tests/quantization/test_quark.py`。PR 描述补充为：1. Fix fp8 activation dynamic and weight per-channel 2. Fix int8 weight per-channel and weight_zero_point/input_zero_point loading issues
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark.py`；关联提交 `a41351f363f3`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 8 个文件，+105/-20，可读 patch 212 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[Quantization][FP8] Add support for FP8 models with input_scale for output projection and QK quantization」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark.py`。PR 描述补充为：This PR adds fp8 quantization support for: 1) Preserving FP8 quantization after FA output, so that output of FA into the next layer will be FP8 - this uses the model parameter s...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark.py`；关联提交 `68af5f6c5ce4`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+0/-29，可读 patch 43 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 修复已暴露的启动、加载、解析或数值问题，标题为「[AMD][FP8][BugFix] Remove V1 check in arg_utils.py for FP8 since it is not necessary」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark.py`。PR 描述补充为：This PR removes a check in arg_utils.py for FP8 that is not necessary. It turns out that a different backend is used for FP8 when running FP8 on V1 so it will not hit the Triton...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark.py`, `vllm/model_executor/layers/quantization/quark/schemes/__init__.py`；关联提交 `db593aa67f8d`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 9 个文件，+289/-3，可读 patch 378 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[Quantization] Quark MXFP4 format loading」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark.py`, `vllm/model_executor/layers/quantization/quark/schemes/__init__.py`。PR 描述补充为：Initial PR to integrate loading MXFP4 models quantized by Quark. This PR supports running MXFP4 emulation for devices where micro-scaling datatype is not natively supported. ###...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `tests/quantization/test_quark.py`, `vllm/model_executor/layers/quantization/quark/schemes/quark_w8a8_fp8.py`；关联提交 `bb239a730f2a`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+38/-9，可读 patch 71 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[Bugfix] Fix quark fp8 format loading on AMD GPUs」，变更集中在 `vllm/model_executor/layers/quantization/quark/schemes/quark_w8a8_fp8.py`, `tests/quantization/test_quark.py`。PR 描述补充为：https://github.com/vllm-project/vllm/pull/10765 was merged into vllm to support Quark quantized models. Unfortunately, tests are somewhat limited and the issue fixed in this PR...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark.py`, `vllm/model_executor/layers/quantization/quark/schemes/quark_w8a8_fp8.py`；关联提交 `1c50e100a9c5`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+23/-16，可读 patch 99 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] fix quark ptpc」，变更集中在 `vllm/model_executor/layers/quantization/quark/schemes/quark_w8a8_fp8.py`, `vllm/model_executor/layers/quantization/quark/quark.py`。PR 描述补充为：This is a fix for importing the **quark** ptpc quantization format.
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `docs/features/quantization/quark.md`, `tests/quantization/test_quark.py`, `vllm/model_executor/layers/quantization/quark/quark.py`, `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `332d4cb17b7c`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 15 个文件，+875/-106，可读 patch 1416 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[Feature][Quantization] MXFP4 support for MOE models」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`, `vllm/model_executor/layers/quantization/quark/quark.py`, `tests/quantization/test_quark.py`。PR 描述补充为：This PR follows https://github.com/vllm-project/vllm/pull/16943, and adds the possibility to load MOE models using MXFP4 weights with dynamic per-group MXFP4 quantization for ac...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/schemes/quark_w8a8_fp8.py`；关联提交 `31d5c1797f32`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 18 个文件，+368/-104，可读 patch 858 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 修复已暴露的启动、加载、解析或数值问题，标题为「[Perf][fp8] Use CustomOp abstraction for fp8 quant for better perf」，变更集中在 `vllm/model_executor/layers/quantization/quark/schemes/quark_w8a8_fp8.py`。PR 描述补充为：## Essential Elements of an Effective PR Description Checklist - [x] The purpose of the PR, such as "Fix some issue (link existing issues this PR will resolve)". - [x] The test...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `a99b9f7dee0a`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 7 个文件，+128/-20，可读 patch 251 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[Quantization] add BNB for MixtralForCausalLM」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：## Essential Elements of an Effective PR Description Checklist - [ ] The purpose of the PR, such as "Fix some issue (link existing issues this PR will resolve)". - [ ] The test...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `8ad7285ea28a`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 54 个文件，+2022/-1305，可读 patch 5535 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[Kernels] Clean up FusedMoeMethodBase and modular kernel setup. Remove extra arguments from modular kernel methods.」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：## Purpose - Pass `FusedMoEConfig` to all `FusedMoEMethodBase` object constructors - Make sure `self.fused_experts` is set to None in the constructor and only set once by `init_...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `8fb85b7bb674`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 19 个文件，+77/-4，可读 patch 570 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「Add routed_scaling_factor to MoE grouped topk」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：## Purpose This PR add `routed_scaling_factor` to MoE grouped topk. * In `vllm/model_executor/layers/fused_moe/layer.py`, add `routed_scaling_factor` parameter to `grouped_topk(...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `tests/quantization/test_quark.py`, `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `ca2d1925ef5a`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+196/-52，可读 patch 325 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[Rocm] [quantization] Fix quark ptpc moe and add test case」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`, `tests/quantization/test_quark.py`。PR 描述补充为：1.Add support for loading quark's ptpc-format moe models 2.add test case for quark ptpc
...
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[EPLB] Support EPLB for Mixtral Model」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：# Essential Elements of an Effective PR Description Checklist - [x] The purpose of the PR, such as "Fix some issue (link existing issues this PR will resolve)". - [x] The test p...
...
- 状态/时间: merged / 2025-09-17
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `5963b98b4650`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 68 个文件，+2675/-2503，可读 patch 9071 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[Kernel] Delegate construction of FusedMoEQuantConfig to FusedMoEMethodBase subclasses」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：## Purpose - Delegate construction of `FusedMoEQuantConfig` objects to the subclass of `FusedMoEMethodBase` that will use that info. - Move all/most quantization info into `Fuse...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `tests/quantization/test_quark.py`；关联提交 `a38c1bfe09f4`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+2/-2，可读 patch 18 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[ci] Rename `test_mxfp4_moe.py` to `test_ocp_mx_moe.py`」，变更集中在 `tests/quantization/test_quark.py`。PR 描述补充为：As per title, as reported in https://github.com/vllm-project/vllm/pull/21166#issuecomment-3377626487 `test_mxfp4_moe.py` was renamed, but not in `.buildkite/test-pipeline.yaml`....
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `8ae169286f93`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 10 个文件，+22/-52，可读 patch 228 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[torch.compile] Unwrap fused_marlin_moe custom op」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：## Purpose Unwrap unnecessary CustomOP'ing of `fused_marlin_moe`. `fused_marlin_moe` was first wrapped into a custom op in the PR https://github.com/vllm-project/vllm/pull/9632...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `0925b28a8e92`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+73/-24，可读 patch 144 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[ROCM] MoE fp4 CK kernel」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：server: client or returns reasonable results. Correctness:
...
- 状态/时间: merged / 2025-10-17
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `e33ee23ee3cd`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+14/-3，可读 patch 38 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[Bugfix] [AITER] [ROCm] Fix Quark MoE Quant Config and AITER Fused MoE quant type logic」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：## Purpose Bug fix the incorrect initialization of Quant Config in Quark MoE and the missing PTPC quant type logic in Fused MoE and the w8a8_utils.py incorrect condition checks....
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `938772af03ce`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 16 个文件，+271/-311，可读 patch 1049 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[Kernels] Isolate modular kernel code from FusedMoEMethodBase subclasses.」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：## Purpose Make a new `FusedMoEModularMethod` subclass of `FusedMoeMethodBase` for use with modular kernels. Instead of having every subclass of `FusedMoEMethodBase` check `self...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `tests/lora/test_mixtral.py`；关联提交 `6b2b9fd934af`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+18/-11，可读 patch 34 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[CI] lora/test_mixtral.py : Add additional expected outputs due to flakiness」，变更集中在 `tests/lora/test_mixtral.py`。PR 描述补充为：## Purpose Distributed A100 tests fail the nightly due to `lora/test_mixtral.py` link This PR attempts to fix that test. Running the test locally, I found it to be flaky. Howeve...
...
- 状态/时间: merged / 2025-11-10
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `b06b9470ca88`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+12/-0，可读 patch 26 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[Rocm][fused_moe][fp4] view weight to torch.float4_e2m1fn_x2 when running aiter fused moe for fp4 model」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：doing view for moe weight to torch.float4_e2m1fn_x2 for aiter FP4 fused moe kernel with this PR, aiter cannot find the suitable kernel for the weight, whose type is uint8 with t...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `docs/features/quantization/quark.md`, `vllm/model_executor/layers/quantization/quark/quark.py`；关联提交 `05576df85c52`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+127/-8，可读 patch 162 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[ROCm][Quantization] extend AMD Quark to support mixed-precision quantized model」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark.py`, `docs/features/quantization/quark.md`。PR 描述补充为：## Purpose This PR aims to support _**layerwise**_ mixed-precision quantization model inference, extending from quantized models in single scheme such as MXFP4, FP8 (aka PTQ mod...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark.py`；关联提交 `0b25498990f0`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+9/-3，可读 patch 48 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[Misc] add ignore mapper for quark quantization」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark.py`。PR 描述补充为：Qwen3vlmoe has the following mapping, and quark format requires a function to complete this mapping. orig_to_new_prefix={ "model.visual.": "visual.", "lm_head.": "language_model...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark.py`；关联提交 `d0a73620cc85`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+30/-5，可读 patch 70 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[ROCm][Quantization] add apply_vllm_mapper in quark config for models like gpt-oss」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark.py`。PR 描述补充为：## Purpose Need to call https://github.com/vllm-project/vllm/blob/c9fe6abe7c0b03d552420edd63c6c678ed683dea/vllm/model_executor/models/gpt_oss.py#L656 to build a map, such as `se...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `8f066146c395`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 18 个文件，+163/-472，可读 patch 1200 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[MoE][Refactor] Make select_experts a non-static method」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：## Purpose There didn't appear to be a good reason why `select_experts` was a static method. Making it non-static allows access to the attributes of the `FusedMoE` layer so that...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `c6df05ebb499`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+5/-0，可读 patch 19 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 修复已暴露的启动、加载、解析或数值问题，标题为「[ROCm] [Fused Moe EP] Use binary expert mask for aiter fused moe kernel」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：## Purpose Quark Fused Moe meets acc issue in DeepSeek-R1 Expert Parallelism. This PR fixes it through setting the right expert mask with format required by aiter kernel. The ai...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `00e5cbb96789`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 18 个文件，+318/-872，可读 patch 2081 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[MoE][Refactor] Remove most arguments to FusedMoEMethodBase.apply」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：## Purpose After making `select_experts` a non-static method (https://github.com/vllm-project/vllm/pull/29067), we can avoid passing most of the arguments to `FusedMoEMethodBase...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark.py`, `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `0c738b58bc0e`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+201/-2，可读 patch 224 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[Quantization] Support Quark int4-fp8 w4a8 for MoE」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`, `vllm/model_executor/layers/quantization/quark/quark.py`。PR 描述补充为：This PR extends support of Quark quantized model for int4-fp8 w4a8 quantization spec. Specifically: - Weights are double quantized: first at INT4 per channel, then further at FP...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `54c892438479`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 19 个文件，+263/-108，可读 patch 709 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 修复已暴露的启动、加载、解析或数值问题，标题为「[MoE Refactor][5/N] Isolate zero expert to LongCatFlash」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：## Purpose #28152 #28745 Fix dimension mismatch error in LongCat Flash MoE model when using zero experts. The issue occurred when `router_logits` (including zero experts) and `e...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `9152a30d8f6f`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+92/-76，可读 patch 270 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[MoE Refactor][12/N] Marlin Fp8 MoE Pure Function」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：## Purpose SUMMARY: * make `prepare_moe_fp8_layer_for_marlin` into a pure function for consistency with other methods and compatibility with the `replace_parameter` semantics th...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `tests/evals/gsm8k/configs/moe-refactor/Mixtral-8x7B-Fp8-AutoFp8-fi-cutlass.yaml`, `tests/evals/gsm8k/configs/moe-refactor/Mixtral-8x7B-Fp8-AutoFp8-triton.yaml`；关联提交 `d3e477c01342`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 30 个文件，+247/-0，可读 patch 280 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[MoE Refactor] Add Temporary Integration Tests - H100/B200」，变更集中在 `tests/evals/gsm8k/configs/moe-refactor/Mixtral-8x7B-Fp8-AutoFp8-fi-cutlass.yaml`, `tests/evals/gsm8k/configs/moe-refactor/Mixtral-8x7B-Fp8-AutoFp8-triton.yaml`。PR 描述补充为：## Purpose * add ci job to validate MoE refactor * this is very compute intensive and duplicative, so this is a temporary job that I will run on my PRs --- Essential Elements of...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `5dcd7ef1f219`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 38 个文件，+1446/-1535，可读 patch 4055 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[MoE Refactor][15/N] Apply Refactor to Fp8」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：## Purpose SUMMARY: * continue refactoring fp8 moe * CUTLASS MoE --> make CutlassExpertsFp8 manage the strides data * CUTLASS MoE --> remove cutlass_moe_fp8 entrypoint * factor...
...
- 状态/时间: merged / 2026-01-08
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `e74698c27ad5`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 20 个文件，+165/-36，可读 patch 744 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[Misc][Refactor] Add FusedMoERouter object」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：## Purpose Add abstract `FusedMoERouter` class that provides `select_experts`. There's a concrete subclass that wraps `FusedMoE._select_experts` that is passed to all the quanti...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `tests/evals/gsm8k/configs/moe-refactor/Mixtral-8x7B-BF16-fi-cutlass.yaml`, `tests/evals/gsm8k/configs/moe-refactor/Mixtral-8x7B-BF16-triton.yaml`；关联提交 `31c29257c852`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 12 个文件，+257/-87，可读 patch 445 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[MoE Refactor][17/N] Apply Refactor to Bf16」，变更集中在 `tests/evals/gsm8k/configs/moe-refactor/Mixtral-8x7B-BF16-fi-cutlass.yaml`, `tests/evals/gsm8k/configs/moe-refactor/Mixtral-8x7B-BF16-triton.yaml`。PR 描述补充为：## Purpose ## Test Plan gsm8k result for Triton kernel, flashinfer_cutlass kernel and aiter rocm kernel for `Qwen/Qwen3-30B-A3B` in TP(triton), TEP (flashinfer cutlass) and rocm...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `327a02d8db86`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 45 个文件，+1754/-688，可读 patch 2998 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[MoE Refactor] Separate Router into OO Classes」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：## Purpose This PR is part of the effort to separate the expert selection code from `FusedMoEMethod`. Move all the MoE router implementations into separate subclasses of `FusedM...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/schemes/quark_w8a8_fp8.py`, `vllm/model_executor/layers/quantization/quark/schemes/quark_w8a8_int8.py`；关联提交 `148117ea2e68`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 30 个文件，+1458/-1029，可读 patch 3752 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[Refactor] Make FP8 Linear Ops use kernel abstraction」，变更集中在 `vllm/model_executor/layers/quantization/quark/schemes/quark_w8a8_fp8.py`, `vllm/model_executor/layers/quantization/quark/schemes/quark_w8a8_int8.py`。PR 描述补充为：## Purpose This PR refactors the FP8 linear kernel integration in vLLM to improve code clarity, maintainability, and path consistency. Changes: 1- A more generic ScaledMMLinearK...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `dc917cceb877`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 22 个文件，+495/-530，可读 patch 1822 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[MoE Refactor] Move `select_experts` from `FusedMoEQuantMethod` -> `FusedMoE`」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：## Purpose Move (almost all) the `select_experts` calls to the `FusedMoE` layer instead of each `FusedMoEMethodBase.apply` method. Change the `apply` signature to take `topk_wei...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `a57c8228ffb3`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 37 个文件，+132/-109，可读 patch 869 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[Moe Refactor] Make Inplace Flag for FusedMoEModularKernel part of the constructor」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：## Purpose Pass the inplace flag to the FusedMoEModularKernel constructor. Knowing the inplace (or being able to disable) behavior ahead of time will help simplify some of the r...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark.py`, `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `b129136c7a73`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 13 个文件，+1094/-213，可读 patch 1860 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[ROCm][Quantization] GPT_OSS in amd-quark format model loading and emulations」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`, `vllm/model_executor/layers/quantization/quark/quark.py`。PR 描述补充为：## Purpose This PR aims for: - [x] quark model loading, combined with `mxfp4` loading function for original openai/gpt-oss-20b & openai/gpt-oss-120b - [x] OCPMX_W4A16, OCPMX_W4A...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `d1481ba78323`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 25 个文件，+913/-753，可读 patch 2246 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[MoE Refactor] Introduce MoERunner abstraction and move execution logic from FusedMoE to DefaultMoERunner」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：## Purpose The execution logic in `FusedMoE` is rather convoluted due to the number of inter-dependent features, e.g. shared experts, gating, modular vs. non-modular kernels, lo...
...
- 状态/时间: merged / 2026-02-11
- 反查来源: `git log --name-only -- <model-files>` 反查到 `tests/evals/gsm8k/configs/moe-refactor/Mixtral-8x7B-BF16-fi-cutlass.yaml`；关联提交 `275e0d2a993b`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 7 个文件，+296/-1，可读 patch 337 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[NVIDIA][test] Tests for flashinfer TRTLLM BF16 MoE」，变更集中在 `tests/evals/gsm8k/configs/moe-refactor/Mixtral-8x7B-BF16-fi-cutlass.yaml`。PR 描述补充为：## Purpose Adding tests for trtllm bf16 moe backend added in PR [[NVIDIA] [feat] Integrate flashinfer Trtllmgen bf16 moe #32954](https://github.com/vllm-project/vllm/pull/32954)...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `ff1f83b056ae`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 48 个文件，+474/-282，可读 patch 2289 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 调整模型相关实现，标题为「[Refactor] Replace `activation: str` with `MoEActivation` enum」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：## Purpose We have had `activation` defined, validated, and passed around as a raw string forever. Now that we have popular models that don't just use the `silu` default and als...
...
- 状态/时间: merged / 2026-02-12
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `8a798be929d6`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+11/-3，可读 patch 41 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[ROCm] Enable MXFP4 MoE weight pre-shuffling on gfx950 and update aiter」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：This commit introduces hardware-specific optimization for MXFP4 quantized Mixture-of-Experts (MoE) layers on AMD gfx950 GPUs and updates the aiter dependency to support this fun...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `d9e62c03eb98`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+3/-3，可读 patch 26 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 修复已暴露的启动、加载、解析或数值问题，标题为「[Quark] Fix MoE fp8 activation scale handling on mi300」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：Small fix follow-up after #29008 for running gpt-oss on mi300. Also ensure the fp8 scale conversion only runs when activation is fp8 quantized.
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `tests/evals/gsm8k/configs/moe-refactor/Mixtral-8x7B-BF16-fi-cutlass.yaml`, `tests/evals/gsm8k/configs/moe-refactor/Mixtral-8x7B-Fp8-AutoFp8-fi-cutlass.yaml`；关联提交 `de527e1cec82`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 37 个文件，+260/-140，可读 patch 720 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[UX] Add `--moe-backend` arg for explicit kernel selection」，变更集中在 `tests/evals/gsm8k/configs/moe-refactor/Mixtral-8x7B-BF16-fi-cutlass.yaml`, `tests/evals/gsm8k/configs/moe-refactor/Mixtral-8x7B-Fp8-AutoFp8-fi-cutlass.yaml`。PR 描述补充为：## Purpose Adds `--moe-backend` argument for explicit MoE kernel selection, allowing users to override the automatic backend selection logic (e.g., `--moe-backend triton`, `--mo...
...
- 状态/时间: merged / 2026-02-26
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `01923eec7092`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+315/-37，可读 patch 494 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[ROCm][Quantization] GPT OSS Upstream MoE wmxfp4_afp8 with static scales」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：# gpt-oss120b-w-mxfp4-a-fp8 ### server: > HIP_VISIBLE_DEVICES=1 VLLM_DISABLE_COMPILE_CACHE=1 VLLM_ROCM_USE_AITER=1 VLLM_ROCM_USE_AITER_UNIFIED_ATTENTION=1 VLLM_ROCM_USE_AITER_MH...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `tests/quantization/test_quark.py`；关联提交 `f26650d649aa`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+24/-6，可读 patch 73 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[ROCm] add amd-quark package in requirements for rocm to use quantized models」，变更集中在 `tests/quantization/test_quark.py`。PR 描述补充为：Fix https://github.com/vllm-project/vllm/issues/35633 ## Purpose - Added amd-quark to requirements/rocm.txt. This way, it can be picked up for building docker, wheel or building...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `5dc3538736e4`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+63/-1，可读 patch 99 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[ROCm][Bugfix] Fall back from CK MXFP4 MoE when GEMM dimensions are unsupported」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：## Summary Fix a `RuntimeError` crash when using AITER fused MoE with MXFP4 quantization on certain model/TP configurations where CK's pre-compiled GEMM kernel instances don't s...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark.py`, `vllm/model_executor/layers/quantization/quark/schemes/__init__.py`, `vllm/model_executor/layers/quantization/quark/schemes/quark_w4a8_mxfp4_fp8.py`；关联提交 `6341d4304351`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+311/-1，可读 patch 362 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[ROCm][Quantization] add quark w4a8 mxfp4_fp8 for LinearLayer」，变更集中在 `vllm/model_executor/layers/quantization/quark/schemes/quark_w4a8_mxfp4_fp8.py`, `vllm/model_executor/layers/quantization/quark/quark.py`, `vllm/model_executor/layers/quantization/quark/schemes/__init__.py`。PR 描述补充为：weights: mxfp4 with static scales activations: fp8 1. Supports Eager + Torch.compile mode 2. Supports Emulation mode 3. Updated test results below. ~~Tested on this script for q...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark.py`；关联提交 `24b4272a8ca6`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+11/-4，可读 patch 22 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 修复已暴露的启动、加载、解析或数值问题，标题为「Fix infinite recursive search issue in quark.py」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark.py`。PR 描述补充为：Fix infinite recursive search issue in _find_matched_config() when test granite 4 small fp8 model on rocm/vllm-dev:nightly docker, the issue fix will be generic for other enviro...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`, `vllm/model_executor/layers/quantization/quark/schemes/quark_ocp_mx.py`；关联提交 `44eea10f6846`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+19/-5，可读 patch 58 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 修复已暴露的启动、加载、解析或数值问题，标题为「[ROCm][Quantization] make quark ocp mx dtype parser robust for weight-only quantization」，变更集中在 `vllm/model_executor/layers/quantization/quark/schemes/quark_ocp_mx.py`, `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：## Purpose This PR aims to improve the robustness of Quark's weight-only OCP MX quantization. Changes here are to fix errors due to None type of `input_quant_spec`. ## Test Plan...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `87bd91892f8c`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 18 个文件，+1695/-1369，可读 patch 3508 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[MoE Refactor] Mxfp4 oracle rebased」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：## Purpose Rebased and improve version of #34983 Ongoing MXFP4 MoE refactor - Refactor MXFP4 MoE from a monolithic 1299-line `Mxfp4MoEMethod` class to the oracle pattern used by...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`, `vllm/model_executor/layers/quantization/quark/schemes/quark_ocp_mx.py`；关联提交 `e99fb98867c2`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+16/-26，可读 patch 122 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 修复已暴露的启动、加载、解析或数值问题，标题为「[ROCm] Fix fused_moe_fake signature mismatch and other AITER bugs」，变更集中在 `vllm/model_executor/layers/quantization/quark/schemes/quark_ocp_mx.py`, `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：## Summary Fix a real logic bug and several related issues in ROCm AITER ops: ### Main bugfix: `_rocm_aiter_fused_moe_fake` signature mismatch The fake/meta implementation for t...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `7d6917bef552`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 12 个文件，+480/-88，可读 patch 928 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 修复已暴露的启动、加载、解析或数值问题，标题为「[ROCm] Fix MoE kernel test failures on gfx950」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：Fixes ROCm-specific MoE kernel test failures on MI355 (gfx950). - `gpt_oss_triton_kernels_moe.py`: Handle installed `triton_kernels.topk` returning tuple instead of `SparseMatri...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `0ae89f18fd75`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 12 个文件，+204/-222，可读 patch 868 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[Refactor] Move FusedMoE hidden_size roundup to quant_method」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：- Refactor hidden_size and intermediate roundup logic to be handled by QuantMethod. - Store padded and unpadded sizes under MoeConfig. - ~~Update ROCm padding logic to improve p...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `7cf56a59a267`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 34 个文件，+556/-397，可读 patch 1816 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 调整模型相关实现，标题为「[MoE Refactor] Make SharedExperts class for use with DefaultMoERunner」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：## Purpose Move shared experts functionality into `SharedExperts` class. The `DefaultMoERunner` and modular kernels defer to `SharedExperts` for executing shared experts (making...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `tests/evals/gpt_oss/configs/gpt-oss-20b-rocm-quark-mxfp4-bf16-aiter.yaml`, `tests/evals/gpt_oss/configs/gpt-oss-20b-rocm-quark-mxfp4-bf16-triton.yaml`, `tests/evals/gpt_oss/configs/gpt-oss-20b-rocm-quark-mxfp4-fp8-triton.yaml`；关联提交 `2df2c85be494`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 7 个文件，+84/-216，可读 patch 444 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 修复已暴露的启动、加载、解析或数值问题，标题为「[Kernels][MoE] Fix legacy_routing to use bitmatrix-based routing path」，变更集中在 `tests/evals/gpt_oss/configs/gpt-oss-20b-rocm-quark-mxfp4-bf16-aiter.yaml`, `tests/evals/gpt_oss/configs/gpt-oss-20b-rocm-quark-mxfp4-bf16-triton.yaml`, `tests/evals/gpt_oss/configs/gpt-oss-20b-rocm-quark-mxfp4-fp8-triton.yaml`。PR 描述补充为：Three issues prevented `test_gpt_oss_triton_kernels` and GPQA serving from working on gfx950 (CDNA4) after the legacy routing deprecation: 1. `pack_bitmatrix` sets bit 31 spurio...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/schemes/quark_w8a8_fp8.py`；关联提交 `2e9034c998e2`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 35 个文件，+1710/-904，可读 patch 3623 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[W8A8 Block Linear Refactor][2/N] Remove W8A8Fp8BlockLinearOp and adopt Fp8 block linear kernel selections.」，变更集中在 `vllm/model_executor/layers/quantization/quark/schemes/quark_w8a8_fp8.py`。PR 描述补充为：## Purpose This PR refactors block scaled linear kernel into kernel abstraction. changes: - Introduces `MMLinearKernel` base interface for all linear kernels. - Introduces `Para...
...
- 状态/时间: merged / 2026-04-09
- 反查来源: `git log --name-only -- <model-files>` 反查到 `tests/quantization/test_quark.py`, `vllm/model_executor/layers/quantization/quark/quark.py`, `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `827268e98d92`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+360/-2，可读 patch 411 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[Quantization] Support Quark W8A8 INT8 MoE inference」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`, `vllm/model_executor/layers/quantization/quark/quark.py`, `tests/quantization/test_quark.py`。PR 描述补充为：## Purpose MoE models quantized by AMD Quark with W8A8 INT8 (per-channel weight + per-token dynamic activation) cannot be loaded in vLLM. For example, quantizing MiniMax-M2.1 (4...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `739e5945dc4b`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 18 个文件，+129/-34，可读 patch 431 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[Quantization] [Refactor] Create special "GptOssMxfp4MoeMethod"」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：## Purpose (Generated with Claude) This PR scopes the MXFP4 quantization implementation to GPT-OSS checkpoints while keeping generic MXFP4 support for other models (e.g. Quark)....
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `e1e318af010b`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 20 个文件，+76/-389，可读 patch 782 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[MoE Refactor] Remove MoE DP chunking」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：## Purpose Remove DP chunking MoE runner. Use `max_num_batched_tokens` as default for `max_num_tokens` in `FusedMoEConfig`. ## Test Plan CI Ran DeepEP related tests/kernels/moe...
...
- 状态/时间: merged / 2026-04-14
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `1a9353bb02e6`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 8 个文件，+16/-12，可读 patch 100 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[MoE] Move GPT OSS Triton kernel experts into fused_moe/experts/」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：## Summary - Moves `gpt_oss_triton_kernels_moe.py` from `fused_moe/` root into `fused_moe/experts/`, consistent with the ongoing migration of expert kernel files (e.g. `trtllm_n...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `726efe177bf2`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 53 个文件，+325/-702，可读 patch 2430 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[MoE Refactor] Move the shared/fused expert output sum into MoERunnerBase」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：## Purpose - Move the summation of the shared and fused expert outputs into `MoERunnerBase` and cleanup the final shared/fused all reduce code. Remove corresponding code from mo...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/layers/quantization/quark/quark_moe.py`；关联提交 `d622e27d2be9`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 12 个文件，+601/-121，可读 patch 1072 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 补齐模型支持入口或运行时能力，标题为「[NVFP4] NVFP4 MOE emulation fallback for H100/MI300/MI350, standardize `TritonExperts` usage for OCP MX emulation」，变更集中在 `vllm/model_executor/layers/quantization/quark/quark_moe.py`。PR 描述补充为：## Purpose This PR enables running NVFP4 MOE models on Hopper and AMD Instinct MI300, MI350. This is useful for researchers, anybody trying out microscaling formats, and people...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/mixtral.py`；关联提交 `1c2c1eb8b9fd`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 53 个文件，+254/-98，可读 patch 1073 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Mixtral Quark INT4/FP8 MoE 优化关键推理路径或后端选择，标题为「[MoE Refactor] Rename FusedMoE.make_expert_params_mapping to fused_moe_make_expert_params_mapping」，变更集中在 `vllm/model_executor/models/mixtral.py`。PR 描述补充为：## Purpose This is prep work for deleting the `FusedMoE` class and replacing it with `MoERunner`. This PR is just a rename of `FusedMoE.make_expert_params_mapping` to `fused_moe...
