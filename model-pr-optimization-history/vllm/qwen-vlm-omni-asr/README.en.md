...
- Total PRs in this document: 102
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_vl.py`; associated commits `3b7fea770f44`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Model][VLM] Add Qwen2-VL model support". The diff centers on `vllm/model_executor/models/qwen2_vl.py`. PR body context: This PR adding support for Qwen2-VL model. FIX #8139 FIX #8281 ## Requirements - ~This PR requires `transformers` with this PR merged and this bugfix PR merged (You can install...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_vl.py`; associated commits `06311e295666`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Misc] Skip loading extra bias for Qwen2-VL GPTQ-Int8". The diff centers on `vllm/model_executor/models/qwen2_vl.py`. PR body context: FILL IN THE PR DESCRIPTION HERE FIX #8406 ping @DarkLight1337 **BEFORE SUBMITTING, PLEASE READ THE CHECKLIST BELOW AND FILL IN THE DESCRIPTION ABOVE** --- section, markdown rend...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_vl.py`; associated commits `a79e5229843e`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Model] Support pp for qwen2-vl". The diff centers on `vllm/model_executor/models/qwen2_vl.py`. PR body context: FILL IN THE PR DESCRIPTION HERE Follow #7860 , this pr introduce pp support for the latest qwen2-vl. Already tested on Qwen2-VL-72B-Instruct-GPTQ-Int4 **BEFORE SUBMITTING, PLEAS...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_vl.py`; associated commits `c23953675f78`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Hardware][CPU] Enable mrope and support Qwen2-VL on CPU backend". The diff centers on `vllm/model_executor/models/qwen2_vl.py`. PR body context: FILL IN THE PR DESCRIPTION HERE FIX #xxxx (*link existing issues this PR will resolve*) - This PR enables mrope and qwen2-vl on CPU backend. **BEFORE SUBMITTING, PLEASE READ THE...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_vl.py`; associated commits `4bb98f2190aa`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR changes model-related implementation. Title: "[Misc] Update config loading for Qwen2-VL and remove Granite". The diff centers on `vllm/model_executor/models/qwen2_vl.py`. PR body context: This PR ports Qwen2-VL config from transformers v4.45 but remove rope type override to address config loading issue, and remove ported Granite config that's no longer required....
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_vl.py`; associated commits `7e7eae338d27`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Misc] Standardize RoPE handling for Qwen2-VL". The diff centers on `vllm/model_executor/models/qwen2_vl.py`. PR body context: https://github.com/huggingface/transformers/issues/33401 has been fixed in Transformers v4.45.2, but the devs have clarified that M-ROPE is intended to be configured as `rope_ty...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_vl.py`; associated commits `999df95b4eef`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Make image processor respect `mm_processor_kwargs` for Qwen2-VL". The diff centers on `vllm/model_executor/models/qwen2_vl.py`. PR body context: Without this PR, if one wants to pass more images with lower resolution to Qwen2-VL models (e.g. 127 images with 256 tokens each): the LLM instance will be failed to create due...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_vl.py`; associated commits `f83feccd7f66`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Ignore GPTQ quantization of Qwen2-VL visual module". The diff centers on `vllm/model_executor/models/qwen2_vl.py`. PR body context: FIX https://github.com/vllm-project/vllm/issues/9832 This is a workaround for the fact that GPTQ configs generated by AutoGPTQ do not have a list of ignored modules to check if...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_vl.py`; associated commits `3945c82346da`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Model] Add support for Qwen2-VL video embeddings input & multiple image embeddings input with varied resolutions". The diff centers on `tests/models/decoder_only/vision_language/test_qwen2_vl.py`, `vllm/model_executor/models/qwen2_vl.py`. PR body context: ## Goal 1. Add support for Qwen2-VL multiple image embeddings input with **varied resolutions** currently, vLLM implementation of Qwen2-VL's image embedding input **requires** a...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_audio.py`, `vllm/model_executor/models/qwen2_vl.py`; associated commits `e24113a8fe5d`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR changes model-related implementation. Title: "[Model] Refactor Qwen2-VL to use merged multimodal processor". The diff centers on `vllm/model_executor/models/qwen2_vl.py`, `tests/models/decoder_only/vision_language/mm_processor_kwargs/test_qwen2_vl.py`, `vllm/model_executor/models/qwen2_audio.py`. The PR body has no extra context, so this assessment comes from the title, file list, and patch.
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_vl.py`; associated commits `b1b1038fbdc1`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Bugfix] Fix Qwen2-VL LoRA weight loading". The diff centers on `vllm/model_executor/models/qwen2_vl.py`. PR body context: FIX #11406 ## Upcoming PRs include: - Convert hf_to_vllm_mapper to static variable for all models - Support `substr` and `subfix` mapping
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_vl.py`; associated commits `11d8a091c6c7`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR optimizes an inference path or backend selection. Title: "[Misc] Optimize Qwen2-VL LoRA test". The diff centers on `vllm/model_executor/models/qwen2_vl.py`. PR body context: I retrained a QWen-VL LoRA. In my local environment using A800, I tested the model on 100 image samples, the generation results were completely aligned with the `transformers` o...
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[V1] Add V1 support of Qwen2-VL". The diff centers on `vllm/model_executor/models/qwen2_vl.py`, `tests/models/decoder_only/vision_language/test_qwen2_vl.py`. PR body context: Continued from #11668 for supporting Qwen2-VL on V1. Co-authored-by: @imkero who has done the great work on the V1 MRoPE implementation.
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_vl.py`; associated commits `4c8dd12ef347`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Misc] Add qwen2.5-vl BNB support". The diff centers on `vllm/model_executor/models/qwen2_5_vl.py`. PR body context: FIX https://github.com/vllm-project/vllm/pull/12604#discussion_r1940626980 - Add tp compatible BNB support to Qwen2.5-VL models.
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Fix num video tokens calculation for Qwen2-VL". The diff centers on `vllm/model_executor/models/qwen2_vl.py`. PR body context: FIX #13099
...
- Motivation: For Qwen VLM/Omni/ASR, this PR optimizes an inference path or backend selection. Title: "[Misc] Qwen2.5-VL Optimization". The diff centers on `vllm/model_executor/models/qwen2_5_vl.py`, `vllm/model_executor/models/qwen2_vl.py`. PR body context: Thanks to the VLLM team for their great work! This PR includes the following: ꔷ Replaced `Qwen2RMSNorm` with vllm's official `RMSNorm`. Extensive testing has confirmed that this...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_vl.py`, `vllm/model_executor/models/qwen2_vl.py`; associated commits `7fdaaf48ef2a`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Fix qwen2.5-vl image processor". The diff centers on `vllm/model_executor/models/qwen2_5_vl.py`, `vllm/model_executor/models/qwen2_vl.py`. PR body context: FIX #13285 **Need to wait Qwen team update the model repo with correct preprocessor_config.json (see https://huggingface.co/Qwen/Qwen2.5-VL-3B-Instruct/discussions/14)** - `tran...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_vl.py`; associated commits `041e29471671`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Misc] add mm_processor_kwargs to extra_body for Qwen2.5-VL". The diff centers on `vllm/model_executor/models/qwen2_5_vl.py`. PR body context: In Qwen2.5-VL online inference, the `fps` parameter in `mm_processor_kwargs` is essential for accurately calculating the `second_pre_grid_t` value. However, the OpenAI interface...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_vl.py`; associated commits `78648758794e`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Fix qwen2.5-vl overflow issue". The diff centers on `vllm/model_executor/models/qwen2_5_vl.py`. PR body context: FIX #13817 (*link existing issues this PR will resolve*) - Qwen2.5-VL-3B will overflow with float16 in its ViT encoder, we need to cast the overflow value manually.
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_vl.py`, `vllm/model_executor/models/qwen2_vl.py`; associated commits `70b808fe1a63`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR optimizes an inference path or backend selection. Title: "[Perf]:Optimize qwen2-vl to reduce cudaMemcpyAsync". The diff centers on `vllm/model_executor/models/qwen2_vl.py`, `vllm/model_executor/models/qwen2_5_vl.py`. PR body context: qwen2-vl logic optimization: During each forward propagation, the xformer branch of Qwen2VisionTransformer will execute multiple tensor tolist methods (flash attn branch will ex...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_vl.py`; associated commits `1e508343e1ec`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Fix incorrect qwen2.5-vl attention mask pre-computation". The diff centers on `vllm/model_executor/models/qwen2_5_vl.py`. PR body context: FIX #15122 FIX #15197 - Fix incorrect attention mask creation for qwen2.5-vl windows attention from #14377
...
- Status/date: merged / 2025-03-21
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_vl.py`, `vllm/model_executor/models/qwen2_vl.py`; associated commits `47c712621316`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Misc] Add attention mask pre-computation optimization back to Qwen2.5-VL". The diff centers on `vllm/model_executor/models/qwen2_5_vl.py`, `vllm/model_executor/models/qwen2_vl.py`. PR body context: - #15200 actually disabled the seqlens pre-computation for Qwen2.5-VL by mistake, because seqlens are computed for each layer again. - This PR adds the optimization introduced i...
...
- Trace source: `git log --name-only -- <model-files>` found it through `examples/offline_inference/qwen2_5_omni/README.md`, `examples/offline_inference/qwen2_5_omni/only_thinker.py`, `vllm/model_executor/models/qwen2_5_omni_thinker.py`, `vllm/model_executor/models/qwen2_5_vl.py`; associated commits `2c1bd848a668`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Model][VLM] Add Qwen2.5-Omni model support (thinker only)". The diff centers on `vllm/model_executor/models/qwen2_5_omni_thinker.py`, `vllm/model_executor/models/qwen2_5_vl.py`, `examples/offline_inference/qwen2_5_omni/only_thinker.py`. PR body context: This PR adding support for Qwen2.5-Omni model (thinker only). ## Requirements This PR requires this corresponding transformers PR. **Note: You need to install transformers from...
...
- Status/date: merged / 2025-04-19
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_omni_thinker.py`; associated commits `5124f5bf51b8`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR changes model-related implementation. Title: "[Model] Qwen2.5-Omni Cleanup". The diff centers on `vllm/model_executor/models/qwen2_5_omni_thinker.py`. PR body context: Clean up some oversighted changes from #15130
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_vl.py`; associated commits `26c0406555a5`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Bugfix] Fix distributed bug in Qwen2.5-VL & Qwen2.5-Omni". The diff centers on `vllm/model_executor/models/qwen2_5_vl.py`. PR body context: This PR fixes the distribution bug in Qwen2.5-VL & Qwen2.5-Omni (introduced in #15130). When `tp_group != world_size`, current implementation will cause an allgather error.
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_vl.py`; associated commits `571e8dd65e2a`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Fix distributed bug again in Qwen2.5-VL & Qwen2.5-Omni". The diff centers on `vllm/model_executor/models/qwen2_5_vl.py`. PR body context: I am very sorry that previous fix #16907 still have distributed bug, need to fix it agin. Related PR: #15130 and #16907
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_omni_thinker.py`; associated commits `8b464d9660a8`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR changes model-related implementation. Title: "[Misc] Clean up Qwen2.5-Omni code". The diff centers on `vllm/model_executor/models/qwen2_5_omni_thinker.py`. PR body context: Split up `apply` even more so that the override in Qwen2.5-Omni doesn't have to repeat as much code
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_vl.py`, `vllm/model_executor/models/qwen2_vl.py`; associated commits `c3e9d5060e89`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR optimizes an inference path or backend selection. Title: "[Misc] Use `apply_rotary_emb` from vllm_flash_attn for Qwen2-VL vision RoPE". The diff centers on `vllm/model_executor/models/qwen2_5_vl.py`, `vllm/model_executor/models/qwen2_vl.py`. PR body context: - Since we have ported FA's RoPE kernel in `vllm_flash_attn`, there is no need to use original FA's `apply_rotary_emb` anymore - Replace original FA's `apply_rotary_emb` with `v...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_omni_thinker.py`, `vllm/model_executor/models/qwen2_5_vl.py`, `vllm/model_executor/models/qwen2_vl.py`; associated commits `015815fe0141`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Bugfix] `use_fast` failing to be propagated to Qwen2-VL image processor". The diff centers on `vllm/model_executor/models/qwen2_5_omni_thinker.py`, `vllm/model_executor/models/qwen2_5_vl.py`, `vllm/model_executor/models/qwen2_vl.py`. PR body context: As discussed in https://vllm-dev.slack.com/archives/C07QCGVDNUF/p1746687303802249?thread_ts=1745944078.067259&cid=C07QCGVDNUF I decided not to add regression tests because the d...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_vl.py`; associated commits `67da5720d4ed`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR optimizes an inference path or backend selection. Title: "[PERF] Speed up Qwen2.5-VL model by speed up rotary position embedding const…". The diff centers on `vllm/model_executor/models/qwen2_5_vl.py`. PR body context: ### Description of Problem In Qwen2.5-VL rotary position embedding constant tensors creates in the beginning of model's `forward`. Before this PR there were a mix of CPU and GPU...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_vl.py`, `vllm/model_executor/models/qwen2_vl.py`; associated commits `ec2dcd80bc17`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Misc] Update `WeightsMapper` for qwen2-vl/qwen2.5-vl". The diff centers on `vllm/model_executor/models/qwen2_5_vl.py`, `vllm/model_executor/models/qwen2_vl.py`. PR body context: FIX #18976 - Transformers v4.52 will map weights name for Qwen2-VL/Qwen2.5-VL: https://github.com/huggingface/transformers/blob/de4cf5a38e9678b9e465867a8a6b88ea727bea52/src/tran...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_vl.py`, `vllm/model_executor/models/qwen2_vl.py`; associated commits `d3a6f2120bb6`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[FEAT][ROCm] Enable running Flash Attention as ViT attn backend for Qwen-VL models on ROCm platform.". The diff centers on `vllm/model_executor/models/qwen2_5_vl.py`, `vllm/model_executor/models/qwen2_vl.py`. PR body context: # Essential Elements of an Effective PR Description Checklist - [x] The purpose of the PR, such as "Fix some issue (link existing issues this PR will resolve)". - [x] The test p...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_vl.py`; associated commits `cbc8457b2663`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Model] Switch to Fused RMS norm in Qwen2.5_VL model.". The diff centers on `vllm/model_executor/models/qwen2_5_vl.py`. PR body context: # Essential Elements of an Effective PR Description Checklist - [x] The purpose of the PR, such as "Fix some issue (link existing issues this PR will resolve)". - [x] The test p...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_omni_thinker.py`; associated commits `9f1c6422549d`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Bugfix] fix Qwen2.5-Omni processor output mapping". The diff centers on `vllm/model_executor/models/qwen2_5_omni_thinker.py`. PR body context: ## Purpose Fix #23056 Add a processor output mapping for Qwen2.5-Omni thinker, to compute video rotary embedding correctly. ## Test Plan Use the script I provided in #23056 and...
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Bugfix] Fix Qwen2.5-VL quantized model weights loading". The diff centers on `vllm/model_executor/models/qwen2_5_vl.py`. PR body context: This PR makes sure quantized model weights are loaded correctly. Currently, loading `RedHatAI/Qwen2.5-VL-7B-Instruct-FP8-Dynamic` will crash on A100s: The issue is introduced in...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_omni_thinker.py`; associated commits `c9f7081f9c84`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[LoRA]: Add lora support to qwen-2.5-omni". The diff centers on `vllm/model_executor/models/qwen2_5_omni_thinker.py`. PR body context: ## Purpose Add LoRA support to `qwen-2.5-omni`, scoped strictly to the language model components. - Implements `SupportsLoRA` on `Qwen2_5OmniThinkerForConditionalGeneration`. -...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_omni_thinker.py`; associated commits `6f4a82f8b5a1`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Model] Enable BNB support for qwen2_5_omni_thinker". The diff centers on `vllm/model_executor/models/qwen2_5_omni_thinker.py`. PR body context: ## Purpose - Enable bitsandbytes quantization for qwen2_5_omni_thinker, fix https://github.com/vllm-project/vllm/issues/23240 - Add `get_mm_mapping` and `SupportLoRA`, deleted b...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_vl.py`; associated commits `b0d1213ac395`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR optimizes an inference path or backend selection. Title: "[Models] Prevent CUDA sync in Qwen2.5-VL". The diff centers on `vllm/model_executor/models/qwen2_5_vl.py`. PR body context: This is a followup to #24443 from @david6666666 When I profiled Qwen2.5-VL it seems like an implicit CUDA sync is still happening during the indexing: https://github.com/vllm-pr...
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Model] Support Qwen3-VL Model Series". The diff centers on `vllm/model_executor/models/qwen3_vl.py`, `vllm/model_executor/models/qwen3_vl_moe.py`, `vllm/model_executor/models/qwen2_vl.py`. PR body context: ## Purpose This PR adds model support for the upcoming Qwen3-VL models, including both dense and MoE variants. Reference HF implementation - https://github.com/huggingface/trans...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen3_vl.py`, `vllm/model_executor/models/qwen3_vl_moe.py`; associated commits `3127274d022b`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[MM Encoder] Apply DP ViT for Qwen3-VL model series". The diff centers on `vllm/model_executor/models/qwen3_vl.py`, `vllm/model_executor/models/qwen3_vl_moe.py`. PR body context: ## Purpose Add DP ViT for Qwen3-VL models - this PR should be merged only after #24727 is merged. Part of #22743 ## Test Plan Running on `Qwen3-VL-30B-A3B-Instruct` with 4xL40s...
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Kernel][Performance] Add Triton kernel for Qwen3-VL interleaved MRoPE". The diff centers on `vllm/model_executor/layers/rotary_embedding/mrope.py`, `tests/kernels/core/test_mrope.py`. PR body context: ## Purpose - Following PR for #24727, implement corresponding Triton kernel for interleaved MRoPE. ## Test Plan ## Test Result Test should pass --- Essential Elements of an Effe...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen3_vl_moe.py`; associated commits `be874c020196`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Bugfix] Fix Qwen3-VL-MoE weight loading for EP". The diff centers on `vllm/model_executor/models/qwen3_vl_moe.py`. PR body context: ## Purpose Previously `self.load_fused_expert_weights` check is too strict and will prevent server from launching with `--enable-expert-parallel`. This PR fixes it. ## Test Plan...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen3_vl.py`; associated commits `30d08911f7cf`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR optimizes an inference path or backend selection. Title: "[MM][Perf] Minor Optimization on Qwen3-VL `fast_pos_embed_interpolate`". The diff centers on `vllm/model_executor/models/qwen3_vl.py`. PR body context: ## Purpose ## Test Plan 10 QPS of VisionArena on Qwen3-VL 4B on A100 ## Test Result Main This branch MMMU matched --- Essential Elements of an Effective PR Description Checklist...
...
- Status/date: merged / 2025-09-21
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen3_vl.py`; associated commits `af7dfb0d1a95`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR optimizes an inference path or backend selection. Title: "[Perf] Further optimization for Qwen3-VL `fast_pos_embed_interpolate`". The diff centers on `vllm/model_executor/models/qwen3_vl.py`. PR body context: ## Purpose - Following PR for #25337 - Just found that we can further optimize weights computation for `fast_pos_embed_interpolate` by reducing duplicated multiply. - Also impro...
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Model] Enable DP for ViT in Qwen2-VL". The diff centers on `vllm/model_executor/models/qwen2_vl.py`. PR body context: ## Purpose Part of https://github.com/vllm-project/vllm/issues/22743 ## Test Plan ## Test Result --- Essential Elements of an Effective PR Description Checklist - [ ] The purpos...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen3_vl.py`; associated commits `7be9ffcd9f5c`
- Diff scope read: GitHub Pull Request files API returned 1 files, +1/-1, 9 readable patch lines; this card prioritizes model-related and high-change files.
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Misc] Fix Qwen3-VL `video_grid_thw` typing". The diff centers on `vllm/model_executor/models/qwen3_vl.py`. PR body context: ## Purpose Without this change, torch will throw a warning `vllm/vllm/model_executor/models/qwen3_vl.py:480: UserWarning: To copy construct from a tensor, it is recommended to u...
...
- Status/date: merged / 2025-09-25
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_vl.py`, `vllm/model_executor/models/qwen3_vl.py`; associated commits `17b4c6685ce6`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Bugfix] Fix Qwen3-VL max_num_video_tokens calculation for video profiling". The diff centers on `vllm/model_executor/models/qwen3_vl.py`, `vllm/model_executor/models/qwen2_vl.py`. PR body context: ## Purpose - Suspend of #25557 before we support configurable mm profiling ## Test Plan ## Test Result --- Essential Elements of an Effective PR Description Checklist - [x] The...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_vl.py`, `vllm/model_executor/models/qwen3_vl.py`; associated commits `c242c98031b8`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Allow Only SDPA Backend for ViT on B200 for Qwen3-VL". The diff centers on `vllm/model_executor/models/qwen2_5_vl.py`, `vllm/model_executor/models/qwen3_vl.py`. PR body context: ## Purpose Temple fix for https://github.com/vllm-project/vllm/issues/25582 Special thanks to @ywang96 for the context The default XFORMERS backend has problem and we will meet...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_vl.py`, `vllm/model_executor/models/qwen3_vl.py`; associated commits `0efd540dbc54`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[VLM] Update Qwen3-VL max_num_video_tokens calculation for configurable video profiling". The diff centers on `vllm/model_executor/models/qwen3_vl.py`, `vllm/model_executor/models/qwen2_vl.py`. PR body context: ## Purpose - Qwen3-VL's max_num_video_tokens calculation is implemented wrong, this PR corrects it ## Test Plan ## Test Result --- Essential Elements of an Effective PR Descript...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen3_vl.py`, `vllm/model_executor/models/qwen3_vl_moe.py`; associated commits `66bca9b8bd0a`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[MM] Add text-only mode for Qwen3-VL". The diff centers on `vllm/model_executor/models/qwen3_vl.py`, `vllm/model_executor/models/qwen3_vl_moe.py`. PR body context: ## Purpose Since this model is performing pretty well on text only tasks we might want to allow people to serve it as a text-only model. ## Test Plan ## Test Result Running `vll...
...
- Status/date: merged / 2025-10-01
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_omni_thinker.py`; associated commits `84d57342b66e`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[BugFix][MM] Fix Nonetype error when video is cache in qwen2.5-omni-thinker". The diff centers on `vllm/model_executor/models/qwen2_5_omni_thinker.py`. PR body context: Fix https://github.com/vllm-project/vllm/issues/25970 When omni is handling the cached video, it will produced `mm_kwargs: {'video': [None]}`. Then the error will occur when acc...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_vl.py`; associated commits `5e4a8223c644`, `dd96465fd744`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR optimizes an inference path or backend selection. Title: "[Qwen][ROCm] Flash Attention Rotary Embeddings". The diff centers on `vllm/model_executor/models/qwen2_vl.py`. PR body context: ## Purpose Qwen VL models previously relied on a basic PyTorch implementation for applying rotary positional embeddings on ROCm architectures. This PR adds a ROCm specialisation...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_vl.py`; associated commits `dd96465fd744`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[BugFix][QWEN-VL]fix wrong apply_rotary_emb_torch selection introduced by #24642". The diff centers on `vllm/model_executor/models/qwen2_vl.py`. PR body context: ## Purpose #24642 introduced dispatch_rotary_emb_function method in common.py while for non_cuda or non_rocm, apply_rotary_emb_torch is differently defined in some modeling code...
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "Add Qwen3-Omni moe thinker". The diff centers on `vllm/model_executor/models/qwen3_omni_moe_thinker.py`. PR body context: This PR from the Qwen team for: qwen3-omni-moe thinker part. Testing has been conducted internally across four configurations (v0/v1, eager/CUDA) on several representative bench...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen3_omni_moe_thinker.py`; associated commits `ddaff2938e0b`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR extends deployment docs, tests, or CI coverage. Title: "[MM] Move Qwen3Omni MRoPE impl to model file". The diff centers on `vllm/model_executor/models/qwen3_omni_moe_thinker.py`. PR body context: ## Purpose Move the mrope impl to the model file, and clean up some code there as well. ## Test Plan ## Test Result --- Essential Elements of an Effective PR Description Checkli...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen3_omni_moe_thinker.py`; associated commits `8c851f6d044b`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Fix qwen3-omni audio truncation issue". The diff centers on `vllm/model_executor/models/qwen3_omni_moe_thinker.py`. PR body context: ## Purpose - Fix #26630 - Before https://github.com/huggingface/transformers/pull/41473, Qwen3-omni will still truncate audio to 30s. - This PR enforces no truncation in Qwen3-o...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen3_vl.py`; associated commits `4c91a28e301d`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[bugfix] Qwen3-VL fix video incorrect timestamp calculations while do_sample_frames=True". The diff centers on `vllm/model_executor/models/qwen3_vl.py`. PR body context: The current code overwrites `video_fps` with `sampled_fps`, which causes incorrect timestamp calculations. @ywang96 Hey bro, could you please help review this? Thanks!
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_vl.py`, `vllm/model_executor/models/qwen3_omni_moe_thinker.py`, `vllm/model_executor/models/qwen3_vl.py`; associated commits `0d8161b07504`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Model] Fix Qwen3VL and Qwen3Omni after torch.compile changes". The diff centers on `vllm/model_executor/models/qwen3_omni_moe_thinker.py`, `vllm/model_executor/models/qwen3_vl.py`, `vllm/model_executor/models/qwen2_5_vl.py`. PR body context: ## Purpose #23207 Broke Qwen3VL since it relies on `Qwen2_5_VisionAttention` as well but still used the old signature: This PR updates Qwen3VL and Qwen3Omni to fix this. ## Test...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_vl.py`; associated commits `e806178d2a9b`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[BugFix][VL] Fix FA selection on Qwen2.5-VL". The diff centers on `vllm/model_executor/models/qwen2_5_vl.py`. PR body context: ## Purpose https://github.com/vllm-project/vllm/pull/27190 breaks AMD CI (and also qwen2.5 vl): tests/v1/entrypoints/openai/responses/test_image.py : with _Backend.FLASH_ATTN it...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_omni_thinker.py`, `vllm/model_executor/models/qwen3_omni_moe_thinker.py`; associated commits `853a8eb53b89`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Fix Qwen Omni audio inference". The diff centers on `vllm/model_executor/models/qwen2_5_omni_thinker.py`, `vllm/model_executor/models/qwen3_omni_moe_thinker.py`. PR body context: ## Purpose FIX https://github.com/vllm-project/vllm/issues/27906 FIX #27907 ## Test Plan ## Test Result --- Essential Elements of an Effective PR Description Checklist - [ ] The...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_vl.py`, `vllm/model_executor/models/qwen2_vl.py`; associated commits `48b8456ff992`, `bc5bd45c7d1a`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR changes model-related implementation. Title: "[Refactor] Remove redundant TP gather/split in split_qkv in QwenVL". The diff centers on `vllm/model_executor/models/qwen2_5_vl.py`, `vllm/model_executor/models/qwen2_vl.py`. PR body context: ## Purpose This code path uses head-parallel attention, where each rank holds full Q/K/V vectors for its own subset of heads. All attention backends operate per-rank on local he...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen3_vl.py`; associated commits `cec275efcef6`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] resolve Qwen3-VL GPTQModel quantized model loading failure". The diff centers on `vllm/model_executor/models/qwen3_vl.py`. PR body context: ## Purpose A quantized Qwen3-VL-32B model using GPTQModel==5.4.0 fails to load with `vllm serve Guan1794/qwen3VL-32B-4bit-gptq` The error message is: The GPTQModel quantization...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_vl.py`; associated commits `d84d8f4429a5`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "Fix EVS crash when using `video_embeds` inputs in Qwen2.5-VL". The diff centers on `vllm/model_executor/models/qwen2_5_vl.py`. PR body context: ### **Summary** Fix a crash in `Qwen2.5-VL` when EVS (Efficient Video Sampling) is enabled and video inputs are provided through the `video_embeds` pathway. The regression occur...
...
- Trace source: `git log --name-only -- <model-files>` found it through `examples/offline_inference/qwen3_omni/only_thinker.py`, `tests/model_executor/test_qwen3_omni.py`, `vllm/model_executor/models/qwen2_5_omni_thinker.py`, `vllm/model_executor/models/qwen3_omni_moe_thinker.py`; associated commits `839c6b7b72bc`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Multimodal][Qwen3 Omni] Make Qwen3 Omni work with audio-in-video inputs in V1 engine.". The diff centers on `tests/model_executor/test_qwen3_omni.py`, `vllm/model_executor/models/qwen3_omni_moe_thinker.py`, `vllm/model_executor/models/qwen2_5_omni_thinker.py`. PR body context: ## Purpose FIX #22268 FIX #22364 CLOSE #23888 CLOSE #25473 CLOSE https://github.com/vllm-project/vllm/issues/28046 ## Test Plan Sanity checked with https://qianwen-res.oss-cn-be...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen3_omni_moe_thinker.py`; associated commits `a2b053dc858d`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "feat(model): Add BitsAndBytes quantization support for Qwen3-Omni-MoE". The diff centers on `vllm/model_executor/models/qwen3_omni_moe_thinker.py`. PR body context: Add the necessary attributes for BitsAndBytes quantization support: - Add packed_modules_mapping with qkv_proj and gate_up_proj mappings - Add get_mm_mapping() method for multim...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen3_omni_moe_thinker.py`; associated commits `3f1b03739ae1`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[ROCm] [Bugfix] [AITER] `compute_attn_mask_seqlen` for qwen3 omni". The diff centers on `vllm/model_executor/models/qwen3_omni_moe_thinker.py`. PR body context: ## Purpose This is a bugfix for qwen3-omni model when using AITER Flash Attention ## Test Plan Evaluate the qwen3-omni chartqa ## Test Result --- Essential Elements of an Effect...
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "support qwen3-vl handle requests with embeddings". The diff centers on `vllm/model_executor/models/qwen3_vl.py`. PR body context: 为了支持qwen3-vl 处理带image embedding的请求，需要使用自定义的 函数，用 类替代 即可 ## Test Script ## Test Result --- Essential Elements of an Effective PR Description Checklist - [ ] The purpose of the PR...
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Revert Qwen2-VL part of change in #28271". The diff centers on `vllm/model_executor/models/qwen2_vl.py`. PR body context: This PR reverts qwen2_vl.py part of commit bc5bd45c7d1abbac4a63d97d383212c108e55308. Fixes #30250, the model generates incorrect output when TP is used.
...
- Status/date: merged / 2025-12-14
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen3_vl.py`; associated commits `ae88aada38ec`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Feature]Add EVS (Efficient Video Sampling) Support for Qwen3-VL". The diff centers on `vllm/model_executor/models/qwen3_vl.py`. PR body context: ## Summary This PR implements **EVS (Efficient Video Sampling)** support for Qwen3-VL models, enabling **dynamic video token pruning** to improve inference efficiency while main...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_omni_thinker.py`; associated commits `6fe588765287`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR extends deployment docs, tests, or CI coverage. Title: "[Chore] Remove v0 dead code for Qwen2.5-omni". The diff centers on `vllm/model_executor/models/qwen2_5_omni_thinker.py`. PR body context: ## Purpose - Just found we missed Qwen2.5-omni's `embed_multimodal_v0` when removing `embed_multimodal_v0` from models. ## Test Plan ## Test Result --- Essential Elements of an...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen3_omni_moe_thinker.py`; associated commits `bb24592d139b`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Qwen3-Omni] fixed _get_feat_extract_output_lengths function". The diff centers on `vllm/model_executor/models/qwen3_omni_moe_thinker.py`. PR body context: ## Purpose ## Test Plan ## Test Result --- Essential Elements of an Effective PR Description Checklist - [ ] The purpose of the PR, such as "Fix some issue (link existing issues...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/glmasr.py`, `vllm/model_executor/models/glmasr_utils.py`; associated commits `d722e9e614f6`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "Add GLM-ASR multimodal support". The diff centers on `vllm/model_executor/models/glmasr.py`, `vllm/model_executor/models/glmasr_utils.py`. PR body context: ## Purpose Support that model :https://huggingface.co/zai-org/GLM-ASR-Nano-2512 ## Test Plan ## Test Result for download and convert test audio: https://paste.ubuntu.com/p/K7S9T...
...
- Trace source: `git log --name-only -- <model-files>` found it through `tests/models/multimodal/processing/test_qwen3_omni.py`, `vllm/model_executor/models/qwen3_omni_moe_thinker.py`; associated commits `97a01308e9ce`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR changes model-related implementation. Title: "Improve HF qwen3_omni: preserve audio_sample_rate in kwargs restructuring". The diff centers on `tests/models/multimodal/processing/test_qwen3_omni.py`, `vllm/model_executor/models/qwen3_omni_moe_thinker.py`. PR body context: The Qwen3OmniMoeProcessor was losing the audio_sample_rate parameter during kwargs restructuring for transformers
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen3_omni_moe_thinker.py`; associated commits `2c1a4f2488da`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix]: avoid overriding audio/text kwargs (Qwen3-Omni)". The diff centers on `vllm/model_executor/models/qwen3_omni_moe_thinker.py`. PR body context: This PR fixes an issue in Qwen3OmniMoeThinker where `audio_kwargs/text_kwargs` could be overridden when restructuring kwargs for `Transformers < 4.58.0`. We now preserve existin...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/glmasr.py`, `vllm/model_executor/models/glmasr_utils.py`; associated commits `974138751bdb`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR optimizes an inference path or backend selection. Title: "[Refactor] GLM-ASR Modeling". The diff centers on `vllm/model_executor/models/glmasr.py`, `vllm/model_executor/models/glmasr_utils.py`. PR body context: ### 🚀 Key Improvements **1. Native vLLM Audio Encoder Implementation** (`glmasr.py`) - Completely rewrote `GlmAsrEncoder` as a vLLM-native implementation with full optimization...
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Models] Allow converting Qwen3-VL into Reranker model". The diff centers on `examples/pooling/score/template/qwen3_vl_reranker.jinja`. PR body context: ## Purpose - Enable reranker support for Qwen3-VL ## Test Plan ## Test Result --- Essential Elements of an Effective PR Description Checklist - [x] The purpose of the PR, such a...
...
- Motivation: For Qwen VLM/Omni/ASR, this PR optimizes an inference path or backend selection. Title: "[Model] Use mm_position to compute mrope positions for Qwen2-VL/2.5-VL". The diff centers on `vllm/model_executor/models/qwen2_5_vl.py`, `vllm/model_executor/models/qwen2_vl.py`. PR body context: ## Purpose Followup to #28399 and #28730. This PR optimizes the M-RoPE position computation for both **Qwen2-VL** and **Qwen2.5-VL**. - Leveraging the pre-calculated mm_features...
...
- Motivation: For Qwen VLM/Omni/ASR, this PR changes model-related implementation. Title: "[Model] Re-implement Qwen3Omni Audio Encoder". The diff centers on `vllm/model_executor/models/qwen3_omni_moe_thinker.py`. PR body context: ## Purpose Re-implement Qwen3-Omni Audio Encoder with vLLM primitives with some vectorization improvements. - roughly 10% speedup at high batch sizes according to profiling run...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/glmasr.py`; associated commits `38bf2ffb21d5`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Fix GLM-ASR audio encoder RoPE dim". The diff centers on `vllm/model_executor/models/glmasr.py`. PR body context: ## Purpose - Fix https://github.com/vllm-project/vllm/issues/32445 - GLM-ASR audio encoder's RoPE should be half rotary. - This issue only occured in native code path, because v...
...
- Trace source: `git log --name-only -- <model-files>` found it through `examples/offline_inference/qwen2_5_omni/only_thinker.py`, `vllm/model_executor/models/qwen2_5_omni_thinker.py`; associated commits `a698e8e7ad4b`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Model] Use mm_position to compute mrope positions for Qwen2.5-Omni". The diff centers on `vllm/model_executor/models/qwen2_5_omni_thinker.py`, `examples/offline_inference/qwen2_5_omni/only_thinker.py`. PR body context: ## Summary - Refactor get_mrope_input_positions() to use mm_feature.mm_position.offset directly - Follows pattern from PR #32126 for Qwen2-VL/2.5-VL ## Changes - Add iter_mm_fea...
...
- Trace source: `git log --name-only -- <model-files>` found it through `examples/offline_inference/qwen3_omni/only_thinker.py`, `vllm/model_executor/models/qwen3_omni_moe_thinker.py`; associated commits `6ca2c91b9663`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR optimizes an inference path or backend selection. Title: "[Model] Use mm_position to compute mrope positions for Qwen3-Omni". The diff centers on `vllm/model_executor/models/qwen3_omni_moe_thinker.py`, `examples/offline_inference/qwen3_omni/only_thinker.py`. PR body context: Optimizes M-RoPE position calculation for Qwen3-Omni by using `mm_position.offset` directly from `MultiModalFeatureSpec` instead of token-by-token searching through `input_token...
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Models] Qwen3-ASR". The diff centers on `vllm/model_executor/models/qwen3_asr.py`, `vllm/transformers_utils/configs/qwen3_asr.py`, `vllm/transformers_utils/processors/qwen3_asr.py`. PR body context: ## Purpose Add support for Qwen3-ASR model series - see recipe at https://github.com/vllm-project/recipes/blob/main/Qwen/Qwen3-ASR.md ## Test Plan ## Test Result --- Essential E...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen3_asr.py`; associated commits `e77f162cf59d`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Fix `Qwen3ASR` language asr tag in output". The diff centers on `vllm/model_executor/models/qwen3_asr.py`. PR body context: It appears Qwen3-ASR necessitates of post-processing to remove spurious language tags `language English ` inserted in the prompt (https://github.com/QwenLM/Qwen3-ASR/blob/c17a13...
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[BUGFIX] Fix hipErrorIllegalState in Qwen3-Omni during startup profiling allow inference Omni on ROCM". The diff centers on `vllm/model_executor/models/qwen3_omni_moe_thinker.py`. PR body context: This PR fixes a critical crash occurring on AMD (ROCm) hardware when initializing the Qwen3-Omni model (specifically the Qwen3Omni_VisionTransformer). During the memory profilin...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen3_asr.py`; associated commits `ceab70c89d2b`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] fix qwen3-asr response error". The diff centers on `vllm/model_executor/models/qwen3_asr.py`. PR body context: ## Purpose fix https://github.com/vllm-project/vllm/issues/33643 ## Test Plan ## Test Result --- Essential Elements of an Effective PR Description Checklist - [ ] The purpose of...
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Bugfix][Model] Fix audio-in-video support for Qwen2.5-Omni and Qwen3-Omni". The diff centers on `vllm/model_executor/models/qwen2_5_omni_thinker.py`, `vllm/model_executor/models/qwen3_omni_moe_thinker.py`. PR body context: ## Purpose Fix bugs preventing `use_audio_in_video=True` from working correctly with Qwen2.5-Omni and Qwen3-Omni. **~~Bug 1: `KeyError: 'audio'` in `MultiModalBudget`~~** ~~Both...
...
- Status/date: merged / 2026-02-04
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen3_omni_moe_thinker.py`; associated commits `535de06cb1d9`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Model] Add transcription support for Qwen3-Omni". The diff centers on `vllm/model_executor/models/qwen3_omni_moe_thinker.py`. PR body context: ## Purpose Only 4 models are supported as a Transcription model according to the docs. This PR adds Qwen3-Omni per the feature request: https://github.com/vllm-project/vllm/issu...
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Realtime] Add Qwen3-ASR realtime streaming support". The diff centers on `vllm/model_executor/models/qwen3_asr_realtime.py`. PR body context: ## Purpose This PR adds real time Websocket streaming transcription support for Qwen3-ASR. Referenced issue: https://github.com/vllm-project/vllm/issues/34232 ## Test Plan Machi...
...
- Trace source: `git log --name-only -- <model-files>` found it through `tests/models/multimodal/processing/test_qwen2_5_omni_embed.py`, `vllm/model_executor/models/qwen2_5_omni_thinker.py`, `vllm/model_executor/models/qwen3_omni_moe_thinker.py`; associated commits `c0615a296d44`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Bugfix] Fix Qwen2.5-Omni and Qwen3-Omni mixed-modality embed regression". The diff centers on `tests/models/multimodal/processing/test_qwen2_5_omni_embed.py`, `vllm/model_executor/models/qwen2_5_omni_thinker.py`, `vllm/model_executor/models/qwen3_omni_moe_thinker.py`. PR body context: ## Summary Fixes #34506 — regression introduced in #33605 where mixed modalities (audio + image + video) no longer worked correctly for Qwen2.5-Omni: the model failed to recogni...
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Fix missing sequence_lengths in qwen3_omni_moe_thinker". The diff centers on `vllm/model_executor/models/qwen3_omni_moe_thinker.py`. PR body context: ## Purpose PR #34580 added a `sequence_lengths` parameter to `Qwen2_5_VisionAttention.forward()` for the FlashInfer cuDNN backend and updated callers in `qwen3_vl.py` and `qwen2...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen3_asr_realtime.py`; associated commits `36bf2131816e`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Bugfix] Add missing dynamic_arg_dims for Qwen3-ASR torch.compile". The diff centers on `vllm/model_executor/models/qwen3_asr_realtime.py`. PR body context: ## Summary `Qwen3ASRForConditionalGeneration` (and its realtime subclass `Qwen3ASRRealtimeGeneration`) crashes during `torch.compile` warmup when `--enforce-eager` is not set. T...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen_vl.py`, `vllm/tokenizers/qwen_vl.py`; associated commits `719634815791`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Fix Qwen-VL tokenizer implementation". The diff centers on `vllm/tokenizers/qwen_vl.py`, `vllm/model_executor/models/qwen_vl.py`, `vllm/renderers/qwen_vl.py`. PR body context: ## Purpose The recent Renderer refactorings broke `Qwen/Qwen-VL` because `_get_tokenizer_without_image_pad` wasn't being used by the Renderer. To solve this, I implemented a cus...
...
- Status/date: merged / 2026-03-05
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen3_omni_moe_thinker.py`; associated commits `3ee68590c7fa`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR changes model-related implementation. Title: "refactor funasr model.". The diff centers on `vllm/model_executor/models/qwen3_omni_moe_thinker.py`. PR body context: This PR is a small refactor for the FunASR model. Please take a look @DarkLight1337 @Isotr0py HF's allendou/Fun-ASR-Nano-2512-vllm has also been updated.
...
- Status/date: merged / 2026-03-05
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen2_5_omni_thinker.py`, `vllm/model_executor/models/qwen2_audio.py`, `vllm/model_executor/models/qwen3_omni_moe_thinker.py`; associated commits `e998fa76b99a`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[BUGFIX]Fix Qwen-Omni models audio max_token_per_item estimation error leading to encoder_cache_size is 0". The diff centers on `vllm/model_executor/models/qwen2_5_omni_thinker.py`, `vllm/model_executor/models/qwen3_omni_moe_thinker.py`, `vllm/model_executor/models/qwen2_audio.py`. PR body context: **Bug Description** When running offline inference with the Qwen2.5-Omni model using audio inputs `python examples/offline_inference/audio_language.py --model-type qwen2_5_omni`...
...
- Trace source: `git log --name-only -- <model-files>` found it through `tests/models/multimodal/processing/test_qwen3_vl.py`, `vllm/model_executor/models/qwen3_vl.py`; associated commits `724759684cd9`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Fix Qwen3-VL timestamp mismatch when using num_frames without fps". The diff centers on `tests/models/multimodal/processing/test_qwen3_vl.py`, `vllm/model_executor/models/qwen3_vl.py`. PR body context: ## Summary Fixes #35909. When `num_frames` is provided via `mm_processor_kwargs` without `fps`, `_get_video_second_idx` was falling back to the default fps (2) to compute the nu...
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Fix Qwen2.5-omni/Qwen3-omni mm_processor cache for audio_in_video request". The diff centers on `vllm/model_executor/models/qwen2_5_omni_thinker.py`, `vllm/model_executor/models/qwen3_omni_moe_thinker.py`. PR body context: ## Purpose - Following PR for https://github.com/vllm-project/vllm/pull/36319 - Actually, Qwen2.5-Omni and Qwen3-Omni processor's `apply_prompt_updates` is incorrect for `audio_...
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Fix Qwen2.5-Omni/Qwen3-Omni use_audio_in_video with multi-video inputs". The diff centers on `vllm/model_executor/models/qwen2_5_omni_thinker.py`, `vllm/model_executor/models/qwen3_omni_moe_thinker.py`. PR body context: ## Purpose - Fix index out of range when Qwen2.5-Omni/Qwen3-omni received multiple video inputs, because ` audio_in_video_item_idx + item_idx` is incorrect: ## Test Plan ## Test...
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "Remove unused EVS functions in qwen3_vl.py". The diff centers on `vllm/model_executor/models/qwen3_vl.py`. PR body context: ## Purpose In qwen3_vl.py, functions including _get_evs_mask_segments, _extract_frame_offsets_from_mask, and _get_actual_frame_token_counts were introduced in #29752. However, t...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen3_vl.py`; associated commits `738d0a281fab`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Fix incorrect use of merge_size in Qwen3-VL video timestamp calculation". The diff centers on `vllm/model_executor/models/qwen3_vl.py`. PR body context: ## Purpose This PR fixes an incorrect use of `merge_size` in Qwen3-VL video timestamp processing. (This bug also affects Qwen3.5) Previously, `_get_video_second_idx` passed `vid...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen3_asr.py`, `vllm/model_executor/models/qwen3_omni_moe_thinker.py`; associated commits `8d0f908b98cd`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[Model] Implement LoRA support for Qwen3ASRForConditionalGeneration". The diff centers on `vllm/model_executor/models/qwen3_asr.py`, `vllm/model_executor/models/qwen3_omni_moe_thinker.py`. PR body context: ## Purpose This PR adds LoRA support for `Qwen3ASRForConditionalGeneration` model. For this to work for the audio tower, I had to make a few additional changes: - Implement `get...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/qwen3_vl.py`; associated commits `80118853f42a`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR adds or enables a model support/runtime surface. Title: "[MM][Perf][CG] Support ViT full CUDA graph for Qwen3-VL video inference". The diff centers on `vllm/model_executor/models/qwen3_vl.py`. PR body context: ## Purpose Following https://github.com/vllm-project/vllm/pull/35963 (only supports image inference), this PR continues to work on it to support video inference for Qwen3-VL. **...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/glmasr.py`; associated commits `aeee7ef93910`
...
- Motivation: For Qwen VLM/Omni/ASR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Fix k_proj's bias for GLM-ASR". The diff centers on `vllm/model_executor/models/glmasr.py`. PR body context: ## Purpose GLM-ASR has `bias = true` only for `q_proj` and `v_proj` not for `k_proj` So, if we use `QKVParallelLinear` to fuse its `qkv_proj`, the bias part for `k_proj` will re...
