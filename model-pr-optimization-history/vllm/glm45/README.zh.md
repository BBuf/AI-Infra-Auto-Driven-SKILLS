...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/glm4_moe.py`；关联提交 `85bda9e7d053`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+2/-3，可读 patch 26 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 GLM-4.5 修复已暴露的启动、加载、解析或数值问题，标题为「remove GLM-4 quantization wrong Code」，变更集中在 `vllm/model_executor/models/glm4_moe.py`。PR 描述补充为：need to remove this line
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/glm4_moe.py`；关联提交 `d3c18c9cb0b6`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+2/-3，可读 patch 12 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 GLM-4.5 优化关键推理路径或后端选择，标题为「fuse fp32 for GLM-4.5 e_score_correction_bias」，变更集中在 `vllm/model_executor/models/glm4_moe.py`。PR 正文没有提供额外背景，判断主要来自标题、文件列表和 patch。
...
- 动机: 该 PR 围绕 GLM-4.5 修复已暴露的启动、加载、解析或数值问题，标题为「[Misc] Modify the organization of GLM series」，变更集中在 `docs/models/supported_models.md`, `tests/models/registry.py`, `tests/models/multimodal/generation/test_common.py`。PR 描述补充为：# Essential Elements of an Effective PR Description Checklist - [ ] The purpose of the PR, such as "Fix some issue (link existing issues this PR will resolve)". - [ ] The test p...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/glm4_moe.py`；关联提交 `6fa41e0c32f3`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+4/-3，可读 patch 35 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 GLM-4.5 优化关键推理路径或后端选择，标题为「self.gate dtype update for GLM-4.5」，变更集中在 `vllm/model_executor/models/glm4_moe.py`。PR 描述补充为：The entire self.gate module needs to remain in float32 to ensure benchmark performance for GLM-4.5 and GLM-4.5V during propagation.
...
- 动机: 该 PR 围绕 GLM-4.5 调整模型相关实现，标题为「not tie_word_embeddings for glm-4.5 and glm-4.5v」，变更集中在 `vllm/model_executor/models/glm4_moe.py`。PR 正文没有提供额外背景，判断主要来自标题、文件列表和 patch。
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/glm4_moe.py`；关联提交 `a6022e6fbcbd`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 5 个文件，+13/-6，可读 patch 61 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 GLM-4.5 调整模型相关实现，标题为「GLM-4.5V with new class name at transformers」，变更集中在 `vllm/model_executor/models/glm4_moe.py`。PR 描述补充为：As this PR mention: https://github.com/huggingface/transformers/pull/39805
...
- 动机: 该 PR 围绕 GLM-4.5 修复已暴露的启动、加载、解析或数值问题，标题为「[Model] Modify the gate implementation of glm4_moe」，变更集中在 `vllm/model_executor/models/glm4_moe.py`。PR 描述补充为：# Essential Elements of an Effective PR Description Checklist - [ ] The purpose of the PR, such as "Fix some issue (link existing issues this PR will resolve)". - [ ] The test p...
...
- 动机: 该 PR 围绕 GLM-4.5 补齐模型支持入口或运行时能力，标题为「feat: add triton fused moe config for GLM-4.5-Air-FP8 on B200」，变更集中在 `vllm/model_executor/layers/fused_moe/configs/E=128,N=704,device_name=NVIDIA_B200,dtype=fp8_w8a8.json`。PR 描述补充为：## Purpose Add fused MoE config for GLM-4.5-Air-FP8 on NVIDIA B200, tp = 2 ## Test Result With config: Without config:
...
- 动机: 该 PR 围绕 GLM-4.5 补齐模型支持入口或运行时能力，标题为「[Doc] Add documentation for GLM-4.5 series models: tool-calling and reasoning parser」，变更集中在 `docs/features/reasoning_outputs.md`, `docs/features/tool_calling.md`。PR 描述补充为：Add documentation for GLM-4.5 series models: tool-calling and reasoning parser.
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/glm4_moe.py`；关联提交 `c15309a730fa`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+55/-30，可读 patch 114 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 GLM-4.5 优化关键推理路径或后端选择，标题为「[Model] Apply SharedFusedMoE to glm4_moe.」，变更集中在 `vllm/model_executor/models/glm4_moe.py`。PR 描述补充为：## Purpose The class `SharedFusedMoE` was proposed by @bnellnm in PR #23273. The model glm4_moe has shared experts but we don't use `SharedFusedMoE` for glm4_moe, I'm not sure w...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/glm4_moe.py`；关联提交 `b1ded114b976`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+7/-5，可读 patch 40 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 GLM-4.5 补强部署文档、测试或 CI 验证面，标题为「Update GLM-4.5 Doc transformers version」，变更集中在 `vllm/model_executor/models/glm4_moe.py`。PR 描述补充为：Update for GLM-4.5 Doc transformers version
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/glm4_moe_mtp.py`；关联提交 `d3ade61e429f`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+11/-4，可读 patch 23 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 GLM-4.5 修复已暴露的启动、加载、解析或数值问题，标题为「[Model] fix glm4_moe_mtp load weights with GLM-4.6 checkpoint.」，变更集中在 `vllm/model_executor/models/glm4_moe_mtp.py`。PR 描述补充为：## Purpose As described in issue#25993, when serving GLM-4.6 with mtp using the following command: It raises the following error: The root cause is that the GLM-4.6 checkpoint d...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/glm4_moe_mtp.py`；关联提交 `ab01cd14e5e2`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+3/-4，可读 patch 17 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 GLM-4.5 补齐模型支持入口或运行时能力，标题为「[BugFix] Fix glm4_moe_mtp load weights bug」，变更集中在 `vllm/model_executor/models/glm4_moe_mtp.py`。PR 描述补充为：## Purpose As comment in issue#25993,PR#27597 introduced bugs causing GLM-4.5/GLM-4.6 serving with mtp error because: 1. spec_layer is overwritten by later None assignment in "e...
