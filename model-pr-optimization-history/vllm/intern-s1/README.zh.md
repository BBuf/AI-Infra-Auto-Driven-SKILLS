...
- 动机: 该 PR 围绕 Intern-S1 补齐模型支持入口或运行时能力，标题为「[VLM] Add video support for Intern-S1」，变更集中在 `vllm/model_executor/models/interns1.py`。PR 描述补充为：## Essential Elements of an Effective PR Description Checklist - [x] The purpose of the PR, such as "Fix some issue (link existing issues this PR will resolve)". - [ ] The test...
...
- 动机: 该 PR 围绕 Intern-S1 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] Fix wrong method name in Intern-S1 image processor」，变更集中在 `vllm/model_executor/models/interns1.py`。PR 描述补充为：# Essential Elements of an Effective PR Description Checklist - [x] The purpose of the PR, such as "Fix some issue (link existing issues this PR will resolve)". - [ ] The test p...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/interns1.py`；关联提交 `56d04089ef50`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+50/-51，可读 patch 167 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Intern-S1 调整模型相关实现，标题为「Migrate Interns1 inputs to TensorSchema」，变更集中在 `vllm/model_executor/models/interns1.py`。PR 描述补充为：## Purpose This PR migrates Interns1 inputs from a TypedDict-based definition to a structured TensorSchema model with runtime shape validation. This brings it in line with recen...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/interns1.py`；关联提交 `03858e6d1c85`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+68/-3，可读 patch 128 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Intern-S1 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] Fix InternS1 video processing after Transformers v4.56」，变更集中在 `vllm/model_executor/models/interns1.py`。PR 描述补充为：## Purpose - FIX #25451 ## Test Plan ## Test Result --- Essential Elements of an Effective PR Description Checklist - [x] The purpose of the PR, such as "Fix some issue (link ex...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/interns1_vit.py`；关联提交 `acc78aeb88c8`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+3/-4，可读 patch 20 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Intern-S1 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] Fix interns1-vit qk norm code path」，变更集中在 `vllm/model_executor/models/interns1_vit.py`。PR 描述补充为：## Purpose - Fix https://github.com/InternLM/Intern-S1/issues/29 ## Test Plan ## Test Result --- Essential Elements of an Effective PR Description Checklist - [x] The purpose of...
...
- 动机: 该 PR 围绕 Intern-S1 补齐模型支持入口或运行时能力，标题为「[Models] Intern-S1-Pro」，变更集中在 `vllm/model_executor/models/interns1_pro.py`。PR 描述补充为：## Purpose Intern-S1-Pro model support. ## Test Plan ## Test Result --- Essential Elements of an Effective PR Description Checklist - [ ] The purpose of the PR, such as "Fix som...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/interns1_pro.py`；关联提交 `192ad4648b20`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 6 个文件，+43/-22，可读 patch 163 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Intern-S1 补齐模型支持入口或运行时能力，标题为「[Bugfix] Fix interns1-pro initialization and PP」，变更集中在 `vllm/model_executor/models/interns1_pro.py`。PR 描述补充为：## Purpose - Fix broken InternS1-PRO intialization because the previous PR is drafted on an old version vLLM. 😅 - Also fix PP for InternS1-PRO ## Test Plan ## Test Result --- Es...
