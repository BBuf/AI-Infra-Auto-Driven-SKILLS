...
- 动机: 该 PR 围绕 MiniMax M2 Series 补齐模型支持入口或运行时能力，标题为「[Model][MiniMaxText01] Support MiniMaxText01 model inference」，变更集中在 `vllm/model_executor/models/minimax_text_01.py`。PR 描述补充为：# Purpose This PR is intended to support the MiniMaxText01 model inference. It can run on a single machine with 8xH800 and 8xH20, where a single H800 machine can handle a maximu...
...
- 动机: 该 PR 围绕 MiniMax M2 Series 补齐模型支持入口或运行时能力，标题为「[Model] support MiniMax-VL-01 model」，变更集中在 `vllm/model_executor/models/minimax_vl_01.py`, `tests/models/multimodal/processing/test_minimax_vl_01.py`, `vllm/transformers_utils/configs/minimax_vl_01.py`。PR 描述补充为：CLOSES #12073 This PR is intended to support the MiniMaxVL01 model inference. It adopts the “ViT-MLP-LLM” framework, which is a commonly used technique in the field of multimoda...
...
- 状态/时间: merged / 2025-04-29
- 反查来源: `git log --name-only -- <model-files>` 反查到 `tests/models/multimodal/processing/test_minimax_vl_01.py`, `vllm/model_executor/models/minimax_vl_01.py`；关联提交 `00ee37efa236`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+38/-283，可读 patch 442 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 MiniMax M2 Series 补齐模型支持入口或运行时能力，标题为「[Bugfix] Clean up MiniMax-VL and fix processing」，变更集中在 `vllm/model_executor/models/minimax_vl_01.py`, `tests/models/multimodal/processing/test_minimax_vl_01.py`。PR 描述补充为：Follow-up to #16328 @qscqesze can you verify this fix? Also, can you open another PR after this one to add example scripts under `examples/offline_inference/vision_language.py`...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/minimax_text_01.py`；关联提交 `a24cb91600bd`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+3/-3，可读 patch 27 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 MiniMax M2 Series 修复已暴露的启动、加载、解析或数值问题，标题为「[Model] Fix minimax model cache & lm_head precision」，变更集中在 `vllm/model_executor/models/minimax_text_01.py`。PR 描述补充为：Change the precision of the MiniMax model in vLLM: update the LM head and KV cache from bfloat16 (bf16) to float32 (fp32). Purpose: To improve numerical stability and output acc...
...
- 动机: 该 PR 围绕 MiniMax M2 Series 补齐模型支持入口或运行时能力，标题为「[Model] Add support for MiniMaxM1ForCausalLM (shares architecture with MiniMaxText01ForCausalLM)」，变更集中在 `tests/models/registry.py`, `docs/models/supported_models.md`, `vllm/model_executor/models/registry.py`。PR 描述补充为：We would like to propose official support for the MiniMaxM1ForCausalLM model within vLLM. This model shares the exact same architecture and inference behavior as MiniMaxText01Fo...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `examples/tool_chat_template_minimax_m1.jinja`；关联提交 `363528de27db`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 5 个文件，+842/-1，可读 patch 866 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 MiniMax M2 Series 补齐模型支持入口或运行时能力，标题为「[Feature] Support MiniMax-M1 function calls features」，变更集中在 `examples/tool_chat_template_minimax_m1.jinja`。PR 描述补充为：Support MiniMax-M1 function calls features How to use: test_scripts: Outpt: Streaming Test: Output:
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/minimax_text_01.py`；关联提交 `922f316441ce`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+36/-11，可读 patch 101 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 MiniMax M2 Series 补齐模型支持入口或运行时能力，标题为「[Model] Support HF format of minimax」，变更集中在 `vllm/model_executor/models/minimax_text_01.py`。PR 描述补充为：## Purpose https://github.com/vllm-project/vllm/pull/20199 set us up to test the older format of `MiniMaxAI/MiniMax-Text-01` we initially implemented. This PR implements the sma...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/minimax_text_01.py`；关联提交 `6ade99eafa37`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 5 个文件，+234/-42，可读 patch 448 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 MiniMax M2 Series 补齐模型支持入口或运行时能力，标题为「[V1] [Hybrid] Support Minimax-Text-01 in V1」，变更集中在 `vllm/model_executor/models/minimax_text_01.py`。PR 描述补充为：# Essential Elements of an Effective PR Description Checklist - [x] The purpose of the PR, such as "Fix some issue (link existing issues this PR will resolve)". - [x] The test p...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/minimax_text_01.py`；关联提交 `75531a6c1342`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 23 个文件，+467/-87，可读 patch 1435 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 MiniMax M2 Series 补齐模型支持入口或运行时能力，标题为「[V1] [Hybrid] Support using float32 for state in Hybrid Models (Mamba2, Mamba1, Minimax)」，变更集中在 `vllm/model_executor/models/minimax_text_01.py`。PR 描述补充为：## Purpose This PR adds support for customizing the dtype of the state stored for models based on mamba/mamba2/linear attention. It introduces a new CLI argument `mamba_ssm_cach...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/minimax_vl_01.py`；关联提交 `31fd3265c8b2`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+123/-32，可读 patch 258 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 MiniMax M2 Series 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] Fix broken Minimax-01-VL model」，变更集中在 `vllm/model_executor/models/minimax_vl_01.py`。PR 描述补充为：# Essential Elements of an Effective PR Description Checklist - [x] The purpose of the PR, such as "Fix some issue (link existing issues this PR will resolve)". - [ ] The test p...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/minimax_text_01.py`；关联提交 `dd589322801e`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+98/-137，可读 patch 387 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 MiniMax M2 Series 补齐模型支持入口或运行时能力，标题为「[V1] [Hybrid] Enable compile and piecewise CUDA graph for MiniMax-Text models」，变更集中在 `vllm/model_executor/models/minimax_text_01.py`。PR 描述补充为：# Essential Elements of an Effective PR Description Checklist - [x] The purpose of the PR, such as "Fix some issue (link existing issues this PR will resolve)". - [x] The test p...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/minimax_text_01.py`；关联提交 `4071c76cf3cf`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+448/-410，可读 patch 917 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 MiniMax M2 Series 调整模型相关实现，标题为「[V1] [Hybrid] Move MiniMaxLinearAttention into layers/mamba」，变更集中在 `vllm/model_executor/models/minimax_text_01.py`。PR 描述补充为：## Purpose This PR just moves the `MinimaxLinearAttention` layer into the `layers/mamba` directory, to be consistent with the other mamba-like layers (e.g., mamba1, mamba2, shor...
...
- 动机: 该 PR 围绕 MiniMax M2 Series 补齐模型支持入口或运行时能力，标题为「[Model][MiniMax-M2] Support MiniMax-M2 Model」，变更集中在 `vllm/model_executor/models/minimax_m2.py`, `vllm/reasoning/minimax_m2_reasoning_parser.py`。PR 描述补充为：## Purpose This PR is intended to support the MiniMax-M2 model inference. It has been tested on a single machine with both 8x96GB and 4x96GB GPU configurations. ## Deploy Deploy...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/minimax_m2.py`；关联提交 `5980604c44d8`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+2/-3，可读 patch 13 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 MiniMax M2 Series 修复已暴露的启动、加载、解析或数值问题，标题为「Fix MiniMax-M2 copyright」，变更集中在 `vllm/model_executor/models/minimax_m2.py`。PR 描述补充为：## Purpose Fix MiniMax-M2 copyright. ## Test Plan ## Test Result --- Essential Elements of an Effective PR Description Checklist - [ ] The purpose of the PR, such as "Fix some i...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/minimax_m2.py`；关联提交 `d6704dd099b7`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+1/-19，可读 patch 41 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 MiniMax M2 Series 修复已暴露的启动、加载、解析或数值问题，标题为「Fix MiniMax-M2 rmsnorm precision and remove useless code」，变更集中在 `vllm/model_executor/models/minimax_m2.py`。PR 描述补充为：## Purpose - Fix rmsnorm precision - Remove useless code ## Test Plan ## Test Result --- Essential Elements of an Effective PR Description Checklist - [ ] The purpose of the PR,...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/minimax_m2.py`；关联提交 `d017bceb08ea`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-1，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 MiniMax M2 Series 修复已暴露的启动、加载、解析或数值问题，标题为「[BugFix] Fix minimax m2 model rotary_dim」，变更集中在 `vllm/model_executor/models/minimax_m2.py`。PR 描述补充为：## Purpose After #29966, get_rope always reads partial_rotary_factor from the configuration and performs the multiplication again, even if rotary_dim has already been scaled. Th...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `tests/reasoning/test_minimax_m2_append_reasoning_parser.py`, `tests/reasoning/test_minimax_m2_reasoning_parser.py`, `vllm/reasoning/minimax_m2_reasoning_parser.py`；关联提交 `6299628d326f`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+468/-0，可读 patch 484 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 MiniMax M2 Series 修复已暴露的启动、加载、解析或数值问题，标题为「[bugfix] fix MiniMaxM2ReasoningParser streaming output not separating reasoning_content.」，变更集中在 `tests/reasoning/test_minimax_m2_reasoning_parser.py`, `tests/reasoning/test_minimax_m2_append_reasoning_parser.py`, `vllm/reasoning/minimax_m2_reasoning_parser.py`。PR 描述补充为：## Summary Fix `minimax_m2` reasoning parser not separating `content` and `reasoning_content` in streaming mode. ## Problem MiniMax-M2 models don't generate ` ` start token, onl...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/tool_parsers/minimax_m2_tool_parser.py`；关联提交 `20fda431515d`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+137/-4，可读 patch 186 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 MiniMax M2 Series 补齐模型支持入口或运行时能力，标题为「[Bugfix][Frontend] Prevent IndexError in MiniMax M2 tool parser during streaming extraction」，变更集中在 `tests/tool_use/test_minimax_m2_tool_parser.py`, `vllm/tool_parsers/minimax_m2_tool_parser.py`。PR 描述补充为：Fix https://github.com/vllm-project/vllm/issues/30554. Prevent IndexError in MiniMax M2 tool parser during streaming extraction. Add args str to streamed_args_for_tool field.
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/tool_parsers/minimax_m2_tool_parser.py`；关联提交 `c02a2705f9ce`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+167/-48，可读 patch 257 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 MiniMax M2 Series 补齐模型支持入口或运行时能力，标题为「Update MiniMax-M2 ToolCall and add MiniMax-M2.1 in Docs」，变更集中在 `vllm/tool_parsers/minimax_m2_tool_parser.py`。PR 描述补充为：## Purpose The MiniMax-M2 tool call parser supports `anyOf`, `oneOf`, `allOf`, type arrays, and enum fields. And add MiniMax-M2.1 to the documentation. ## Test Plan ## Test Resu...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/minimax_m2.py`；关联提交 `5bc664110f12`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+25/-2，可读 patch 41 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 MiniMax M2 Series 优化关键推理路径或后端选择，标题为「Optimize QKNorm for MiniMax-M2/M2.1」，变更集中在 `vllm/model_executor/models/minimax_m2.py`。PR 描述补充为：## Purpose Fuses QKNorm in tensor-parallel mode so that only a single all_reduce is launched, reducing communication overhead. Part of #31478. ## Test Plan Benchmarks were condu...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/tool_parsers/minimax_m2_tool_parser.py`；关联提交 `19b251fe3d26`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+2/-5，可读 patch 19 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 MiniMax M2 Series 修复已暴露的启动、加载、解析或数值问题，标题为「Fix optional parameter parsing in MiniMax M2 tool parser #32278」，变更集中在 `vllm/tool_parsers/minimax_m2_tool_parser.py`。PR 描述补充为：## Purpose #32278 ## Test Plan ## Test Result main branch: "logprobs":null, \"logfile\": \"/tmp/foo\"}" --- Essential Elements of an Effective PR Description Checklist - [ ] The...
...
- 动机: 该 PR 围绕 MiniMax M2 Series 补齐模型支持入口或运行时能力，标题为「feat: Complete LoRA support for MiniMaxM2 Fixes #32736」，变更集中在 `vllm/model_executor/models/minimax_m2.py`。PR 描述补充为：FIX #32736 This PR builds upon #31703 by adding the missing components to enable full LoRA support for MiniMaxM2ForCausalLM. Changes: - Add SupportsLoRA interface to MiniMaxM2Fo...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/reasoning/minimax_m2_reasoning_parser.py`；关联提交 `7fea7250a46c`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+6/-1，可读 patch 17 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 MiniMax M2 Series 修复已暴露的启动、加载、解析或数值问题，标题为「[Bug] Fix missing tag after tool call in MiniMax 2.1」，变更集中在 `vllm/reasoning/minimax_m2_reasoning_parser.py`。PR 描述补充为：Fix: #35349 ## Purpose This PR improves the reasoning end detection logic in MiniMaxM2AppendThinkReasoningParser.is_reasoning_end. After a successful tool call, the following li...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `tests/tool_parsers/test_minimax_m2_tool_parser.py`, `vllm/tool_parsers/minimax_m2_tool_parser.py`；关联提交 `8647c6cf510b`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+534/-532，可读 patch 1119 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 MiniMax M2 Series 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] Fix minimax_m2 tool parser when stream interval > 1」，变更集中在 `vllm/tool_parsers/minimax_m2_tool_parser.py`, `tests/tool_parsers/test_minimax_m2_tool_parser.py`, `tests/tool_use/test_minimax_m2_tool_parser.py`。PR 描述补充为：## Purpose Fix MiniMax M2 tool parser dropping arguments when stream_interval > 1. ### Background MiniMax M2 uses XML-based tool call format: The old parser used an incremental...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/minimax_text_01.py`；关联提交 `17808394bc48`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+235/-219，可读 patch 527 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 MiniMax M2 Series 修复已暴露的启动、加载、解析或数值问题，标题为「standardize load_weights using AutoWeightsLoader for kimi_linear and minimax_text_01」，变更集中在 `vllm/model_executor/models/minimax_text_01.py`。PR 描述补充为：FIX (partial) #15697 ## Test Plan Verified the refactor with a mock-weight loading script using a "Tiny Model Hack" (reducing layers to 1 for fast validation): Mock Weights: Gen...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/minimax_m2.py`；关联提交 `74056039b776`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+11/-0，可读 patch 18 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 MiniMax M2 Series 修复已暴露的启动、加载、解析或数值问题，标题为「Fix minimax m2.5 nvfp4 kv scales weight loading」，变更集中在 `vllm/model_executor/models/minimax_m2.py`。PR 描述补充为：## Purpose Fix kv scale weight loading issue in minimax m2.5 nvfp4: ## Test Plan ## Test Result --- Essential Elements of an Effective PR Description Checklist - [ ] The purpose...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/minimax_m2.py`；关联提交 `63babd17f1b1`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 5 个文件，+137/-10，可读 patch 238 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 MiniMax M2 Series 补齐模型支持入口或运行时能力，标题为「[Model][Quantization] Add GGUF support for MiniMax-M2.1」，变更集中在 `vllm/model_executor/models/minimax_m2.py`。PR 描述补充为：## Purpose FIX #28724 Add GGUF loading support for MiniMax-M2.1 (456B MoE, 45.9B active, 256 experts, 8 active per token). This enables serving GGUF-quantized MiniMax-M2.1 check...
...
- 动机: 该 PR 围绕 MiniMax M2 Series 补齐模型支持入口或运行时能力，标题为「MiniMax-M2: add Eagle3 speculative decoding support」，变更集中在 `vllm/model_executor/models/minimax_m2.py`。PR 描述补充为：- Add SupportsEagle3 interface to MiniMaxM2ForCausalLM with aux hidden state collection in MiniMaxM2Model.forward() - Register Eagle3MiniMaxM2ForCausalLM in speculative decoding...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `tests/kernels/core/test_minimax_reduce_rms.py`, `vllm/compilation/passes/fusion/minimax_qk_norm_fusion.py`, `vllm/model_executor/models/minimax_m2.py`；关联提交 `ecd1ea13634e`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 14 个文件，+1861/-4，可读 patch 1936 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 MiniMax M2 Series 优化关键推理路径或后端选择，标题为「[Kernel] Porting the TRTLLM minimax_allreduce_rms kernels」，变更集中在 `vllm/model_executor/models/minimax_m2.py`, `vllm/compilation/passes/fusion/minimax_qk_norm_fusion.py`, `tests/kernels/core/test_minimax_reduce_rms.py`。PR 描述补充为：## Purpose See: https://github.com/NVIDIA/TensorRT-LLM/pull/12163 ## Plan - [x] Clean up the code - [x] Verify the accuracy on GSM8K Benchmark - [x] Performance Benchmark - [x]...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/parser/minimax_m2_parser.py`；关联提交 `d2130a47bb97`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+5/-2，可读 patch 25 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 MiniMax M2 Series 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix]: Fix MinimaxM2ToolParser missing tools parameter」，变更集中在 `vllm/parser/minimax_m2_parser.py`。PR 描述补充为：## Purpose [Bugfix]: Fix MinimaxM2ToolParser missing tools parameter ## Test Plan before ## Test Result --- Essential Elements of an Effective PR Description Checklist - [ ] The...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/parser/minimax_m2_parser.py`；关联提交 `8d7c9628337a`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+9/-3，可读 patch 21 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 MiniMax M2 Series 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] Accept **kwargs in MiniMaxM2Parser.__init__()」，变更集中在 `vllm/parser/minimax_m2_parser.py`。PR 描述补充为：Fixes #39847. ### Summary `MiniMaxM2Parser.__init__()` does not accept `**kwargs`, causing a `TypeError` on every streaming chat completion request when serving MiniMax M2 model...
