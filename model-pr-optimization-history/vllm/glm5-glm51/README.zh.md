...
- 动机: 标题「[Model] GLM adaptation」；模型线: GLM-5/5.1；类别: 模型实现调整；主要 diff: `vllm/model_executor/models/deepseek_v2.py`, `tests/models/registry.py`, `tests/models/test_initialization.py`；未提供可用技术摘要。
...
- 动机: 标题「[Bugfix] Fix MTP accuracy for GLM-5」；模型线: GLM-5/5.1；类别: 缺陷修复；主要 diff: `vllm/v1/spec_decode/eagle.py`；技术摘要: 覆盖「[Bugfix] Fix MTP accuracy for GLM-5」；主要实现面是 `vllm/v1/spec_decode/eagle.py`。下方保留文件级证据、代码摘录和验证风险。
