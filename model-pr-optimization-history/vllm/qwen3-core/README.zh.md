...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[Model] Add Qwen3 and Qwen3MoE」，变更集中在 `vllm/model_executor/models/qwen3_moe.py`, `vllm/model_executor/models/qwen3.py`。PR 描述补充为：## Description Recently, I have submitted a pull request to Hugging Face Transformers containing the implementation of the Qwen3 and Qwen3MoE model. I would also like to contrib...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/qwen3_moe.py`；关联提交 `5a1e1c8353b9`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+220/-198，可读 patch 514 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3 Core 修复已暴露的启动、加载、解析或数值问题，标题为「[Model] use AutoWeightsLoader for phimoe,qwen2_moe,qwen3_moe」，变更集中在 `vllm/model_executor/models/qwen3_moe.py`。PR 描述补充为：FIX https://github.com/vllm-project/vllm/issues/15697 - Qwen3MoeForCausalLM Because can't download Qwen/Qwen3-MoE-15B-A2B this model, so not test. - Qwen2MoeForCausalLM - PhiMoE...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/qwen3.py`, `vllm/model_executor/models/qwen3_moe.py`；关联提交 `f80ae5bdcfa7`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 7 个文件，+19/-15，可读 patch 97 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3 Core 优化关键推理路径或后端选择，标题为「[Kernel] Use fused rmsnorm for some models like qwen3 series」，变更集中在 `vllm/model_executor/models/qwen3.py`, `vllm/model_executor/models/qwen3_moe.py`。PR 描述补充为：Some models like qwen3 series still use forward_native method when q and k need to be normalized. The reason why fused rmsnorm kernel is not being used is that before q norm and...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/qwen3_moe.py`；关联提交 `63dc3426e078`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+11/-0，可读 patch 18 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[Model] Add packed_modules_mapping for Qwen3-MOE」，变更集中在 `vllm/model_executor/models/qwen3_moe.py`。PR 正文没有提供额外背景，判断主要来自标题、文件列表和 patch。
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[New Model]: Support Qwen3 Embedding & Reranker」，变更集中在 `vllm/model_executor/models/qwen3.py`。PR 描述补充为：# Summary - Qwen3 Embedding - Qwen/Qwen3-Embedding-0.6B - Qwen/Qwen3-Embedding-4B - Qwen/Qwen3-Embedding-8B - Qwen3 Reranker - Qwen/Qwen3-Reranker-0.6B - Qwen/Qwen3-Reranker-4B...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/qwen3_moe.py`；关联提交 `e41bf15cd04e`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-1，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3 Core 修复已暴露的启动、加载、解析或数值问题，标题为「[Chore]: qwen3-moe-type-hints-mistake」，变更集中在 `vllm/model_executor/models/qwen3_moe.py`。PR 描述补充为：## Essential Elements of an Effective PR Description Checklist - [ ] The purpose of the PR, such as "Fix some issue (link existing issues this PR will resolve)". - [ ] The test...
...
- 动机: 该 PR 围绕 Qwen3 Core 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] Skip loading extra parameters for modelopt Qwen3 MoE model」，变更集中在 `vllm/model_executor/models/qwen3_moe.py`。PR 描述补充为：## Essential Elements of an Effective PR Description Checklist - [x] The purpose of the PR, such as "Fix some issue (link existing issues this PR will resolve)". - [x] The test...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/qwen3_moe.py`；关联提交 `d979dd6bebb1`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+142/-24，可读 patch 273 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[Feature][EPLB] Add eplb support for Qwen3」，变更集中在 `vllm/model_executor/models/qwen3_moe.py`。PR 描述补充为：## Essential Elements of an Effective PR Description Checklist - [x] The purpose of the PR, such as "Fix some issue (link existing issues this PR will resolve)". - [x] The test...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/qwen3.py`, `vllm/model_executor/models/qwen3_moe.py`；关联提交 `7377131a2ccb`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+60/-31，可读 patch 176 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[Qwen3] Enable dual-chunk-attention support for Qwen3 models.」，变更集中在 `vllm/model_executor/models/qwen3.py`, `vllm/model_executor/models/qwen3_moe.py`。PR 描述补充为：# Essential Elements of an Effective PR Description Checklist - [ ] The purpose of the PR, such as "Fix some issue (link existing issues this PR will resolve)". - [ ] The test p...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/qwen3_moe.py`；关联提交 `d57dc2364e88`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+58/-37，可读 patch 129 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「Add ModelOpt Qwen3 nvfp4 support」，变更集中在 `vllm/model_executor/models/qwen3_moe.py`。PR 描述补充为：## Essential Elements of an Effective PR Description Checklist - [x] The purpose of the PR, such as "Fix some issue (link existing issues this PR will resolve)". - [x] The test...
...
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-1，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3 Core 修复已暴露的启动、加载、解析或数值问题，标题为「[BUGFIX] KeyError 'layers.14.mlp.gate.g_idx' for Qwen3-MoE with GPTQ on ROCm」，变更集中在 `vllm/model_executor/models/qwen3_moe.py`。PR 描述补充为：Description: This PR fixes a KeyError for 'layers.14.mlp.gate.g_idx' that was popping up when loading GPTQ-quantized Qwen3-MoE models, particularly in ROCm setups. The problem s...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「Fix GGUF loader for Qwen3 MoE.」，变更集中在 `vllm/model_executor/models/qwen3_moe.py`。PR 描述补充为：## Purpose Despite upstream repositories (gguf-py, transformers) having added support for Qwen3 MoE GGUF quantization, vLLM GGUF loading is still broken. This PR aims to fix the...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/qwen3_moe.py`；关联提交 `4f510bc2a175`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-5，可读 patch 20 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3 Core 优化关键推理路径或后端选择，标题为「[Model] Removes redundant all-reduce operation in Qwen3MoeSparseMoeBlock」，变更集中在 `vllm/model_executor/models/qwen3_moe.py`。PR 描述补充为：## Purpose Remove redundant all-reduce operation and reuse the all-reduce implementation provided by `FusedMoE` (see `vllm/model_executor/layers/fused_moe/layer.py#L1672`). ## T...
...
- 动机: 该 PR 围绕 Qwen3 Core 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] Fix Qwen3 MoE GPTQ inference」，变更集中在 `vllm/model_executor/models/qwen3_moe.py`。PR 描述补充为：## Purpose - Fix #23467 ## Test Plan ## Test Result ## (Optional) Documentation Update --- Essential Elements of an Effective PR Description Checklist - [ ] The purpose of the P...
...
- 动机: 该 PR 围绕 Qwen3 Core 修复已暴露的启动、加载、解析或数值问题，标题为「[BUGFIX] GPTQ quantization compatibility for Qwen3 MOE models (AutoGPTQ and AutoRound-GPTQ)」，变更集中在 `vllm/model_executor/models/qwen3_moe.py`。PR 描述补充为：The following PR attempts to make the quantized MOE model chart compatible with AutoGPTQ and Autoround-GPTQ. The PR: https://github.com/vllm-project/vllm/issues/23467 attempted...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/qwen3_moe.py`；关联提交 `0f7acdd73ca6`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 13 个文件，+2084/-17，可读 patch 2262 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[Model] Support Qwen3-VL Model Series」，变更集中在 `vllm/model_executor/models/qwen3_moe.py`。PR 描述补充为：## Purpose This PR adds model support for the upcoming Qwen3-VL models, including both dense and MoE variants. Reference HF implementation - https://github.com/huggingface/trans...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[Bugfix][WideEP] Apply TP Attn + EP MoE fix to other models」，变更集中在 `vllm/model_executor/models/qwen3_moe.py`。PR 描述补充为：## Purpose Prior to this PR, in many cases, using TP Attn and EP MoEs with `--tensor-parallel-size N --data-parallel-size M --enable-expert-parallel` would result in factor `N`...
...
- 动机: 该 PR 围绕 Qwen3 Core 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] Fix Qwen3-VL regression from #24982」，变更集中在 `vllm/model_executor/models/qwen3_moe.py`。PR 描述补充为：## Purpose #24982 caused a regression since the Qwen3-VL needs to access the text config of Qwen3-MoE via `get_text_config()` ## Test Plan ## Test Result Tested locally to make...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「Add EAGLE-3 Speculative Decoding Support for Qwen3 MoE」，变更集中在 `vllm/model_executor/models/qwen3_moe.py`。PR 描述补充为：This PR adds support for EAGLE-3 speculative decoding to the `Qwen3MoeForCausalLM` model, enabling faster inference with draft models like `nm-testing/Mockup-qwen235-eagle3-fp16...
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[Performance] Support FP8 flashinfer TRTLLM MOE on Qwen3 and Qwen-3next」，变更集中在 `vllm/model_executor/models/qwen3_moe.py`。PR 描述补充为：## Purpose - Integrate multiple routing methods for FP8 flashinfer trtllm MOE, currently only DS and Llama4 - Add FP8 flashinfer trtllm MOE support on Qwen3 and Qwen3-next ## Te...
...
- 动机: 该 PR 围绕 Qwen3 Core 修复已暴露的启动、加载、解析或数值问题，标题为「[bugfix][quantization] fix quark qwen3 kv_cache quantization」，变更集中在 `vllm/model_executor/models/qwen3_moe.py`。PR 描述补充为：Qwen3moe cannot effectively recognize the kv_cache scale. We need to call the get_cache_scale method in the base class to identify it. test scripts:
...
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[Models] Add `SharedFusedMoE` support to Qwen3MoE」，变更集中在 `vllm/model_executor/models/qwen3_moe.py`。PR 描述补充为：## Purpose - Fix https://github.com/vllm-project/vllm-omni/pull/560#discussion_r2655368800 - Qwen3-omni's MoE talker has share experts in its sparse moe block, while vLLM's Qwen...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/qwen3.py`, `vllm/model_executor/models/qwen3_moe.py`；关联提交 `2991dd3d2241`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 6 个文件，+132/-13，可读 patch 188 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[Bugfix][Model] Support LoRA on Qwen3 Output Embedding」，变更集中在 `vllm/model_executor/models/qwen3.py`, `vllm/model_executor/models/qwen3_moe.py`。PR 描述补充为：## Purpose This PR adds support for LoRA on the embed/unembed layers for Qwen3 dense/MoE models. It is a simplified version of #26115 that removes the changes for supporting zer...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `tests/models/multimodal/pooling/test_colqwen3.py`, `vllm/model_executor/models/colqwen3.py`, `vllm/transformers_utils/configs/colqwen3.py`；关联提交 `d1ea65d0a1c6`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 10 个文件，+935/-0，可读 patch 982 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[new model] add COLQwen3 code & Inference」，变更集中在 `vllm/model_executor/models/colqwen3.py`, `tests/models/multimodal/pooling/test_colqwen3.py`, `vllm/transformers_utils/configs/colqwen3.py`。PR 描述补充为：## Purpose Add native support for **ColQwen3** multi-modal late interaction models in vLLM. ColPali (arXiv:2407.01449) introduces a ColBERT-style multi-vector retrieval approach...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `tests/models/multimodal/pooling/test_colqwen3.py`, `vllm/model_executor/models/colqwen3.py`；关联提交 `5719a4e4e601`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 10 个文件，+532/-66，可读 patch 843 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3 Core 补齐模型支持入口或运行时能力，标题为「[Frontend] Support multimodal inputs for late-interaction scoring (ColQwen3) + NewModel: nvidia/nemotron-colembed」，变更集中在 `tests/models/multimodal/pooling/test_colqwen3.py`, `vllm/model_executor/models/colqwen3.py`。PR 描述补充为：## Purpose Follow-up to #34398 (`[new model] add COLQwen3 code & Inference`). PR #34398 added native support for ColQwen3 multi-modal late interaction models, but the `/score` a...
...
- 动机: 该 PR 围绕 Qwen3 Core 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix][Model] Fix FP8 k_scale/v_scale not loaded for Qwen3-MoE」，变更集中在 `vllm/model_executor/models/qwen3_moe.py`。PR 描述补充为：FP8 KV cache scales from llm-compressor checkpoints (e.g. `qkv_proj.k_scale`) were silently dropped during weight loading in Qwen3MoeModel, causing fallback to scale=1.0 and acc...
...
- 动机: 该 PR 围绕 Qwen3 Core 修复已暴露的启动、加载、解析或数值问题，标题为「[BugFix]fix Qwen3 MoE call gate twice」，变更集中在 `vllm/model_executor/models/qwen3_moe.py`。PR 描述补充为：## Purpose Qwen3 MoE model will call gate gemm twice, we find this in xpu kernel profiling. thanks @zufangzhu raise this. see discussion here https://github.com/vllm-project/vll...
