...
- Motivation: For ERNIE 4.5, this PR adds or enables a model support/runtime surface. Title: "[Model] Add Ernie4.5 and Ernie4.5MoE Model Support". The diff centers on `vllm/model_executor/models/ernie45_moe.py`, `vllm/model_executor/models/ernie45.py`, `tests/models/registry.py`. PR body context: ## Purpose Support Baidu Ernie4.5 model for vllm In this PR, I have provided the implementation of the Ernie4.5 and Ernie4.5MoE model structure through two files: ernie45.py and...
...
- Motivation: For ERNIE 4.5, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Fix Ernie4_5_MoeForCausalLM shared experts". The diff centers on `vllm/model_executor/models/ernie45_moe.py`. PR body context: ## Essential Elements of an Effective PR Description Checklist - [ ] The purpose of the PR, such as "Fix some issue (link existing issues this PR will resolve)". - [ ] The test...
...
- Status/date: merged / 2025-07-28
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/ernie45.py`; associated commits `656c24f1b5d8`
...
- Motivation: For ERNIE 4.5, this PR changes model-related implementation. Title: "[`Ernie 4.5`] Name Change for Base 0.3B Model". The diff centers on `vllm/model_executor/models/ernie45.py`. PR body context: Internally discussed with baidu/ernie team to have the name change from `Ernie4_5_For...` to `Ernie4_5For...` Relevant hub PRs that need to be merged before: - https://huggingfa...
...
- Motivation: For ERNIE 4.5, this PR adds or enables a model support/runtime surface. Title: "[Model] Add Ernie4.5 VL Model Support". The diff centers on `vllm/model_executor/models/ernie45_vl.py`, `vllm/model_executor/models/ernie45_vl_moe.py`, `vllm/model_executor/layers/rotary_embedding/mrope.py`. PR body context: ## Purpose Support Baidu Ernie4.5 VL model for vllm Note: torch.compile is not supported. Due to the model limitations of multimodal experts and text experts, using torch.compil...
...
- Motivation: For ERNIE 4.5, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[BugFix][Model] Fix Ernie4.5-VL hanging on long inputs". The diff centers on `vllm/model_executor/models/ernie45_vl.py`, `vllm/model_executor/models/ernie45_vl_moe.py`. PR body context: ## Purpose Implement the `get_mm_max_tokens_per_item` method to ensure a larger `encoder_compute_budget` and prevent hanging on long inputs. ## Test Plan `python examples/offlin...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/ernie45_moe.py`, `vllm/model_executor/models/ernie45_vl_moe.py`; associated commits `ef6e0e7132ec`
...
- Motivation: For ERNIE 4.5, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix][Model]fix ernie45 moe gate&bias dtype to float32". The diff centers on `vllm/model_executor/models/ernie45_vl_moe.py`, `vllm/model_executor/models/ernie45_moe.py`. PR body context: fix issue https://github.com/vllm-project/vllm/issues/25833 refer transformers:https://github.com/huggingface/transformers/blob/main/src/transformers/models/ernie4_5_moe/modelin...
...
- Trace source: `git log --name-only -- <model-files>` found it through `tests/reasoning/test_ernie45_reasoning_parser.py`, `vllm/reasoning/ernie45_reasoning_parser.py`; associated commits `782505ed8eb4`
...
- Motivation: For ERNIE 4.5, this PR adds or enables a model support/runtime surface. Title: "[Model] Add reasoning_parser and tool_parser for Ernie45 thinking". The diff centers on `vllm/reasoning/ernie45_reasoning_parser.py`, `tests/reasoning/test_ernie45_reasoning_parser.py`. PR body context: ## Purpose Add reasoning_parser and tool_parser for Ernie45 thinking reasoning format: `abc\n \n\n\n \ndef\n \n` toolcall format: `abc\n \n\n\n \nxyz\n \n` ## Test **start serve...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/ernie45_moe.py`; associated commits `01ad27faff35`
...
- Motivation: For ERNIE 4.5, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Model][Bugfix]fix ernie45 load failed due to ernie45 eplb code". The diff centers on `vllm/model_executor/models/ernie45_moe.py`. PR body context: ## Purpose fix following issue by PR https://github.com/vllm-project/vllm/pull/22100 ## Test Plan Running the following test script python test_eplb.py --mode eplb python test_e...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/ernie45_vl_moe.py`; associated commits `e51928793e10`
...
- Motivation: For ERNIE 4.5, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Model][Bugfix] fix ernie45 vl run failed from shared experts optimization". The diff centers on `vllm/model_executor/models/ernie45_vl_moe.py`. PR body context: ## Purpose Fix the following issue Due to `SharedFusedMoE` `forward` return is tuple (https://github.com/vllm-project/vllm/issues/26145), it is no `flatten()` method ## Test Pla...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/ernie45_moe.py`; associated commits `63b22e0dbb90`
...
- Motivation: For ERNIE 4.5, this PR adds or enables a model support/runtime surface. Title: "[Model][Bugfix] fix ernie45 moe 300B SharedFusedMoE output tuple". The diff centers on `vllm/model_executor/models/ernie45_moe.py`. PR body context: ## Purpose fix the following issue ERNIE-4.5-300B-A47B-PT non shared expert model, but the output of `SharedFuseMoE` is still tuple Add an else branch to handle this situation #...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/reasoning/ernie45_reasoning_parser.py`; associated commits `43a6acfb7de8`
...
- Motivation: For ERNIE 4.5, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Model] fix ernie45 reasoning_parser". The diff centers on `vllm/reasoning/ernie45_reasoning_parser.py`. PR body context: ## Purpose Fix the following issues with ernie45 reasoning parser
...
- Motivation: For ERNIE 4.5, this PR adds or enables a model support/runtime surface. Title: "[Model][Ernie4.5-VL] Support video metadata for timestamp rendering". The diff centers on `vllm/model_executor/models/ernie45_vl.py`, `tests/models/multimodal/processing/test_common.py`. PR body context: ## Purpose ERNIE-4.5-VL requires video metadata to render timestamps. This PR forwards video inputs as (frames, metadata) tuples to the HuggingFace processor when supported. For...
...
- Trace source: `git log --name-only -- <model-files>` found it through `tests/model_executor/test_ernie45_vl_mrope.py`, `vllm/model_executor/models/ernie45_vl.py`; associated commits `0008729abfbd`
...
- Motivation: For ERNIE 4.5, this PR changes model-related implementation. Title: "[Model] Use mm_features for Ernie-4.5 VL M-RoPE". The diff centers on `vllm/model_executor/models/ernie45_vl.py`, `tests/model_executor/test_ernie45_vl_mrope.py`. PR body context: ## Purpose Implement the Ernie-4.5 VL slice of #32656 by switching M-RoPE position calculation from the legacy `input_tokens` marker-scanning path to `mm_features`. This change...
