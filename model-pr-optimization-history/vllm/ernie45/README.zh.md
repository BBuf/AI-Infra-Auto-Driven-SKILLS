...
- 动机: 该 PR 围绕 ERNIE 4.5 补齐模型支持入口或运行时能力，标题为「[Model] Add Ernie4.5 and Ernie4.5MoE Model Support」，变更集中在 `vllm/model_executor/models/ernie45_moe.py`, `vllm/model_executor/models/ernie45.py`, `tests/models/registry.py`。PR 描述补充为：## Purpose Support Baidu Ernie4.5 model for vllm In this PR, I have provided the implementation of the Ernie4.5 and Ernie4.5MoE model structure through two files: ernie45.py and...
...
- 动机: 该 PR 围绕 ERNIE 4.5 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] Fix Ernie4_5_MoeForCausalLM shared experts」，变更集中在 `vllm/model_executor/models/ernie45_moe.py`。PR 描述补充为：## Essential Elements of an Effective PR Description Checklist - [ ] The purpose of the PR, such as "Fix some issue (link existing issues this PR will resolve)". - [ ] The test...
...
- 状态/时间: merged / 2025-07-28
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/ernie45.py`；关联提交 `656c24f1b5d8`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+8/-8，可读 patch 51 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 ERNIE 4.5 调整模型相关实现，标题为「[`Ernie 4.5`] Name Change for Base 0.3B Model」，变更集中在 `vllm/model_executor/models/ernie45.py`。PR 描述补充为：Internally discussed with baidu/ernie team to have the name change from `Ernie4_5_For...` to `Ernie4_5For...` Relevant hub PRs that need to be merged before: - https://huggingfa...
...
- 动机: 该 PR 围绕 ERNIE 4.5 补齐模型支持入口或运行时能力，标题为「[Model] Add Ernie4.5 VL Model Support」，变更集中在 `vllm/model_executor/models/ernie45_vl.py`, `vllm/model_executor/models/ernie45_vl_moe.py`, `vllm/model_executor/layers/rotary_embedding/mrope.py`。PR 描述补充为：## Purpose Support Baidu Ernie4.5 VL model for vllm Note: torch.compile is not supported. Due to the model limitations of multimodal experts and text experts, using torch.compil...
...
- 动机: 该 PR 围绕 ERNIE 4.5 修复已暴露的启动、加载、解析或数值问题，标题为「[BugFix][Model] Fix Ernie4.5-VL hanging on long inputs」，变更集中在 `vllm/model_executor/models/ernie45_vl.py`, `vllm/model_executor/models/ernie45_vl_moe.py`。PR 描述补充为：## Purpose Implement the `get_mm_max_tokens_per_item` method to ensure a larger `encoder_compute_budget` and prevent hanging on long inputs. ## Test Plan `python examples/offlin...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/ernie45_moe.py`, `vllm/model_executor/models/ernie45_vl_moe.py`；关联提交 `ef6e0e7132ec`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+13/-7，可读 patch 83 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 ERNIE 4.5 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix][Model]fix ernie45 moe gate&bias dtype to float32」，变更集中在 `vllm/model_executor/models/ernie45_vl_moe.py`, `vllm/model_executor/models/ernie45_moe.py`。PR 描述补充为：fix issue https://github.com/vllm-project/vllm/issues/25833 refer transformers:https://github.com/huggingface/transformers/blob/main/src/transformers/models/ernie4_5_moe/modelin...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `tests/reasoning/test_ernie45_reasoning_parser.py`, `vllm/reasoning/ernie45_reasoning_parser.py`；关联提交 `782505ed8eb4`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 7 个文件，+870/-0，可读 patch 909 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 ERNIE 4.5 补齐模型支持入口或运行时能力，标题为「[Model] Add reasoning_parser and tool_parser for Ernie45 thinking」，变更集中在 `vllm/reasoning/ernie45_reasoning_parser.py`, `tests/reasoning/test_ernie45_reasoning_parser.py`。PR 描述补充为：## Purpose Add reasoning_parser and tool_parser for Ernie45 thinking reasoning format: `abc\n \n\n\n \ndef\n \n` toolcall format: `abc\n \n\n\n \nxyz\n \n` ## Test **start serve...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/ernie45_moe.py`；关联提交 `01ad27faff35`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+22/-12，可读 patch 71 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 ERNIE 4.5 修复已暴露的启动、加载、解析或数值问题，标题为「[Model][Bugfix]fix ernie45 load failed due to ernie45 eplb code」，变更集中在 `vllm/model_executor/models/ernie45_moe.py`。PR 描述补充为：## Purpose fix following issue by PR https://github.com/vllm-project/vllm/pull/22100 ## Test Plan Running the following test script python test_eplb.py --mode eplb python test_e...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/ernie45_moe.py`；关联提交 `63b22e0dbb90`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+2/-0，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 ERNIE 4.5 补齐模型支持入口或运行时能力，标题为「[Model][Bugfix] fix ernie45 moe 300B SharedFusedMoE output tuple」，变更集中在 `vllm/model_executor/models/ernie45_moe.py`。PR 描述补充为：## Purpose fix the following issue ERNIE-4.5-300B-A47B-PT non shared expert model, but the output of `SharedFuseMoE` is still tuple Add an else branch to handle this situation #...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/reasoning/ernie45_reasoning_parser.py`；关联提交 `43a6acfb7de8`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+2/-2，可读 patch 11 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 ERNIE 4.5 修复已暴露的启动、加载、解析或数值问题，标题为「[Model] fix ernie45 reasoning_parser」，变更集中在 `vllm/reasoning/ernie45_reasoning_parser.py`。PR 描述补充为：## Purpose Fix the following issues with ernie45 reasoning parser
...
- 动机: 该 PR 围绕 ERNIE 4.5 补齐模型支持入口或运行时能力，标题为「[Model][Ernie4.5-VL] Support video metadata for timestamp rendering」，变更集中在 `vllm/model_executor/models/ernie45_vl.py`, `tests/models/multimodal/processing/test_common.py`。PR 描述补充为：## Purpose ERNIE-4.5-VL requires video metadata to render timestamps. This PR forwards video inputs as (frames, metadata) tuples to the HuggingFace processor when supported. For...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `tests/model_executor/test_ernie45_vl_mrope.py`, `vllm/model_executor/models/ernie45_vl.py`；关联提交 `0008729abfbd`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+196/-123，可读 patch 339 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 ERNIE 4.5 调整模型相关实现，标题为「[Model] Use mm_features for Ernie-4.5 VL M-RoPE」，变更集中在 `vllm/model_executor/models/ernie45_vl.py`, `tests/model_executor/test_ernie45_vl_mrope.py`。PR 描述补充为：## Purpose Implement the Ernie-4.5 VL slice of #32656 by switching M-RoPE position calculation from the legacy `input_tokens` marker-scanning path to `mm_features`. This change...
