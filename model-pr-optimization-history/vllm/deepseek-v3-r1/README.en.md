...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/deepseek_v2.py`; associated commits `24679788ed38`
...
- Motivation: For DeepSeek V3/R1, this PR extends deployment docs, tests, or CI coverage. Title: "DeepSeek V2/V3/R1 only place `lm_head` on last pp rank". The diff centers on `vllm/model_executor/models/deepseek_v2.py`. PR body context: Balances memory usage across pipeline ranks by reducing the amount used on all ranks that are not the last rank.
...
- Trace source: `git log --name-only -- <model-files>` found it through `examples/tool_chat_template_deepseekv3.jinja`; associated commits `3a5ea7512926`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Feature] Support DeepSeekV3 Function Call". The diff centers on `examples/tool_chat_template_deepseekv3.jinja`. PR body context: Support DeepSeek-V3-0324 function call. usage: test script (no streaming): Output: test script (streaming): Output:
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Bugfix] Add missing `packed_modules_mapping` to `DeepseekV2ForCausalLM`". The diff centers on `vllm/model_executor/models/deepseek_v2.py`. PR body context: This PR adds `packed_modules_mapping` class attribute to `DeepseekV2ForCausalLM`. https://github.com/vllm-project/vllm/pull/21116 added `fused_qkv_a_proj` for DeepseekV2ForCausa...
...
- Trace source: `git log --name-only -- <model-files>` found it through `examples/tool_chat_template_deepseekv31.jinja`; associated commits `b8f17f5d980e`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Support DeepSeek-V3.1 tool call". The diff centers on `examples/tool_chat_template_deepseekv31.jinja`. PR body context: ## Purpose Support DeepSeek-V3.1 tool call. The tool call format of DeepSeek-V3.1 is different from DeepSeek-V3/R1: DeepSeek-V3.1: tool_call_name tool_call_arguments DeepSeek-R1...
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "Add LoRA support for DeepSeek models (V2, V3, R1-0528)". The diff centers on `vllm/model_executor/models/deepseek_v2.py`. PR body context: - Add SupportsLoRA interface to DeepseekV2ForCausalLM - Enable LoRA fine-tuning for DeepSeek-V2, DeepSeek-V3, and DeepSeek-R1-0528 - MoE detection already handled by existing is...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/deepseek_v2.py`; associated commits `930a24144c07`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Bug] R1 Accuracy: Fix `routed_scaling_factor` Double Mul Issue". The diff centers on `vllm/model_executor/models/deepseek_v2.py`. PR body context: ## Purpose Fixes https://github.com/vllm-project/vllm/issues/24118 that is introduced by https://github.com/vllm-project/vllm/pull/23123 ## Test `lm_eval --model local-completio...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/deepseek_mtp.py`, `vllm/model_executor/models/deepseek_v2.py`; associated commits `fa7e254a7f3e`
...
- Motivation: For DeepSeek V3/R1, this PR changes model-related implementation. Title: "[New Model] DeepSeek-V3.2 (Rebased to Main)". The diff centers on `vllm/model_executor/models/deepseek_v2.py`, `vllm/model_executor/models/deepseek_mtp.py`. PR body context: Rebased dsv32, based on #25869 Run command gsm8k gsm8k, 20-shot
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/deepseek_v2.py`; associated commits `1e50f1be7058`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Deepseek v3.2] Support indexer prefill chunking". The diff centers on `vllm/model_executor/models/deepseek_v2.py`. PR body context: ## Purpose Split the prefill to multiple steps, with each step contains a subset of prefill requests. With this approach, we can avoid the large output caused by gather kv cache...
...
- Trace source: `git log --name-only -- <model-files>` found it through `tests/reasoning/test_deepseekv3_reasoning_parser.py`; associated commits `85a65e7f51ad`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Model] Add DeepSeek-V3.1 reasoning parser (split from PR #24972)". The diff centers on `tests/reasoning/test_deepseekv3_reasoning_parser.py`. PR body context: ## Purpose This PR adds a new reasoning parser for the DeepSeek-V3.1 model, named deepseek_v3. Unlike previous models such as deepseek_r1, the reasoning parser for DeepSeek-V3.1...
...
- Status/date: merged / 2025-10-15
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/deepseek_v2.py`; associated commits `f5ed68ef63d0`
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "[Deepseek-V3.2][Kernel] Integrate cuda indexer k cache gather". The diff centers on `vllm/model_executor/models/deepseek_v2.py`. PR body context: Replace torch `cp_gather_indexer_k_quant_cache` to cuda op. Follow up for #25931 gsm8k 20 shots
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/deepseek_v2.py`; associated commits `80e94529845d`
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "[Deepseek v3.2] Optimize top_k_per_row". The diff centers on `vllm/model_executor/models/deepseek_v2.py`. PR body context: ## Purpose This PR optimizes kernel top_k_per_row. Local testing shows it is about 2.5x from its previous version. ## Test Plan ## Test Result --- Essential Elements of an Effec...
...
- Status/date: merged / 2025-10-21
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/deepseek_v2.py`; associated commits `09a7e6f6179b`
...
- Motivation: For DeepSeek V3/R1, this PR extends deployment docs, tests, or CI coverage. Title: "[Deepseek v3.2] Remove extra logics in indexer". The diff centers on `vllm/model_executor/models/deepseek_v2.py`. PR body context: ## Purpose Remove extra logics around `index_end_pos` in the indexer. CC @dcampora ## Test Plan `lm-eval --model local-completions --tasks gsm8k --model_args model=DeepSeek-V3.2...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/deepseek_v2.py`; associated commits `88f5b19f0bc6`
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[DeepSeek] Fix DeepSeek V3.2 Rope Embedding". The diff centers on `vllm/model_executor/models/deepseek_v2.py`. PR body context: ## Purpose Deepseek recently find error in their official implementation that ROPE in indexer shouldn't be interleaved. ## Test Plan gsm8k 20-shots ## Test Result --- Essential...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/deepseek_v2.py`; associated commits `06c20c990464`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[ROCm] Add AMD GPU support on Deepseek v3.2 and SparseMLA". The diff centers on `vllm/model_executor/models/deepseek_v2.py`. PR body context: ## Purpose The PR add Deepseek v3.2 support on ROCm platforms. The main change in this PR include: - Replace all hardcode float8_e4m3fn to platform supported fp8 dtype, and add...
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Fix DeepSeek R1 MTP weight loading". The diff centers on `vllm/model_executor/models/deepseek_mtp.py`. PR body context: ## Purpose Fixes #29448 by adding some logic from `deepseek_v2.py` that was missing in `deepseek_mtp.py` ## Test Plan with ## Test Result Previously crashed during weight loadin...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/deepseek_v2.py`; associated commits `184076c3fecf`
...
- Motivation: For DeepSeek V3/R1, this PR changes model-related implementation. Title: "[DeepSeek v3.2] Make top-k work for any logit values.". The diff centers on `vllm/model_executor/models/deepseek_v2.py`. PR body context: ## Purpose This PR allows top_k_per_row work for any values in logits. Even if the logits differ only in the least significant bytes, top-k is now guaranteed to always give a co...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/deepseek_v2.py`; associated commits `3e41992fecdc`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Attention] Use sparse prefill kernel for fp8 kv-cache in DeepSeek-v3.2". The diff centers on `vllm/model_executor/models/deepseek_v2.py`. PR body context: When doing prefill up-convert the kv-cache from fp8 to bf16 and call the bf16 prefill kernel instead of the decode kernel. This PR introduce global workspace management to have...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/deepseek_v2.py`; associated commits `84896fda22d3`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Bugfix] deepseek-V3.2 self.weights_proj has no bias". The diff centers on `vllm/model_executor/models/deepseek_v2.py`. PR body context: ## Purpose self.weights_proj has no bias,some other hardware bias maybe not initial with 0 maybe not correct H20 bias initial with 0 !image kunlun bias not initial with 0 !image...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/deepseek_v2.py`; associated commits `4cf9429897c1`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Bug] Fix `error 'Dynamo failed to run FX node with fake tensors` for Deepseek V3.2". The diff centers on `vllm/model_executor/models/deepseek_v2.py`. PR body context: ## Purpose `export MODEL="deepseek-ai/DeepSeek-V3.2"` `vllm serve "$MODEL" -tp 8 --port 9256 --enable-expert-parallel -cc '{"mode":3,"pass_config":{"fuse_norm_quant":true,"elimi...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/deepseek_v2.py`; associated commits `76e6a951925b`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Bug] Fix `Number of dimensions of tensors must match.` for Deepseek V3.2". The diff centers on `vllm/model_executor/models/deepseek_v2.py`. PR body context: ## Purpose `export MODEL="deepseek-ai/DeepSeek-V3.2"` `vllm serve "$MODEL" -tp 8 --port 9256 --enable-expert-parallel` Will trigger error The root cause: Number of dimensions of...
...
- Trace source: `git log --name-only -- <model-files>` found it through `tests/evals/gsm8k/configs/DeepSeek-R1-DP.yaml`, `tests/evals/gsm8k/configs/DeepSeek-R1-TP.yaml`; associated commits `276e03b92c16`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[CI][DeepSeek] Add nightly DeepSeek R1 `lm_eval` tests on H200". The diff centers on `tests/evals/gsm8k/configs/DeepSeek-R1-DP.yaml`, `tests/evals/gsm8k/configs/DeepSeek-R1-TP.yaml`. PR body context: ## Purpose DeepSeek R1 is frequently broken on main. This PR adds an `lm_eval` test for DeepSeek R1 on 8xH200 to catch this earlier. Currently set to be run nightly (no `source_...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/deepseek_v2.py`; associated commits `5de6dd0662da`
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] [DeepSeek-V3.2] fix sparse_attn_indexer padding". The diff centers on `vllm/model_executor/models/deepseek_v2.py`. PR body context: ## Purpose Fix https://github.com/vllm-project/vllm/issues/32172 ## Test Plan ## Test Result vllm bench ok. lm_eval: Main: This PR: --- Essential Elements of an Effective PR Des...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/deepseek_v2.py`; associated commits `6c20e89c0209`
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "[ROCm][Deepseekv3.2] Refactor Sparse Indexer as CustomOp". The diff centers on `vllm/model_executor/models/deepseek_v2.py`. PR body context: ## Purpose This PR optimize the deepseekv3.2's performance on AMD's device, and separate `SparseAttnIndexer` out as a `CustomOp` as it contains lots of heavy kernels like `fp8_m...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/tool_parsers/deepseekv32_tool_parser.py`; associated commits `7bec4351305f`
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Fix the issue where tool calling does not work when using fast detokenization with dsv32". The diff centers on `vllm/tool_parsers/deepseekv32_tool_parser.py`. PR body context: ## Purpose Fix the issue where tool calling does not work when using fast detokenization with dsv32 dsv32 uses special tokens like `｜DSML｜function_calls>`, which are skipped by...
...
- Motivation: For DeepSeek V3/R1, this PR changes model-related implementation. Title: "[Model] Deepseek-V3.1 reasoning parser". The diff centers on `tests/reasoning/test_deepseekv3_reasoning_parser.py`. PR body context: ## Purpose This PR adds a new reasoning parser for the DeepSeek-V3.1 model, named deepseek_v3. Unlike previous models such as deepseek_r1, the reasoning parser for DeepSeek-V3.1...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/deepseek_v2.py`; associated commits `6874638bc443`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Model Bash] DeepSeek R1 BF16 Min Latency QKV A GEMM (0.5% E2E Speedup)". The diff centers on `vllm/model_executor/models/deepseek_v2.py`. PR body context: ## Purpose - add min latency bf16 qkv_a_gemm - adapted from sgl: * https://github.com/sgl-project/sglang/blob/main/sgl-kernel/csrc/gemm/dsv3_fused_a_gemm.cu, which adapted from...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/deepseek_v2.py`; associated commits `7f51e9386470`
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bug] Fix DeepSeek V3 weight loading caused by incorrect prefix". The diff centers on `vllm/model_executor/models/deepseek_v2.py`. PR body context: ## Purpose Fix #34869 ## Test Plan ## Test Result --- Essential Elements of an Effective PR Description Checklist - [ ] The purpose of the PR, such as "Fix some issue (link exis...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/deepseek_v2.py`; associated commits `8435b2e04925`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[ModelBash][DSV3] Add TRTLLM DSV3 Router GEMM kernel (6% B1 Speedup)". The diff centers on `vllm/model_executor/models/deepseek_v2.py`. PR body context: Port the optimized router GEMM kernel from sglang's sgl-kernel for DeepSeek V3 MoE models. This kernel is specifically optimized for small batch sizes (1-16 tokens) common in de...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/deepseek_v2.py`; associated commits `9319044ee9a1`
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "[MoE][Perf] Wrap DSV3 QKVAProj GEMM in custom op for torch.compile". The diff centers on `vllm/model_executor/models/deepseek_v2.py`. PR body context: ## Summary - `DeepSeekV2FusedQkvAProj.forward` had a data-dependent conditional `0 🤖 Generated with Claude Code
...
- Diff scope read: GitHub Pull Request files API returned 1 files, +2/-2, 18 readable patch lines; this card prioritizes model-related and high-change files.
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Bugfix] Fix compressed-tensors quantization failure for DeepSeek-R1 on MI300x". The diff centers on `vllm/model_executor/models/deepseek_v2.py`. PR body context: ## Purpose When running DeepSeek-R1 FP8 on MI300x, vLLM crashed during model initialization with: The compressed-tensors quantization config matches layers by checking if a targ...
...
- Trace source: `git log --name-only -- <model-files>` found it through `tests/tool_parsers/test_deepseekv32_tool_parser.py`, `vllm/tool_parsers/deepseekv32_tool_parser.py`; associated commits `be12afd284f3`
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Fix Deepseekv32 tool parser when stream interval > 1". The diff centers on `vllm/tool_parsers/deepseekv32_tool_parser.py`, `tests/tool_parsers/test_deepseekv32_tool_parser.py`. PR body context: ## Purpose The deepseek 3.2 tool parser used an incremental state machine (~20 instance variables) that parsed DSML tags character-by-character as tokens streamed in. With strea...
...
- Trace source: `git log --name-only -- <model-files>` found it through `tests/tool_parsers/test_deepseekv32_tool_parser.py`, `vllm/tool_parsers/deepseekv32_tool_parser.py`; associated commits `a6db99ba02ec`
...
- Motivation: For DeepSeek V3/R1, this PR adds or enables a model support/runtime surface. Title: "[Bugfix] Support multi-type params parsing for DeepSeek v3.2". The diff centers on `tests/tool_parsers/test_deepseekv32_tool_parser.py`, `vllm/tool_parsers/deepseekv32_tool_parser.py`. PR body context: ## Purpose Kilo code uses multi typed params for some reason, and such calls fails to render with exception `'list' object has no attribute 'lowercase'` when Kilo code passes `t...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/deepseek_mtp.py`, `vllm/model_executor/models/deepseek_v2.py`; associated commits `5f96f9aff10f`
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "[Perf] DSV3.2 Indexer Fused Weights Projection". The diff centers on `vllm/model_executor/models/deepseek_v2.py`, `vllm/model_executor/models/deepseek_mtp.py`. PR body context: ## Purpose Fuse the WK and Weights_Proj projections in the DSV3.2 Indexer. This is an alternative optimization to https://github.com/vllm-project/vllm/pull/35968, which overlaps...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/deepseek_mtp.py`, `vllm/model_executor/models/deepseek_v2.py`; associated commits `8617f8676b5a`
...
- Motivation: For DeepSeek V3/R1, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Fix DSV32 weight loading". The diff centers on `vllm/model_executor/models/deepseek_v2.py`, `vllm/model_executor/models/deepseek_mtp.py`. PR body context: ## Purpose #38684 intorude bug on the fp8 checkpoint ## Test Plan gsm8k score ## Test Result --- Essential Elements of an Effective PR Description Checklist - [x] The purpose of...
...
- Trace source: `git log --name-only -- <model-files>` found it through `vllm/model_executor/models/deepseek_v2.py`; associated commits `b55d830ec782`
...
- Motivation: For DeepSeek V3/R1, this PR optimizes an inference path or backend selection. Title: "[Perf][Kernel] Persistent TopK scheduler: unified CUDAGraph-safe kernel with dynamic per-row dispatch - DeepSeek-V3.2 DSA decode". The diff centers on `vllm/model_executor/models/deepseek_v2.py`. PR body context: ## Summary Redesigns the persistent TopK kernel used by DSA as a true persistent scheduler with dynamic per-row path selection. This supersedes and closes #34265, which took a C...
