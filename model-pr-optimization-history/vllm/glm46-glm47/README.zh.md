...
- 动机: 该 PR 围绕 GLM-4.6/4.7 补齐模型支持入口或运行时能力，标题为「[Kernel][MoE] Add MoE tunings for GLM 4.6-FP8 and GLM 4.5 Air on NVidia B200」，变更集中在 `vllm/model_executor/layers/fused_moe/configs/E=32,N=1408,device_name=NVIDIA_B200.json`, `vllm/model_executor/layers/fused_moe/configs/E=40,N=1536,device_name=NVIDIA_B200,dtype=fp8_w8a8.json`, `vllm/model_executor/layers/fused_moe/configs/E=64,N=1408,device_name=NVIDIA_B200.json`。PR 描述补充为：## Purpose See title. I ran the following: --- Essential Elements of an Effective PR Description Checklist - [x] The purpose of the PR, such as "Fix some issue (link existing is...
...
- 动机: 该 PR 围绕 GLM-4.6/4.7 补齐模型支持入口或运行时能力，标题为「[Bugfix]: Fix glm46 awq marlin moe wna16 compatibility」，变更集中在 `vllm/model_executor/layers/fused_moe/fused_moe.py`, `vllm/model_executor/layers/quantization/moe_wna16.py`。PR 描述补充为：## Purpose Fixes two issues preventing GLM-4.6-AWQ from loading in vLLM: 1AWQ Marlin fallback compatibility: When AWQ Marlin doesn't support a MoE layer and falls back to MoeWNA...
...
- 动机: 该 PR 围绕 GLM-4.6/4.7 补齐模型支持入口或运行时能力，标题为「GLM-4.7 Tool Parser and Doc Update」，变更集中在 `vllm/tool_parsers/glm47_moe_tool_parser.py`, `vllm/model_executor/models/glm4_moe.py`。PR 描述补充为：Added support for GLM-4.7's Tool Parser and improved documentation.
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/tool_parsers/glm4_moe_tool_parser.py`；关联提交 `02dbb933cb28`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+68/-0，可读 patch 76 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 GLM-4.6/4.7 修复已暴露的启动、加载、解析或数值问题，标题为「Fix GLM-4.6v flash tool calling in transformers 5.x」，变更集中在 `vllm/tool_parsers/glm4_moe_tool_parser.py`。PR 描述补充为：## Purpose fix #31485 ## Test Plan main branch this branch --- Essential Elements of an Effective PR Description Checklist - [ ] The purpose of the PR, such as "Fix some issue (...
...
- 动机: 该 PR 围绕 GLM-4.6/4.7 补齐模型支持入口或运行时能力，标题为「[GLM-4.7] GLM Model support for GLM-Lite」，变更集中在 `vllm/model_executor/models/glm4_moe_lite.py`, `vllm/model_executor/models/glm4_moe_lite_mtp.py`, `vllm/model_executor/models/glm4_moe_mtp.py`。PR 描述补充为：using with transformers 5.0.0 with GLM-Lite model， transformers PR here
...
- 动机: 该 PR 围绕 GLM-4.6/4.7 修复已暴露的启动、加载、解析或数值问题，标题为「fix(glm47): improve tool call parsing and content normalization」，变更集中在 `tests/tool_parsers/test_glm47_moe_tool_parser.py`, `vllm/tool_parsers/glm47_moe_tool_parser.py`, `vllm/tool_parsers/glm4_moe_tool_parser.py`。PR 描述补充为：## Summary - **Improve GLM-4.7 `func_detail_regex`**: Use `\S+?` instead of `.*?` for the function name capture group, and make the arg group greedy (`.*` vs `.*?`) so all argum...
