...
- 动机: 该 PR 围绕 Qwen3.5 补齐模型支持入口或运行时能力，标题为「[MODEL] Adding Support for Qwen3.5 Models」，变更集中在 `vllm/model_executor/models/qwen3_5.py`, `vllm/model_executor/models/qwen3_5_mtp.py`。PR 描述补充为：## Purpose This PR adds model support for the upcoming Qwen3.5 models, including both dense and MoE variants. 🫡 Many thanks to @wulipc and @sighingnow for model verification and...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/qwen3_5.py`；关联提交 `047a457fa4af`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+3/-0，可读 patch 17 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3.5 补齐模型支持入口或运行时能力，标题为「[Bugfix] Adopt `ChunkGatedDeltaRule` for Qwen3.5」，变更集中在 `vllm/model_executor/models/qwen3_5.py`。PR 描述补充为：## Purpose `ChunkGatedDeltaRule` was recently added in #32846. Qwen3.5 missed this in its initialization which causes an error since its `_forward_core` inherits from Qwen3-Next...
...
- 状态/时间: merged / 2026-02-10
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/qwen3_5.py`；关联提交 `9615575afc0d`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+2/-1，可读 patch 11 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3.5 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] Fix mamba cache dtype for Qwen3.5」，变更集中在 `vllm/model_executor/models/qwen3_5.py`。PR 描述补充为：## Purpose Qwen3.5 uses float32 for mamba cache dtype and it's rather inconvenient to ask users to pass ` --mamba-cache-dtype float32` every single time. Since it's not part of...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/qwen3_5.py`；关联提交 `0b20469c627e`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-1，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3.5 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] Fix weight naming in Qwen3.5」，变更集中在 `vllm/model_executor/models/qwen3_5.py`。PR 描述补充为：## Purpose ## Test Plan ## Test Result --- Essential Elements of an Effective PR Description Checklist - [ ] The purpose of the PR, such as "Fix some issue (link existing issues...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/qwen3_5.py`；关联提交 `eea3024f43e0`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+42/-6，可读 patch 91 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3.5 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] Fix mamba state dtype setting for Qwen3-Next and Qwen3.5」，变更集中在 `vllm/model_executor/models/qwen3_5.py`。PR 描述补充为：## Purpose Previously conv and ssm state dtypes are coupled for Qwen3-Next, and therefore affected Qwen3.5 which inherits from it. This PR fixes the dtype setting for both model...
...
- 状态/时间: merged / 2026-02-13
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/qwen3_5.py`, `vllm/model_executor/models/qwen3_5_mtp.py`, `vllm/transformers_utils/configs/qwen3_5.py`, `vllm/transformers_utils/configs/qwen3_5_moe.py`；关联提交 `5885e330efea`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 6 个文件，+410/-12，可读 patch 473 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3.5 补强部署文档、测试或 CI 验证面，标题为「[Misc] Port Qwen3.5 Configs」，变更集中在 `vllm/transformers_utils/configs/qwen3_5_moe.py`, `vllm/transformers_utils/configs/qwen3_5.py`, `vllm/model_executor/models/qwen3_5.py`。PR 描述补充为：## Purpose so that users don't need to install transformers ## Test Plan ## Test Result --- Essential Elements of an Effective PR Description Checklist - [ ] The purpose of the...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/transformers_utils/configs/qwen3_5.py`, `vllm/transformers_utils/configs/qwen3_5_moe.py`；关联提交 `2f186635cbcb`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+18/-10，可读 patch 72 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3.5 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] Fix Qwen3.5 config loading」，变更集中在 `vllm/transformers_utils/configs/qwen3_5.py`, `vllm/transformers_utils/configs/qwen3_5_moe.py`。PR 描述补充为：## Purpose #34512 actually didn't port it completely correctly and resulted in degradation - this PR fixes it. ## Test Plan ## Test Result --- Essential Elements of an Effective...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/transformers_utils/configs/qwen3_5.py`, `vllm/transformers_utils/configs/qwen3_5_moe.py`；关联提交 `9521002f0ace`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+4/-4，可读 patch 26 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3.5 修复已暴露的启动、加载、解析或数值问题，标题为「[Misc] fix qwen3.5 config」，变更集中在 `vllm/transformers_utils/configs/qwen3_5.py`, `vllm/transformers_utils/configs/qwen3_5_moe.py`。PR 描述补充为：## Purpose fix qwen3.5 config loading @ywang96 before:
...
- 状态/时间: merged / 2026-02-16
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/transformers_utils/configs/qwen3_5.py`, `vllm/transformers_utils/configs/qwen3_5_moe.py`；关联提交 `b5475d053442`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+4/-4，可读 patch 26 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3.5 修复已暴露的启动、加载、解析或数值问题，标题为「Revert "[Misc] fix qwen3.5 config"」，变更集中在 `vllm/transformers_utils/configs/qwen3_5.py`, `vllm/transformers_utils/configs/qwen3_5_moe.py`。PR 描述补充为：Reverts vllm-project/vllm#34604 It was meant for transformers v5
- 实现要点: `vllm/transformers_utils/configs/qwen3_5.py` modified +2/-2 (4 lines); hunks: -68,10 +68,10 @@ def __init__(; symbols: __init__，涉及 `__init__`；`vllm/transformers_utils/configs/qwen3_5_moe.py` modified +2/-2 (4 lines); hunks: -75,10 +75,10 @@ def __init__(; symbols: __init__，涉及 `__init__`。
- 代码 diff 细节:
  - `vllm/transformers_utils/configs/qwen3_5.py` modified +2/-2 (4 lines); hunks: -68,10 +68,10 @@ def __init__(; symbols: __init__
  - `vllm/transformers_utils/configs/qwen3_5_moe.py` modified +2/-2 (4 lines); hunks: -75,10 +75,10 @@ def __init__(; symbols: __init__
...
- 状态/时间: merged / 2026-02-16
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/qwen3_5.py`；关联提交 `3bb4e4311c6d`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+87/-182，可读 patch 404 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3.5 优化关键推理路径或后端选择，标题为「[Models] Fuse Qwen3.5 GDN's qkvz_proj and ba_proj」，变更集中在 `vllm/model_executor/models/qwen3_5.py`。PR 描述补充为：## Purpose - Fuse Qwen3.5's GDN qkvz proj and ba proj to get better performance gain. ## Test Plan ## Test Result **Main branch** **PR** --- Essential Elements of an Effective P...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/qwen3_5.py`；关联提交 `909b14719725`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+6/-5，可读 patch 32 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3.5 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] Fix prefix creation for Qwen3.5」，变更集中在 `vllm/model_executor/models/qwen3_5.py`。PR 描述补充为：## Purpose It seems that the prefix creation for Qwen3.5 is incorrect since it adds a "model" module after the "language_model" module in the HF checkpoint. This will cause issu...
...
- 动机: 该 PR 围绕 Qwen3.5 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] Redo Qwen3.5/Qwen3-Next GDN projector fusion」，变更集中在 `vllm/model_executor/models/qwen3_5.py`。PR 描述补充为：## Purpose - Redo https://github.com/vllm-project/vllm/pull/34683 and fix the root issue - Actualy, Qwen3-Next's qkvz_proj output_sizes should be `output_sizes=[sum((key_dim, ke...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/qwen3_5.py`；关联提交 `6fff24f30fe2`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+7/-0，可读 patch 21 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3.5 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] Qwen3.5 kv-scale weight remapping」，变更集中在 `vllm/model_executor/models/qwen3_5.py`。PR 描述补充为：## Purpose - Fix for loading KV cache scaling factors by remapping the names, similar to how it is done in `models/qwen3_next.py` https://github.com/vllm-project/vllm/pull/32199...
...
- 动机: 该 PR 围绕 Qwen3.5 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] [Qwen3.5]Fix Qwen3.5 FP8 quantization: tuple shard_id weight loading」，变更集中在 `vllm/model_executor/layers/linear.py`。PR 描述补充为：## Purpose Fix #35287 Fix online FP8 quantization (`--quantization fp8`) for Qwen3.5 models. ### Bug — Weight loading crash: `NotImplementedError` for tuple `shard_id` Qwen3.5's...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/qwen3_5_mtp.py`；关联提交 `63d7972f13d1`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-1，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3.5 修复已暴露的启动、加载、解析或数值问题，标题为「Fix Qwen3_5MTP packed_modules_mapping for gate_up_proj」，变更集中在 `vllm/model_executor/models/qwen3_5_mtp.py`。PR 描述补充为：### What does this PR do? Fixes the `packed_modules_mapping` in `Qwen3_5MTP` class where `gate_up_proj` incorrectly included `down_proj` instead of `gate_proj`. ### Why is this...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/qwen3_5.py`；关联提交 `afd089f231d7`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+0/-5，可读 patch 40 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3.5 补齐模型支持入口或运行时能力，标题为「[Bugfix][Model] Fix Qwen3.5/Qwen3Next ignoring --dtype flag on older GPUs」，变更集中在 `vllm/model_executor/models/qwen3_5.py`。PR 描述补充为：## Summary - **Bug**: Qwen3.5 and Qwen3Next models ignore `--dtype float16`, creating bfloat16 parameters that crash on GPUs without bfloat16 support (e.g. 2080 Ti) - **Root cau...
...
- 动机: 该 PR 围绕 Qwen3.5 补齐模型支持入口或运行时能力，标题为「Add: Eagle3 support for Qwen3.5」，变更集中在 `vllm/model_executor/models/qwen3_5.py`。PR 描述补充为：This PR adds support for EAGLE-3 speculative decoding to `Qwen3.5`, enabling faster inference with draft models like `BLR2/Qwen3.5-9B-Eagle3-ShareGPT`. ## Changes ### Modified F...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `examples/pooling/score/colqwen3_5_rerank_online.py`, `tests/models/multimodal/pooling/test_colqwen3_5.py`, `vllm/model_executor/models/colqwen3_5.py`；关联提交 `c0745a851a4f`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 8 个文件，+579/-0，可读 patch 619 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3.5 补齐模型支持入口或运行时能力，标题为「[Model] Add ColQwen3.5 4.5B support」，变更集中在 `vllm/model_executor/models/colqwen3_5.py`, `tests/models/multimodal/pooling/test_colqwen3_5.py`, `examples/pooling/score/colqwen3_5_rerank_online.py`。PR 描述补充为：## Purpose Add support for **ColQwen3.5** late interaction model for multi-modal document retrieval and reranking. ColQwen3.5 extends the ColPali architecture using the Qwen3.5...
...
- 动机: 该 PR 围绕 Qwen3.5 修复已暴露的启动、加载、解析或数值问题，标题为「Fix AttributeError in Qwen3.5 GDN layers with quantized models」，变更集中在 `vllm/model_executor/models/qwen3_5.py`。PR 描述补充为：## Summary - Replace `self.in_proj_qkvz.weight.shape[0]` and `self.in_proj_ba.weight.shape[0]` with `sum(self.in_proj_qkvz.output_sizes)` and `sum(self.in_proj_ba.output_sizes)`...
...
- 动机: 该 PR 围绕 Qwen3.5 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix][LoRA] Fix Qwen35 LoRA」，变更集中在 `vllm/model_executor/models/qwen3_5.py`。PR 描述补充为：## Purpose ## Test Plan ## Test Result --- Essential Elements of an Effective PR Description Checklist - [ ] The purpose of the PR, such as "Fix some issue (link existing issues...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `tests/lora/test_qwen35_densemodel_lora.py`；关联提交 `1f0d21064137`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+369/-135，可读 patch 529 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3.5 修复已暴露的启动、加载、解析或数值问题，标题为「[CI/Build][LoRA] Update Qwen35 LoRA testing」，变更集中在 `tests/lora/test_qwen35_densemodel_lora.py`。PR 描述补充为：## Purpose ## Test Plan ## Test Result --- Essential Elements of an Effective PR Description Checklist - [ ] The purpose of the PR, such as "Fix some issue (link existing issues...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `tests/evals/gsm8k/configs/Qwen3.5-35B-A3B-DEP2.yaml`, `tests/evals/gsm8k/configs/Qwen3.5-35B-A3B-FP8-DEP2.yaml`, `tests/evals/gsm8k/configs/Qwen3.5-397B-A17B-NVFP4-DEP2.yaml`, `tests/evals/gsm8k/configs/models-qwen35-blackwell.txt`；关联提交 `52069012fe53`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 10 个文件，+69/-11，可读 patch 177 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3.5 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] Fix DeepGemm E8M0 accuracy degradation for Qwen3.5 FP8 on Blackwell」，变更集中在 `tests/evals/gsm8k/configs/Qwen3.5-397B-A17B-NVFP4-DEP2.yaml`, `tests/evals/gsm8k/configs/Qwen3.5-35B-A3B-DEP2.yaml`, `tests/evals/gsm8k/configs/Qwen3.5-35B-A3B-FP8-DEP2.yaml`。PR 描述补充为：## Summary Fixes #37804 — `Qwen/Qwen3.5-35B-A3B-FP8` accuracy drops ~12pp on Blackwell (B200) when DeepGemm is active. DeepGemm's mandatory E8M0 (power-of-2 ceiling) scale forma...
...
- 状态/时间: merged / 2026-03-26
- 反查来源: `git log --name-only -- <model-files>` 反查到 `tests/evals/gsm8k/configs/models-qwen35-mi355.txt`；关联提交 `9c3ae04bfe65`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+25/-0，可读 patch 30 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3.5 补齐模型支持入口或运行时能力，标题为「[ROCm][CI] Add LM Eval Qwen3.5 Models test for MI355」，变更集中在 `tests/evals/gsm8k/configs/models-qwen35-mi355.txt`。PR 描述补充为：This PR adds a new CI entry for running Qwen3.5 model evaluation on MI355 GPUs: - Added `LM Eval Qwen3.5 Models (B200-MI355)` entry in `test-amd.yaml` - Created `tests/evals/gsm...
...
- 动机: 该 PR 围绕 Qwen3.5 调整模型相关实现，标题为「[Model] Extract GatedDeltaNetAttention into shared layer for Qwen3Next and Qwen3.5」，变更集中在 `vllm/model_executor/models/qwen3_5.py`。PR 描述补充为：## Purpose Move the GDN (Gated Delta Net) layer implementation from `qwen3_next.py` into a dedicated `gdn_linear_attn.py`, and unify Qwen3Next and Qwen3.5 under a single `GatedD...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `tests/evals/gsm8k/configs/Qwen3.5-397B-A17B-NVFP4-DEP2.yaml`；关联提交 `ea7bfde6e40d`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-0，可读 patch 5 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3.5 补齐模型支持入口或运行时能力，标题为「[CI] fix LM Eval Qwen3.5 Models (B200)」，变更集中在 `tests/evals/gsm8k/configs/Qwen3.5-397B-A17B-NVFP4-DEP2.yaml`。PR 描述补充为：https://buildkite.com/vllm/ci/builds/58942/steps/canvas?sid=019d427b-900f-4987-9ed9-07480b73fc65&tab=output This was introduced in https://github.com/vllm-project/vllm/pull/3827...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/qwen3_5_mtp.py`；关联提交 `771913e4a024`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+10/-1，可读 patch 24 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3.5 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] Fix NVFP4+MTP crash: force unquantized mtp.fc for Qwen3.5」，变更集中在 `vllm/model_executor/models/qwen3_5_mtp.py`。PR 描述补充为：## Description Fix `AssertionError` when loading `nvidia/Qwen3.5-397B-A17B-NVFP4` with `method="mtp"`. The NVFP4 checkpoint stores the entire MTP branch in BF16, but `hf_quant_c...
...
- 状态/时间: merged / 2026-04-03
- 反查来源: `git log --name-only -- <model-files>` 反查到 `tests/evals/gsm8k/configs/Qwen3.5-35B-A3B-MXFP4-TP2.yaml`, `tests/evals/gsm8k/configs/models-qwen35-mi355.txt`；关联提交 `201d2ea5bfb9`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+9/-0，可读 patch 12 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3.5 补齐模型支持入口或运行时能力，标题为「[CI][ROCm] Add Qwen3.5-35B-A3B-MXFP4 model eval into CI」，变更集中在 `tests/evals/gsm8k/configs/Qwen3.5-35B-A3B-MXFP4-TP2.yaml`, `tests/evals/gsm8k/configs/models-qwen35-mi355.txt`。PR 描述补充为：As title, TP2 validated to work locally with https://huggingface.co/amd/Qwen3.5-35B-A3B-MXFP4. Will mark this PR as ready once the mxfp4 model is made public.
...
- 动机: 该 PR 围绕 Qwen3.5 补齐模型支持入口或运行时能力，标题为「[Bugfix][LoRA] Fix missing in_proj_z in Qwen3_5ForConditionalGenerati…」，变更集中在 `vllm/model_executor/models/qwen3_5.py`。PR 描述补充为：## Purpose Fix missing `in_proj_z` entry in `Qwen3_5ForConditionalGeneration.update_packed_mapping()` when LoRA is enabled. When LoRA is enabled, GDN layers use separate `in_pro...
...
- 动机: 该 PR 围绕 Qwen3.5 补齐模型支持入口或运行时能力，标题为「[Bugfix]Fix EP precision for Qwen3.5, Qwen3-Next」，变更集中在 `vllm/model_executor/models/qwen2_moe.py`, `vllm/model_executor/models/qwen3_next.py`。PR 描述补充为：## Purpose Do not shard shared experts weights when sequence parallel is enabled to fix precision issue for Qwen3.5/Qwen3-Next with EP. At present, when sequence_parallel is ena...
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `vllm/model_executor/models/qwen3_5.py`, `vllm/model_executor/models/qwen3_5_mtp.py`；关联提交 `908a713488db`
- 代码 diff 已读范围: GitHub Pull Request files API 返回 6 个文件，+34/-16，可读 patch 104 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 该 PR 围绕 Qwen3.5 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] LoRA: extend expert base_layer loading to Qwen3.5 and Step3.x」，变更集中在 `vllm/model_executor/models/qwen3_5.py`, `vllm/model_executor/models/qwen3_5_mtp.py`。PR 描述补充为：## Purpose This PR extends https://github.com/vllm-project/vllm/pull/31104 to the remaining model-specific MoE loaders that still hardcode expert parameter names without `.base_...
