...
- 动机: 该 PR 围绕 DeepSeek V3.1 补齐模型支持入口或运行时能力，标题为「[Feature] Add Hopper DeepGEMM E8M0 for DeepSeekV3.1 scale_fmt」，变更集中在 `vllm/model_executor/layers/quantization/fp8.py`, `vllm/model_executor/layers/fused_moe/fused_moe.py`, `vllm/model_executor/layers/fused_moe/triton_deep_gemm_moe.py`。PR 描述补充为：## Purpose Recently DeepGEMM has supported E8M0 scale on Hopper, and this is also required by DeepSeekV3.1 This PR adds the support for it ## Test ### Unit Test ### Accuracy `VL...
...
- 动机: 该 PR 围绕 DeepSeek V3.1 补齐模型支持入口或运行时能力，标题为「[Model] Add DeepSeek-V3.1 reasoning parser (split from PR #24972)」，变更集中在 `tests/reasoning/test_deepseekv3_reasoning_parser.py`, `vllm/reasoning/deepseek_v3_reasoning_parser.py`, `vllm/reasoning/identity_reasoning_parser.py`。PR 描述补充为：## Purpose This PR adds a new reasoning parser for the DeepSeek-V3.1 model, named deepseek_v3. Unlike previous models such as deepseek_r1, the reasoning parser for DeepSeek-V3.1...
...
- 动机: 该 PR 围绕 DeepSeek V3.1 修复已暴露的启动、加载、解析或数值问题，标题为「[BugFix] Fix DeepSeek-V3.1 + DeepGEMM incompatible scale shapes」，变更集中在 `vllm/model_executor/layers/quantization/utils/quant_utils.py`。PR 描述补充为：https://github.com/vllm-project/vllm/pull/29867 broke with For DeepGEMM revert to the behavior before https://github.com/vllm-project/vllm/pull/29867 Credit to: Eldar Kurtić for...
