...
- 动机: 该 PR 围绕 DeepSeek V4 补齐模型支持入口或运行时能力，标题为「[New Model] Support DeepseekV4」，变更集中在 `vllm/model_executor/models/deepseek_v4.py`, `vllm/model_executor/layers/deepseek_v4_attention.py`, `vllm/tokenizers/deepseek_v4_encoding.py`。PR 描述补充为：Congratulations on Deepseek-ai to release the model. Thanks for all Inferact member's effort for support this. Note: This model implementation is highly optimized. All the compo...
...
- 动机: 该 PR 围绕 DeepSeek V4 修复已暴露的启动、加载、解析或数值问题，标题为「[Bugfix] Fix the DSML token leakage in DSV4/3.2」，变更集中在 `tests/tool_parsers/test_deepseekv32_tool_parser.py`, `vllm/tool_parsers/deepseekv32_tool_parser.py`。PR 描述补充为：Co-authored-by: @Windswithyou ## Purpose ## Test Plan ## Test Result before after --- Essential Elements of an Effective PR Description Checklist - [ ] The purpose of the PR, su...
...
- 动机: 该 PR 围绕 DeepSeek V4 补齐模型支持入口或运行时能力，标题为「[Perf][Kernel] BF16 input support for persistent topK - DeepSeekV4」，变更集中在 `csrc/persistent_topk.cuh`, `csrc/topk.cu`, `tests/kernels/test_top_k_per_row.py`。PR 描述补充为：## Motivation > We further quantize the index scores 𝐼:,: from FP32 to BF16 during this QAT process. This optimization achieves a 2× speedup for the top-k selector, while preser...
