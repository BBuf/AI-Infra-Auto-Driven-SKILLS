...
- Motivation: For Hunyuan3 Preview, this PR adds or enables a model support/runtime surface. Title: "[Model] add Hunyuan V1 Dense Model support.". The diff centers on `vllm/model_executor/models/hunyuan_v1.py`, `vllm/model_executor/models/registry.py`, `tests/models/registry.py`. PR body context: ## Essential Elements of an Effective PR Description Checklist - [x] The purpose of the PR, such as "Fix some issue (link existing issues this PR will resolve)". - [x] The test...
...
- Motivation: For Hunyuan3 Preview, this PR adds or enables a model support/runtime surface. Title: "[Model] Add HunyuanOCR support". The diff centers on `vllm/model_executor/models/hunyuan_vision.py`, `vllm/transformers_utils/processors/hunyuan_vl_image.py`, `vllm/transformers_utils/configs/hunyuan_vl.py`. PR body context: ## Purpose - Add HunyuanOCR support ## Test Plan ## Test Result --- Essential Elements of an Effective PR Description Checklist - [ ] The purpose of the PR, such as "Fix some is...
...
- Motivation: For Hunyuan3 Preview, this PR adds or enables a model support/runtime surface. Title: "feature: support eagle3 for HunyuanVL & Hunyuan". The diff centers on `vllm/model_executor/models/hunyuan_v1.py`, `vllm/model_executor/models/hunyuan_vision.py`, `vllm/v1/spec_decode/eagle.py`. PR body context: ## Purpose Eagle3 models: https://huggingface.co/collections/AngelSlim/eagle3 ## Test Plan ## Test Result 详细结果见：AngelSlim Speculative-Decoding ### HunyuanOCR Model | Model | Met...
...
- Motivation: For Hunyuan3 Preview, this PR adds or enables a model support/runtime surface. Title: "[Model] Support Hy3 preview". The diff centers on `vllm/model_executor/models/hy_v3.py`, `vllm/tool_parsers/hy_v3_tool_parser.py`, `vllm/model_executor/models/hy_v3_mtp.py`. PR body context: ## Purpose Support Hy3-preview model ## Test Plan Test model, reasoning parser and tool parser. ## Test Result All pass. --- Hy3-preview model Hy3 preview is a Mixture-of-Expert...
