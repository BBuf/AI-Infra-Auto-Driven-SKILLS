...
- Motivation: For Qwen3 Coder, this PR adds or enables a model support/runtime surface. Title: "Preliminary Support for Qwen3XMLDetector". The diff centers on `python/sglang/srt/function_call/qwen3_detector.py`, `python/sglang/srt/function_call/function_call_parser.py`, `python/sglang/srt/server_args.py`. PR body context: ## Motivation This is a very basic, preliminary implementation of the Qwen3XMLDetector, intended to provide initial support. Please note that the related functionality still req...
...
- Motivation: For Qwen3 Coder, this PR adds or enables a model support/runtime surface. Title: "[Bugfix][Feat] Add XML-ish grammar in EBNFComposer and fix misc bugs in Qwen3 detector". The diff centers on `test/srt/test_function_call_parser.py`, `python/sglang/srt/function_call/ebnf_composer.py`, `python/sglang/srt/function_call/qwen3_coder_detector.py`. PR body context: ## Motivation Follow up to #8260 Here are some issues of `Qwen3XMLDetector`: 1. **EBNFComposer XML Support:** `tool_choice="required"` and specific tool selection fail because E...
...
- Motivation: For Qwen3 Coder, this PR changes model-related implementation. Title: "Update qwen3_coder_detector.py for streaming". The diff centers on `python/sglang/srt/function_call/qwen3_coder_detector.py`. PR body context: ## Motivation This PR is to improve the streaming tool call parser from reading all values to incremental flow. ## Modifications Update the tool call parser for qwen3_coder_dete...
...
- Motivation: For Qwen3 Coder, this PR changes model-related implementation. Title: "Forward unknown tool calls instead of dropping". The diff centers on `python/sglang/srt/function_call/qwen3_coder_detector.py`, `test/srt/function_call/test_unknown_tool_name.py`, `python/sglang/srt/function_call/base_format_detector.py`. PR body context: Closes #12223 ## Motivation - Avoid silent data loss when models emit unknown tool names. - Make behavior consistent with paths that already forward malformed arguments (no sche...
...
- Motivation: For Qwen3 Coder, this PR changes model-related implementation. Title: "Remove EBNF Composer". The diff centers on `test/srt/test_function_call_parser.py`, `python/sglang/srt/function_call/ebnf_composer.py`, `test/srt/function_call/test_json_schema_constraint.py`. PR body context: ## Motivation Resolve issue: https://github.com/sgl-project/sglang/issues/11078 We are now using json schema to constrain model output for required and named tool_choice cases,...
...
- Motivation: For Qwen3 Coder, this PR changes model-related implementation. Title: "Improve Qwen3CoderDetector with schema-aware parameter type conversion". The diff centers on `python/sglang/srt/function_call/qwen3_coder_detector.py`, `test/per_commit/function_call/test_function_call_parser.py`. PR body context: ## Motivation In the original implementation of Qwen3CoderDetector, there is a function called `_safe_val` to help convert the parameter type of function calls: However, due to...
...
- Motivation: For Qwen3 Coder, this PR adds or enables a model support/runtime surface. Title: "Add Qwen3-Coder-480B to nightly tests". The diff centers on `.github/workflows/nightly-test-nvidia.yml`, `test/nightly/test_qwen3_coder_480b_perf.py`, `test/nightly/nightly_utils.py`. PR body context: ## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according to the Format code with pre-commit. - [ ] Add unit t...
...
- Motivation: For Qwen3 Coder, this PR adds or enables a model support/runtime surface. Title: "support new qwen3_coder_detector". The diff centers on `python/sglang/srt/function_call/qwen3_coder_detector.py`. PR body context: # UT !39a93fe0305fa84ec404a08211d84853 # E2E Test !76e2386d0e007fb9c295f7f8b61f2904 The test results, provided by Zeyu Cui from the Qwen Team @cyente, have been confirmed to mee...
...
- Motivation: For Qwen3 Coder, this PR adds or enables a model support/runtime surface. Title: "[Fix] Triton TP MoE Dpsk V3/Qwen3 Coder with SwapAB". The diff centers on `python/sglang/srt/layers/moe/fused_moe_triton/configs/triton_3_5_1/E=80,N=640,device_name=NVIDIA_H200,dtype=fp8_w8a8,block_shape=[128, 128]_down.json`, `python/sglang/srt/layers/moe/fused_moe_triton/configs/triton_3_5_1/E=80,N=640,device_name=NVIDIA_H200,dtype=fp8_w8a8,block_shape=[128, 128].json`, `python/sglang/srt/layers/moe/fused_moe_triton/configs/triton_3_5_1/E=257,N=256,device_name=NVIDIA_H200,dtype=fp8_w8a8,block_shape=[128, 128]_down.json`. PR body context: ## Motivation Enable SwapAB on H200, which has been verified in this PR. And also retune the configuration of TP=8 EP=1 DeepseekV3 and TP=8 EP=2 Qwen3 Coder, in the latency scen...
...
- Motivation: For Qwen3 Coder, this PR adds or enables a model support/runtime surface. Title: "Add MoE fused config for Qwen3-Coder-Next-FP8 on H100 TP=2". The diff centers on `python/sglang/srt/layers/moe/fused_moe_triton/configs/triton_3_5_1/E=512,N=256,device_name=NVIDIA_H100_80GB_HBM3,dtype=fp8_w8a8,block_shape=[128, 128].json`. PR body context: ## Motivation Add optimized Triton MoE kernel configuration for Qwen3-Coder-Next-FP8 on H100 with TP=2. ## Modifications ## Accuracy Tests ## Benchmarking and Profiling `python...
...
- Motivation: For Qwen3 Coder, this PR adds or enables a model support/runtime surface. Title: "[ModelOPT] Support Qwen 3 Next Coder NVFP4". The diff centers on `python/sglang/srt/models/qwen3_next.py`. PR body context: ## Motivation This branch include important bugfix for qwen 3 coder next nvfp4 # B300 `sglang serve --model vincentzed-hf/Qwen3-Coder-Next-NVFP4 --quantization modelopt_fp4` **W...
...
- Motivation: For Qwen3 Coder, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[NPU] bugfix for model Qwen3-Coder-Next at weight shape transpose for npu.". The diff centers on `python/sglang/srt/layers/attention/hybrid_linear_attn_backend.py`, `python/sglang/srt/hardware_backend/npu/quantization/fused_moe_method_npu.py`. PR body context: ## Motivation 1. The original code contained an import error that affected the proper loading of weight-related modules. 2. Duplicate dimension transformations were applied to w...
...
- Motivation: For Qwen3 Coder, this PR adds or enables a model support/runtime surface. Title: "[AMD] Support Qwen3-Coder-Next on AMD platform". The diff centers on `python/sglang/srt/layers/attention/aiter_backend.py`, `python/sglang/srt/models/qwen3_next.py`. PR body context: ## Motivation Enable Qwen3-Coder-Next model on AMD GPU platform. With this PR, we are able to support non-MTP (fp8 kv cache) and MTP on Qwen3-Coder-Next. ## Modifications - aite...
...
- Motivation: For Qwen3 Coder, this PR adds or enables a model support/runtime surface. Title: "[AMD] Add Qwen3-Coder-Next accuracy and functionality test scripts for MI35x 8-GPU". The diff centers on `test/registered/amd/accuracy/mi35x/test_qwen3_coder_next_eval_mi35x.py`, `test/registered/amd/test_qwen3_coder_next_8gpu.py`. PR body context: ## Motivation Add CI test coverage for the Qwen3-Coder-Next model on AMD MI35x (gfx950) 8-GPU systems. This model features a hybrid architecture combining full attention (GQA) w...
...
- Motivation: For Qwen3 Coder, this PR adds or enables a model support/runtime surface. Title: "feat: Add FP8 KV cache support for Triton attention backend". The diff centers on `python/sglang/srt/layers/attention/triton_backend.py`, `python/sglang/srt/layers/attention/triton_ops/decode_attention.py`, `python/sglang/srt/layers/attention/triton_ops/extend_attention.py`. PR body context: ## Motivation The Triton attention backend does not correctly handle FP8 KV cache. It does not pass k_scale/v_scale to set_kv_buffer, causing scales to default to 1.0. In additi...
...
- Motivation: For Qwen3 Coder, this PR adds or enables a model support/runtime surface. Title: "[AMD] Fix Qwen3-Coder-Next: Add missing k_scale/v_scale args to extend_attention_fwd in aiter_backend". The diff centers on `python/sglang/srt/layers/attention/aiter_backend.py`. PR body context: ## Summary - Fix `TypeError: extend_attention_fwd() missing 1 required positional argument: 'v_scale'` crash in aiter_backend when running non-MLA speculative decoding (target_v...
...
- Motivation: For Qwen3 Coder, this PR adds or enables a model support/runtime surface. Title: "[Feature] Support incremental streaming for tool_call arguments in Qwen3CoderDetector". The diff centers on `python/sglang/srt/function_call/qwen3_coder_detector.py`. PR body context: ## Motivation When using --tool-call-parser qwen3_coder with streaming enabled, the arguments field of toolcall deltas is not streamed incrementally. Instead, the entire paramet...
