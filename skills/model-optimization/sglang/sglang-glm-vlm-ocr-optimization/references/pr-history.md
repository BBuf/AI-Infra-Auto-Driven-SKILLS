...
- Extra preserved PRs from prior docs: 23
...
- Motivation: For GLM VLM/OCR, this PR adds or enables a model support/runtime surface. Title: "Support glm4.1v and glm4.5v". The diff centers on `python/sglang/srt/models/glm4v.py`, `python/sglang/srt/models/glm4v_moe.py`, `python/sglang/srt/layers/rotary_embedding.py`. PR body context: ## Motivation Close https://github.com/sgl-project/sglang/issues/7993. Adapted from https://github.com/sgl-project/sglang/pull/8015 and depend on https://github.com/huggingface/...
...
- Motivation: For GLM VLM/OCR, this PR extends deployment docs, tests, or CI coverage. Title: "Set the default attention backend for GLM-4.5v to fa3". The diff centers on `python/sglang/srt/utils.py`. PR body context: As suggested in https://huggingface.co/zai-org/GLM-4.5V#sglang ## Motivation ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code...
...
- Motivation: For GLM VLM/OCR, this PR adds or enables a model support/runtime surface. Title: "[GLM4.1V and GLM4.5V] Add vision transformer num_dummy_head support: max tp=4 -> max tp=8". The diff centers on `python/sglang/srt/layers/attention/vision_utils.py`, `python/sglang/srt/models/glm4v.py`, `python/sglang/srt/models/internvl.py`. PR body context: ## Motivation In some GLM4v models, `num_key_value_heads=8` in text attention and `num_heads=12` in vision attention so `tp=4` is the max supported tp_size today `(num_key_value...
...
- Motivation: For GLM VLM/OCR, this PR adds or enables a model support/runtime surface. Title: "Add support for GLM 4.5V FP8". The diff centers on `python/sglang/srt/layers/moe/fused_moe_triton/configs/triton_3_4_0/E=128,N=352,device_name=NVIDIA_L40S,dtype=fp8_w8a8.json`, `benchmark/kernels/fused_moe_triton/tuning_fused_moe_triton.py`. PR body context: ## Motivation Add moe kernel generation support for GLM 4.5V FP8 ## Modifications Write according to the GLM 4.5V FP8 configuration. ## Accuracy Tests N/A ## Benchmarking and Pr...
...
- Trace source: preserved from an explicit existing history/skill citation
- Diff scope read: GitHub Pull Request files API returned 1 files, +1/-0, 8 readable patch lines; this card prioritizes model-related and high-change files.
- Motivation: For GLM VLM/OCR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "Fix GLM45v launch server cuda torch compile bug". The diff centers on `python/sglang/srt/models/qwen2_5_vl.py`. PR body context: ## Motivation After recent sglang code changes, glm45v launch server would fail with this error during torch compile ## Modifications - From https://discuss.pytorch.org/t/index-...
...
- Motivation: For GLM VLM/OCR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bug Fix] Fix Glm4vVisionBlock norm". The diff centers on `python/sglang/srt/models/glm4v.py`. PR body context: ## Motivation This PR fixes a runtime crash in `Glm4vVisionBlock` when running vision prefill. After #9709, `Qwen2_5_VisionBlock` calls `norm2(x2d, residual=attn2d)` and expects...
...
- Motivation: For GLM VLM/OCR, this PR adds or enables a model support/runtime surface. Title: "Fix: (glm4v) Add missing field". The diff centers on `python/sglang/srt/models/glm4v.py`. PR body context: ## Motivation Fix: https://github.com/sgl-project/sglang/actions/runs/17539177242/job/49807888401 ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist -...
...
- Trace source: preserved from an explicit existing history/skill citation
- Diff scope read: GitHub Pull Request files API returned 1 files, +3/-0, 10 readable patch lines; this card prioritizes model-related and high-change files.
- Motivation: For GLM VLM/OCR, this PR adds or enables a model support/runtime surface. Title: "Add self.capture_aux_hidden_states For GLM-4.5V". The diff centers on `python/sglang/srt/models/glm4v_moe.py`. The PR body has no extra context, so this assessment comes from the title, file list, and patch.
...
- Motivation: For GLM VLM/OCR, this PR optimizes an inference path or backend selection. Title: "Replace pad with cat for better performance". The diff centers on `python/sglang/srt/models/dots_vlm_vit.py`, `python/sglang/srt/models/glm4v.py`, `python/sglang/srt/models/qwen2_5_vl.py`. PR body context: ## Motivation Using concatenation tends to be faster than padding. ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [ ] Format your code according...
...
- Motivation: For GLM VLM/OCR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[lint] improve ruff check". The diff centers on `python/sglang/srt/layers/moe/fused_moe_triton/triton_kernels_moe.py`, `python/sglang/srt/layers/quantization/compressed_tensors/compressed_tensors.py`, `python/sglang/srt/layers/attention/flashinfer_mla_backend.py`. PR body context: Prev PR #11685 added `python/sglang` to the Ruff lint scope, but the pre-commit args were written as: These arguments are interpreted as `--select=F401 F821 --fixable=F401`, whi...
...
- Motivation: For GLM VLM/OCR, this PR changes model-related implementation. Title: "GLM-4-0414 and GLM-4.1V Code Refactor". The diff centers on `python/sglang/srt/models/glm4.py`, `python/sglang/srt/models/glm4v.py`, `python/sglang/srt/layers/rotary_embedding.py`. PR body context: Used new interfaces, including the addition of PPMissingLayer, and discarded some useless old code
...
- Motivation: For GLM VLM/OCR, this PR adds or enables a model support/runtime surface. Title: "Cleanup vision attention related codes". The diff centers on `python/sglang/srt/models/glm4v.py`, `python/sglang/srt/models/qwen2_5_vl.py`, `python/sglang/srt/models/qwen3_vl.py`. PR body context: ## Motivation Most of the codes for vision attention are hard coded or dead codes. This PR removes them, supports the auto set of `--mm-attention-backend triton_attn` on B200. #...
...
- Motivation: For GLM VLM/OCR, this PR adds or enables a model support/runtime surface. Title: "support GLM-V vision model dp". The diff centers on `python/sglang/srt/models/glm4v.py`, `python/sglang/srt/models/glm4.py`, `python/sglang/srt/models/glm4_moe.py`. PR body context: Based on https://github.com/sgl-project/sglang/pull/13126 and https://github.com/sgl-project/sglang/pull/13724, add support for the GLM-V vision model DP.
...
- Motivation: For GLM VLM/OCR, this PR adds or enables a model support/runtime surface. Title: "[Glm46v] support ktransformers". The diff centers on `python/sglang/srt/models/glm4v_moe.py`. PR body context: ## Motivation support ktransformers for GLM-4.6V ## Modifications ## Accuracy Tests ## Benchmarking and Profiling ## Checklist - [x] Format your code according to the Format cod...
...
- Motivation: For GLM VLM/OCR, this PR adds or enables a model support/runtime surface. Title: "[GLM-4.6V] Support Pipeline Parallelism for GLM-4.6V & GLM-4.1V". The diff centers on `python/sglang/srt/models/glm4v.py`, `test/srt/test_pp_single_node.py`, `python/sglang/test/test_utils.py`. PR body context: ## Motivation This PR is to support GLM-4.6V and GLM-4.1V Pipeline Parallelism. In main branch GLM-4.6V and GLM-4.1V PP don't work correctly. There are two reasons: 1. In pp-siz...
...
- Trace source: preserved from an explicit existing history/skill citation
- Diff scope read: GitHub Pull Request files API returned 1 files, +3/-0, 10 readable patch lines; this card prioritizes model-related and high-change files.
- Motivation: For GLM VLM/OCR, this PR adds or enables a model support/runtime surface. Title: "[CI]add nightly CI for glm4v_moe arch model". The diff centers on `test/nightly/test_vlms_mmmu_eval.py`. PR body context: ## Motivation add nightly CI for glm4v_moe arch model to prevent silent regression like this https://github.com/sgl-project/sglang/issues/14582. Note: existing `zai-org/GLM-4.1V...
...
- Motivation: For GLM VLM/OCR, this PR adds or enables a model support/runtime surface. Title: "add transformers version validation for glm-4.6v moe models". The diff centers on `python/sglang/srt/configs/model_config.py`. PR body context: ## Motivation This PR adds a version check for the installed transformers library to ensure compatibility with GLM-4.6v MoE vision models. For models requiring Transformers ≥ 5....
...
- Motivation: For GLM VLM/OCR, this PR adds or enables a model support/runtime surface. Title: "[VLM] Support cos sin cache for Qwen3-VL & GLM-4.1V". The diff centers on `python/sglang/srt/models/glm4v.py`, `python/sglang/srt/models/qwen3_vl.py`, `python/sglang/srt/layers/attention/vision.py`. PR body context: ## Motivation Support cos sin cache for Qwen3-VL & GLM-4.1V. This PR refactors the rotary positional embedding (RoPE) implementation to expose an explicit cosine/sine cache inte...
...
- Motivation: For GLM VLM/OCR, this PR optimizes an inference path or backend selection. Title: "Convert cu_seqlens to CPU for npu_flash_attention_unpad operator". The diff centers on `python/sglang/srt/models/qwen3_vl.py`, `python/sglang/srt/models/glm4v.py`, `python/sglang/srt/models/paddleocr_vl.py`. PR body context: ## Motivation In order to improve the performance of VisionAscendAttention, we convert cu_seqlens to CPU before the first transformer layer, because converting it to CPU per lay...
...
- Motivation: For GLM VLM/OCR, this PR adds or enables a model support/runtime surface. Title: "[GLM-OCR] Support GLM-OCR Model". The diff centers on `python/sglang/srt/models/glm_ocr.py`, `python/sglang/srt/models/glm_ocr_nextn.py`, `python/sglang/srt/multimodal/processors/glm4v.py`. PR body context: Support for GLM-OCR Model, transformers PR here Need transformer>=5.0.0dev0, but not 5.0.0, so also changed min requirements in glm4v.
...
- Motivation: For GLM VLM/OCR, this PR optimizes an inference path or backend selection. Title: "[VLM] Optimize get_rope_index for GLM4v". The diff centers on `python/sglang/srt/layers/rotary_embedding.py`, `benchmark/bench_rope/benchmark_rope_index.py`. PR body context: ## Motivation Speedup **12%** to **600%**(long token length) for get_rope_index for GLM4v. Benchmark test added. lmms_evals no drop. PR: Main: ## Modifications ## Accuracy Tests...
...
- Motivation: For GLM VLM/OCR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "Fix GLM-4V processor registration when glm_ocr is unavailable". The diff centers on `python/sglang/srt/multimodal/processors/glm4v.py`. PR body context: ## Motivation Fix nightly test failures where GLM-4V models fail with "No processor registered for architecture" or show "Model not evaluated". Affected tests: - **4-GPU**: `tes...
...
- Motivation: For GLM VLM/OCR, this PR optimizes an inference path or backend selection. Title: "[VLM] Replace conv3d proj with linear for GLM4V". The diff centers on `python/sglang/srt/models/glm4v.py`. PR body context: ## Motivation Inspired by https://github.com/sgl-project/sglang/pull/19788 with some optimizations: lmms_evals no drops. Main and PR are the same score: More performance test wi...
...
- Motivation: For GLM VLM/OCR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bugfix] Fix GLM-4.6V vision regression in glm4v_moe and glm_ocr". The diff centers on `python/sglang/srt/models/glm4v_moe.py`, `python/sglang/srt/models/glm_ocr.py`. PR body context: ## Motivation PR #20033 replaced `Conv3d` with `Linear` in `Glm4vVisionPatchEmbed` and added `copy_conv3d_weight_to_linear()` at the end of `glm4v.py`'s `load_weights()`. Howeve...
...
- Motivation: For GLM VLM/OCR, this PR adds or enables a model support/runtime surface. Title: "Add Conv2dLayer/Conv3dLayer to fix PyTorch 2.9.1 CuDNN Conv3d bug". The diff centers on `python/sglang/srt/layers/conv.py`, `python/sglang/srt/models/glm4v.py`, `python/sglang/srt/models/pixtral.py`. PR body context: ## Motivation Very thanks for the benchmark data in this PR: https://github.com/sgl-project/sglang/pull/19788 - Add `Conv2dLayer`/`Conv3dLayer` in `sglang/srt/layers/conv.py`. C...
...
- Motivation: For GLM VLM/OCR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "Revert "[Bugfix] Fix GLM-4.6V vision regression in glm4v_moe and glm_ocr"". The diff centers on `python/sglang/srt/models/glm4v_moe.py`, `python/sglang/srt/models/glm_ocr.py`. PR body context: Reverts sgl-project/sglang#20463 per request by @JustinTong0323
...
- Motivation: For GLM VLM/OCR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[Bug Fix] GLM-V / GLM-OCR: field detection for transformers 5.x and MTP omission fix". The diff centers on `python/sglang/srt/models/glm4v_moe.py`, `python/sglang/srt/models/glm_ocr.py`. PR body context: Mainly modify several issues 1. Modified the position of the replacement model module to ensure that the MTP has a normal acceptance rate after reading, otherwise the accept len...
...
- Motivation: For GLM VLM/OCR, this PR fixes a launch, loading, parsing, or numerical issue. Title: "[bugfix]GLM-4V model". The diff centers on `python/sglang/srt/models/glm4v.py`, `python/sglang/srt/multimodal/processors/base_processor.py`, `test/registered/ascend/vlm_models/test_ascend_glm_4_5v.py`. PR body context: ## Motivation GLM-4V model bugfix ## Modifications 1. In VisionAttention, it is necessary to use num_dumy_heads to calculate. If num_dumy_heads=0 and the result of num_dumy_head...
...
- Motivation: For GLM VLM/OCR, this PR adds or enables a model support/runtime surface. Title: "[NPU] Support GLM-4.5V". The diff centers on `python/sglang/srt/models/glm4_moe.py`. PR body context: ## Motivation Support GLM-4.5V on NPU. ## Modifications 1. When calling the split_qkv_rmsnorm_rope function, pass the correct arguments based on the use_qk_norm parameter.The `s...
